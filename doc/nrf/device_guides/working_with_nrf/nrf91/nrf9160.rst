...
Build the sample (located under ``ncs/zephyr/samples/hello_world``) for the nrf9160dk/nrf52840 board.
...
In Zephyr, the firmware for the application core of :ref:`zephyr:nrf9160dk_nrf9160` is divided into two different board targets:

* ``nrf9160dk/nrf9160`` for board targets that have Cortex-M Security Extensions (CMSE) disabled.
* ``nrf9160dk/nrf9160/ns`` for board targets that have CMSE enabled and have the Secure Processing Environment (SPE) firmware alongside the Non-Secure Processing Environment (NSPE) firmware.
...
.. |sim_card| replace:: For the iBasis SIM card provided with the nRF9160 DK, see `iBasis IoT network coverage`_.

.. |firmware_filename| replace:: :file:`mfw_nrf9160_`

Download and extract the latest application and modem firmware from the `nRF9160 DK Downloads`_ page.

.. dk_update_firware_start
...
Modem firmware
  ...
  The archive is named |firmware_filename| followed by the firmware version number.
...
.. note::
   When you update the application firmware on an nRF9160 DK, make sure that the modem firmware and application firmware are compatible versions.
...
.. |vsc_sample_board_target_line| replace:: you must use the board target ``nrf9160dk/nrf9160/ns`` when building the application code for the nRF9160 DK
...
.. |cmd_board_target| replace:: ``nrf9160dk/nrf9160/ns`` when building the application code for the nRF9160 DK
...
.. |firmware_update| replace:: :ref:`nrf9160_gs_updating_fw_application`

.. |firmware_file| replace:: :file:`nrf9160dk_at_client_<version-number>.hex`

.. at_client_test_start

The :ref:`at_client_sample` sample enables you to send AT commands to the modem of your |DK| to test and monitor the cellular connection.
...
1. Follow the steps in |firmware_update| to program the sample to the DK.
   When selecting the HEX file, select |firmware_file| instead of the one for Asset Tracker v2.
...
For example, when building a non-secure application for nRF9160 DK v0.9.0, use ``nrf9160dk@0.9.0/nrf9160/ns`` as board target.
