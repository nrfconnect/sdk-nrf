...
* Prompts:
  ...
  * Include units (for example, "in milliseconds" or "in bytes").
  * Depending on the option status:

    * If it is experimental, add ``[EXPERIMENTAL]`` to the prompt.
    * If it is deprecated, add ``[DEPRECATED]`` to the prompt.
    * If it is informational only, add ``(informative only, do not change)`` to the prompt.
