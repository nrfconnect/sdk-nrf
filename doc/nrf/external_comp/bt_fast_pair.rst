...
This top-level integration guide includes additional guides for the Fast Pair helper modules.
These helper modules may provide additional functionality in your application, but they are not strictly required for the Fast Pair integration.
...
.. note::
   If you use the :ref:`bt_fast_pair_adv_manager_readme` helper module for Fast Pair advertising management, you can enable the locator tag use case with the :kconfig:option:`CONFIG_BT_FAST_PAIR_ADV_MANAGER_USE_CASE_LOCATOR_TAG` Kconfig option.
   This option includes additional logic for handling the `Fast Pair Locator Tag Specific Guidelines`_ that are related to Fast Pair advertising.
   See :ref:`ug_bt_fast_pair_adv_manager` for more details on how to integrate it in your application.
...
It also provides additional layer for the locator tag use case, available using the :kconfig:option:`CONFIG_BT_FAST_PAIR_ADV_MANAGER_USE_CASE_LOCATOR_TAG` Kconfig option, that implements Fast Pair advertising requirements specified in the `Fast Pair Locator Tag Specific Guidelines`_ section of the FMDN Accessory specification.
