...
.. note::
   The dynamic generation of the ``bt_fast_pair`` partition definition is only supported if you enable the :kconfig:option:`CONFIG_BT_FAST_PAIR` Kconfig option in your project image.
...
To generate a hexadecimal file with the Fast Pair provisioning data during the build run, you must use the default |NCS| build system configuration with sysbuild and set the following sysbuild Kconfig options:
...
.. note::
   The Fast Pair partition is only recognized by the sysbuild code when it is defined in the main project domain.
   For example, the CPUNET domain in the case of nRF53 Series does not work.

For example, when building an application with the |nRFVSC|, you need to add the following parameters in the **Extra CMake arguments** field on the **Add Build Configuration view**: ``-DSB_CONFIG_BT_FAST_PAIR_MODEL_ID=0xFFFFFF -DSB_CONFIG_BT_FAST_PAIR_ANTI_SPOOFING_PRIVATE_KEY=\"AbAbAbAbAbAbAbAbAbAbAbAbAbAbAbAbAbAbAbAbAbA=\"``.
Make sure to replace ``0xFFFFFF`` and ``AbAbAbAbAbAbAbAbAbAbAbAbAbAbAbAbAbAbAbAbAbA=`` with values obtained for your device.

Apart from passing the Fast Pair sysbuild Kconfig options as additional build parameters, these options can also be supplied to the build system with the following alternative methods:

* A project configuration file for modifying the sysbuild configuration (:file:`sysbuild.conf`).
* A default redefinition in the project Kconfig file for sysbuild (:file:`Kconfig.sysbuild`).

.. note::
   The Anti-Spoofing Private Key is confidential and must be handled securely.
   It is recommended to store this key securely outside the code repository and pass it to the project as an additional build parameter (with the ``-DSB_CONFIG_BT_FAST_PAIR_ANTI_SPOOFING_PRIVATE_KEY`` parameter).
...
The Fast Pair provisioning data is automatically merged with the final hexadecimal output from the build command (:file:`merged.hex`) and is uploaded on your target device with the `west flash` command.
...
To support Google Fast Pair in your project, you must enable :kconfig:option:`CONFIG_BT_FAST_PAIR` Kconfig option at the main application image level.

.. note::
   The Fast Pair Kconfig configuration at the sysbuild level is only used to generate the Fast Pair provisioning data hex file during the build run.
