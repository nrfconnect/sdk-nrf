...
* In a typical integration, the DULT association follows the lifetime of the accessory-locating network stack.
  The network stack claims the DULT association with the :c:func:`dult_enable` function during its enable operation and releases it with the :c:func:`dult_reset` function during its disable operation.
...
* In a typical integration, the DULT association follows the association state with the accessory-locating network.
  The network stack claims the DULT association when the accessory is being associated with this network, or during the stack enable procedure if the accessory is already associated.
  For example, the FHN extension of the Fast Pair module claims the association during its provisioning operation.
  The stack releases the DULT association when the accessory association with this network is being removed or during the stack disable procedure.

Whenever the integration of the DULT subsystem is different for the selected API variant, it is highlighted with tabs consistently throughout this guide.
...
.. tabs::
   ...
   .. group-tab:: API variant v1
      ...
      Call the :c:func:`dult_reset` function.
   ...
   .. group-tab:: API variant v2
      ...
      Call the :c:func:`dult_user_unregister` function.
...
The lifetime of the registered callbacks depends on the selected :ref:`ug_dult_api_variant`:

.. tabs::

   .. group-tab:: API variant v1

      The callbacks are cleared by the :c:func:`dult_reset` function and must be registered again after each subsequent DULT user registration.

   .. group-tab:: API variant v2

      The callbacks are preserved across the :c:func:`dult_reset` function call and cleared only by the :c:func:`dult_user_unregister` function call.
...
.. tabs::
   ...
   .. group-tab:: API variant v1
      ...
      The :c:func:`dult_reset` function unregisters the current DULT user and callbacks, resets the preset configuration, and disables the DULT subsystem.

   .. group-tab:: API variant v2

      The :c:func:`dult_reset` function releases the association and keeps the DULT user, its callbacks, and its preset configuration registered.
...
While another accessory-locating network stack holds the DULT association, every evicted user must block its own association flow, so that the accessory cannot be associated with two accessory-locating networks at the same time.
...
The lifetime of the preset state depends on the selected :ref:`ug_dult_api_variant`:

.. tabs::

   .. group-tab:: API variant v1

      The state is also reset to the near-owner mode when the :c:func:`dult_reset` function is called.

   .. group-tab:: API variant v2

      The state is preserved across the :c:func:`dult_reset` function call and cleared by the :c:func:`dult_user_unregister` function call.
...
Your accessory-locating network stack can override this gate for the Accessory Information operations by registering the :c:struct:`dult_bt_anos_cb` structure with the :c:func:`dult_bt_anos_cb_register` function.
...
While a DULT user is associated, the ANOS consults the access policy of the associated user.
The callback can then relax the near-owner state gate and allow the Accessory Information operations in the near-owner mode.

When no DULT user is associated, the ANOS rejects all operations unless the Bluetooth LE connection is claimed with the :c:func:`dult_multi_user_conn_claim` function.
This function is available only with the :kconfig:option:`CONFIG_DULT_API_VARIANT_V2` Kconfig option, see the :ref:`ug_dult_anos_pre_association_access` section for details.
...
.. tabs::
   ...
   .. group-tab:: API variant v1
      ...
      Register the access policy callback while the DULT subsystem is still disabled, and call the :c:func:`dult_enable` function as soon as your accessory-locating network stack is ready to serve the Accessory Information operations.
...
If your accessory-locating network does integrate the DULT location-enabled advertising payload, the network stack is responsible for advertising it.
...
The point at which you must set the battery level depends on the selected :ref:`ug_dult_api_variant`:

.. tabs::

   .. group-tab:: API variant v1

      You must set the battery level before enabling DULT, as the :c:func:`dult_enable` function fails otherwise.

   .. group-tab:: API variant v2

      The :c:func:`dult_enable` function succeeds with the battery level unset, but the accessory non-owner service rejects the battery read operations until the associated user sets its battery level.
