...
When the accessory becomes the associated DULT user depends on the selected DULT API variant (see the :ref:`ug_dult_api_variant` section of the DULT integration guide):
...
When the callback reports that another accessory-locating network claimed the association, we recommend to call the :c:func:`bt_fast_pair_disable` function to disable the Fast Pair module together with the FHN extension, so that the winning network can operate exclusively.
