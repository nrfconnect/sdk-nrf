...
* API functions used to register callbacks:
  ...
  * The :c:func:`bt_fast_pair_fhn_info_cb_register` function (mandatory with the :c:member:`bt_fast_pair_fhn_info_cb.dult_ownership_state_changed` callback set if the :kconfig:option:`CONFIG_DULT_USER_MAX` Kconfig option is set to a value greater than ``1``)
...
For details on how to declare the accessory type, see the :ref:`ug_dult_accessory_type` section of the DULT integration guide.
...
The point at which the accessory becomes the associated DULT user depends on the selected DULT API variant (see the :ref:`ug_dult_api_variant` section of the DULT integration guide):

.. tabs::

   .. group-tab:: API variant v1

      The FHN extension enables the DULT module during the :c:func:`bt_fast_pair_enable` function call and keeps it enabled throughout the Fast Pair lifetime.
      In the Fast Pair enabled state, you must ensure that no other accessory-locating network stack requires the DULT module in your application, as only one DULT user can be registered at a time.

   .. group-tab:: API variant v2

      The FHN extension ties the DULT association to its provisioning state.
      It claims the association during the :c:func:`bt_fast_pair_enable` function call if the accessory is already provisioned, and upon provisioning otherwise.
      It releases the association upon unprovisioning and during the :c:func:`bt_fast_pair_disable` function call.
      This lets the FHN extension coexist with other accessory-locating networks during the pre-association window.

The following FHN extension sections describe the additional steps required to integrate the DULT module after the DULT user is registered during the :c:func:`bt_fast_pair_enable` function call and the FHN extension holds the DULT association.
...
* :c:member:`bt_fast_pair_fhn_info_cb.dult_ownership_state_changed` - Notification about the DULT ownership (association) state update
...
.. _ug_bt_fast_pair_gatt_service_fhn_info_callbacks_dult_ownership_state:

DULT ownership state - handling multiple accessory-locating networks
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

This section applies only to products that support multiple DULT-based accessory-locating networks.
...
This section describes the FHN-specific actions for the workflow that is defined in the :ref:`ug_dult_multi_user` section of the DULT integration guide.

Register the :c:member:`bt_fast_pair_fhn_info_cb.dult_ownership_state_changed` callback to learn about the DULT ownership (association) state updates.
The callback is mandatory when the :kconfig:option:`CONFIG_DULT_USER_MAX` Kconfig option is set to a value greater than ``1``, because it is the only way to learn that the FHN extension was evicted by another accessory-locating network stack.

When the callback reports that another accessory-locating network stack claimed the association, we recommend to call the :c:func:`bt_fast_pair_disable` function to disable the Fast Pair module together with the FHN extension, so that the winning network can operate exclusively.
Make sure to also stop the application-level Fast Pair activities that are not managed by the :c:func:`bt_fast_pair_disable` function, such as the Fast Pair advertising.
The callback is delivered from the system workqueue, so you can call this function directly from it.

.. note::
   Provisioning the FHN extension fails if another accessory-locating network stack holds the DULT association.
