...
The following page provides guidelines on how to integrate this module in your application.

.. note::
   This page is a helper guide to the main Fast Pair integration guide and covers only integration steps for the :ref:`bt_fast_pair_adv_manager_readme` module.
   For the complete Fast Pair integration guide, see :ref:`ug_bt_fast_pair`.

Prerequisites
*************

Before using the :ref:`bt_fast_pair_adv_manager_readme`, ensure that the following prerequisites are fulfilled:

* Read through the main :ref:`ug_bt_fast_pair` guide to understand the integration requirements of the overall Fast Pair solution and make sure you satisfy all the prerequisites in this top-level document.
* Accept the usage of the Bluetooth LE Extended Advertising feature (:kconfig:option:`CONFIG_BT_EXT_ADV`), which is the dependency for the :ref:`bt_fast_pair_adv_manager_readme` module.
...
* Accept the usage of the :ref:`bt_le_adv_prov_readme` library (:kconfig:option:`CONFIG_BT_ADV_PROV`) as a way of encoding the advertising payload for the Fast Pair advertising set.
  Additionally, the advertising provider for the Fast Pair payload (:kconfig:option:`CONFIG_BT_ADV_PROV_FAST_PAIR` Kconfig option) is required to work with this module.
...
The Fast Pair Advertising Manager integration consists of the following steps:
...
To start integrating the Fast Pair Advertising Manager in your project, complete the following initial steps:
...
To support the Fast Pair Advertising Manager in your project, you must enable the :kconfig:option:`CONFIG_BT_FAST_PAIR_ADV_MANAGER` Kconfig option in the configuration file of your application image.
To use the :kconfig:option:`CONFIG_BT_FAST_PAIR_ADV_MANAGER`, you must also enable the :kconfig:option:`CONFIG_BT_FAST_PAIR` Kconfig option that enables the general support for the Fast Pair solution.
Additionally, enable all dependencies of the :kconfig:option:`CONFIG_BT_FAST_PAIR_ADV_MANAGER` Kconfig option that are listed in the :ref:`bt_fast_pair_adv_manager_config` section.
...
Before you can start using the Fast Pair Advertising Manager, you must first enable it.

The Fast Pair Advertising Manager is enabled with the :c:func:`bt_fast_pair_adv_manager_enable` function.
Before performing the :c:func:`bt_fast_pair_adv_manager_enable` operation, you must enable the Fast Pair subsystem with the :c:func:`bt_fast_pair_enable` function.
...
The module can be disabled with the :c:func:`bt_fast_pair_adv_manager_disable` function.
...
The module readiness can be checked with the :c:func:`bt_fast_pair_adv_manager_is_ready` function.
Some optional API functions must only be used in the *unready* state (when it is not enabled).
The following subsections describe them in more details.

Bluetooth identity configuration
--------------------------------

The custom Bluetooth identity configuration is optional.
The module uses the :c:macro:`BT_ID_DEFAULT` identity by default.
...
The Fast Pair Advertising Manager uses the information callbacks to notify the application about the module related events.

The callback registration is optional.
However, if you want to subscribe to information callbacks, you must register them with the :c:func:`bt_fast_pair_adv_manager_info_cb_register` function before the module is enabled with the :c:func:`bt_fast_pair_adv_manager_enable` function.
...
This :c:struct:`bt_fast_pair_adv_manager_info_cb` structure supports only one callback.

The :c:member:`bt_fast_pair_adv_manager_info_cb.adv_state_changed` provides notifications about the advertising state change.
...
When the advertising starts, the callback *active* parameter is set to ``true``.
Otherwise, when the advertising stops, the *active* parameter is set to ``false``.
...
The :c:func:`bt_fast_pair_adv_manager_is_adv_active` API function, that is associated with this asynchronous callback, can also be used to synchronously check the current state of the Fast Pair advertising.
...
Before you can start using the advertising trigger, you must first define it using the :c:macro:`BT_FAST_PAIR_ADV_MANAGER_TRIGGER_REGISTER` macro.
...
The :c:struct:`bt_fast_pair_adv_manager_trigger` structure is used to represent the advertising trigger.

The :c:macro:`BT_FAST_PAIR_ADV_MANAGER_TRIGGER_REGISTER` requires two mandatory parameters.
The first parameter defines the variable name of the :c:struct:`bt_fast_pair_adv_manager_trigger` structure and the second parameter defines the trigger identifier in the string format.

The :c:macro:`BT_FAST_PAIR_ADV_MANAGER_TRIGGER_REGISTER` macro also accepts the third optional parameter.
The parameter is a pointer to the :c:struct:`bt_fast_pair_adv_manager_trigger_config` structure that describes the trigger configuration.
The trigger configuration is a constant field of the :c:struct:`bt_fast_pair_adv_manager_trigger` structure.
Due to this, it cannot be changed at runtime.
If you do not need to configure the trigger, you can pass ``NULL`` as the third parameter.

Below, you can see an example of how to define a trigger for the user-initiated pairing mode that uses a dedicated configuration:
...
The trigger configuration is represented by the :c:struct:`bt_fast_pair_adv_manager_trigger_config` structure that is part of the :c:struct:`bt_fast_pair_adv_manager_trigger` structure and cannot be changed at runtime once it is defined.

Each trigger can be configured with the following options:

* :c:member:`bt_fast_pair_adv_manager_trigger_config.pairing_mode` - used to create a pairing mode trigger.
* :c:member:`bt_fast_pair_adv_manager_trigger_config.suspend_rpa` - used to create a trigger that suspends RPA rotations.
...
Otherwise, the not discoverable payload is used.
...
The change of the pairing mode state always causes the advertising payload update.

You can check if the Fast Pair Advertising Manager is in the pairing mode using the :c:func:`bt_fast_pair_adv_manager_is_pairing_mode` function.

Suspension of RPA rotations
---------------------------

The :c:member:`bt_fast_pair_adv_manager_trigger_config.suspend_rpa` option is used to create a trigger that suspends RPA rotations used by the Fast Pair advertising set.

The RPA rotation is suspended if at least one Fast Pair Advertising Manager trigger is active and is configured to suspend RPA rotations.

You should use this option with caution, as it decreases the privacy of the advertiser by freezing its RPA rotations.
It is typically used for triggers that are only active for a short period of time.

Trigger activation
==================

To activate and deactivate a specific trigger, you must use the :c:func:`bt_fast_pair_adv_manager_request` function.
The function requires pointer to the :c:struct:`bt_fast_pair_adv_manager_trigger` structure to identify the trigger instance and a boolean parameter that indicates whether the trigger should be activated or deactivated.
Initially, the trigger is in the inactive state.
...
The :c:func:`bt_fast_pair_adv_manager_request` function can also be called to set the trigger state when the Fast Pair Advertising Manager module is in the *unready* state and has not been enabled yet with the :c:func:`bt_fast_pair_adv_manager_enable` function.
However, in this case, the changes in the trigger state do not have any effect on the Fast Pair advertising.
When the module is in the *unready* state, the advertising is disabled regardless of the trigger state.
The advertising can be started in the context of the :c:func:`bt_fast_pair_adv_manager_enable` function if at least one trigger is activated before the enable operation.
...
This module acts as middleware between data providers and the application module for controlling Fast Pair advertising with triggers, which allows for flexible composition of advertising data.
...
The Fast Pair Advertising Manager requires the Fast Pair advertising data provider (:kconfig:option:`CONFIG_BT_ADV_PROV_FAST_PAIR`) to be enabled in the project Kconfig configuration.
...
To see how to implement the custom providers, see the :ref:`ug_bt_fast_pair_adv_manager_payload_custom` section.
...
Please note that providers can include their data conditionally based on the current advertising state.
It is necessary to ensure that data length is within the limits in all cases.
...
You can extend the advertising payload by implementing custom advertising data providers.
...
Here is an example that conditionally includes the Bluetooth Advertising Data (AD) with the Manufacturer Specific Data type and the battery level payload:
...
.. code-block:: c
   ...
   static uint8_t battery_level = 85; /* Example battery level */

   static int manufacturer_ad_provider_get_data(struct bt_data *d,
                                                const struct bt_le_adv_prov_adv_state *state,
                                                struct bt_le_adv_prov_feedback *fb)
   {
       static uint8_t manufacturer_data[MANUFACTURER_DATA_LEN];

       /* Only include manufacturer data when not in pairing mode and battery is low */
       if (state->pairing_mode || battery_level > 20) {
           return -ENOENT;
       }

       /* Create manufacturer-specific data with battery level */
       manufacturer_data[0] = 0xFF; /* Company ID LSB (example) */
       manufacturer_data[1] = 0xFF; /* Company ID MSB (example) */
       manufacturer_data[2] = battery_level;

       d->type = BT_DATA_MANUFACTURER_DATA;
       d->data_len = sizeof(manufacturer_data);
       d->data = manufacturer_data;

       return 0;
   }

   BT_LE_ADV_PROV_AD_PROVIDER_REGISTER(manufacturer_ad_provider, manufacturer_ad_provider_get_data);

   /* Function to update battery level */
   void battery_level_update(uint8_t new_level)
   {
       battery_level = new_level;
       /* Trigger advertising update if needed */
   }
...
The module supports use case-specific extensions that handle common advertising scenarios automatically.
...
If you want to support the specific use case, you must enable the corresponding Kconfig choice manually.
Currently, this layer supports only the locator tag use case.

Locator tag
===========

To enable the module extension for the locator tag use case, you must enable the :kconfig:option:`CONFIG_BT_FAST_PAIR_ADV_MANAGER_USE_CASE_LOCATOR_TAG` Kconfig option.

.. note::
   If the locator tag extension of this module does not fit your application requirements due to its underlying assumptions, you can desist from enabling the :kconfig:option:`CONFIG_BT_FAST_PAIR_ADV_MANAGER_USE_CASE_LOCATOR_TAG` Kconfig option and replace it with your custom implementation.
...
* The FMDN provisioning trigger used to maintain Fast Pair not discoverable advertising after the Account Key write operation.
  The FMDN provisioning trigger is active until the FMDN provisioning is completed or when the timeout period expires.
* The FMDN clock synchronization trigger used to maintain Fast Pair not discoverable advertising after a power loss to synchronize time between the locator tag and the phone.
  The FMDN clock synchronization trigger is active until the clock synchronization is completed or when the timeout period expires.
  This trigger can be disabled with the :kconfig:option:`CONFIG_BT_FAST_PAIR_ADV_MANAGER_USE_CASE_LOCATOR_TAG_CLOCK_SYNC_TRIGGER` Kconfig option and replaced with custom implementation.

See the :ref:`fast_pair_locator_tag` sample for an example of how to use locator tag extension of the Fast Pair Advertising Manager module in the example application.
