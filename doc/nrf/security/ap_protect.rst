...
.. list-table:: AP-Protect implementations
   ...
   * - Hardware and software
     ...
     - Issuing an ``ERASEALL`` command through CTRL-AP.
       ...
       Additionally:
        With nRF52, nRF53 and nRF91 Series, firmware must write ``SwDisable`` to ``APPROTECT.DISABLE``.

        With nRF54L series, firmware must open the `debugger signals in Tamper Controller <nRF54L15 Debugger signals>`_.
