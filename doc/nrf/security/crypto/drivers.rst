...
Driver selection in more detail
===============================
...
These directives are used by the driver wrapper to select the most appropriate driver for each cryptographic operation.
...
* You can configure the application to use CC310 and require the AES-192 key size through the :kconfig:option:`CONFIG_PSA_WANT_AES_KEY_SIZE_192` and :kconfig:option:`CONFIG_PSA_USE_CC3XX_KEY_MANAGEMENT_DRIVER` Kconfig options.
  ...
  If the CC310 peripheral does not support this feature, nRF Security selects the nrf_oberon driver instead, even if the nrf_cc3xx driver is available for the hardware platform.
...
* You can configure the application to require the SHA-512 hashing functionality through the :kconfig:option:`CONFIG_PSA_WANT_ALG_SHA_512` and :kconfig:option:`CONFIG_PSA_USE_CC3XX_HASH_DRIVER` Kconfig options.
  ...
  If the nrf_cc3xx driver is not available or does not support the SHA-512 hashing functionality, the wrapper falls back to the nrf_oberon software implementation.
...
#. If a specific hash algorithm is not supported, the enabled hardware-accelerated driver returns ``NOT_SUPPORTED``.
