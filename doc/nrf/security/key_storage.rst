...
The PSA Crypto API supports two methods for managing keys:

Key generation
  Use :c:func:`psa_generate_key` to create random keys.
  Generated keys can have one of the following lifetimes:

  * Volatile - Stored in RAM, lost on reset.
  * Persistent - Stored in PSA Internal Trusted Storage, retained during reboots.

Key import
  Use :c:func:`psa_import_key` to import existing key material.
...
.. note::
   ...
   When the key is imported from the NSPE, it temporarily resides in an untrusted environment.
   Whenever possible, it is safer to generate new keys without exposing them to the NSPE.
...
* Key derivation - HUK is used to derive other keys rather than directly perform cryptographic operations.
  Keys are then passed to the application, which does not use the HUK.
...
Do not use HUKs directly.
Instead, consider using the :ref:`PSA Protected Storage APIs <ug_psa_certified_api_overview_secstorage>` to use HUKs.
...
If you use the HUK library, ensure that the HUK is generated.
See the :ref:`TF-M Provisioning image <provisioning_image>` sample or the :ref:`Crypto persistent key usage <crypto_persistent_key>` sample for an example.
...
* Hardware protection - Encrypted using MKEK and stored in :ref:`data_storage_psa_protected` (when using TF-M) or in Zephyr's :ref:`secure_storage` subsystem or the :ref:`trusted_storage_readme` library (when not using TF-M).
...
* PSA's Internal Trusted Storage backend - Benefits from encryption and integrity protection.
