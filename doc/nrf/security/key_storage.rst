...
Use the :ref:`PSA Protected Storage APIs <ug_psa_certified_api_overview_secstorage>` to use them.

If you use HUKs through PSA Protected Storage without TF-M, for example through the :ref:`trusted_storage_readme` library, the HUK library will by default transparently encrypt storage entries.
...
If you use the HUK library, ensure that the HUK are written.
...
* Initial Attestation Key (for devices other than nRF54L Series, where the IAK is derived)
* User-defined keys
...
* Hardware protection - Encrypted using MKEK and either stored in :ref:`key_storage_kmu` (for devices other than nRF54L Series) or derived every time (for nRF54L Series devices).
