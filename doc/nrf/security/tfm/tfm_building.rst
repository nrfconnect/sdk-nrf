...
1. Enable :kconfig:option:`CONFIG_NRF_SECURITY` to enable PSA Crypto APIs through :ref:`nRF Security <nrf_security>`.
...
After building the application, the TF-M secure image enables the use of the hardware acceleration and the Kconfig configurations in the nRF Security subsystem control the features enabled in TF-M.
