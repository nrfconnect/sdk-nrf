...
* Generating random numbers using the random generator available in the device (see ``psa_generate_random()`` in the `crypto.h`_ file in the Oberon PSA Crypto repository).
