...
The following figure gives an overview of functionality and principles in Trusted Firmware-M and the entire firmware architecture in applications implementing TF-M in the |NCS|.
...
* Immutable Root of Trust (Immutable RoT) - "the combination of hardware and non-modifiable firmware and data installed during manufacturing" (as per `Trusted Base System Architecture for M (TBSA-M) Specification`_).
  For nRF5340 and nRF9160, the Immutable RoT consists of an :ref:`immutable_bootloader`, plus certain hardware peripherals such as the Key Management Unit (KMU) and One-Time Programmable (OTP) memory.
...
For details on the hardware implementation of Non-Secure Callable Interface, see the :term:`System Protection Unit (SPU)` documentation in the datasheets' Peripherals section.
For example, in the `nRF9160 Product Specification <nRF9160 System Protection Unit_>`_ or the `nRF9161 Product Specification <nRF9161 System Protection Unit_>`_.
...
Some SoCs, such as `nRF9160 <nRF9160 System Protection Unit_>`_ and `nRF9161 <nRF9161 System Protection Unit_>`_, use :term:`System Protection Unit (SPU)` hardware peripheral to handle TF-M Core functionalities.
