...
* Internal Trusted Storage - The Internal Trusted Storage API is used internally by the SPE to store data persistently in secure flash.
  The Internal Trusted Storage API is one of :ref:`ug_tfm_architecture_rot_services_platform` and is by default not available in the Non-Secure Callable interface.
...
For more information about the secure storage implementation in Nordic Semiconductor's SoCs and the |NCS|, see :ref:`secure_storage_in_ncs`.
