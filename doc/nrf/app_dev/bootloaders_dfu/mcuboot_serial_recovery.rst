...
If the ``MCUBOOT_PERUSER_MGMT_GROUP_ENABLED`` [optionTBC] Kconfig option is enabled, it can also support system-specific MCUmgr commands depending on the given mcuboot-port.
...
You can load an image to other slots only if you have enabled the ``MCUBOOT_SERIAL_DIRECT_IMAGE_UPLOAD`` [optionTBC] Kconfig option.
To use MCUboot's feature for progressively erasing a slot during image upload, enable the ``MCUBOOT_ERASE_PROGRESSIVELY`` [optionTBC] Kconfig option.
...
You can enable the :zephyr:code-sample:`smp-svr` implementation by setting the ``CONFIG_MCUBOOT_SERIAL`` [TBC] Kconfig option.
To set a type of an interface, use the ``BOOT_SERIAL_DEVICE`` [TBC] Kconfig, and select either the ``CONFIG_BOOT_SERIAL_UART`` or the ``CONFIG_BOOT_SERIAL_CDC_ACM`` value [TBC].
...
How to enable and configure the serial recovery feature depends on the given MCUboot build configuration.

.. +API for serial recovery (link to be delivered)
...
Enable the ``CONFIG_BOOT_SERIAL_WAIT_FOR_DFU`` [TBC] Kconfig option to use this mode.
The ``CONFIG_BOOT_SERIAL_WAIT_FOR_DFU_TIMEOUT`` [TBC] Kconfig option defines the amount of time in milliseconds the device will wait for the trigger.
...
You can use the MCUmgr command-line tool as an SMP client for evaluation purposes.
Install it by following the :ref:`installation instructions <dfu_tools_mcumgr_cli>`.

Configuring connection
======================

Use the following command to configure the connection:
...
To upload to a different slot, use the image upload MCUmgr command with a selected image number, ensuring the ``CONFIG_MCUBOOT_SERIAL_DIRECT_IMAGE_UPLOAD`` [TBC] Kconfig option is enabled.
...
Set the ``CONFIG_ENABLE_MGMT_PERUSER`` [TBC] Kconfig option to enable the following additional commands:

* Storage erase (``CONFIG_BOOT_MGMT_CUSTOM_STORAGE_ERASE`` [TBC]) - This Kconfig option allows erasing the storage partition.
* Custom image list (``CONFIG_BOOT_MGMT_CUSTOM_IMG_LIST`` [TBC]) - This Kconfig option allows fetching version and installation status (custom properties) for all images.
...
The device must be reset after updating or uploading a new firmware image through the serial recovery mode.
