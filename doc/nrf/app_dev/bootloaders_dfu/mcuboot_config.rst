...
For memory layout parameters, use devicetree to keep all memory-related configuration in a single location and improve consistency.

Examples of parameters that require synchronization include the following:

* NVM partitioning scheme.
...
To do so, set the ``compatible`` property to ``nordic,mcuboot``.

To configure the node to reference the MCUboot code partition, use the ``partitions`` property.
This reference is used by the MCUboot code to match the bootloader configuration that should be used.
