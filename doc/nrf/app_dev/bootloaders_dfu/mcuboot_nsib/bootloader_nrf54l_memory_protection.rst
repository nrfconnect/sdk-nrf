...
BOOTCONF protection is disabled by default on the nRF54L05, nRF54L10, and nRF54L15 SoCs, where the standard bootloader partitions exceed the 31-kB hardware limit.
On these SoCs, enable the relevant option explicitly only after sizing the immutable bootloader partition to no more than 31 kB.
The build fails if BOOTCONF protection is enabled and the bootloader partition exceeds the region-size limit of the selected SoC.
