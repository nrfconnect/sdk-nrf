...
In particular, it is the following include lines that enable them, with the first ``<runner>`` listed becoming the default runner:

.. code-block:: cmake

   include(${ZEPHYR_BASE}/boards/common/<runner>.board.cmake)

You can read more about runners in the Zephyr documentation in the :ref:`zephyr:flash-and-debug-support` and :ref:`zephyr:west-flashing` sections.
...
Programming the nRF54H20 DK
   ...
   Programming the nRF54H20 DK with the |NCS| version prior to v3.0.0 requires installing the `nrfutil device command <Installing and upgrading nRF Util commands_>`_ for the ``west flash`` command to work with this device.
