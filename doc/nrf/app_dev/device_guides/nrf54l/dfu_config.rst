...
.. note::
   ...
   The nRF54LS05A and nRF54LS05B SoCs currently do not support this configuration.
...
On the nRF54LS05A and nRF54LS05B SoCs, MCUboot can support RSA, ECDSA P-256, ed25519, and ed25519-pure signatures using software-based verification.
On these SoCs, ECDSA P-256 is recommended and is selected by default.
NSIB supports only the ed25519-pure signature, which is hardcoded.

Signature keys
**************

The :ref:`Key Management Unit (KMU)<ug_kmu_guides_cracen_overview>` retains the keys necessary for image signature verification.
...
MCUboot supports AES-encrypted images on the nRF54L Series, using ECIES-X25519 or ECIES-P256 for key exchange.
For detailed information on ECIES support, refer to the :ref:`ug_nrf54l_ecies` documentation page.
