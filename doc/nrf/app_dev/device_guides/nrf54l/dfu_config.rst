...
* MCUboot protection - MCUboot can be protected from overwrites using the FPROTECT library.
  ...
  To determine the protection mechanism for sizes between 31 kB and 62 kB, configure the :kconfig:option:`CONFIG_FPROTECT_ALLOW_COMBINED_REGIONS` Kconfig option.
...
Enable this option during the MCUboot build process if images signed with a compromised key contain critical vulnerabilities.
