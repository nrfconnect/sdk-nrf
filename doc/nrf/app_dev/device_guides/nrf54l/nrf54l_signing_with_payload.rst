...
1. Adjust the following partitions:
   ...
   * ``cpuflpr_code_partition`` - This partition defines where the FLPR code is placed on the storage device.
   ...
   .. note::
      ...
      The Partition Manager will ignore this setting while allocating space for the application running on CPUAPP, but the :ref:`FLPR minimal sample <vpr_flpr_nrf54l_initiating>` will still utilize it.
