...
Introducion to KMU key provisioning
###################################
...
In PSA abstraction key types are mapped by ``psa_set_key_lifetime`` attribute, refer to :ref:`PSA Key programming model<ug_nrf54l_crypto_kmu_key_programming_model>`.
...
Limitations of operating without TEE (TF-M)
*******************************************

Operating without Trusted Execution Environment (TEE) has a few limitations.
...
Revocable KMU keys are vulnerable to revocation by applications running in secure mode.
If you cannot trust the application running in the secure mode, you should provision locked keys.
This consideration is particularly important for applications without a TEE setup.
For this reason, you must evaluate the security aspects of your application.
