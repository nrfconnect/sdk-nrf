...
You can provision keys during the flashing when the build directory contains the :file:`keyfile.json` file with commands such as ``west flash --recover`` or ``west flash --erase``.
When flashing a project that contains NSIB, only the ``west flash --recover`` command is available, as the programming file contains UICR provisioning data as well.

The :file:`keyfile.json` file might be generated during the build process (for example, when running ``west build``), if the :Kconfig:option:`SB_CONFIG_SECURE_BOOT_GENERATE_DEFAULT_KMU_KEYFILE` and/or the :Kconfig:option:`SB_CONFIG_MCUBOOT_GENERATE_DEFAULT_KMU_KEYFILE` options are enabled.
