...
For MCUboot notice that it uses one key by default and KMU support in its configuration need to be enabled, otherwise MCUboot fallbacks to compiled in key.
