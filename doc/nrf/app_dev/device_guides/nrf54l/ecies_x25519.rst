...
An encrypted image includes a TLV that contains the public key used for ECIES-X25519 key exchange, the encrypted AES key, and the MAC tag of the encrypted key.
The key used to encrypt the AES key is derived using HKDF, and the MAC tag is generated using HMAC.
Both HKDF and HMAC currently use the SHA-256 hash algorithm.

While the use of SHA-256 does not pose a security concern and has minimal impact on performance, it increases code size because SHA-256 support must be included alongside SHA-512, which is already used by the ED25519 signature algorithm.
...
For the nRF54L15 SoC, the ED25519 signature algorithm is selected by default.

When encryption is enabled, the encrypted image is generated in the application build directory under the names :file:`zephyr.signed.encrypted.bin` and :file:`zephyr.signed.encrypted.hex`.

The BIN file is a binary image suitable for DFU operations using :ref:`MCUmgr<dfu_tools_mcumgr_cli>`.
...
Generally, enabling encryption in projects created using |nRFVSC| requires passing Kconfig options, mentioned in above paragraphs, to project configuration. This action is best described in :ref:`cmake_options`.
