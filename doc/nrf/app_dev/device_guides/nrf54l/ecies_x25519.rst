...
You will need to get familiar with `How to work with build configurations`_ page to be able to setup encryption in |nRFVSC|.
You will have to add the :kconfig:option:`SB_CONFIG_BOOT_ENCRYPTION` and :kconfig:option:`SB_CONFIG_BOOT_ENCRYPTION_KEY_FILE` using Extra CMake arguments.
In case when already existing project is being modified, it has to be re-generated, otherwise the new settings may not take effect.
