...
1. The push model - where all necessary candidate images are uploaded to the device along with the envelope before triggering the envelope processing.

#. The fetch model - where not all of the required payloads are uploaded to the device before triggering the envelope processing.
...
In the push model all of the candidate images have to be uploaded to the device before the envelope processing starts.
