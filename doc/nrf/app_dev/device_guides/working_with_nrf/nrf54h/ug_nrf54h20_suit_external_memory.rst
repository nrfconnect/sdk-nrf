...
#. Modify the application manifest file :file:`app_envelope.yaml.jinja2` by completing the following steps:
   ...
   #. Modify the ``suit-install`` sequence in the application manifest file (:file:`app_envelope.yaml.jinja2`) to load into RAM and boot the companion image before accessing the candidate images stored in the external memory:
      ...
      The update procedure robustness may be further improved by copying all integrity checks from the ``suit-install`` to the ``suit-candidate-verification`` sequence.

   #. Append flash companion binary as integrated payload inside SUIT envelope:
      ...
      The flash companion binary must be available before the driver to the external memory is booted, so it must be transferred as part of the update candidate envelope and fit within the DFU partition inside the internal MRAM memory.
...
#. If your application uses radio core, modify the radio manifest file :file:`rad_envelope.yaml.jinja2` by completing the following steps:
...
#. Modify the root manifest file :file:`root_with_binary_nordic_top_extflash.yaml.jinja2` by completing the following steps:
   ...
   #. If your application uses radio core, append to the ``suit-payload-fetch`` sequence:
...
* The companion image needs either a dedicated area in the executable region of the MRAM or fit within the local RAM that is assigned to the application domain.
