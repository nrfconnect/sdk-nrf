...
By default, the command uses the manifest templates from the ``default`` variant. Manifest template variants serve different use cases, as described in the following table.
...
In certain samples you may see the use of :kconfig:option:`SB_CONFIG_SUIT_BASE_MANIFEST_VARIANT`. 
This Kconfig exists only for the convenience of the |NCS| developers and its use is strongly discouraged in user applications.
You can safely remove this Kconfig as long as you have created your own custom manifest templates.
