...
The CRACEN driver implements driver-specific parts of the :ref:`PSA Crypto APIs <ug_psa_certified_api_overview_crypto>` and adds some vendor-specific functionalities to this API.
...
CRACEN is a hardware peripheral that provides hardware-accelerated cryptographic operations and entropy generation.
...
It can execute these operations using different types of keys, including keys that are stored in the KMU or derived from the :ref:`CRACEN Isolated Key Generator (IKG) <ug_kmu_guides_cracen_ikg>`.
...
The driver carries out :ref:`cryptographic operations <ug_crypto_supported_features>` using the CRACEN hardware peripheral and supports the following KMU operations:
