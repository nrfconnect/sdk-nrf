...
The nRF54L Series devices are equipped with two hardware peripherals that work together to provide secure cryptographic operations: the Key Management Unit (KMU) and the Crypto Accelerator Engine (CRACEN).
The hardware peripherals are supported in the |NCS| through the CRACEN driver, one of :ref:`nrf_security_drivers`.
The CRACEN driver provides a set of standard :ref:`PSA Crypto APIs <ug_psa_certified_api_overview_crypto>` (with some additional vendor-specific extensions).
