...
Secure key storage and provisioning
   ...
   This provides crucial security for symmetric keys, as the KMU can transfer them directly to CRACEN's protected RAM.
...
Key provisioning for cryptographic operations
   ...
   Once provisioned, using encryption or decryption keys does not expose the key material to the CPU.
...
Integration with Trusted Firmware-M
   ...
   This provides additional hardware-level key protection while maintaining compatibility with TF-M security services.
