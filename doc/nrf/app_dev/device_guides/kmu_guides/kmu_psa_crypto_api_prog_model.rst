...
Expand the following box to see how the CRACEN driver handles KMU metadata.
...
* Keys with the usage scheme Raw (``CRACEN_KMU_KEY_USAGE_SCHEME_RAW``) are temporarily pushed to a RAM location by the CRACEN driver, and then loaded by CRACEN into the asymmetric engine before running operations like sign or verify.
...
Depending on whether the application is built with TF-M or not:
