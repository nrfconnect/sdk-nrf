...
The sum of all size configurations specified in ``UICR.SECURESTORAGE.*`` must be 4kB aligned.
...
UICR.SECURESTORAGE.ADDRESS
  ...
  This address must be 4kB aligned and point to a valid memory region that can be used for secure storage.
...
This feature enables recovery scenarios and dual-firmware configurations.
...
UICR.SECONDARY.PROCESSOR
  ...
  * ``APPLICATION`` - Boot secondary firmware on the APPLICATION processor
  * ``RADIOCORE`` - Boot secondary firmware on the RADIOCORE processor
...
UICR.SECONDARY.ADDRESS
  ...
  The address must be aligned to a 4 kiB boundary (bits [11:0] are ignored).

UICR.SECONDARY.TRIGGER
  Configures automatic triggers that cause |ISE| to boot into secondary firmware instead of the primary firmware.

  UICR.SECONDARY.TRIGGER.ENABLE
    Controls whether automatic triggers are enabled for secondary firmware boot.

  UICR.SECONDARY.TRIGGER.RESETREAS
    Specifies which reset reasons will trigger automatic boot into secondary firmware.
    ...
    * ``RADIOCOREWDT0`` - Radiocore domain watchdog 0 reset
    * ``RADIOCOREWDT1`` - Radiocore domain watchdog 1 reset
    * ``RADIOCORELOCKUP`` - Radiocore domain CPU lockup reset
...
UICR.SECONDARY.WDTSTART
  ...
  Note that if RADIOCORE is specified in ``UICR.SECONDARY.PROCOSSOR`` the WDT instances used will be the ones in the radio domain.
...
The secondary firmware can be triggered automatically, through ``CTRLAP.BOOTMODE`` or through an IPC service.
