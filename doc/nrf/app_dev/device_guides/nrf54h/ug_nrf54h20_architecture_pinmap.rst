...
If a pin and the peripheral it is routed to are in the same domain, the peripheral will force the domain on when in use, thus powering the pin and the required multiplexers, as well.
...
It is possible to route pins across domains, though this is not recommended because of the following reasons:

* A fast peripheral in the FAST_ACTIVE_1 domain is limited by the slower slew rate of pins in the slow domain.
* The firmware must force on the domain the pins are in before the peripheral is used if power management (CONFIG_PM) is enabled.

To force on either the FAST_ACTIVE_1 or SLOW_MAIN power domain, power management is used as follows:
