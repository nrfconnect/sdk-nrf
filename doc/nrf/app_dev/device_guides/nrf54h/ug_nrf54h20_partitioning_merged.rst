...
A compatibility between those firmares can be ensured through a mutual versioning dependency scheme, where each image depends on the rest of images, forcing the newest available version for each of them.
...
* Include the modified memory map devicetree overlay file in all images (bootloader, application core, radio core).
  ...
  The main image (usuallly the application core) should define the overaly under the :file:`boards/<board_name>.overlay`.
...
#. Upload the opposite variant of the merged update candidate to the device i.e. using SMP protocol.
