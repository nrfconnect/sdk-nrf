...
To achieve optimal low power consumption, it is important to ensure that both the radio and application domains are populated with firmware, as each domain is responsible for sending its own power and clock requirements to the System Controller.

For reference implementations, refer to the :ref:`multicore_idle_test` samples.
These samples demonstrate configurations that leverage an always idle radio core combined with an active application on the application core.
