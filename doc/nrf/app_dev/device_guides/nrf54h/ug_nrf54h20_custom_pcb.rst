...
* ``mode`` - Defines the oscillator mode. Can be ``CRYSTAL`` for crystal operation, ``EXT_SINE`` for external sine wave input, or ``EXT_SQUARE`` for external square wave input.
* ``builtInLoadCapacitancePf`` - Specifies the built-in load capacitance in picofarads (pF).
  Valid range is 1 to 25 pF.
...
* ``startupTimeMs`` - Defines the startup time for the LFXO oscillator in milliseconds.
  Valid range is 1 to 25 ms.
* ``source`` - Specifies the low-frequency clock source.
  Can be ``LFXO`` when an external crystal oscillator is in place, or ``LFRC`` when an external crystal oscillator is not in place.
...
+------------------------+-------------------------------+
| HFXO.STARTUPTIME: TIME | startupTimeUs\ :sup:`1`       |
+------------------------+-------------------------------+

.. [1] Depends on ``"builtInLoadCapacitors": true``
