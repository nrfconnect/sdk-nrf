...
When the LFXO oscillator is used as the LFCLK clock source and LFOSC.LFXOCAL is ``0xFFFFFFFF`` at device boot, the firmware calibrates the LFXO oscillator and writes the calibration data to the LFOSC.LFXOCAL register.
...
Until calibration is complete, the SoC will use the LFRC oscillator.
...
.. note::
   If you select ``LFXO`` as the LFCLK source, the SoC might perform an automated LFXO calibration during startup (for example, after programming a new BICR configuration).
   This can take 3 to 3.5 seconds, during which the SoC must remain powered.
   Until calibration is complete, the SoC will use the LFRC oscillator.
