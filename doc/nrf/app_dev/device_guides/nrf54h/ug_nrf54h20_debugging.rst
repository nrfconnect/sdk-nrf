...
You can also attach to a running core using the ``west attach`` command.
...
A local domain reset triggers a global reset, which resets the debugging state. As a result, a new debugging connection is required.
To debug early boot issues, you can trigger a reset while keeping the cores in a halted state. This allows time to attach the debugger. The cores can then be started individually.

The following example demonstrates debugging the early boot phase on the application core:
