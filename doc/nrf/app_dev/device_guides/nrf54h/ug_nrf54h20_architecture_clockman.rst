...
The LFCLK source is selected by the clock control drivers based on system configuration (including BICR), and when ``LFXO`` is used the SoC might perform an automated LFXO calibration during startup; see :ref:`ug_nrf54h20_custom_pcb_bicr`.
