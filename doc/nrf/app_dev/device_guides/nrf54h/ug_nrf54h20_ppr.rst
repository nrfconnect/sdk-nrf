...
.. note::
   The PPR core support in the |NCS| is currently :ref:`experimental<software_maturity>`.

The nRF54H20 SoC includes a dedicated VPR CPU, based on RISC-V architecture, known as the *Peripheral Processor* (PPR).
You can use the PPR core to manage specific peripherals through the appropriate Zephyr Device Driver API.
These peripherals have IRQs routed to PPR:
...
The primary purpose of this snippet is to enable the transfer of the PPR code to the designated memory region (if required) and initiate the PPR core.
...
To mitigate this, use ``RAM_30`` exclusively for PPR code, PPR data, and non-time-sensitive information from the application CPU.
