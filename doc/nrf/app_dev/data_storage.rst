...
* NVS - For most applications requiring simple persistent storage.
  Stores data as values identified by an 16-bit integer ID.
* ZMS - Recommended for nRF54H and nRF54L Series devices, instead of NVS.
  Stores data as values identified by an 32-bit integer ID.
...
* Region protection - `Configurable memory regions <nRF54L15 REGION.CONFIG_>`_ with read, write, execute, among others.
...
* When used with TF-M, PSA Protected Storage is an :ref:`Application RoT Service <ug_tfm_architecture_rot_services_application>` that runs in the :ref:`Secure Processing Environment (SPE) <app_boards_spe_nspe>`.
  It is available to both NSPE and SPE, but data stored from one environment is not accessible from the other.
* When used without TF-M, PSA Protected Storage does not support storing data to external flash.
  Instead, at the time of writing to memory, the application will write PSA Protected Storage data to the same location as PS Internal Trusted Storage.
...
When using :ref:`TF-M <ug_tfm>`, PSA Protected Storage is automatically enabled when its requirements are met and the :kconfig:option:`CONFIG_TFM_PARTITION_PROTECTED_STORAGE` Kconfig option is enabled.
