...
* NVS - For most applications requiring simple persistent storage.
  Stores data as values identified by an integer ID.
* ZMS - Recommended for nRF54H and nRF54L Series devices, instead of NVS.
* RRAMC and ZMS - Recommended for nRF54L Series devices, instead of NVMC and NVS.
  RRAMC provides the hardware capabilities for RRAM memory management, while ZMS offers an optimized data management system that extends memory lifespan.
...
.. list-table:: Storage features comparison
   ...
   * - :ref:`NVS <data_storage_nvs>`
     ...
     - No
     - No
   * - :ref:`RRAMC for nRF54L <data_storage_rramc>`
     - Yes
     - Yes
     - Yes
     - No
   * - :ref:`ZMS for nRF54L and nRF54H20 <data_storage_zms>`
     - Yes
     - Yes
     - Yes
     - No
...
This is typically implemented using Cyclic Redundancy Check (CRC) or cryptographic hash functions, such as SHA-256.

The integrity can be inherited between a subsystem and its backends.
For example, the Settings subsystem can use different backends.
If the backend has integrity, Settings also does.
...
Authentication and encryption provide additional data-at-rest protection for stored data.
...
NVS stores data as values identified by an integer ID in non-volatile memory using a FIFO-managed circular buffer.
...
.. note::
   RRAMC is only available on the `nRF54L Series <nRF54L15 RRAMC_>`_ and should be used with :ref:`ZMS <data_storage_zms>`.
...
* Write without erase - RRAMC allows writing and overwriting without requiring erase operations.
...
* Region protection - Configurable memory regions with read, write, execute, and secure access controls.
...
* Power-failure protection - Built-in power-fail comparator to prevent writes during low voltage conditions.

The RRAMC provides several protection mechanisms, including:

* Region protection - Memory can be divided into configurable regions with different access permissions.
...
For more information, see the `nRF54L Series SoC datasheets <nRF54L15 RRAMC_>`_.
...
.. note::
   ZMS is recommended for the nRF54L and nRF54H Series devices.
   For nRF54L Series devices, ZMS should be used with :ref:`RRAMC <data_storage_rramc>` instead of NVMC and NVS.
...
ZMS stores data as values identified by an integer ID.
...
For more information, see :ref:`zms_memory_storage`.
...
It gives modules a way to store per-device configuration, which is organized in a tree structure and retained during reboots.
...
To use Settings with the NVS backend, enable the following Kconfig options:

* :kconfig:option:`CONFIG_SETTINGS`
* :kconfig:option:`CONFIG_SETTINGS_NVS`
* :kconfig:option:`CONFIG_NVS`

To use Settings with the ZMS backend, enable the following Kconfig options:
...
PSA Protected Storage can be used with or without :ref:`TF-M <ug_tfm>`:

* When used with TF-M, using PSA Protected Storage is mandatory.
  In such case, PSA Protected Storage is an :ref:`Application RoT Service <ug_tfm_architecture_rot_services_application>` that runs in the :ref:`Secure Processing Environment (SPE) <app_boards_spe_nspe>`.
  It is available to both NSPE and SPE, but data stored from one environment is not accessible from the other.
* When used without TF-M, PSA Protected Storage is optional for use in the |NCS|.
  In such case, PSA Protected Storage does not support storing data to external flash.
...
* You need data authentication and data encryption at rest.
...
* Choose NVS or ZMS when you need:
  ...
  * Lower overhead
  * No security isolation for data stored directly using NVS or ZMS

Configuration
-------------

When used with :ref:`TF-M <ug_tfm>`, PSA Protected Storage is automatically enabled.

When used without :ref:`TF-M <ug_tfm>`, you must enable the following Kconfig options:
