...
ESB monitor mode allows you to listen in on ESB communication between other devices, without sending any packets.
When ESB is configured as PRX or PTX, it switches between RX, TX, and DISABLE states.
This causes delays related to radio ramp-up time that is 40 µs when the value of the ``use_fast_ramp_up`` parameter is ``true`` and 130 µs without it.
To eliminate the delays, the monitor mode remains in the RX state until it is manually disabled.
This feature allows you to capture both packets and ACKs sent by other ESB devices, which can be useful when analyzing or debugging the protocol.
