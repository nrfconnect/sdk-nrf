...
A UE can operate in a variety of DRX modes.
...
The UE can negotiate with the network for a timer called as Active Time.
...
Proprietary Power Saving Mode (PSM) or Smart PSM
================================================
...
If the network notices this, it can perform local deregistration for the device, which would lead to a new registration and extra signalling when the device wakes up.
...
Connection Quality Indicator (CQI) or Energy estimation
=======================================================
...
The section below explins the Access Stratum (AS-RAI) in detail.
...
Therefore, when waiting to transit to RRC idle mode, a large timer value may unnecessarily increase power usage.
In RRC Connected mode, If the UE keeps the radio on for an extra 60 seconds each time.
The timer lasts from 5 to 60 seconds, which can significantly shorten the battery life.
