...
On the nRF54L SoC, Thread samples support :ref:`app_boards_spe_nspe` with Trusted Firmware-M (TF-M).
...
For example to build the Thread CLI sample for the nRF54L15DK board with the TF-M support, use the following command:
...
Depending on the platform you can use PSA Internal Trusted Storage or CRACEN Key Management Unit (KMU) for storing the Thread cryptographic materials.

By default, the Thread stack uses PSA ITS backend for storing the Thread cryptographic materials.
It is chosen by setting the :kconfig:option:`CONFIG_OPENTHREAD_PSA_NVM_BACKEND_ITS` Kconfig option to ``y``.
...
The PSA Internal Trusted Storage (ITS) is a software-based key storage solution that can be used to store the Thread cryptographic materials.
It is accelerated by the hardware-accelerated by the nRF crypto driers such as CryptoCell or CRACEN.
In Thread stack we use PSA ITS range defined in the :kconfig:option:`CONFIG_OPENTHREAD_PSA_ITS_NVM_OFFSET` Kconfig option, and by default it is set to ``0x20000``.
...
It is available on the nRF54L series SoCs, and Development Kits such as nRF54L15DK, nRF54L10DK, and nRF54L05DK.
...
To change the slots start number, set the :kconfig:option:`CONFIG_OPENTHREAD_KMU_SLOT_START` Kconfig option to the desired start slot number.
...
In the Thread stack there are the following cryptographic materials to be stored in the non-volatile memory:
...
.. list-table:: Thread cryptographic materials
   ...
   * - Network key
     ...
     - 1
   * - PSKc
     - Pre-shared key for the Thread network for the commissioner
     - Persisted
   ...
   * - Mesh Link Establishment (MLE) key
     ...
     - N/A
   * - PSKd
     - Pre-shared key for the Thread network for the device
     - Volatile
     - Asymmetric ECC secp256r1 key pair, ECDSA-SHA-256
     - 256 bits
     - 2
   ...
   * - Commissioning Credential
     - Human readable commissioning credential for the Thread network
     - Volatile
     - Password
     - 8-255 bytes
     - 1
