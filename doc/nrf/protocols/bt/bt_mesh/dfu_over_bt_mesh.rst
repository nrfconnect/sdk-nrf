...
.. note::
   ...
   If the application contains more than one composition data structure (for instance when the structure to use is picked at runtime), the script will not print any encoded metadata.
   In this case, the JSON file must be used to find the encoded metadata matching the composition data to be used by the device after the update.
