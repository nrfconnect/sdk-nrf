...
As part of this guide, you will create a custom cluster for the :ref:`Matter template <matter_template_sample>` sample.
:ref:`Matter manufacturer-specific <matter_manufacturer_specific_sample>` sample already contains a custom ``NordicDevkit`` cluster that you can use as a reference.
...
The cluster ``<code>`` is a 32-bit combination of the vendor ID and cluster ID and should be unique and not conflict with existing clusters.
The most significant 16 bits are the Vendor ID, and the least significant 16 bits are the cluster ID.
Vendor ID must be configured according to the Matter specification (section 2.5.2 Vendor Identifier).
Cluster ID for manufacturer-specific cluster must be in the range from ``0xFC00`` to ``0xFFFE``.
In the example, there is a cluster with the code ``0xFFF1FC01``, which means ``0xFFF1`` is the Test Manufacturer ID, and ``0xFC01`` is the cluster ID.
...
.. tabs::
   ...
   .. tab:: ``<cluster>``
      ...
      ``<cluster>`` defines the cluster and consist of the following:
   ...
   .. tab:: ``<deviceType>``
      ...
      ``<deviceType>`` elements define the device types that can be used in the cluster and consist of the following:
...
1. Run the following command:
   ...
   .. note::
      Command should be executed from your application's directory as ZAP tool searches recursively for the :file:`.zap` files in the current directory.

#. Add Endpoint with the new device type in the zap tool.
...
Generating zap files with ``--full`` option creates new source files under :file:`zap-generated/app-common` that need to override the default files located in the Matter SDK in :file:`zzz_generated/app-common` directory.
To override the path, you need to set the ``CHIP_APP_ZAP_DIR`` variable in the CMakeLists.txt file, pointing to the parent of generated :file:`app-common` directory, before initializing the Matter Data Model.
...
For example, the following code snippet shows how to modify the Matter template :file:`CMakeLists.txt` file with the new cluster:
