...
Currently, all :ref:`matter_samples` in the |NCS| use the :ref:`trusted_storage_readme` library as the secure storage backend for all supported platforms.
You can find an overview of the secure storage layer configuration supported for each |NCS| Matter-enabled platform in the :ref:`matter_platforms_security_support` section.
