...
The Nordic Semiconductor Wi-Fi devices support Promiscuous mode.
...
The Nordic Wi-Fi device supports raw 802.11 packet reception from a specific BSSID to which the device is connected in Station mode, when Promiscuous mode is enabled for the nRF Wi-Fi driver.
Promiscuous mode setting can be enabled when the Nordic Wi-Fi device operates in Station mode, however, raw IEEE 802.11 packets in Promiscuous mode are only received when the Nordic Wi-Fi device connects to a specific BSSID and operates as a connected station.
...
The IEEE 802.11 packet that is captured by the Nordic Wi-Fi device will be prepended with a proprietary header ``raw_rx_pkt_header``, which contains the following information about the received 802.11 packet:
...
An IEEE 802.11 packet captured by the Nordic Wi-Fi device in Promiscuous mode will be sent up to the nRF Wi-Fi driver, which will prepend a radio information header to the received packet and present the resulting set to the user application.
...
TX injection mode can be enabled concurrently with Promiscuous mode on the Nordic Wi-Fi device.
