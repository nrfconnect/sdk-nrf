...
The Nordic Semiconductor Wi-Fi devices can be used as offloaded raw transmit devices, where the Nordic Wi-Fi device can transmit frames at regular intervals utilizing very low power.
The contents of the frame as well as parameters such as frequency and channel of transmission are programmable.

The major functionality of transmitting the frames is offloaded to the Nordic Wi-Fi device, thereby placing minimal requirements on the host (mainly programming capability).
...
The offloaded raw transmit functionality of Nordic Wi-Fi devices can be utilized by using the APIs provided by the driver.
...
The power consumed by the Nordic Wi-Fi device during the offloaded raw TX operation depends on the following parameters:
