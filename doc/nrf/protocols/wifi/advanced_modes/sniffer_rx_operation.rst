...
The Nordic Semiconductor Wi-Fi devices support Monitor mode.
You must configure the Nordic Wi-Fi device in Monitor mode to operate it as an 802.11 wireless packet sniffer.
...
Monitor mode can be enabled or disabled on the Nordic Wi-Fi device at runtime, using the ``NET_REQUEST_WIFI_MODE`` network management runtime API.
...
Monitor mode on the Nordic Wi-Fi device cannot operate concurrently with other modes, such as Station mode or SoftAP mode.

You must configure the operating Wi-Fi® channel on which the Nordic Wi-Fi device will monitor and receive 802.11 packets.
When the device operates in Monitor mode, all 802.11 packets received on the configured channel will be sent up the stack by the Nordic Wi-Fi device and the nRF Wi-Fi driver.

To set the desired channel for 802.11 packet reception, use the ``NET_REQUEST_WIFI_CHANNEL`` runtime network management API.
The channel configuration runtime network management API can be used to set the channel after setting the Nordic Wi-Fi device in Monitor mode.
...
The IEEE 802.11 packet that is captured by the Nordic Wi-Fi device will prepend a proprietary header ``raw_rx_pkt_header``, which contains the following information about the received 802.11 packet:
...
An IEEE 802.11 packet captured by the Nordic Wi-Fi device in Monitor mode will be sent up to the nRF Wi-Fi driver, which will prepend a radio information header to the received packet and present the resulting set to the user application.
...
TX injection mode can be enabled for operation when the Nordic Wi-Fi device is configured to operate in Monitor mode.
