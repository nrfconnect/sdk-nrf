...
The Nordic Semiconductor Wi-Fi device supports the transmission of raw IEEE 802.11 packets.
Raw IEEE 802.11 packets are packets that are not modified by the 802.11 Medium Access Control (MAC) layer during transmission by the Nordic Wi-Fi device.
...
The MAC layer on the Nordic Wi-Fi device will transmit the raw 802.11 packets over-the-air using the provided packet data, TX, and PHY parameters.
...
The Nordic Wi-Fi device supports raw 802.11 packet transmission when TX injection mode is enabled in the nRF Wi-Fi driver.

The TX injection mode can be enabled when the primary mode of operation for the Nordic Wi-Fi device is set to either Station mode or Monitor mode.
When the Nordic Wi-Fi device is configured in Station mode, TX injection mode can be enabled regardless of whether the device is connected to an Access Point (AP) or not.
...
This channel will be used as the transmit channel when the Nordic Wi-Fi device operates in Station mode, but is not connected to an AP.
When the Nordic Wi-Fi device operates in Station mode and is connected to an AP on a given channel, the connected channel will be used for raw 802.11 packet transmission.
When the Nordic Wi-Fi device operates in Monitor mode, the configured channel for Monitor mode will be used as the transmit channel for TX injection operation.
...
* The Frame Check Sequence (FCS) is not added to the packet by the application and is concatenated by the Nordic Wi-Fi device.

.. note::
   If the packet is not a properly framed 802.11 packet, it will be dropped by the Nordic Wi-Fi device and will not be transmitted over-the-air.
...
The packet is forwarded to the Nordic Wi-Fi device, which transmits the packet over-the-air.
All 802.11 MAC layer rules for the 802.11 packet (such as, link-layer acknowledgements, retransmissions) are handled by the Nordic Wi-Fi device.
