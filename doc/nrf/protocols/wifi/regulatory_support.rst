...
.. note::
   The supported bands, channels, and regulatory domain configuration described on this page apply to the nRF70 companion ICs.
   Other Nordic Semiconductor Wi-Fi devices may support additional bands and use their own Kconfig options.

The Nordic Wi-Fi devices operate in the license-exempt 2.4 GHz and 5 GHz radio frequency spectrum bands.
...
Once a regulatory domain is configured, the following restrictions are imposed on the operation of the Nordic Wi-Fi device:
...
Without user configuration, the Nordic Wi-Fi device will default to operating with world regulatory rules and utilize regulatory hints extracted from the AP beacon packet in order to adjust to more appropriate regional rules.
...
Once the Nordic Wi-Fi device disconnects from the AP, the device restores the regulatory region to world regulatory (code: 00) if automatic mode was enabled.
...
.. note::
   The maximum allowed transmit power values (shown in the table) may exceed the actual transmit power values for the Nordic Wi-Fi device.
