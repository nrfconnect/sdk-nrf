...
The Nordic Semiconductor Wi-Fi device supports multiple power save modes, enabling the device to minimize power consumption by shutting down most of the digital logic and RF circuits.

.. _ug_nrf70_developing_powersave_device_states:

Nordic Wi-Fi device states
**************************

The power save state of the Nordic Wi-Fi device is described through a combination of the physical power state of the logic or circuits and the logical functional state as observed by the access point (AP).

Power state
===========

The Nordic Wi-Fi device can be in one of the following power states:

.. note::
   The specific power-consumption values and the wake behavior described in this section are for the nRF70 companion ICs.
...
The transitions of the Nordic Wi-Fi device to and from the Shutdown state are automatically managed by the nRF Wi-Fi driver.
When the network interface is brought down, the nRF Wi-Fi driver puts the Nordic Wi-Fi device in Shutdown state.
When the network interface is brought up, the nRF Wi-Fi driver puts the Nordic Wi-Fi device in Active state.
...
The functional state information of the Nordic Wi-Fi device at the AP database can be either Active or Power Save.
...
The Nordic Wi-Fi device can be configured in one of the following Power Save modes:
...
* The default value for the Nordic Wi-Fi device is 100 ms.
...
.. note::
  ...
  The Nordic Wi-Fi device consumes less power in Power Save mode, that is, when the inactivity timer value is ``0``, in low traffic scenarios.
...
The Nordic Wi-Fi devices use DTIM-based Power Save mode by default.
...
The Legacy Power Save mode is the default option in the Nordic Wi-Fi devices.
...
When operating in Trigger Enabled mode, the Nordic Wi-Fi device:
...
When operating in Non-trigger Enabled mode, the Nordic Wi-Fi device:
...
When operating in Power Save mode, the Nordic Wi-Fi device can remain idle for extended periods, which may cause such access points to silently drop the connection.
...
The null data frame is an IEEE 802.11 MAC data frame with no payload that is generated and transmitted by the Nordic Wi-Fi device firmware.
...
When operating in DTIM-based Power Save mode, the Nordic Wi-Fi device:
