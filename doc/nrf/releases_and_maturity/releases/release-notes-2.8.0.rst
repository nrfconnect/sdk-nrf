...
See the `Migration notes for nRF Connect SDK v2.8.0`_ for the changes required or recommended when migrating your application from |NCS| v2.7.0 to |NCS| v2.8.0.
...
* Added:
  ...
  * A section in the sysbuild-related migration notes about the migration of :ref:`child_parent_to_sysbuild_migration_qspi_xip` from child/parent image to sysbuild.
...
* Updated:
  ...
  * Storing the Device Attestation Certificates private key in the Key Management Unit (KMU) to be enabled for nRF54L Series devices.
    See ``matter_platforms_security_dac_priv_key_kmu`` for more information.
