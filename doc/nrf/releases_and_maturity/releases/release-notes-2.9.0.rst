...
* Added the :ref:`nRF54l_signing_app_with_flpr_payload` page that includes instructions for building separate applications, merging them, and signing them for MCUboot.
...
* Updated:
  ...
  * The nRF54L15 DK's configurations (``nrf54l15dk/nrf54l15/cpuapp``) to enable hardware cryptography for the MCUboot bootloader.
    ...
    The change increases the MCUboot partition size (modifies Partition Manager configuration) and changes the MCUboot image signing algorithm.
    Because of that, the nRF Desktop application images built for an nRF54L15 DK from the |NCS| v2.9.0 are not compatible with MCUboot bootloader built from previous releases.
  ...
  * The :ref:`nrf_desktop_ble_conn_params` to:
    ...
    * Improve the log to display also the information if USB is suspended.
...
* Updated:
  ...
  * :ref:`power_profiling` sample that now has added support for the :ref:`zephyr:nrf54h20dk_nrf54h20` board target.
...
* :ref:`lib_nrf_cloud` library:
  ...
  * Fixed an issue that would cause resumption to fail when FOTA images are downloaded using the CoAP backend.
