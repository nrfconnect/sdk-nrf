...
* Added:
  ...
  * Support for GATT long/reliable writes (:kconfig:option:`CONFIG_BT_ATT_PREPARE_COUNT`) to Fast Pair and Works With ChromeBook (WWCB) configurations.
    This allows to perform :ref:`fwupd <nrf_desktop_fwupd>` DFU image upload over Bluetooth LE with GATT Clients that do not perform MTU exchange (for example ChromeOS using Floss Bluetooth stack).
