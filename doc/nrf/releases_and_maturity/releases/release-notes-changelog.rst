...
* :ref:`lib_at_host` library:
  ...
  * Fixed a bug where AT responses would erroneously be logged on the logging UART instead of being written to the chosen ``ncs,at-host-uart`` UART device when :kconfig:option:`CONFIG_LOG_BACKEND_UART` was set.
