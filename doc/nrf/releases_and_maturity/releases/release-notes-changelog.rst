...
* :ref:`nrf_desktop_config_channel_script` Python script:
  ...
  * Updated:
    ...
    * HID device discovery to ensure that discovery failure of a HID device would not affect other HID devices.
      Without the change, problems with discovery of a HID device could lead to skipping discovery and listing of other HID devices (even if the devices work properly).
