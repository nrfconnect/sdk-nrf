...
SE0: BICR CRC is not populated or recalculated
  ...
  If a strong magnetic field corrupts a BICR that was generated without the BICR CRC fix from `Zephyr commit a6d085`_, the device does not detect the corruption, attempts to use an invalid BICR configuration, and does not boot.
