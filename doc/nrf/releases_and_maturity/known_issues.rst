...
KRKNWK-21227: RXERROR event not detected during NFC frame reception
  ...
  **Workaround:** Remove ``RXERROR`` from the interrupt mask and instead poll the ``RXERROR`` event register when ``RXFRAMEEND`` occurs.
  This ensures the error state is captured atomically during frame end processing, regardless of when the error occurred within the frame.
  See `nrfx PR #1078`_ for the fix.
