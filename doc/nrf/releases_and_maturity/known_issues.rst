...
NCSDK-35650: Possible problems with sending HID input reports over Bluetooth LE after reconnection
  ...
  Enabling the feature leads to introducing an additional network buffer reference with the :c:func:`net_buf_ref` function to extend the lifetime of the network buffer representation until data transmission is confirmed by an ACK from the remote.
  ...
  That leads to missing net buffer reference removal.
