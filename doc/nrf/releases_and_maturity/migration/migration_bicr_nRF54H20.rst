...
.. note::
   If you select ``LFXO`` as the low-frequency clock source, the SoC might perform an automated LFXO calibration during startup when no calibration has been stored yet (for example, when ``LFOSC.LFXOCAL`` is ``0xFFFFFFFF`` after programming a new BICR configuration).
