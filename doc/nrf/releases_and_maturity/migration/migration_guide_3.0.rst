...
.. toggle::
   ...
   * The default runner for the ``west flash`` command has been changed to use `nRF Util`_ instead of ``nrfjprog``, which was part of the archived `nRF Command Line Tools`_.
     ...
     * nRF21540
     ...
     It is recommended to install nRF Util to avoid potantial issues during programming.
     Complete the following steps:

     1. Follow the steps for `Installing nRF Util`_ in its official documentation.
     2. Install the `nrfutil device <Device command overview_>`_ by running the following command:

        .. code-block::

           nrfutil install device

     If you prefer to continue using nrfjprog for programming devices, you can do so by :ref:`specifying the west runner <programming_selecting_runner>` with ``west flash``.
