...
.. toggle::
   ...
   * The OpenThread samples have been updated to use the new architecture option to use the OpenThread stack directly to communicate with the IEEE 802.15.4 radio driver.
     ...
     * :kconfig:option:`CONFIG_NET_L2_OPENTHREAD`
     ...
     For example, to enable the Zephyr networking layer in the :ref:`ot_cli_sample` for nRF54L15 DK, build the sample with the following command:
     ...
     * :kconfig:option:`CONFIG_OPENTHREAD_L2_DEBUG_DUMP_IPV6`
     ...
     For example, to enable logging from the Zephyr networking layer in the :ref:`ot_cli_sample` for nRF54L15 DK, build the sample with the following command:
