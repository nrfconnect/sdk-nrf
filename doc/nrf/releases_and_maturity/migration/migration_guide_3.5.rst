...
.. toggle::
   ...
   * Fast port control for the nRF54L series:
     ...
     * The slew of the GPIO pins on P2 in the E0 and E1 drive mode is now configured in the devicetree and defaults to the highest value (3) if not explicitly set, compared to 0 previously.
       It is set using the optional ``hs-bias`` property of the GPIO P2 devicetree node with nodelabel ``gpio2`` of nRF54L series SoCs:
