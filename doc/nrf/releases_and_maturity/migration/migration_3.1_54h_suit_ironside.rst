...
* You have a working |NCS| 3.0.0 application for the nRF54H20 SoC using SUIT.
* You have installed |NCS| 3.1.0 and its toolchain.
...
* SUIT support is removed in |NCS| 3.1.0.
...
To update prj.conf, do the following:
...
The Zephyr build invokes the :file:`gen_uicr.py` script (:file:`soc/nordic/common/uicr/gen_uicr.py` in the Zephyr tree) via ``nrf-regtool`` in the NCS sysbuild system.
...
Both HEX files must be flashed alongside your firmware image.
