...
Migrating BICR from DTS to JSON
###############################
...
In nRF Connect SDK v3.0.0, the format of the BICR configuration file was changed from DTS to JSON files.
...
* `JSON format definition`_: ``bicr-schema.json`` (from nRF Connect SDK v3.0.0).
...
Field-matching DTS to JSON
==========================

This section disects the ``nrf54h20dk_bicr.dtsi`` BICR configuration example in DTS format and explains how to port the different parts to JSON.
...
However, the nRF54H20 currently supports only two hardware layout options.
...
.. note::
   Configuration values use hexadecimal format, even if the ``0x`` prefix is not shown in DTS syntax.
...
For more information, consult the datasheet.
...
These DTS options configure parts of the ``BICR`` → ``LFOSC.LFXOCONFIG`` register.
...
.. note::
   In the JSON configuration, the load capacitance parameter is only applied if the option ``builtInLoadCapacitors`` is explicitly set to true, as per previous example.
...
Use these parameters to precisely control LFRC autocalibration behavior.
...
.. note::
   ...
   * Use the new default values in please of the old values from the DTS version.
...
For reference, the v3.0.0 DK files indicate that the default values have been updated since v2.9.0.
