...
As described in :ref:`_otdoa_overview`, the hellaPHY OTDOA firmware subsystem consists of a binary library
(delivered in binary object code format), and the adaption layer (delivered as source code as part
...
Adaption layer includes a workqueue thread that allows it to interact with a network server to download
the uBSA.
...
The buffering of the PRS samples is done using a memory slab in the hellaPHY OTDOA adaption layer.
