...
* :kconfig:option:`CONFIG_NTN_MODEM_LOCATION_REQ_MARGIN` - Margin in seconds for requesting location when the modem needs location at a specific time.
...
The :c:enumerator:`NTN_EVT_MODEM_LOCATION_UPDATE` event is sent to notify the application when location updates are requested and the accuracy requested by the modem.

When location updates are requested, the application must call the :c:func:`ntn_modem_location_set` function with the current location.
...
The validity time parameter in the :c:func:`ntn_modem_location_set` function call must be longer than the time between subsequent location updates.
The location can be invalidated using the :c:func:`ntn_modem_location_invalidate` function.
...
You can configure this margin using the :kconfig:option:`CONFIG_NTN_MODEM_LOCATION_REQ_MARGIN` Kconfig option.
