...
* Extension compatibility - Is compatible with the Fast Pair extensions that are supported by the :ref:`bt_fast_pair_readme` module (for example, the Find Hub Network extension).
...
* :kconfig:option:`BT_FAST_PAIR_ADV_MANAGER_USE_CASE` - This Kconfig option allows to select the Fast Pair use case (device type) and to enable additional advertising behaviors that are specific to your use case.
  ...
  * :kconfig:option:`CONFIG_BT_FAST_PAIR_ADV_MANAGER_USE_CASE_LOCATOR_TAG` - The option provides a tailored configuration for Fast Pair locator tags that use the FHN extension.
    It depends on the :kconfig:option:`CONFIG_BT_FAST_PAIR_FHN` Kconfig option.
...
.. note::
   If the FHN extension is used, the FHN advertising configures the RPA timeout in the FHN provisioned state.
   This module is tightly integrated with the FHN extension and respects this requirement.
...
The module is compatible with the Find Hub Network (FHN) extension and handles the complex advertising state transitions automatically.
The advertising policy of the Fast Pair advertising set respects the requirements of the FHN advertising set.
The module rotates the RPA and the payload of the Fast Pair advertising set synchronously with the FHN advertising set.

The module yields control over the RPA timeout configuration to the FHN advertising set once the FHN provisioning is completed.
...
The locator tag extension registers the :c:struct:`bt_fast_pair_info_cb` and the :c:struct:`bt_fast_pair_fhn_info_cb` callback structures to manage the advertising triggers that are specific to the locator tag use case.

This extension defines the FHN provisioning trigger that is used to maintain the Fast Pair advertising between the Owner Account Key write (:c:member:`bt_fast_pair_info_cb.account_key_written`) and the successful FHN provisioning (:c:member:`bt_fast_pair_fhn_info_cb.provisioning_state_changed`).
This advertising trigger is also configured to suspend the RPA rotation during the FHN provisioning.
If the FHN provisioning is not completed within the allowed time, the extension deactivates the FHN provisioning with the :c:func:`bt_fast_pair_adv_manager_request` function.
The trigger behavior is aligned with the requirements of the `Fast Pair Locator Tag Specific Guidelines`_ section of the FHN Accessory specification.

.. note::
   If the FHN provisioning trigger is deactivated prematurely with the :c:func:`bt_fast_pair_adv_manager_disable` function, it will not be restored during the :c:func:`bt_fast_pair_adv_manager_enable` function call.
...
The extension also defines the FHN clock synchronization trigger that is used to maintain the Fast Pair advertising after a power loss until the clock synchronization (:c:member:`bt_fast_pair_fhn_info_cb.clock_synced`) is completed.
