...
* The Beacon Actions GATT characteristic when the Find Hub Network (FHN) extension is enabled (:kconfig:option:`CONFIG_BT_FAST_PAIR_FHN`).
  The characteristic is described in the `Fast Pair Find Hub Network extension`_ documentation.
...
* :kconfig:option:`CONFIG_BT_FAST_PAIR_FHN` - The option enables the `Fast Pair Find Hub Network extension`_.

  * :kconfig:option:`CONFIG_BT_FAST_PAIR_FHN_DULT` - The option enables the Detecting Unwanted Location Trackers (DULT) support in the FHN extension (see :ref:`ug_bt_fast_pair_prerequisite_ops_fhn_dult_integration`):

    * :kconfig:option:`CONFIG_BT_FAST_PAIR_FHN_DULT_MANUFACTURER_NAME` - The option configures the manufacturer name parameter.
    * :kconfig:option:`CONFIG_BT_FAST_PAIR_FHN_DULT_MODEL_NAME` - The option configures the model name parameter.
    * :kconfig:option:`CONFIG_BT_FAST_PAIR_FHN_DULT_ACCESSORY_CATEGORY` - The option configures the accessory category parameter.
  ...
  * There are following advertising configuration options for the FHN extension (see :ref:`ug_bt_fast_pair_advertising_fhn`):
    ...
    * :kconfig:option:`CONFIG_BT_FAST_PAIR_FHN_MAX_CONN` - The option configures a maximum number of FHN connections.
  ...
  * There are following ringing configuration options for the FHN extension (see :ref:`ug_bt_fast_pair_gatt_service_fhn_ring_callbacks`):
    ...
    * :kconfig:option:`CONFIG_BT_FAST_PAIR_FHN_RING_VOLUME` - The option enables ringing volume support.
    * :kconfig:option:`CONFIG_BT_FAST_PAIR_FHN_RING_REQ_TIMEOUT_DULT_BT_GATT` - The option configures the ringing timeout for connected peers that use DULT-based ringing mechanism.
      You can only use this option when the :kconfig:option:`CONFIG_BT_FAST_PAIR_FHN_DULT` Kconfig option is enabled.
...
.. note::
   If you use the FHN extension, and your Provider is provisioned as an FHN beacon, do not use the :c:func:`bt_le_oob_get_local` function.
...
The FHN extension (see :kconfig:option:`CONFIG_BT_FAST_PAIR_FHN`) selects the :kconfig:option:`CONFIG_BT_EXT_ADV` Kconfig option.
...
For more details regarding the advertising policy of the FHN extension, see the :ref:`Setting up Bluetooth LE advertising <ug_bt_fast_pair_advertising>` section of the Fast Pair integration guide.

DULT module for the FHN extension
---------------------------------

The :kconfig:option:`CONFIG_BT_FAST_PAIR_FHN_DULT` Kconfig option of the FHN extension selects the :kconfig:option:`CONFIG_DULT` Kconfig option to enable the DULT module.
...
The :kconfig:option:`CONFIG_BT_FAST_PAIR_FHN_BATTERY_DULT` Kconfig option of the FHN extension selects the :kconfig:option:`CONFIG_DULT_BATTERY` Kconfig option to enable the battery support in the DULT module.
