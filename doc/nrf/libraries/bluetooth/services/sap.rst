...
After authentication, it derives an application AES-GCM transport key for protected application traffic.
...
Do not ship the sample credentials.
Production devices must be provisioned with product-specific private keys and certificates.
...
Enable the library with :kconfig:option:`CONFIG_BT_SAP`.
...
The library invokes :c:struct:`bt_sap_cb` callbacks when authentication succeeds, authentication fails, a protected payload is received, or a session disconnects.
