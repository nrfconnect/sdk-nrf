...
* **Trigger-based advertising configuration**: Provides flexible configuration of the advertising parameters such as the pairing mode and RPA rotation behavior based on trigger requirements.
...
.. note::
   The module is designed to work in cooperative thread context.
...
The Fast Pair Advertising Manager is enabled with the :kconfig:option:`CONFIG_BT_FAST_PAIR_ADV_MANAGER` Kconfig option.
...
With the :kconfig:option:`CONFIG_BT_FAST_PAIR_ADV_MANAGER` Kconfig option enabled, the following Kconfig options are available for this module:

* :kconfig:option:`BT_FAST_PAIR_ADV_MANAGER_USE_CASE` - This Kconfig choice is defined to select the Fast Pair use case (device type) and to enable additional advertising behaviours that are specific to the chosen use case.

  * :kconfig:option:`CONFIG_BT_FAST_PAIR_ADV_MANAGER_USE_CASE_UNSPECIFIED` - The option provides a neutral configuration without use case specific behaviors.
...
The :kconfig:option:`CONFIG_BT_FAST_PAIR_ADV_MANAGER` Kconfig option depends on several Bluetooth LE features:
...
2. **Configuration**: The majority of the configuration (such as information callbacks or Bluetooth identity) is performed before enabling the module.
3. **Enabling**: The module is enabled with the :c:func:`bt_fast_pair_adv_manager_enable`, which requires that the Fast Pair subsystem is ready (:c:func:`bt_fast_pair_enable`).
4. **Operation**: Triggers can request advertising activation or deactivation.
5. **Disabling**: The module is disabled with the :c:func:`bt_fast_pair_adv_manager_disable` before the Fast Pair subsystem is disabled.
...
Additionally, during advertising set creation, the advertising parameters are set:

* Advertising interval ``100ms``

  .. note::
     Currently, the advertising interval is fixed and cannot be changed with the Kconfig option or API functions.
...
This operation happens right after the advertising set creation and before the advertising process is started.

The module may start the advertising in the :c:func:`bt_fast_pair_adv_manager_enable` context if the appropriate conditions are met.
...
.. note::
   In case the FMDN extension is used, the FMDN advertising configures the RPA timeout in the FMDN provisioned state.
   This module is tightly integrated with the FMDN extension and respects this requirement.

The timeout value is recalculated for every RPA rotation and is being randomized to increase privacy.
...
The RPA rotation is suspended if any active advertising trigger has :c:member:`bt_fast_pair_adv_manager_trigger_config.suspend_rpa` flag set to ``true``.
...
* The :c:func:`bt_fast_pair_adv_manager_payload_refresh` API function is called and the advertising is ongoing.
...
The module tracks the advertising activity and is able to report each change with the :c:member:`bt_fast_pair_adv_manager_info_cb.adv_state_changed` callback to the application.
...
* Active connections with the :c:func:`bt_conn_disconnect` function.
  This disconnect operation is only confined to the connections that are derived from the Fast Pair advertising set.
...
By default, the :kconfig:option:`CONFIG_BT_FAST_PAIR_ADV_MANAGER_USE_CASE_UNSPECIFIED` is enabled which means that this layer is disabled.

If this default configuration is changed and the specific use case is selected, the core library uses the enable callback to activate the use case extension in the :c:func:`bt_fast_pair_adv_manager_enable` function.
...
The trigger behaviour is aligned with the requirements of the `Fast Pair Locator Tag Specific Guidelines`_ section of the FMDN Accessory specification.
