...
Release for modem firmware version 1.3.7 and 2.0.2.
...
* Removed the configuration ``disable_bootstrap_from_smartcard`` (and corresponding Kconfig ``LWM2M_CARRIER_BOOTSTRAP_SMARTCARD``).

  * Instead, the LwM2M carrier library will bootstrap from smartcard if the external :ref:`lib_uicc_lwm2m` library is included in the project. (:kconfig:option:`UICC_LWM2M`)

* Changed the CoAP response if an unsupported image is pushed during in-band FOTA.

  * Previous CoAP return value was ``4.00 Bad Request``, and is now ``5.00 Internal Server Error``.

* Changed the timing of the lifetime update in order to better allow CoAP retransmissions. (Effectively, the updates will now trigger earlier.)

  * This can reduce the amount of overall signalling needed.
    For example: in poor network conditions, the Server could have de-registered the client prematurely, when in fact the client was still retransmitting and attempting to update with the server.

* The library now uses an internal workqueue instead of the system workqueue.
...
* Extended the use use of the modem library hooks (:c:macro:`NRF_MODEM_LIB_ON_CFUN`) to better handle changes in the modem functional states.
