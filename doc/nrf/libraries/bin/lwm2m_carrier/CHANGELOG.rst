...
* Added a new event :c:macro:`LWM2M_CARRIER_DEFERRED_SIM_MSISDN`.
  ...
  * This event can trigger on devices with new a SIM card which has not been registered on the network yet.
...
* Changed the :c:func:`lwm2m_os_lte_link_up` function to perform asynchronous connect.
* Removed the following unused functions from glue layer: ``lwm2m_os_sec_ca_chain_exists()``, ``lwm2m_os_sec_ca_chain_cmp()``, ``lwm2m_os_sec_ca_chain_write()``.
