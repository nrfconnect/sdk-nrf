...
* Enabling the :kconfig:option:`CONFIG_NRF_SECURITY` replaces using the Kconfig option :kconfig:option:`CONFIG_NORDIC_SECURITY_BACKEND` to enable the legacy crypto support.
...
These legacy crypto backends are provided as "alternative implementation" of the drivers that are also used for the PSA Crypto API support.
