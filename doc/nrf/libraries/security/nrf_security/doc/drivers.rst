...
Note that on nRF54L Series devices CRACEN is the only source of entropy, so disabling :kconfig:option:`CONFIG_PSA_CRYPTO_DRIVER_CRACEN` is not possible when the Zephyr entropy driver has been enabled.
