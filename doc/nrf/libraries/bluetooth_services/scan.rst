...
Use the :kconfig:option:`CONFIG_BT_SCAN_CONN_ATTEMPTS_FILTER` to enable this filter.
...
You can increase the device number by setting the configuration option :kconfig:option:`CONFIG_BT_SCAN_CONN_ATTEMPTS_FILTER_LEN`.
The option :kconfig:option:`CONFIG_BT_SCAN_CONN_ATTEMPTS_COUNT` is responsible for the number of connection attempts.
...
Use the option :kconfig:option:`CONFIG_BT_SCAN_BLOCKLIST` to enable the blocklist.

In the default configuration, the scanning module allows to add up to two devices to the blocklist.
You can increase the blocklist size by setting the option :kconfig:option:`CONFIG_BT_SCAN_BLOCKLIST_LEN`.
...
It is recommended to enable the :kconfig:option:`CONFIG_BT_PRIVACY` option to support directed advertising only between bonded peers.
Use the :kconfig:option:`CONFIG_BT_SCAN_WITH_IDENTITY` option only when the :kconfig:option:`CONFIG_BT_PRIVACY` option is not available.
