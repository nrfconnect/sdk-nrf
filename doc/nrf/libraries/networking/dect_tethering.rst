...
Address sourcing on ``dect0`` may use :kconfig:option:`CONFIG_NET_L2_DECT_IPV6_IFACE_UNICAST_INSTALL` (disabled by default for this library) when ULA/GUA are not listed on the interface address table.
...
The library requires the following configurations:
...
To enable the library, set the :kconfig:option:`CONFIG_DECT_TETHER_IPV6_LIB` Kconfig option.

Sub-features are controlled by the following dedicated Kconfig options (all under ``DECT NR+ libraries`` → ``DECT IPv6 tethering``):
...
Call the :c:func:`dect_tether_ipv6_init` function after ``nrf_modem_lib_init()`` and with the network stack initialized.
Call the :c:func:`dect_tether_ipv6_deinit` function to stop RA, DHCPv6, forwarding, and mDNS threads or handlers.

.. code-block:: c

   #include <net/dect/dect_tether_ipv6.h>
...
The :ref:`dect_tether_ipv6_sample` sample uses this library.
...
* Ethernet ingress source filtering by tethered host address is not implemented.
  A design based on :kconfig:option:`CONFIG_NET_PKT_FILTER` and the IPv6 receive hook (using the committed DHCPv6 lease as the allowlist) is tracked as a possible enhancement.
* Cross-leg unicast mDNS is dropped when the source or destination is link-local (``fe80::``); only multicast queries/responses to ``ff02::fb`` are forwarded.
* If DECT NR+-derived addresses change between exchanges, Request/Renew/Rebind Replies include the previous IAADDR entries with lifetime 0 before any new addresses.
