...
The library therefore asks the server for a content-range which must be supported by the host server when using HTTPS with the nRF91 Series devices.
...
If requirements cannot be met, the downloader fails with error ``-EMSSIZE``.
