...
* :kconfig:option:`CONFIG_NRF_PROVISIONING_SCHEDULED` - Activates scheduled provisioning at a given interval, if disabled, the client will only provision when requested manually using the :c:func:`nrf_provisioning_trigger_manually` function.
...
This is done by calling the :c:func:`nrf_provisioning_init` function and passing the following event callback handler to receive events from the library:
...
When the option :kconfig:option:`CONFIG_NRF_PROVISIONING_AUTO_START_ON_INIT` is set, the library initializes and then starts provisioning according to the configured interval.
