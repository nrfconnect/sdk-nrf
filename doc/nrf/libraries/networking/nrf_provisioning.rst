...
* :kconfig:option:`CONFIG_NRF_PROVISIONING_SCHEDULED` - Activates scheduled provisioning at a given interval, if disabled, the client will only provision when requested manually via the :c:func:`nrf_provisioning_trigger_manually` function.
* :kconfig:option:`CONFIG_NRF_PROVISIONING_WAIT_FOR_VALID_DATETIME` - Time to wait for valid date time before starting provisioning
...
* :kconfig:option:`CONFIG_NRF_PROVISIONING_PROVIDE_ATTESTATION_TOKEN` - Provides attestation token when the device is unclaimed/unauthorized via the event handler
...
This is done by calling the :c:func:`nrf_provisioning_init` function passing in an event callback handler to receive events from the library:
...
Once initialized, provisioning can occur in one of two ways:

* Automatically, at a configured interval set by the :kconfig:option:`CONFIG_NRF_PROVISIONING_INTERVAL_S` if :Kconfig:option:`CONFIG_NRF_PROVISIONING_SCHEDULED` is enabled.
  Provisioning can be triggered asynchronously using the function :c:func:`nrf_provisioning_trigger_manually`.
...
For an example of how to use the library, see the :ref:`nrf_provisioning_sample`.

Provisioning
============

During provisioning the device will received a set of commands from the server.
This requires the device to deactivate LTE (putting the modem into offline mode) to write the commands to the modem's non-volatile memory.
This is because the modem cannot be connected while any data is being written to its storage area.

When the option :kconfig:option:`CONFIG_NRF_PROVISIONING_AUTO_START_ON_INIT` is set library starts provisioning when it initializes, then according to the configured interval.
When setting this option you should ensure that it is called when the device has obtained a network connection and the modem is ready to communicate with the server.
