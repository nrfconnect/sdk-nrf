...
Overview
********
The emergency data storage (EMDS) library provides persistent storage functionality designed to prevent the wear and tear of the flash memory or RRAM.
...
Allocating a larger storage area will demand more resources, but also increase the life expectancy of the flash/RRAM.
The chosen size should reflect the amount of data stored, the available flash/RRAM resources, and how the application calls the :c:func:`emds_store` function.
...
When all entries are added, the :c:func:`emds_load` function restores the entries into the memory areas from the flash/RRAM.
...
After completion of :c:func:`emds_store`, the :c:func:`emds_is_ready` function call will return error, since it can no longer guarantee that the data will fit into the flash/RRAM area.
...
:math:`s_i` is the size of the :math:`i`\ th entry in bytes and :math:`s_\text{block}` is the number of bytes in one word of flash/RRAM.
...
Limitations
***********
    The power-fail comparator cannot be active when EMDS is used, as it will prevent the NVMC/RRAMC from performing write operations to flash/RRAM.
