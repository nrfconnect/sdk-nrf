...
The storage partitions must be defined in the devicetree file used for the application.
The devicetree file must contain two partitions with names ``emds_partition_0`` and ``emds_partition_1``.
...
Partitions must be placed in the persistent memory area of the device.
