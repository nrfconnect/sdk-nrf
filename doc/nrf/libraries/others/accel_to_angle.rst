...
1. (Optional) If you want to use filtering in the data conversion pipeline, you must define the filter instance.
   ...
   You have one of the following options to define a filter:

    a. Use one of the already implemented filters that are delivered together with the library.
    ...
    b. Use a custom filter that is compatible with the library API.
       For an example on how to develop a custom filter, see the EMA filter.
...
#. Define and initialize the library context structure by calling the :c:macro:`ACCEL_TO_ANGLE_CTX_DEFINE` macro.
   ...
   If you want to use optional filtering step from step, pass the filter instance to the :c:macro:`ACCEL_TO_ANGLE_CTX_DEFINE` macro.
