...
* :kconfig:option:`CONFIG_DULT_MULTI_USER` - This read-only convenience option indicates that more than one DULT user can be registered.
  It is enabled automatically when the :kconfig:option:`CONFIG_DULT_USER_MAX` option is set to a value greater than ``1``.
