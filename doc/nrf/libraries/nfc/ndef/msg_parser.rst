...
When using an NFC polling device, you can use the NDEF message parser library to interpret the NDEF messages that are read from the NFC tag.
...
The message parser library provides functionality for parsing NDEF messages and printing out their content.
It then uses the NDEF record parser library for parsing NDEF records to record descriptions.
