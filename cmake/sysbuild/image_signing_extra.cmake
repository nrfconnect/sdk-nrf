...
if(num_binaries GREATER 0)
  ...
  foreach(index RANGE ${last_index})
    ...
    if(NOT DEFINED NCS_MCUBOOT_EXTRA_${extra_image_number}_IMAGE_NUMBER OR NCS_MCUBOOT_EXTRA_${extra_image_number}_IMAGE_NUMBER EQUAL -1)
      message(WARNING "Extra image ${extra_image_number} not configured. Set SB_CONFIG_MCUBOOT_EXTRA_IMAGES to at least ${num_binaries} in sysbuild.")
      return()
    endif()
    ...
    if(SB_CONFIG_PARTITION_MANAGER)
      ...
      foreach(partition_node ${fixed_partition_nodes})
        if(${slot_flash} MATCHES ${partition_node}/.*$)
          dt_prop(partition_node_compatible TARGET ${DEFAULT_IMAGE} PATH "${partition_node}" PROPERTY "compatible")
          ...
          if(${partition_node_compatible} MATCHES "soc-nv-flash")
            ...
            if(${reg_names_len} EQUAL "3")
              ...
              dt_reg_addr(base_offset TARGET ${DEFAULT_IMAGE} PATH "${partition_node}" NAME "${reg_name_mm}")
            else()
