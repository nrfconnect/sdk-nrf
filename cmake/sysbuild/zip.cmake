...
function(dfu_app_zip_package)
  ...
  if(SB_CONFIG_DFU_ZIP_APP)
    if(SB_CONFIG_QSPI_XIP_SPLIT_IMAGE)
      ...
      if(NOT SB_CONFIG_MCUBOOT_BUILD_DIRECT_XIP_VARIANT AND NOT SB_CONFIG_MCUBOOT_DIRECT_XIP_GENERATE_VARIANT)
        # Application
        ...
        mcuboot_image_number_to_slot(internal_load_slot ${NCS_MCUBOOT_APPLICATION_IMAGE_NUMBER} n)
        mcuboot_image_number_to_slot(external_load_slot ${NCS_MCUBOOT_QSPI_XIP_IMAGE_NUMBER} n)
        get_address_from_dt_partition_nodelabel("slot${internal_load_slot}_partition"
          internal_load_address
        )
        get_address_from_dt_partition_nodelabel("slot${external_load_slot}_partition"
          external_load_address
        )

        set(generate_script_app_params
          "${app_internal_update_name}load_address=${internal_load_address}"
          ...
          "${app_external_update_name}load_address=${external_load_address}"
        ...
        mcuboot_image_number_to_slot(internal_load_slot ${NCS_MCUBOOT_APPLICATION_IMAGE_NUMBER} n)
        mcuboot_image_number_to_slot(external_load_slot ${NCS_MCUBOOT_QSPI_XIP_IMAGE_NUMBER} n)
        mcuboot_image_number_to_slot(variant_internal_load_slot
          ${NCS_MCUBOOT_APPLICATION_IMAGE_NUMBER} y
        )
        mcuboot_image_number_to_slot(variant_external_load_slot
          ${NCS_MCUBOOT_QSPI_XIP_IMAGE_NUMBER} y
        )
        get_address_from_dt_partition_nodelabel(
          "slot${internal_load_slot}_partition" internal_load_address
        )
        get_address_from_dt_partition_nodelabel(
          "slot${external_load_slot}_partition" external_load_address
        )
        get_address_from_dt_partition_nodelabel(
          "slot${variant_internal_load_slot}_partition" variant_internal_load_address
        )
        get_address_from_dt_partition_nodelabel(
          "slot${variant_external_load_slot}_partition" variant_external_load_address
        )
        ...
        set(generate_script_app_params
          ...
          "${secondary_app_internal_update_name}load_address=${variant_internal_load_address}"
          ...
          "${secondary_app_external_update_name}load_address=${variant_external_load_address}"
  ...
  if(SB_CONFIG_DFU_ZIP_NET AND NOT SB_CONFIG_MCUBOOT_SIGN_MERGED_BINARY)
    # Network core
    set(net_update_name "${SB_CONFIG_NETCORE_IMAGE_NAME}.bin")
    set(secondary_net_update_name "${SB_CONFIG_NETCORE_IMAGE_NAME}_secondary_app.bin")
    sysbuild_get(net_core_board IMAGE ${SB_CONFIG_NETCORE_IMAGE_NAME} VAR BOARD CACHE)

    if(SB_CONFIG_SECURE_BOOT_NETCORE)
      sysbuild_get(net_update_version IMAGE ${SB_CONFIG_NETCORE_IMAGE_NAME} VAR CONFIG_FW_INFO_FIRMWARE_VERSION KCONFIG)
    else()
      sysbuild_get(net_update_version IMAGE ${SB_CONFIG_NETCORE_IMAGE_NAME} VAR CONFIG_MCUBOOT_IMGTOOL_SIGN_VERSION KCONFIG)
    endif()

    if(NCS_MCUBOOT_NETWORK_CORE_IMAGE_NUMBER EQUAL -1)
      mcuboot_image_number_to_slot(net_update_slot_primary ${NCS_MCUBOOT_APPLICATION_IMAGE_NUMBER} n)
      mcuboot_image_number_to_slot(net_update_slot_secondary ${NCS_MCUBOOT_APPLICATION_IMAGE_NUMBER} y)
    else()
    ...
    if(NOT SB_CONFIG_MCUBOOT_BUILD_DIRECT_XIP_VARIANT)
      ...
      if(SB_CONFIG_SECURE_BOOT_NETCORE)
        list(APPEND bin_files "${CMAKE_BINARY_DIR}/signed_by_mcuboot_and_b0_${SB_CONFIG_NETCORE_IMAGE_NAME}.bin")
        list(APPEND signed_targets ${SB_CONFIG_NETCORE_IMAGE_NAME}_signed_packaged_target)
      else()
        sysbuild_get(net_CONFIG_KERNEL_BIN_NAME IMAGE ${SB_CONFIG_NETCORE_IMAGE_NAME} VAR CONFIG_KERNEL_BIN_NAME KCONFIG)
        if(SB_CONFIG_BOOT_ENCRYPTION)
          list(APPEND bin_files "${CMAKE_BINARY_DIR}/${SB_CONFIG_NETCORE_IMAGE_NAME}/zephyr/${net_CONFIG_KERNEL_BIN_NAME}.signed.encrypted.bin")
        else()
          list(APPEND bin_files "${CMAKE_BINARY_DIR}/${SB_CONFIG_NETCORE_IMAGE_NAME}/zephyr/${net_CONFIG_KERNEL_BIN_NAME}.signed.bin")
        endif()
      endif()

      list(APPEND zip_names "${net_update_name}")
      list(APPEND signed_targets ${SB_CONFIG_NETCORE_IMAGE_NAME}_extra_byproducts)
    else()
      ...
      list(APPEND bin_files
         "${CMAKE_BINARY_DIR}/${SB_CONFIG_NETCORE_IMAGE_NAME}/zephyr/${CONFIG_KERNEL_BIN_NAME}.signed.bin"
         "${CMAKE_BINARY_DIR}/${SB_CONFIG_NETCORE_IMAGE_NAME}_secondary_app/zephyr/${CONFIG_KERNEL_BIN_NAME}.signed.bin"
      )
      set(exclude_files EXCLUDE
        ${CMAKE_BINARY_DIR}/${SB_CONFIG_NETCORE_IMAGE_NAME}/zephyr/${CONFIG_KERNEL_BIN_NAME}.signed.bin
        ${CMAKE_BINARY_DIR}/${SB_CONFIG_NETCORE_IMAGE_NAME}_secondary_app/zephyr/${CONFIG_KERNEL_BIN_NAME}.signed.bin
      ...
      set(include_files INCLUDE
        ${CMAKE_BINARY_DIR}/${SB_CONFIG_NETCORE_IMAGE_NAME}/zephyr/${CONFIG_KERNEL_BIN_NAME}.bin
        ${CMAKE_BINARY_DIR}/${SB_CONFIG_NETCORE_IMAGE_NAME}_secondary_app/zephyr/${CONFIG_KERNEL_BIN_NAME}.bin
      )

      list(APPEND zip_names "${net_update_name};${secondary_net_update_name}")
      list(APPEND signed_targets ${SB_CONFIG_NETCORE_IMAGE_NAME}_extra_byproducts)
      list(APPEND signed_targets ${SB_CONFIG_NETCORE_IMAGE_NAME}_secondary_app_extra_byproducts)
    endif()
  endif()
