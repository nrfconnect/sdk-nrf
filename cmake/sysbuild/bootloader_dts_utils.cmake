...
function(get_address_from_dt_partition_nodelabel label address)
  sysbuild_dt_nodelabel(partition_node IMAGE ${DEFAULT_IMAGE} NODELABEL ${label} REQUIRED)
  sysbuild_dt_reg_addr(partition_offset IMAGE ${DEFAULT_IMAGE} PATH ${partition_node})
  ...
  sysbuild_dt_reg_addr(flash_area_addr IMAGE ${DEFAULT_IMAGE} PATH ${flash_area_node})
...
function(code_partition_matches_label_verify label image)
  sysbuild_dt_nodelabel(label_node IMAGE ${image} NODELABEL ${label})
  sysbuild_dt_chosen(code_partition_node IMAGE ${image} PROPERTY "zephyr,code-partition")
...
function(labels_match_for_all_images_verify labels)
  foreach(label ${labels})
    sysbuild_dt_nodelabel(default_image_node IMAGE ${DEFAULT_IMAGE} NODELABEL ${label} REQUIRED)
    foreach(image ${IMAGES})
      sysbuild_dt_nodelabel(node IMAGE ${image} NODELABEL ${label} REQUIRED)
...
function(verify_bootloader_dts_configuration)
  ...
  if(SB_CONFIG_SECURE_BOOT)
    list(APPEND required_labels_list "b0_partition" "provision_partition")
  endif()
  if(SB_CONFIG_BOOTLOADER_MCUBOOT OR SB_CONFIG_SECURE_BOOT_BUILD_S1_VARIANT_IMAGE)
    # Slot 1 will be used either for the MCUBoot secondary image or for the S1 variant image
    # The latter will be the case if MCUBOOT is not present
    list(APPEND required_labels_list "slot1_partition")
  endif()
  if(SB_CONFIG_BOOTLOADER_MCUBOOT)
    list(APPEND required_labels_list "boot_partition")
    if(SB_CONFIG_SECURE_BOOT_BUILD_S1_VARIANT_IMAGE)
      list(APPEND required_labels_list "boot_partition_slot1")
    endif()
  endif()
  ...
  if(SB_CONFIG_SECURE_BOOT_BUILD_S1_VARIANT_IMAGE AND NOT SB_CONFIG_BOOTLOADER_MCUBOOT)
    code_partition_matches_label_verify("slot1_partition" "s1_image")
  endif()

  if(SB_CONFIG_BOOTLOADER_MCUBOOT)
    code_partition_matches_label_verify("boot_partition" "mcuboot")
    if(SB_CONFIG_SECURE_BOOT_BUILD_S1_VARIANT_IMAGE)
      code_partition_matches_label_verify("boot_partition_slot1" "s1_image")
    endif()
  endif()
endfunction()
...
function(nsib_get_s0_address address)
  if(SB_CONFIG_BOOTLOADER_MCUBOOT)
    set(s0_label "boot_partition")
  else()
    set(s0_label "slot0_partition")
  endif()
...
function(nsib_get_s1_address address)
  if(SB_CONFIG_BOOTLOADER_MCUBOOT)
    set(s1_label "boot_partition_slot1")
  else()
    set(s1_label "slot1_partition")
  endif()
