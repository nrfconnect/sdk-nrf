...
static void wifi_setup(void)
{
	...
	/* Make entire RAM_01 accessible to the Wi-Fi domain*/
	init_mpc_region_override(&override);
	override.start_address = 0x20000000;
	override.endaddr = 0x20080000;
	override.index = 0;
	...
	/* Make entire RAM_02 (Different AMBIX) accessible to the Wi-Fi domain for IPC */
	init_mpc_region_override(&override);
	override.start_address = 0x200C0000;
	override.endaddr = 0x20100000;
	override.index = 0;
	...
	nrf_spu_feature_secattr_set(
		NRF_SPU00, NRF_SPU_FEATURE_GPIO_PIN, 2, 2, SPU_FEATURE_GPIO_PIN_SECATTR_NonSecure);

	/* Set permission for TXD */
	nrf_spu_feature_secattr_set(
		NRF_SPU20, NRF_SPU_FEATURE_GPIO_PIN, 1, 4, SPU_FEATURE_GPIO_PIN_SECATTR_NonSecure);

	/* Split security configuration to let Wi-Fi access GRTC */
	nrf_spu_feature_secattr_set(
		NRF_SPU20, NRF_SPU_FEATURE_GRTC_CC, 15, 0, 0);
	nrf_spu_feature_secattr_set(
		NRF_SPU20, NRF_SPU_FEATURE_GRTC_CC, 14, 0, 0);
	nrf_spu_feature_secattr_set(
		NRF_SPU20, NRF_SPU_FEATURE_GRTC_INTERRUPT, 4, 0, 0);
	nrf_spu_feature_secattr_set(
		NRF_SPU20, NRF_SPU_FEATURE_GRTC_INTERRUPT, 5, 0, 0);
	nrf_spu_feature_secattr_set(
		NRF_SPU20, NRF_SPU_FEATURE_GRTC_SYSCOUNTER, 0, 0, 0);
