...
* ``ble`` - Enables the client part of the :ref:`Bluetooth LE RPC <ble_rpc>` and shell interface to serialized API. Enables the :ref:`Bluetooth LE Nordic UART Service <nus_service_readme>`.
* ``coex`` - Enables shell commands for the :ref:`MPSL software coexistence <ug_radio_coex_mpsl_cx_based>` implementation on the server device.
