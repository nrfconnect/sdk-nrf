...
Protocols serialization: server
###############################
...
* set the :makevar:`SNIPPET` option to ``ble;coex;debug;log_rpc;openthread``

.. include:: /includes/sample_snippets.txt

The following snippets are available:

* ``ble`` - Enables the server part of the :ref:`Bluetooth LE RPC <ble_rpc>`. Enables the :ref:`Bluetooth LE Nordic UART Service <nus_service_readme>`.
* ``coex`` - Enables the :ref:`MPSL software coexistence <ug_radio_coex_mpsl_cx_based>` implementation on the server device.
...
.. tabs::
    ...
    .. group-tab:: nRF52840 SoC

        By default, the nRF52840 system-on-chip uses the ``uart0`` peripheral for shell and logging purposes, and the ``uart1`` peripheral for sending OpenThread and Bluetooth remove procedure calls (RPCs).
    ...
    .. group-tab:: nRF54l15 SoC

        By default, the nRF54L15 system-on-chip uses the ``uart20`` peripheral for shell and logging purposes, and the ``uart21`` peripheral for sending OpenThread and Bluetooth remove procedure calls (RPCs).
...
#. Send data over the RX characteristic of the UART service in mobile application.
