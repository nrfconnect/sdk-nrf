...
If you use the RCP architecture (see :kconfig:option:`CONFIG_OPENTHREAD_COPROCESSOR_RCP`), you can alternatively use ``ot-daemon`` or ``ot-cli`` with commands listed in `OpenThread CLI Reference`_.
...
* :kconfig:option:`CONFIG_OPENTHREAD_COPROCESSOR_NCP` - Selects the NCP architecture for the sample.
  This is the default.
