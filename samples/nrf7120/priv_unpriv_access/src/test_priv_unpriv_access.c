...
#define SIZE_OF_SCRATCH_AREA 4096 // This is one page size, must be aligned to 4KB

__attribute__((section(".sram_priv_scratch"))) __aligned(4096)
volatile uint8_t sram_priv_scratch[SIZE_OF_SCRATCH_AREA];
...
static inline void set_memory_privilege(bool privileged)
{
	...
	nrf_mpc_override_perm_set(NRF_MPC00, 4, privileged ? 0x10 : 0x0);	 // PRIVL = 1 or 0
	...
	if (!unpriv_domain_inited && !privileged) {
		...
		int ret = k_mem_domain_init(&unpriv_mem_domain, ARRAY_SIZE(unpriv_mem_parts), unpriv_mem_parts);
		zassert_equal(ret, 0, "k_mem_domain_init failed");
		unpriv_domain_inited = true;
	}
}

// static inline void* privileged_setup()
// {
...
static inline void* unprivileged_setup()
{
	int rc;
	printf("Unpriviliged setup ...\n");
	printf("Test executed on %s\n", CONFIG_BOARD_TARGET);
	...
	set_gpio_privilege(spu_instance, 1, 11, false); // P1.11
	set_gpio_privilege(spu_instance, 1, 12, false); // P1.12
...
{
	memset((void *)sram_priv_scratch  , 0x0, SIZE_OF_SCRATCH_AREA);
 ...
 *
 * @param[in] expected_fault   Whether a fault is expected during memory access
 * @param[in] reason		   Expected fault reason
 * @param[in] priv_mem		 Whether to access privileged memory
 */
static inline void access_memory(void *p1, void *p2, void *p3)
{
	...
	ARG_UNUSED(p2);
	ARG_UNUSED(p3);
	printf("Current Privilege in access_memory = 0x%lx .\n", __get_CONTROL() & CONTROL_nPRIV_Msk); // 0 = privileged, 1 = unprivileged
...
// {
// 	printf("Current nrf_mpc00 index 4 override config: 0x%x\n", NRF_MPC00->OVERRIDE[4].PERM);
// 	access_gpio(NULL, NULL, NULL);
// 	k_sem_take(&done, K_MSEC(20));
// }
...
// 	access_memory(NULL, NULL, NULL);
// 	k_sem_take(&done, K_MSEC(20));

// 	zassert_equal(sram_priv_scratch[0], 0xAA, "Privileged access to privileged memory failed");
// }
...
ZTEST(test_suite_unpriv_settings, test_case_unpriv_access_unpriv_perip)
{
    ...
    k_mem_domain_add_thread(&unpriv_mem_domain, tid);   // <-- add this to unpriv mem domain
