...
This page collects the PowerShell and Command Prompt commands needed to inspect, reset, and troubleshoot the tether link on a Windows 11 host.

Find the tether adapter and gateway address
=============================================

Use the following command to list network adapters and identify the tether NIC's name and interface index.
Run in elevated PowerShell.
Replace ``"Ethernet"`` with the alias from ``Get-NetAdapter`` if your tether NIC has another name.

.. code-block:: powershell

   Get-NetAdapter | Format-Table Name, ifIndex, Status, LinkSpeed, InterfaceDescription

The tether gateway's link-local address (``fe80::...``) is in the device log or ``net iface`` on the nRF, or from a Wireshark RA Source on Ethernet.
In the example, ``fe80::f4ce:36ff:fe04:73b1`` is used.

Reset the Ethernet interface
============================

Run after reflashing the tether gateway, changing its Ethernet MAC, or whenever the tether link looks stale.
Elevated PowerShell or Command Prompt is required.
...
You can also use plain ``ipconfig /all`` (Command Prompt) and look at the tether adapter's block.
Working tether -- ``Default Gateway`` is filled in with the tether gateway's link-local address:
...
Paste into elevated PowerShell to watch the ``::/0`` route on the tether continuously instead of re-running the status check by hand.
Stop with Ctrl+C.

* ``GW valid`` - Route present, with its remaining lifetime and metric.
* ``GW GONE`` - No ``::/0`` on that interface.
  Expected on baseline firmware once Neighbor Unreachability Detection expires the router between Router Advertisements.
  See `Set a default route`_ for a workaround that does not require changing firmware.
...
Set a default route when the monitor above shows ``GW GONE`` (for example, while testing baseline firmware, where the route can expire between Router Advertisements).
Replace the example address with your tether gateway's link-local address.
...
To delete the route, use the following commands.
