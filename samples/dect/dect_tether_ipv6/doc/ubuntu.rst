...
Run the following commands after reflashing the tether gateway or changing its Ethernet MAC.
...
See the status
==============
Run the following commands to see the status.
Replace ``eth0`` with your tether interface.
Replace ``eth0`` with your tether interface.
...
The idea with this monitoring is the same as in a Windows PowerShell loop:
...
* ``expires=…sec`` - Kernel countdown until the RA route expires.
  Should jump back when a new RA arrives (~60 s on baseline).
* ``GW GONE`` - Rare on Ubuntu with baseline firmware.
  Investigate if the tether gateway stopped sending RAs or the link is down.

Extra: capture Router Advertisements
====================================
To capture Router Advertisements, use the following command:
