 ...
 *
 * A UDP socket is bound to [::]:CONFIG_SAMPLE_DECT_TETHER_IPV6_MDNS_DNS_SD_PORT
...
LOG_MODULE_REGISTER(bh6_mdns, CONFIG_SAMPLE_DECT_TETHER_IPV6_LOG_LEVEL);
...
static const uint16_t bh6_dns_sd_port_be =
	sys_cpu_to_be16((uint16_t)CONFIG_SAMPLE_DECT_TETHER_IPV6_MDNS_DNS_SD_PORT);
	...
	case NET_EVENT_IF_UP:
		if (atomic_cas(&bh6_dns_sd_state, BH6_DNS_SD_STATE_WAITING_IFACE,
			...
			LOG_INF("DNS-SD: advertising _dect-nr._udp on %s, instance=%s (port %u)",
				...
				(unsigned int)CONFIG_SAMPLE_DECT_TETHER_IPV6_MDNS_DNS_SD_PORT);
		}
		bh6_dns_sd_refresh();
		break;
...
/* Placeholder listener: bind [::]:CONFIG_SAMPLE_DECT_TETHER_IPV6_MDNS_DNS_SD_PORT
...
static int bh6_dns_sd_placeholder_bind(void)
{
	struct sockaddr_in6 bind_addr = {
		...
		.sin6_port = htons(CONFIG_SAMPLE_DECT_TETHER_IPV6_MDNS_DNS_SD_PORT),
	...
	if (zsock_bind(sock, (struct sockaddr *)&bind_addr, sizeof(bind_addr)) < 0) {
		LOG_ERR("DNS-SD: placeholder bind [::]:%u failed (%d)",
			(unsigned int)CONFIG_SAMPLE_DECT_TETHER_IPV6_MDNS_DNS_SD_PORT, errno);
		zsock_close(sock);
		return -errno;
	}
