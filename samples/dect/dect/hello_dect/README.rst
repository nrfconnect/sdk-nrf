...
* :kconfig:option:`CONFIG_DECT_DEFAULT_LONG_RD_ID`

  * The sample overrides the default long radio device ID to ``0x11223344``.

* :kconfig:option:`CONFIG_DECT_MDM_DEFAULT_DEV_TYPE_FT`
...
* :kconfig:option:`CONFIG_DECT_MDM_DEFAULT_DEV_TYPE_PT`
...
.. |sample path| replace:: :file:`samples/dect/dect/hello_dect`
...
To build the sample for FT operation, use the overlay configuration:

.. code-block:: console

   west build -p -b nrf9151dk/nrf9151/ns -- -DOVERLAY_CONFIG=overlay-ft.conf

Building for PT mode
====================

To build the sample for PT operation, use the overlay configuration:

.. code-block:: console

   west build -p -b nrf9151dk/nrf9151/ns -- -DOVERLAY_CONFIG=overlay-pt.conf

Building with DECT NR+ L2 Shell
===============================

To enable the DECT MAC L2 shell for debugging and configuration, use the shell overlay:

.. code-block:: console

   west build -p -b nrf9151dk/nrf9151/ns -- -DOVERLAY_CONFIG=overlay-shell.conf
...
.. code-block:: console
   ...
   west build -p -b nrf9151dk/nrf9151/ns -- -DOVERLAY_CONFIG="overlay-ft.conf;overlay-shell.conf"
