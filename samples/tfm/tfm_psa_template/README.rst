...
* On the nRF5340 and nRF91x SoCs, a device fresh from factory is already in lifecycle state ``ASSEMBLY``.
...
On nRF54L Series devices, the provisioning image is the primary way to provision the device for this application, and it is built together with the application by default (:kconfig:option:`SB_CONFIG_SAMPLE_TFM_PSA_TEMPLATE_PROVISIONING_IMAGE`).
...
.. code-block:: console
   ...
   west build -b nrf54l15dk/nrf54l15/cpuapp/ns nrf/samples/tfm/tfm_psa_template \
     -- -DSB_CONFIG_SAMPLE_TFM_PSA_TEMPLATE_PROVISIONING_IMAGE=n
...
.. msc::
   ...
   Application note Application [label="Prints token as hex dump"];
...
* The PSA certificate reference embedded in the attestation token is a placeholder ``0000000000000-00000`` set by :file:`sysbuild/nrf54l.conf`, not the ``0632793519539-10100`` value like in the nRF5340 output.
...
* Everything from the ``Requesting initial attestation token with 64 byte challenge.`` line in the `Sample output on nRF5340 DK`_ onward has the same shape; only the token bytes themselves differ (every device's identity key is unique and the optional fields encode device-specific values.)
