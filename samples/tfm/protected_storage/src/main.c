...
void main(void)
{
	...
	if (strcmp(stored_data, TEST_STRING_1) != 0) {
		printk("Read string was not correct!\n");
		return;
	}
	...
	if (status != PSA_ERROR_NOT_PERMITTED) {
		printk("Got unexpected error when overwriting! (%d)\n", status);
		return;
	}
	printk("Got expected error (PSA_ERROR_NO_PERMITTED) when writing to protected UID\n");
}
