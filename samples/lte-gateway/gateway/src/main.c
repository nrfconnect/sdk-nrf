...
static struct nrf_cloud_sensor_data gps_cloud_data_out;
...
static void gps_trigger_handler(struct device *dev, struct gps_trigger *trigger)
{
	...
	if (!(button_state & SWITCH_2)) {
		...
		in_data.type = GPS_POSITION;
		in_data.length = gps_cloud_data.data.len;
		memcpy(&in_data.data[4], gps_cloud_data.data.ptr,
			gps_cloud_data.data.len);
		*((u32_t *)&in_data.data[0]) = gps_cloud_data.tag;

		printk("gps length: %d", gps_cloud_data.data.len);
...
static void send_aggregated_data(void)
{
	...
	printk("flip_alarm triggered.\n");
	while (1) {
		if (aggregator_get(&aggregator_data) == -1) {
			printk("No data in aggregator.\n");
			break;
		}
		switch (aggregator_data.type) {
		case THINGY_ORIENTATION:
			switch (aggregator_data.data[0]) {
			case 0:
				flip_cloud_data.data.ptr = "LEFT";
				flip_cloud_data.data.len = sizeof("LEFT") - 1;
				break;
			case 1:
				flip_cloud_data.data.ptr = "NORMAL";
				flip_cloud_data.data.len = sizeof("NORMAL") - 1;
				break;
			case 2:
				flip_cloud_data.data.ptr = "RIGHT";
				flip_cloud_data.data.len = sizeof("RIGHT") - 1;
				break;
			case 3:
				flip_cloud_data.data.ptr = "UPSIDE_DOWN";
				flip_cloud_data.data.len =
					sizeof("UPSIDE_DOWN") - 1;
				break;
			default:
				printk("Orientation unknown %d\n",
		...
		case GPS_POSITION:
			...
			gps_cloud_data_out.type = NRF_CLOUD_SENSOR_GPS;
			gps_cloud_data_out.data.ptr = gps_data_buffer;
			memcpy((void *)gps_cloud_data_out.data.ptr,
				&aggregator_data.data[4],
				aggregator_data.length);
			gps_cloud_data_out.data.len = aggregator_data.length;
			gps_cloud_data_out.tag =
				*((u32_t *)&aggregator_data.data[0]);
			sensor_data_send(&gps_cloud_data_out);
		break;
