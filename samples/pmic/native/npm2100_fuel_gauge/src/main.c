...
static const char * const battery_model_str[] = {
	[BATTERY_TYPE_ALKALINE_AA] = "Alkaline AA",
...
{
	if (battery_model >= ARRAY_SIZE(battery_model_str)) {
		shell_error(sh, "Invalid battery model");
		return 0;
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(subcmd_battery_model,
...
SHELL_CMD_ARG_REGISTER(battery_model, &subcmd_battery_model, "Select battery mode",
