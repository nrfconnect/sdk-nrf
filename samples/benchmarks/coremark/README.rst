...
If you want to run the sample upon startup, enable the :kconfig:option:`CONFIG_COREMARK_MODE_FLASH_AND_RUN` Kconfig option.
...
#. To start the test, press the button assigned to the respective core.
   ...
   If the :kconfig:option:`CONFIG_COREMARK_MODE_FLASH_AND_RUN` Kconfig option is enabled, the measurement will launch automatically on all available cores when starting the application.
