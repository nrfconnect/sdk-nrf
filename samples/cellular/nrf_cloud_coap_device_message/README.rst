...
.. _CONFIG_COAP_DEVICE_MESSAGE_LTE_LED_NUM:

CONFIG_COAP_DEVICE_MESSAGE_LTE_LED_NUM - LTE LED number
   This configuration option defines which LED is used to indicate LTE connection success.

.. _CONFIG_COAP_DEVICE_MESSAGE_SEND_LED_NUM:

CONFIG_COAP_DEVICE_MESSAGE_SEND_LED_NUM - Send LED number
   This configuration option defines which LED is used to indicate a device message is being sent.
...
Set the :kconfig:option:`CONFIG_NRF_CLOUD_COAP_DEVICE_MESSAGE_SAMPLE_LOG_LEVEL_DBG` option so that log messages are generated on each button press.
