...
#define WIFI_SCANNING_RESULTS_MAX_CNT CONFIG_NRF700X_SCAN_LIMIT
...
int scan_wifi_start(struct k_sem *wifi_scan_ready)
{
	...
	LOG_INF("Triggering start of Wi-Fi scanning");
...
static void scan_wifi_result_handle(struct net_mgmt_event_callback *cb)
{
	...
	if (scan_wifi_info.cnt < WIFI_SCANNING_RESULTS_MAX_CNT) {
		...
		*current = *entry;
		...
		LOG_INF("scan result #%d stored: ssid %s, channel %d,"
...
static void scan_wifi_done_handle(struct net_mgmt_event_callback *cb)
{
	...
	if (status->status) {
		...
		LOG_INF("Scan request done with %d Wi-Fi APs", scan_wifi_info.cnt);
	}
