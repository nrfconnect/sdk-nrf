...
#define TRANSFORM_DESIRED_CFG_INTERVAL                                                             \
	NRF_CLOUD_JSON_KEY_STATE "." NRF_CLOUD_JSON_KEY_DES "." NRF_CLOUD_JSON_KEY_CFG             \
...
static struct nrf_cloud_fota_poll_ctx fota_ctx = {.rest_ctx = &rest_ctx,
						  .device_id = device_id,
...
static void send_device_status(void)
{
	...
	struct nrf_cloud_svc_info_fota fota = {.bootloader = 1,
					       .modem = 1,
					       .application = 1,
	...
	struct nrf_cloud_modem_info mdm_inf = {/* Include all available modem info */
	...
	struct nrf_cloud_device_status dev_status = {
		.modem = &mdm_inf, .svc = &svc_inf, .conn_inf = NRF_CLOUD_INFO_SET};
