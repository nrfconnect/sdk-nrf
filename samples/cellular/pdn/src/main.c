...
static void dynamic_info_print(uint32_t cid)
{
	...
	err = lte_lc_pdn_ctx_dynamic_info_get(cid, &info);
	if (err) {
		printk("lte_lc_pdn_ctx_dynamic_info_get() failed, err %d\n", err);
		return;
	}
...
void lte_lc_evt_handler(const struct lte_lc_evt *const evt)
{
	...
	case LTE_LC_EVT_PDN_ESM_ERROR:
		...
		break;
	default:
		printk("Event: PDP context %d %s\n", evt->pdn.cid, event_str[evt->type]);
		break;
	}
}
...
int main(void)
{
	...
	err = lte_lc_pdn_default_apn_get(apn, sizeof(apn));
	if (err) {
		printk("lte_lc_pdn_default_apn_get() failed, err %d\n", err);
		return 0;
	}
	...
	err = lte_lc_pdn_ctx_activate(cid, &esm, NULL);
	if (err) {
		printk("lte_lc_pdn_ctx_activate() failed, err %d esm %d %s\n",
			err, esm, lte_lc_pdn_esm_strerror(err));
		return 0;
	}

	printk("PDP Context %d, PDN ID %d\n", 0, lte_lc_pdn_ctx_connection_id_get(0));
	printk("PDP Context %d, PDN ID %d\n", cid, lte_lc_pdn_ctx_connection_id_get(cid));
