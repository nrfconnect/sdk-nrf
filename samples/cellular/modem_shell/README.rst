...
.. list-table::
   ...
   * - PPP support (see also `Testing PPP support`_)
     - * :file:`overlay-ppp.conf`
       * :file:`ppp.overlay`
     - * ``-DEXTRA_CONF_FILE=overlay-ppp.conf``
       * ``-DEXTRA_DTC_OVERLAY_FILE=ppp.overlay``
   * - FOTA support
     - :file:`overlay-app_fota.conf`
     - ``-DEXTRA_CONF_FILE=overlay-app_fota.conf``
   * - nRF91 Series DK with full modem FOTA support
     - * :file:`overlay-modem_fota_full.conf`
       * :file:`<device>_ext_flash.overlay`
     - * ``-DEXTRA_CONF_FILE=overlay-modem_fota_full.conf``
       * ``-DEXTRA_DTC_OVERLAY_FILE=<device>_ext_flash.overlay``
   * - LwM2M carrier library support
     - :file:`overlay-carrier.conf`
   ...
   * - Cloud over MQTT (remote control)
     - :file:`overlay-cloud_mqtt.conf`
     - ``-DEXTRA_CONF_FILE=overlay-cloud_mqtt.conf``
   * - Cloud over CoAP
     - :file:`overlay-cloud_coap.conf`
   ...
   * - Location services in MoSh, with P-GPS
     - * :file:`overlay-cloud_mqtt.conf` and :file:`overlay-pgps.conf`
       * :kconfig:option:`CONFIG_LOCATION_SERVICE_EXTERNAL`
       * :kconfig:option:`CONFIG_NRF_CLOUD_PGPS_TRANSPORT_NONE`
     - * ``-DEXTRA_CONF_FILE="overlay-cloud_mqtt.conf;overlay-pgps.conf"``
       * ``-DCONFIG_LOCATION_SERVICE_EXTERNAL=y``
       * ``-DCONFIG_NRF_CLOUD_PGPS_TRANSPORT_NONE=y``
   * - SEGGER Real Time Transfer (RTT) support (instead of UART)
     - :file:`overlay-rtt.conf`
     - ``-DEXTRA_CONF_FILE=overlay-rtt.conf``

.. |variable_feature| replace:: the FOTA support
.. |makevar| replace:: EXTRA_CONF_FILE
.. |cmake_file_name| replace:: overlay-app_fota.conf
