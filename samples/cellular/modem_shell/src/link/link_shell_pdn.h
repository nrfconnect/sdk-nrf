...
int link_family_str_to_pdn_lib_family(enum lte_lc_pdn_family *ret_fam, const char *family);
const char *link_pdn_lib_family_to_string(enum lte_lc_pdn_family pdn_family, char *out_fam_str);

bool link_shell_pdn_info_is_in_list(uint8_t cid);
void link_shell_pdn_info_list_remove(uint8_t cid);

int link_shell_pdn_auth_prot_to_pdn_lib_method_map(int auth_proto, enum lte_lc_pdn_auth *method);
