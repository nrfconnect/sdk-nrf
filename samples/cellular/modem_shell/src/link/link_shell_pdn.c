...
void link_pdn_event_handler(const struct lte_lc_evt *const evt)
{
	...
	default:
		mosh_print("PDN event: PDP context %d %s", evt->pdn.cid,
			   link_pdn_event_to_string(evt->type, str_buf));
		if (evt->pdn.cid == 0) {
#if defined(CONFIG_MOSH_STARTUP_CMDS)
			if (evt->type == LTE_LC_EVT_PDN_ACTIVATED) {
			...
			} else if (evt->type == LTE_LC_EVT_PDN_DEACTIVATED ||
				   evt->type == LTE_LC_EVT_PDN_NETWORK_DETACH) {
			   ...
			   const struct link_shell_pdn_auth *auth_params)
{
	...
	pdn_id = lte_lc_pdn_ctx_connection_id_get(cid);
...
int link_shell_pdn_disconnect(int pdn_cid)
{
	...
	ret = lte_lc_pdn_ctx_deactivate(pdn_cid);
	if (ret) {
		mosh_error("lte_lc_pdn_ctx_deactivate() failed, err %d", ret);
	}
...
int link_shell_pdn_activate(int pdn_cid)
{
	...
	/* Activate a PDN connection */
	ret = lte_lc_pdn_ctx_activate(pdn_cid, &esm, NULL);
	if (ret) {
		mosh_error(
			"lte_lc_pdn_ctx_activate() failed, err %d esm %d %s\n",
