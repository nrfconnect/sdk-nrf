...
static void ntn_evt_handler(const struct ntn_evt *evt)
{
	...
	case NTN_EVT_MODEM_LOCATION_UPDATE:
		mosh_print("NTN modem location update event: requested: %s, accuracy: %d m",
		...
		if (evt->modem_location.updates_requested) {
			ntn_modem_location_update_enable(
				update_interval_calculate(evt->modem_location.accuracy));
		} else {
...
int ntn_modem_location_update_enable(uint32_t interval)
{
#if defined(CONFIG_GNSS)
	if (!update_enabled) {
