...
static struct lte_lc_cell gci_cells[CONFIG_COAP_CELL_GCI_COUNT];
...
/* Location request structure to contain cell info to be sent to nRF Cloud */
...
static void get_cell_info(void)
{
	...
	if (active_cell_pos_type == LOCATION_TYPE_SINGLE_CELL) {
		...
		struct lte_lc_ncellmeas_params ncellmeas_params = {
			...
			.gci_count = CONFIG_COAP_CELL_GCI_COUNT
		};
...
{
	static int config_mode = (IS_ENABLED(CONFIG_COAP_CELL_DEFAULT_HICONF_VAL) ?  0x01 : 0x00) |
				 (IS_ENABLED(CONFIG_COAP_CELL_DEFAULT_FALLBACK_VAL) ? 0x02 : 0x00);
	...
	if (active_cell_pos_type == LOCATION_TYPE_SINGLE_CELL) {
		...
		if (IS_ENABLED(CONFIG_COAP_CELL_CHANGE_CONFIG)) {
...
int main(void)
{
	 ...
	 */
	if (IS_ENABLED(CONFIG_COAP_CELL_SEND_DEVICE_STATUS)) {
		send_device_status();
	}
	...
	while (1) {
		...
		if (config.do_reply) {
			LOG_INF("Lat: %f, Lon: %f, Uncertainty: %u m",
				cell_pos_result.lat,
				cell_pos_result.lon,
				cell_pos_result.unc);
		}
