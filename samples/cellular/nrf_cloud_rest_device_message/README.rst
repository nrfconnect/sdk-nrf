...
Your device must be onboarded with `nRF Cloud`_. If it is not, follow the instructions in `Device on-boarding <nrf_cloud_rest_device_message_onboarding_>`_.
...
To onboard your device, install `nRF Cloud Utils_` and follow the instructions in the README section.
