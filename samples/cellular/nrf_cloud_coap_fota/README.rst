...
You can update your device firmware on the `nRF Cloud`_ portal or via the `Memfault CLI tool`_.
...
To enable delta modem FOTA, add the following parameter to your build command and set :kconfig:option:`CONFIG_MEMFAULT_FOTA_MODEM_PROJECT_KEY` to your Memfault modem firmware project key:
...
Note: SMP and full modem FOTA updated are not currently supported by this sample, however legacy overlay and source files for enabling those features are left for reference:
