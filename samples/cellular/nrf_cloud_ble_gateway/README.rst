.. _nrf_cloud_ble_gateway:

Cellular: nRF Cloud BLE gateway
###############################
...
.. _nrf_cloud_ble_gateway_requirements:
...
.. _nrf_cloud_ble_gateway_features:
...
.. _nrf_cloud_ble_gateway_structure_and_theory_of_operation:
...
* The cloud connection thread (``con_thread``, :file:`src/cloud_connection.c`) runs the :ref:`nrf_cloud_ble_gateway_cloud_connection_loop`, which maintains a connection to `nRF Cloud`_.
* The application thread (``app_thread``, :file:`src/application.c`) runs the :ref:`nrf_cloud_ble_gateway_application_thread_and_main_application_loop`, which controls demo features and submits `device messages <nRF Cloud Device Messages_>`_ to the :ref:`nrf_cloud_ble_gateway_device_message_queue`.
* The message queue thread (``msg_thread``, :file:`src/message_queue.c`) then transmits these messages whenever there is an active connection.
  See :ref:`nrf_cloud_ble_gateway_device_message_queue`.

:file:`src/main.c` also optionally starts a fourth thread, the ``led_thread``, which animates any onboard LEDs if :ref:`nrf_cloud_ble_gateway_led_status_indication` is enabled.

When using CoAP, two additional threads start:

* The CoAP FOTA job checking thread (``coap_fota``, :file:`src/fota_support_coap.c`) runs the :ref:`nrf_cloud_ble_gateway_coap_fota_loop` which periodically asks nRF Cloud for any pending FOTA job.
* The CoAP shadow delta checking thread (``coap_shadow``, :file:`src/shadow_support_coap.c`) runs the :ref:`nrf_cloud_ble_gateway_coap_shadow_loop` which periodically asks nRF Cloud for any shadow changes.

.. _nrf_cloud_ble_gateway_cloud_connection_loop:
...
See :ref:`nrf_cloud_ble_gateway_standard_onboarding` for details.
...
.. _nrf_cloud_ble_gateway_device_message_queue:
...
.. _nrf_cloud_ble_gateway_application_thread_and_main_application_loop:
...
* Sends sensor sample and location device messages to the :ref:`nrf_cloud_ble_gateway_device_message_queue`.
* Checks for and executes :ref:`remote modem AT command requests <nrf_cloud_ble_gateway_remote_at>`.
...
.. _nrf_cloud_ble_gateway_coap_fota_loop:
...
.. _nrf_cloud_ble_gateway_coap_shadow_loop:
...
.. _nrf_cloud_ble_gateway_temperature_sensing:
...
.. _nrf_cloud_ble_gateway_location_tracking:
...
.. _nrf_cloud_ble_gateway_wifi_location_tracking:
...
See :ref:`nrf_cloud_ble_gateway_building_wifi_scan` for details on how to enable Wi-Fi location tracking.
...
.. _nrf_cloud_ble_gateway_remote_at:
...
.. _nrf_cloud_ble_gateway_led_status_indication:
...
See :ref:`nrf_cloud_ble_gateway_customizing_LED_status_indication` for details on customizing the LED behavior.

See :ref:`nrf_cloud_ble_gateway_led_third_party` for details on configuring LED status indication on third-party boards.

.. _nrf_cloud_ble_gateway_test_counter:
...
.. _nrf_cloud_ble_gateway_device_message_formatting:
...
.. _nrf_cloud_ble_gateway_configuration:
...
For examples, see the related minimal overlays in the :ref:`nrf_cloud_ble_gateway_minimal` section.
...
.. _nrf_cloud_ble_gateway_customizing_LED_status_indication:
...
.. _nrf_cloud_ble_gateway_led_third_party:
...
To see all debug output for this sample, enable the :ref:`CONFIG_ble_gateway_LOG_LEVEL_DBG <CONFIG_ble_gateway_LOG_LEVEL_DBG>` option.
...
See also the :ref:`nrf_cloud_ble_gateway_test_counter`.
...
.. _CONFIG_ble_gateway_LOG_LEVEL_DBG:

CONFIG_ble_gateway_LOG_LEVEL_DBG - Sample debug logging
...
CONFIG_AT_CMD_REQUESTS - Enable AT command requests
   ...
   See :ref:`nrf_cloud_ble_gateway_remote_at` for details.
...
.. _nrf_cloud_ble_gateway_building_and_running:

Building and running
********************

.. |sample path| replace:: :file:`samples/cellular/nrf_cloud_ble_gateway`

.. include:: /includes/build_and_run_ns.txt

.. _nrf_cloud_ble_gateway_building_lte:
...
Once the sample is built and flashed, proceed to :ref:`nrf_cloud_ble_gateway_standard_onboarding` for instructions on how to onboard your device.

.. _nrf_cloud_ble_gateway_building_provisioning_service:
...
Once the sample is built and flashed, proceed to :ref:`nrf_cloud_ble_gateway_provisioning_service` for instructions on how to provision your device with the Provisioning Service.
...
.. _nrf_cloud_ble_gateway_building_wifi_scan:
...
.. note::
   ...
   To build the sample with Wi-Fi connectivity instead, see :ref:`nrf_cloud_ble_gateway_building_lte`.
...
See also :ref:`the paragraphs on the Wi-Fi location tracking method <nrf_cloud_ble_gateway_wifi_location_tracking>`.
...
.. _nrf_cloud_ble_gateway_building_wifi_conn:
...
.. important::
   ...
   Even though this is more secure than :ref:`hard-coded credentials <nrf_cloud_ble_gateway_build_hardcoded>`, the device private key still has to be loaded into unprotected memory during TLS connections.
...
Once your device is onboarded, proceed to :ref:`nrf_cloud_ble_gateway_setup_wifi_cred` for instructions on configuring your device to connect to a specific access point.

.. _nrf_cloud_ble_gateway_build_hardcoded:
...
.. note::
   ...
   See :ref:`nrf_cloud_ble_gateway_provisioning_onboarding` for a list of other methods.

If you are certain you understand the inherent security risks, you can use this option as follows:

1. Follow the instructions under :ref:`nrf_cloud_ble_gateway_onboard_hardcoded`.

#. Create a :file:`certs` folder directly in the :file:`nrf_cloud_ble_gateway` folder, and copy :file:`client-cert.pem`, :file:`private-key.pem`, and :file:`ca-cert.pem` files into it.

   Make sure not to place the new folder in the :file:`nrf_cloud_ble_gateway/src` folder by accident.
...
.. _nrf_cloud_ble_gateway_setup_wifi_cred:
...
.. _nrf_cloud_ble_gateway_minimal:
...
.. _nrf_cloud_ble_gateway_provisioning_onboarding:
...
The following sections outline the various onboarding methods that this sample supports.
  ...
  * If you are building with :ref:`provisioning service support <nrf_cloud_ble_gateway_building_provisioning_service>`, proceed to :ref:`nrf_cloud_ble_gateway_provisioning_service`.
    Use this method for nRF91x1-based devices.
  * If you are not building with Provisioning Service support, proceed to :ref:`nrf_cloud_ble_gateway_standard_onboarding`.
    Use this method for nRF9160-based devices.
  * If you are building with :ref:`hard-coded credentials <nrf_cloud_ble_gateway_build_hardcoded>`, proceed to :ref:`nrf_cloud_ble_gateway_onboard_hardcoded`.
...
.. _nrf_cloud_ble_gateway_install_nrf_utils:
...
.. _nrf_cloud_ble_gateway_provisioning_service:
...
For devices using the nRF9160, proceed to :ref:`nrf_cloud_ble_gateway_standard_onboarding`.

If you have :ref:`enabled support <nrf_cloud_ble_gateway_building_provisioning_service>` for the `provisioning service <nRF Cloud Provisioning Service_>`_, you can provision and onboard your device in one of two ways:
...
* Using scripted onboarding, as follows:
   ...
   First, :ref:`create a self-signed CA certificate <nrf_cloud_ble_gateway_create_self_signed_ca>`.
   ...
   #. Use the :file:`claim_and_provision_device.py` Python script :ref:`you installed <nrf_cloud_ble_gateway_install_nrf_utils>` to provision and onboard your device
      ...
      .. tabs::
         ...
         .. group-tab:: MQTT
               ...
               Where the :file:`.pem` files are the self-signed CA certificate and private key files :ref:`you created <nrf_cloud_ble_gateway_create_device_cred_locally>` and *your_api_key* is your `nRF Cloud REST API key`_.
      ...
      Within a few minutes of script completion, it should successfully connect to nRF Cloud and begin demonstrating the :ref:`supported nRF Cloud features <nrf_cloud_ble_gateway_features>`.

.. _nrf_cloud_ble_gateway_standard_onboarding:

Onboarding a device without the nRF Cloud Provisioning Service
==============================================================

If you are not using :ref:`provisioning service support <nrf_cloud_ble_gateway_building_provisioning_service>`, you can onboard your devices as follows:
...
#. Install the device and server credentials using the :file:`device_credentials_installer.py` Python script :ref:`you installed <nrf_cloud_ble_gateway_install_nrf_utils>`:
      ...
      .. group-tab:: MQTT
            .. tabs::
               .. group-tab:: LTE
                  ...
                  Where the :file:`.pem` files are the self-signed CA certificate and private key files :ref:`you created <nrf_cloud_ble_gateway_create_device_cred_locally>`.
               ...
               .. group-tab:: Wi-Fi
                  ...
                  Where:
                    ...
                    * *device id* is the (globally unique) ID for your device.
                      ...
                      See :ref:`nrf_cloud_ble_gateway_building_wifi_conn` for details.
                    * The :file:`.pem` files are the self-signed CA certificate and private key files :ref:`you created <nrf_cloud_ble_gateway_create_device_cred_locally>`.
...
.. _nrf_cloud_ble_gateway_onboard_hardcoded:
...
First, create a :ref:`self-signed CA certificate <nrf_cloud_ble_gateway_create_self_signed_ca>`.

Next, complete the following steps for each device you wish to onboard:

1. :ref:`Locally generate a device credential <nrf_cloud_ble_gateway_create_device_cred_locally>`
   ...
   Do not confuse this CA certificate with your :ref:`self-signed CA certificate <nrf_cloud_ble_gateway_create_self_signed_ca>`.
...
#. :ref:`Manually onboard the device to nRF Cloud <nrf_cloud_ble_gateway_onboard_device_manually>`

#. Follow the instructions under :ref:`nrf_cloud_ble_gateway_build_hardcoded`.

.. _nrf_cloud_ble_gateway_create_self_signed_ca:
...
1. :ref:`Download and install <nrf_cloud_ble_gateway_install_nrf_utils>` the `nRF Cloud Utilities <nRF Cloud Utilities documentation_>`_ repository.
...
.. _nrf_cloud_ble_gateway_create_device_cred_locally:

Generating device credentials locally
=====================================

To generate and sign a device credential locally using your :ref:`self-signed CA certificate <nrf_cloud_ble_gateway_create_self_signed_ca>`, perform the following steps:
...
#. Navigate to the folder where you :ref:`created your self-signed CA certificate <nrf_cloud_ble_gateway_create_self_signed_ca>`, and use the :file:`create_device_credentials.py` Python script :ref:`you installed <nrf_cloud_ble_gateway_install_nrf_utils>` to generate a self-signed certificate for the device:
   ...
   Where *device_id* is the device ID you created, and the :file:`.pem` files are the self-signed CA certificate and private key files :ref:`you created <nrf_cloud_ble_gateway_create_self_signed_ca>`.
...
.. _nrf_cloud_ble_gateway_onboard_device_manually:
...
1. Create an onboarding CSV file named :file:`<device_id>_onboard.csv` and give it the following contents:
   ...
   Where *device_id* is replaced by the device ID :ref:`you created <nrf_cloud_ble_gateway_create_device_cred_locally>`, and *device_cert* is replaced by the exact contents of :file:`<device_id>_crt.pem`.
...
.. _nrf_cloud_ble_gateway_dependencies:
