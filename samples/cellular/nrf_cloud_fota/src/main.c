...
LOG_MODULE_REGISTER(nrf_cloud_fota_sample, CONFIG_NRF_CLOUD_FOTA_SAMPLE_LOG_LEVEL);

#define BTN_NUM		       CONFIG_NRFCLOUD_FOTA_BUTTON_EVT_NUM
#define LTE_LED_NUM	       CONFIG_NRFCLOUD_FOTA_LTE_LED_NUM
...
static uint32_t fota_check_rate = CONFIG_NRFCLOUD_FOTA_JOB_CHECK_RATE_MIN;
...
static void send_device_status(void)
{
	...
	struct nrf_cloud_modem_info mdm_inf = {
		...
		.application_version = CONFIG_NRF_CLOUD_FOTA_SAMPLE_VERSION
	};
...
static bool process_desired_check_rate(long desired_check_rate)
{
		if (desired_check_rate > INTERVAL_MINUTES_MAX) {
			...
			LOG_INF("Desired interval must be greater than zero, setting to %d",
				CONFIG_NRFCLOUD_FOTA_JOB_CHECK_RATE_MIN);
			desired_check_rate = CONFIG_NRFCLOUD_FOTA_JOB_CHECK_RATE_MIN;
		}
...
static void check_config(void)
{
	...
	if (!err) {
		...
		update_reported = process_desired_check_rate(desired_check_rate);
	}
...
static void send_initial_config(void)
{
	...
	err = nrf_cloud_obj_num_add(&cfg_obj, REST_FOTA_CFG_INTERVAL,
...
static void check_config(void)
{
	...
	err = nrf_cloud_obj_num_get(&cfg_obj, REST_FOTA_CFG_INTERVAL, &desired_check_rate);
	if (err) {
...
int main(void)
{
	...
	LOG_INF("nRF Cloud FOTA Sample, version: %s", CONFIG_NRF_CLOUD_FOTA_SAMPLE_VERSION);
	...
	while (1) {
		...
		LOG_INF("Checking for FOTA job in %d minute(s) or when button %d is pressed",
			fota_check_rate, CONFIG_NRFCLOUD_FOTA_BUTTON_EVT_NUM);
