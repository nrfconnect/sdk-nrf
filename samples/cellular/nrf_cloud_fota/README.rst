.. _nrf_cloud_fota_sample:

Cellular: nRF Cloud FOTA
########################
...
The nRF Cloud FOTA sample demonstrates how to use the `nRF Cloud REST API`_ and `nRF Cloud CoAP API`_ to perform Firmware Over-the-Air (FOTA) updates over REST on your device.
This covers modem, application, and full modem FOTA updates (FMFU).
Also, with the nRF9160 DK, it supports SMP FOTA updates to the firmware on the nRF52840 chip.
...
If it is not, follow the instructions in `Device on-boarding <nrf_cloud_fota_sample_onboarding_>`_.
...
This will bypass the wait time specified by the :ref:`CONFIG_NRFCLOUD_FOTA_JOB_CHECK_RATE_MIN <CONFIG_NRFCLOUD_FOTA_JOB_CHECK_RATE_MIN>` option.
**Button 1** is configured by the :ref:`CONFIG_NRFCLOUD_FOTA_BUTTON_EVT_NUM <CONFIG_NRFCLOUD_FOTA_BUTTON_EVT_NUM>` option.

The configured LTE LED (**LED 1** by default, :ref:`CONFIG_NRFCLOUD_FOTA_LTE_LED_NUM <CONFIG_NRFCLOUD_FOTA_LTE_LED_NUM>`) is lit once an LTE connection is established.

.. _nrf_cloud_fota_sample_onboarding:
...
.. _CONFIG_NRFCLOUD_FOTA_JOB_CHECK_RATE_MIN:

CONFIG_NRFCLOUD_FOTA_JOB_CHECK_RATE_MIN - Update check rate
...
.. _CONFIG_NRFCLOUD_FOTA_LTE_LED_NUM:

CONFIG_NRFCLOUD_FOTA_LTE_LED_NUM - LTE LED number
   Defines the LED used to indicate connection to the LTE network.

.. _CONFIG_NRFCLOUD_FOTA_BUTTON_EVT_NUM:

CONFIG_NRFCLOUD_FOTA_BUTTON_EVT_NUM - Button number
...
.. |sample path| replace:: :file:`samples/cellular/nrf_cloud_fota`

.. include:: /includes/build_and_run_ns.txt

The configuration files for this sample are located in :file:`samples/cellular/nrf_cloud_fota`.
See :ref:`configure_application` for information on how to configure the parameters.
By default, the sample uses the REST API for FOTA updates.
...
To use the nRF Cloud CoAP API for FOTA updates, add the following parameter to your build command:

``-DEXTRA_CONF_FILE=overlay_coap_fota.conf``
