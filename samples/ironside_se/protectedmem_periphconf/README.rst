...
Shared Memory Map
  The :file:`dts/nrf54h20dk_nrf54h20_memory_map.dtsi` file defines the partition layout for the board.
  It relocates ``periphconf_partition`` to be placed right after ``cpuapp_boot_partition``, and defines a ``protectedmem_partition`` subpartition grouping both.
  A matching ``secondary_protectedmem_partition`` covers the secondary firmware's boot and periphconf partitions.
  Both :file:`app.overlay` and :file:`secondary/app.overlay` include this file, and :file:`sysbuild/uicr.overlay` applies it to the UICR image so all images see the same partition layout.

Kconfig Configuration
  The :file:`sysbuild/uicr.conf` file enables PROTECTEDMEM (:kconfig:option:`CONFIG_GEN_UICR_PROTECTEDMEM`), secondary firmware (:kconfig:option:`CONFIG_GEN_UICR_SECONDARY`), and SECONDARY.PROTECTEDMEM (:kconfig:option:`CONFIG_GEN_UICR_SECONDARY_PROTECTEDMEM`).
  The PROTECTEDMEM size is derived automatically from the ``protectedmem_partition`` devicetree node.
  Both images enable :kconfig:option:`CONFIG_FLASH` to use the Zephyr flash driver API.

Secondary Firmware
  The :file:`secondary` directory contains the secondary firmware that boots when the PROTECTEDMEM integrity check fails.
...
Flash Ordering
  :file:`sysbuild.cmake` uses ``sysbuild_add_dependencies(FLASH ...)`` to ensure the UICR image is programmed last.
