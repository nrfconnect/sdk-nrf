...
static void iso_sent(struct bt_iso_chan *chan)
{
	...
	/* Update the sent SDU counter before starting the
	 * timer that triggers when the next SDU is sent
	 * to ensure that the counter is updated in time.
	 */
	...
	if (prev_sent_sdu % 100 == 0) {
		...
		printk("Sent SDU counter %u with timestamp %u (controller time %u), ",
