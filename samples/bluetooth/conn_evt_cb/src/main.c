...
#define CONN_EVT_CB_PREPARE_DISTANCE_US 500
...
static void conn_evt_prepare_cb(struct bt_conn *conn, void *user_data)
{
	int err;
...
static struct bt_conn_evt_cb callbacks = {
	.prepare = conn_evt_prepare_cb,
};
...
static void discovery_complete(struct bt_gatt_dm *dm, void *context)
{
	...
	int err = bt_conn_evt_cb_register(bt_gatt_dm_conn_get(dm),
					  callbacks,
					  NULL,
...
int main(void)
{
	...
	printk("Starting connection event callback sample.\n");
