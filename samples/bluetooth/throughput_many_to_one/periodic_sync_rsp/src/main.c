...
/* MSD header (4B) + protocol version (1B) + protocol flags (1B), */
	...
	if (assigned_slot >= 0) {
		...
		if (is_open) {
			if (++open_seen_consecutive >= 2) {
				LOG_INF("[JOIN] Evicted: slot %d marked open; rejoining",
						assigned_slot);
	...
	struct net_buf_simple *buf)
{
	...
	if (buf && buf->len) {
		...
		if (buf->len >= MIN_PAYLOAD_SIZE) {
			parse_pawr_control_payload(buf, &open_len, open_bitmap,
				&ack_count, &acks_ptr,
				&retransmit_bitmap32);
		}
		...
		if (assigned_slot < 0) {
			...
			if ((ev % mod) == 0) {
				try_claim_open_slot(sync, info, open_len,
									open_bitmap,
									&rsp_buf, &rsp_params);
			}
		}
