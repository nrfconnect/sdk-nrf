...
.. _APP_THROUGHPUT_PAWR:

APP_THROUGHPUT_PAWR
   This configuration option enables the throughput many-to-one sample settings using PAwR.
   It is enabled by default.

.. _APP_NUM_RSP_DEVICES:

APP_NUM_RSP_DEVICES
   This configuration option sets the number of PAwR synchronizer devices that will participate in the PAwR communication.
   It affects timing calculations.
   Set this option in :file:`periodic_adv_rsp/prj.conf` to match the number of synchronizer devices you plan to run.

.. note::
   If :ref:`APP_NUM_RSP_DEVICES` does not match the number of synchronizer devices you run, the advertiser might appear stalled while waiting for unassigned or missing responders.
...
* [LOSS] slot X missed response; should retransmit:
  ...
  * The advertiser expected data from that slot for this event but did not receive it.
...
* Failed to set subevent data (err –13):
  ...
  * Ensure the subevent index is modulo the configured number of subevents and the data length fits.
...
* Same-slot collisions - If two unassigned responders choose the same open slot in the same subevent, their claim responses collide at the PHY and the advertiser receives neither.
  No acknowledgment (ACK) is issued for that event.
  Both devices remain unassigned and retry in later events with probabilistic backoff.
