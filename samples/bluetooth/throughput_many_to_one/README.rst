...
.. note::
   Before building the advertiser, set the :option:`CONFIG_BT_NUM_RSP_DEVICES`  Kconfig option in the :file:`periodic_adv_rsp/prj.conf` file to the number of synchronizer devices you plan to run.
...
* Same-slot collisions - If two unassigned responders choose the same open slot in the same subevent, their claim responses collide at the PHY and the advertiser receives neither.
  No acknowledgement (ACK) is issued that event.
