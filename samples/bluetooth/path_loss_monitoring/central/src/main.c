...
/* Set up logging */
LOG_MODULE_REGISTER(central_unit,
                    LOG_LEVEL_INF);

/* After a name match has occured, this is the functionality that occurs: */
static void scan_filter_match(struct bt_scan_device_info *device_info,
                              struct bt_scan_filter_match *filter_match,
                              bool connectable) {
  char addr[BT_ADDR_LE_STR_LEN];
  bt_addr_le_to_str(device_info->recv_info->addr, addr, sizeof(addr));
  LOG_INF("Found peripheral: %s\n", addr);
...
static void connected(struct bt_conn *conn, uint8_t conn_err) {
  ...
  if (conn_err) {
    LOG_ERR("Failed to connect to %s (err 0x%02x)\n", addr, conn_err);
  ...
  if (err) {
    LOG_ERR("Failed to set path loss monitoring parameters (er: %d)\n", err);
    return;
  }
  ...
  if (err) {
    LOG_ERR("Failed to enable path loss monitoring (err: %d)\n", err);
    return;
  }
...
static void scan_init(void) {
  ...
  if (err) {
    LOG_ERR("Failed to add peripheral name filter (err %d)\n", err);
    return;
  }

  err = bt_scan_filter_enable(BT_SCAN_NAME_FILTER, false);
  if (err) {
    LOG_ERR("Failed to enable filter (err: %d)\n", err);
    return;
  }
}
...
static void path_loss_threshold_report(
  ...
  LOG_INF("Zone %d. Path loss: %d dBm\n", report->zone, report->path_loss);
}
