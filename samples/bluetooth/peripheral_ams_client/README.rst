...
After programming the sample to your development kit, you can test it either by connecting to an iOS device or by using `nRF Connect for Desktop`_ that emulates an AMS Server.
...
#. Set up the server:
    ...
    #. Load the ``AMS_central.ncs`` file located under :file:`samples/bluetooth/peripheral_ams_client` in the |NCS| folder structure.
...
#. Connect to the device from nRF Connect for Desktop. The device is advertising as "Nordic_AMS".
...
#. After bonding, verify in nRF Connect for Desktop that the :guilabel:`Client Characteristic Configuration` (CCCD) value for :guilabel:`Apple Remote Command` and :guilabel:`Apple Entity Update` are set to ``01 00``.

Music setup
^^^^^^^^^^^

#. In nRF Connect for Desktop, verify that the :guilabel:`Apple Entity Update` is initiated to ``00 00 01 02``.
...
#. In nRF Connect for Desktop, verify that the :guilabel:`Apple Entity Update` is updated to ``02 00 01 02 03``.
   The following table lists the attributes requested by the MR.
...
#. In nRF Connect for Desktop, verify that the :guilabel:`Apple Remote Command` is updated to ``02``.
...
#. In nRF Connect for Desktop, verify that the :guilabel:`Apple Remote Command` is updated to ``03``.
...
Disconnect the device in nRF Connect for Desktop.
As the bond information is preserved by nRF Connect for Desktop, you can immediately reconnect to the device by clicking :guilabel:`Connect`.
