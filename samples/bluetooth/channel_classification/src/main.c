...
#define REPORTING_MIN_SPACING	      5U
#define REPORTING_MAX_DELAY	      5U
...
static void channel_map_set_bit(uint8_t map[5], uint8_t channel, bool good)
{
			      ...
			      uint8_t map[5])
{
	...
	for (uint8_t channel = 0; channel < DATA_CHANNEL_COUNT; channel++) {
		...
		const bool good = (value == 0x01U);

		channel_map_set_bit(map, channel, good);
	}
...
static void connected(struct bt_conn *conn, uint8_t err)
{
	...
	if (info.role == BT_CONN_ROLE_CENTRAL) {
		err = central_reporting_start(default_conn);
		if (err) {
			LOG_WRN("Failed to start channel classification reporting (err %d)", err);
		}
	} else {
		channel_map_index = 0;
		k_work_schedule(&channel_map_update_work, CHANNEL_MAP_UPDATE_PERIOD);
	}
}
