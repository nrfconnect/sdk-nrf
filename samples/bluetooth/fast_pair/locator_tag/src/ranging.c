...
static void stop_ranging_rsp_send(struct mgmt_conn *mgmt_conn)
{
	...
	LOG_INF("\tRanging technologies stopped successfully bitfield 0x%04X:",
		mgmt_conn->stop_ranging_tech_bm);
	ranging_tech_bm_print(mgmt_conn->stop_ranging_tech_bm);
...
/* The user_data argument is an ordered array of pointers with the following elements:
 * 1. The peer address (const bt_addr_t *).
 * 2. The bond found flag (bool *).
 * Supplying other values is not allowed.
 */
static void ranging_tech_ble_cs_bond_find(const struct bt_bond_info *info,
					  void *user_data)
{
	void **args = user_data;
	const bt_addr_t *peer_addr;
	bool *bond_found;

	__ASSERT_NO_MSG(user_data);

	peer_addr = *args++;
	bond_found = *args++;

	__ASSERT_NO_MSG(peer_addr);
	__ASSERT_NO_MSG(bond_found);

	if (*bond_found) {
		return;
	}

	if (memcmp(&info->addr.a, peer_addr, sizeof(info->addr.a)) == 0) {
		*bond_found = true;
	}
}
...
static bool ranging_tech_ble_cs_conn_bond_validate(struct mgmt_conn *mgmt_conn)
{
	...
	/* Satisfy the requirements of the ranging_tech_ble_cs_bond_find function. */
	void *args[] = {(void *) &conn_info.le.dst->a, (void *) &bond_found};

	bt_foreach_bond(conn_info.id, ranging_tech_ble_cs_bond_find, args);
