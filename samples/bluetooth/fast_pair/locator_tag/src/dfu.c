...
BT_FAST_PAIR_LOCATOR_TAG_FP_ADV_TRIGGER_REGISTER(fp_adv_trigger_dfu, "dfu");
...
{
	enum bt_fast_pair_locator_tag_fp_adv_mode pre_request_fp_adv_mode =
		bt_fast_pair_locator_tag_fp_adv_mode_get();
	...
	bt_fast_pair_locator_tag_fp_adv_request(&fp_adv_trigger_dfu, dfu_mode);

	/* Ensure that the advertising payload is updated if the advertising was already
	 * enabled before the advertising request.
	 */
	if ((pre_request_fp_adv_mode == BT_FAST_PAIR_LOCATOR_TAG_FP_ADV_MODE_DISCOVERABLE) ||
	    (pre_request_fp_adv_mode == BT_FAST_PAIR_LOCATOR_TAG_FP_ADV_MODE_NOT_DISCOVERABLE)) {
		bt_fast_pair_locator_tag_fp_adv_payload_refresh();
	}
		    ...
		    struct bt_le_adv_prov_feedback *fb)
{
	...
	enum bt_fast_pair_locator_tag_fp_adv_mode current_fp_adv_mode =
		bt_fast_pair_locator_tag_fp_adv_mode_get();
	...
	if (current_fp_adv_mode != BT_FAST_PAIR_LOCATOR_TAG_FP_ADV_MODE_DISCOVERABLE) {
		return -ENOENT;
	}
		    ...
		    struct bt_le_adv_prov_feedback *fb)
{
	...
	if (current_fp_adv_mode != BT_FAST_PAIR_LOCATOR_TAG_FP_ADV_MODE_NOT_DISCOVERABLE) {
		return -ENOENT;
	}
