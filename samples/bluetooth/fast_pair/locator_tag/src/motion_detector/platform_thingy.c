...
/* Minimum required rotation in degrees to consider device as moving */
#define ROT_THR_RAD		(ROT_THR_DEG * M_PI / 180)
...
static struct k_thread gyro_poll_thread;
static K_THREAD_STACK_DEFINE(gyro_poll_thread_stack, GYRO_POLL_THREAD_STACK_SIZE);
...
static struct motion_detector motion_detector;
...
static bool motion_detected(void)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(motion_detector.rot); i++) {
		if (fabs(motion_detector.rot[i]) >= ROT_THR_RAD) {
			motion_axis_count++;
		}
	}

	return (motion_axis_count >= 2);
}
...
static void sensor_fetch(void)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(motion_detector.rot); i++) {
		/* Rotation = Angular velocity * Time = Angular velocity / Sampling frequency. */
		motion_detector.rot[i] += sensor_value_to_double(&val[i]) / GYRO_SAMPLING_FREQ;
	}
}
...
int app_motion_detector_init(void)
{
	...
	if (!device_is_ready(pwr.port)) {
		LOG_ERR("FMDN: GYRO_PWR_NODE is not ready");
		return -EIO;
	}

	err = gpio_pin_configure_dt(&pwr, GPIO_OUTPUT_ACTIVE);
	if (err) {
		LOG_ERR("FMDN: Error while configuring GYRO_PWR_NODE (err %d)", err);
		return err;
	}

	sensor = DEVICE_DT_GET(GYRO_NODE);
	if (!sensor) {
		LOG_ERR("FMDN: No sensor device found");
		return -EIO;
	}

	if (!device_is_ready(sensor)) {
		LOG_ERR("FMDN: Device %s is not ready.", sensor->name);
		return -EIO;
	}

	err = sensor_configure(sensor);
	if (err) {
		LOG_ERR("FMDN: Configuring gyroscope sensor failed (err %d)", err);
		return err;
	}

	motion_detector.dev = sensor;
	...
	(void) k_thread_create(&gyro_poll_thread,
			       gyro_poll_thread_stack,
			       K_THREAD_STACK_SIZEOF(gyro_poll_thread_stack),
			       gyro_poll_thread_fn, NULL, NULL, NULL,
			       GYRO_POLL_THREAD_PRIORITY, 0, K_NO_WAIT);
