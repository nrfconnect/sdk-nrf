...
DULT Motion detector active mode
--------------------------------

In the provisioned state, the accessory can enter the DULT motion detector active mode.
It happens when the accessory is separated from the owner for a randomized amount of time between :kconfig:option:`CONFIG_DULT_MOTION_DETECTOR_SEPARATED_UT_TIMEOUT_PERIOD_MIN` and :kconfig:option:`CONFIG_DULT_MOTION_DETECTOR_SEPARATED_UT_TIMEOUT_PERIOD_MAX`.
In this state, if the motion is detected, the accessory starts the ringing action.
