...
To enable the LE pairing authentication with the LE Pairing Responder model support, build this sample with :makevar:`EXTRA_CONF_FILE` set to :file:`overlay-smp-bt-auth.conf` file.
See :ref:`cmake_options` for instructions on how to provide this build variable.

.. |variable_feature| replace:: the LE pairing authentication with the LE Pairing Responder model support
.. |makevar| replace:: EXTRA_CONF_FILE
.. |cmake_file_name| replace:: overlay-smp-bt-auth.conf
