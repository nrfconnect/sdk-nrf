...
Provisioning is handled by the :ref:`bt_mesh_dk_prov`, with the default :file:`prj.conf` configuration.
...
* Health Server is used to call attention to the device during provisioning.
...
If the function returns successfully, the response buffer will contain the response from Mesh Light.
...
Repeat steps 3-6 for each of the elements on the node to control each of the remaining three LEDs on the Mesh Light device.
