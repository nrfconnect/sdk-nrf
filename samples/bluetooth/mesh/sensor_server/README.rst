...
.. note::
   This sample must be paired with the :ref:`bluetooth_mesh_nlc_sensor_observer` sample to show any functionality.
...
Additionally, the sample requires the :ref:`bluetooth_mesh_nlc_sensor_observer` sample application.
...
.. note::
   These values can be requested through shell commands by the :ref:`bluetooth_mesh_nlc_sensor_observer`.
...
.. note::
   The Bluetooth Mesh sensor sample cannot demonstrate any functionality on its own, and needs a device with the :ref:`bluetooth_mesh_nlc_sensor_observer` sample running in the same mesh network.
   Before testing the sensor sample, go through the sensor observer sample's :ref:`testing guide <bluetooth_mesh_nlc_sensor_observer_testing>` with a different development kit.
