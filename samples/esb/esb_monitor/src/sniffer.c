...
static uint8_t rtt_data_buff[CONFIG_ESB_SNIFFER_RTT_DATA_BUFF_SZ];
static uint8_t rtt_comm_ret_buff[CONFIG_ESB_SNIFFER_RTT_COMM_RET_BUFF_SZ];
static uint8_t rtt_comm_buff[CONFIG_ESB_SNIFFER_RTT_COMM_BUFF_SZ];
...
static void radio_reg_to_array(uint32_t addr, char *array)
{
#if __CORTEX_M == (0x04U)
	uint32_t inp = addr;
...
#else
	uint32_t inp = sys_cpu_to_le32(addr);
...
static void rtt_comm_thread_fn(void)
{
	...
	read_len = sizeof(struct esb_payload) + 8;
	read_len = sys_cpu_to_be32(read_len);
	SEGGER_RTT_WriteNoLock(CONFIG_ESB_SNIFFER_RTT_COMM_RET_CHANNEL,
						&read_len, sizeof(uint32_t));

	while (1) {
		read_len = SEGGER_RTT_Read(CONFIG_ESB_SNIFFER_RTT_COMM_CHANNEL,
		...
		if (read_len == sizeof(enum esb_sniffer_rtt_comm)) {
			...
			case SNIFFER_STATUS:
				...
				break;
			default:
				LOG_ERR("Unrecognized rtt command: %d", comm);
			}
		}
...
int sniffer_init(void)
{
	...
	err = SEGGER_RTT_ConfigUpBuffer(CONFIG_ESB_SNIFFER_RTT_DATA_CHANNEL,
					"Data", rtt_data_buff,
					CONFIG_ESB_SNIFFER_RTT_DATA_BUFF_SZ,
					CONFIG_SEGGER_RTT_MODE_NO_BLOCK_SKIP);

	err += SEGGER_RTT_ConfigUpBuffer(CONFIG_ESB_SNIFFER_RTT_COMM_RET_CHANNEL,
					 "Comm_ret", rtt_comm_ret_buff,
					 CONFIG_ESB_SNIFFER_RTT_COMM_RET_BUFF_SZ,
					 CONFIG_SEGGER_RTT_MODE_NO_BLOCK_SKIP);

	err += SEGGER_RTT_ConfigDownBuffer(CONFIG_ESB_SNIFFER_RTT_COMM_CHANNEL,
					   "Commands", rtt_comm_buff,
					   CONFIG_ESB_SNIFFER_RTT_COMM_BUFF_SZ,
					   CONFIG_SEGGER_RTT_MODE_NO_BLOCK_SKIP);
	if (err != 0) {
