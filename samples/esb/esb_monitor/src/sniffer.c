...
static void _rtt_comm_exec(enum esb_sniffer_rtt_comm command)
{
	int err;
	...
	case SNIFFER_START:
		...
		SEGGER_RTT_Write(CONFIG_ESB_SNIFFER_RTT_COMM_RET_CHANNEL,
								&err, sizeof(int));
		break;
	case SNIFFER_STOP:
...
int sniffer_init(void)
{
	...
	BUILD_ASSERT(
		CONFIG_ESB_SNIFFER_RTT_DATA_CHANNEL != CONFIG_ESB_SNIFFER_RTT_COMM_RET_CHANNEL,
