...
static int get_random_bytes_str(uint8_t *output_str, const size_t output_str_length)
{
	int err = -ENODATA;

	if ((NULL == output_str) || (output_str_length > (RANDOM_STR_MAX_SIZE))) {
		/* Bad parameter */
		return -EINVAL;
	}
	...
	psa_status_t psa_status = psa_generate_random(tmp_output_buffer, sizeof(tmp_output_buffer));
	...
	for (uint32_t i = 0; i < ((RANDOM_STR_MAX_SIZE - 1) / 8); i++) {
