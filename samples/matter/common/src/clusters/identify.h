...
namespace Matter
{
	 ...
	 * Kits. It is used to blink the LED2 on the nRF Development Kit.
	 *
	...
	public:
		...
		explicit IdentifyDelegateImplNrf(bool isTriggerEffectEnabled = false,
						 void (*customIdentifyStopCallback)(void) = nullptr)
			: mIsTriggerEffectEnabled(isTriggerEffectEnabled),
			  mCustomIdentifyStopCallback(customIdentifyStopCallback)
		{
		}

		virtual void OnIdentifyStart(chip::app::Clusters::IdentifyCluster &cluster) override
		{
			Nrf::PostTask([] {
				Nrf::GetBoard()
		...
		virtual void OnIdentifyStop(chip::app::Clusters::IdentifyCluster &cluster) override
		{
		...
		virtual void OnTriggerEffect(chip::app::Clusters::IdentifyCluster &cluster) override
		{
		...
		virtual bool IsTriggerEffectEnabled() const override { return mIsTriggerEffectEnabled; }

	protected:
		bool mIsTriggerEffectEnabled = false;
		void (*mCustomIdentifyStopCallback)(void) = nullptr;
	};
	...
	public:
		 ...
		 * Kits.
		 *
		 * The default behavior is to blink the LED2 on the nRF Development Kit.
		 *
