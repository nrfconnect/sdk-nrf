...
CHIP_ERROR BindingHandler::RegisterBindingEventHandler(ReleaseCallback bindingReleaseCallback)
{
	ReturnErrorOnFailure(Nrf::Matter::RegisterEventHandler(BindingMatterEventHandler, 0));

	mReleaseCallbackPointer = bindingReleaseCallback;

	return CHIP_NO_ERROR;
}

void BindingHandler::BindingMatterEventHandler(const DeviceLayer::ChipDeviceEvent *event, intptr_t /* unused */)
{
	switch (event->Type) {
	case DeviceLayer::DeviceEventType::kBindingsChangedViaCluster:
		if (BindingTable::GetInstance().Size() == 0) {
			Instance().mReleaseCallbackPointer();
			UnbindDevice(event->BindingsChanged.fabricIndex);
		}
		break;
	default:
		break;
...
void BindingHandler::UnbindDevice(FabricIndex fabricIndex)
{
	LOG_INF("Removing binding from device: %d", fabricIndex);
	BindingManager::GetInstance().UnicastBindingRemoved(fabricIndex);
}
