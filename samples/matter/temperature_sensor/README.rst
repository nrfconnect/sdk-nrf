...
This sample demonstrates the usage of the :ref:`Matter <ug_matter>` application layer to build a device capable of measuring temperature.
...
The sample does not use real temperature sensor due to hardware limitation.
Instead, it simulates temperature measurement following the lineary increasing values from –20 to +20 Celsius degrees.
...
The temperature sensor works as a Matter Intermittently Connected Device (ICD) with a :ref:`Long Idle Time (LIT)<ug_matter_device_low_power_icd_sit_lit>`.
...
.. tabs::
   ...
   .. group-tab:: nRF52 and nRF53 DKs
      ...
      #. Read the temperature measured by the device by invoking the following command with the *<node_id>* and *<endpoint_id>* replaced with values selected by you (for example, ``1`` and ``1``):
