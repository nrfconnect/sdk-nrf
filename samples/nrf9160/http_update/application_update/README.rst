...
It uses the :ref:`lib_fota_download` library to download a file from a remote server and write it to flash memory.

Overview
********

The sample connects to an HTTP server to download a signed firmware image.
The image is generated when building the sample, but you must upload it to a server and configure the location from where it can be downloaded.

The sample uses *two* firmware images: one having the *application version* set to ``1`` and the other having that set to ``2``.
...
By default, the image is saved to the `MCUboot`_ second-stage bootloader secondary slot.
...
The sample also uses MCUboot as a second-stage bootloader, which is automatically built and merged into the final HEX file when building the sample.
...
Note that two options configure the names of the updated image files:

- ``CONFIG_DOWNLOAD_FILE_V1`` should be set to the file name of version 1 of the update image.
- ``CONFIG_DOWNLOAD_FILE_V2`` should be set to the file name of version 2 of the update image.

To specify the location in |SES|, do the following:

1. Select :guilabel:`Project` -> :guilabel:`Configure nRF Connect SDK Project`.
#. Navigate to :guilabel:`HTTP application update sample` and specify the hostname of the server hosting the images (``CONFIG_DOWNLOAD_HOST``) and the names of the files that have to be downloaded (``CONFIG_DOWNLOAD_FILE_V1`` and ``CONFIG_DOWNLOAD_FILE_V2``).
...
1. Upload the file :file:`app_update.bin` with the application version 1 to the server you have chosen.
...
#. Upload the file :file:`app_update.bin` with the application version 2 to the server you have chosen.
...
#. Press the **RESET** button on the kit.
   MCUboot will now detect the downloaded image and write it to flash memory.
