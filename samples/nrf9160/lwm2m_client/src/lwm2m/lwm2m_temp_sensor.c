...
int lwm2m_init_temp_sensor(void)
{
	...
	lwm2m_engine_get_res_buf(LWM2M_PATH(IPSO_OBJECT_TEMP_SENSOR_ID, 0, SENSOR_VALUE_RID),
				  (void **)&temp_float, NULL, &dummy_data_len, &dummy_data_flags);
	lwm2m_engine_set_res_buf(LWM2M_PATH(IPSO_OBJECT_TEMP_SENSOR_ID, 0, SENSOR_UNITS_RID),
				  TEMP_UNIT, sizeof(TEMP_UNIT), sizeof(TEMP_UNIT), LWM2M_RES_DATA_FLAG_RO);
	...
	if (IS_ENABLED(CONFIG_LWM2M_IPSO_TEMP_SENSOR_VERSION_1_1)) {
		...
		lwm2m_engine_set_res_buf(LWM2M_PATH(IPSO_OBJECT_TEMP_SENSOR_ID, 0,
						    APPLICATION_TYPE_RID),
					 TEMP_APP_TYPE, sizeof(TEMP_APP_TYPE),
					 sizeof(TEMP_APP_TYPE), LWM2M_RES_DATA_FLAG_RO);
		lwm2m_engine_set_res_buf(LWM2M_PATH(IPSO_OBJECT_TEMP_SENSOR_ID, 0, TIMESTAMP_RID),
					  &lwm2m_timestamp, sizeof(lwm2m_timestamp), sizeof(lwm2m_timestamp),
					  LWM2M_RES_DATA_FLAG_RW);
		lwm2m_engine_set_res_buf(LWM2M_PATH(IPSO_OBJECT_TEMP_SENSOR_ID, 0,
						     MEASUREMENT_QUALITY_INDICATOR_RID),
					  &meas_qual_ind, sizeof(meas_qual_ind), sizeof(meas_qual_ind),
					  LWM2M_RES_DATA_FLAG_RW);
	}
