...
static void request_location(enum multicell_service service)
{
	int err;
	struct multicell_location location;

	LOG_INF("Sending location request for %d ...", service);
	...
	LOG_INF("Location obtained from %d: ", service);
...
void main(void)
{
	...
	while (true) {
		...
		request_location(MULTICELL_SERVICE_NRF_CLOUD);
#endif
#if defined(CONFIG_MULTICELL_LOCATION_SERVICE_HERE)
		request_location(MULTICELL_SERVICE_HERE);
#endif
#if defined(CONFIG_MULTICELL_LOCATION_SERVICE_SKYHOOK)
		request_location(MULTICELL_SERVICE_SKYHOOK);
#endif
		request_location(MULTICELL_SERVICE_ANY);
	}
}
