...
The TCP Client sample demonstrates the sequential transmission of TCP packets to a predetermined server identified by an IP address and a port.
...
It configures a TCP socket and continuously transmits data over the socket to the modem's TCP/IP stack, where the data eventually gets transmitted to a server specified by an IP address and a port number.
...
You can configure the frequency with which the packets are transmitted and the size of the TCP payload through the Kconfig system.
...
The TCP Client sample can be used to characterize the current consumption of the nRF9160 SiP.
This is due to the *simple TCP/IP behavior* demonstrated by the sample, which makes it suitable for current measurement.
...
* :option:`CONFIG_TCP_DATA_UPLOAD_SIZE_BYTES`
* :option:`CONFIG_TCP_DATA_UPLOAD_FREQUENCY_SECONDS`
* :option:`CONFIG_TCP_SERVER_ADDRESS_STATIC`
* :option:`CONFIG_TCP_SERVER_PORT`
* :option:`CONFIG_TCP_PSM_ENABLE`
* :option:`CONFIG_TCP_EDRX_ENABLE`
* :option:`CONFIG_TCP_RAI_ENABLE`


Configuration options
=====================

.. option:: CONFIG_TCP_DATA_UPLOAD_SIZE_BYTES - TCP  data upload size configuration

This configuration option sets the number of bytes to be transmitted to the server.

.. option:: CONFIG_TCP_DATA_UPLOAD_FREQUENCY_SECONDS - TCP data upload frequency configuration

This configuration option sets the frequency with which the sample transmits data to the server.

.. option:: CONFIG_TCP_SERVER_ADDRESS_STATIC - TCP Server IP Address configuration

This configuration option sets the static IP address of the server.

.. option:: CONFIG_TCP_SERVER_PORT - TCP server port configuration

This configuration option sets the server address port number.

.. option:: CONFIG_TCP_PSM_ENABLE - PSM mode configuration

This configuration option, if set, allows the sample to request PSM from the modem or cellular network.

.. option:: CONFIG_TCP_EDRX_ENABLE - eDRX mode configuration

This configuration option, if set, allows the sample to request eDRX from the modem or cellular network.

.. option:: CONFIG_TCP_RAI_ENABLE - RAI configuration
 ...
 * :option:`CONFIG_LTE_PSM_REQ_RPTAU` option set to a value greater than the value of :option:`CONFIG_TCP_DATA_UPLOAD_FREQUENCY_SECONDS`.
 * :option:`CONFIG_LTE_PSM_REQ_RAT` set to 0.
 * :option:`CONFIG_SERIAL` disabled in ``prj.conf`` and ``spm.conf``.
 * :option:`CONFIG_TCP_EDRX_ENABLE` set to false.
 * :option:`CONFIG_TCP_RAI_ENABLE` set to true for NB-IoT. It is not supported for LTE-M.
...
They are located in ``samples/nrf9160/tcp_client`` folder.

Building and running
********************

.. |sample path| replace:: :file:`samples/nrf9160/tcp_client`
...
.. code-block:: console
      ...
      *** Booting Zephyr OS build v2.3.0-rc1-ncs1-1451-gb160c8c5caa5  ***
      ...
      Transmitting TCP/IP payload of 38 bytes to the IP address 8.8.8.8, port number 2469
