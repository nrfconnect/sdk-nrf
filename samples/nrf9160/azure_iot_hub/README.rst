...
If DPS is used, use the Kconfig fragment found in :file:`overlay-dps.conf` and change the desired configurations there.
...
* :kconfig:option:`CONFIG_AZURE_IOT_HUB_DPS_REG_ID` - Sets the Azure IoT Hub DPS registration ID. Can be provided at run time. The sample by default uses the device ID as the registration ID and sets it at run time.
* :kconfig:option:`CONFIG_AZURE_IOT_HUB_DPS_ID_SCOPE` - Sets the Azure IoT Hub DPS ID scope. Can be provided at run time.
...
If ta new telemetry interval is set in the device twin, the console output is like below:
