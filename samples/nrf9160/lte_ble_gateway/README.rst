...
Therefore, this sample acts as a gateway between the |BLE| and the LTE connections to the nRF Connect for Cloud.
...
You can then trigger an alarm that sends the aggregated data over LTE to the `nRF Connect for Cloud`_ by flipping the Thingy:52, which causes a change in the flip state to ``UPSIDE_DOWN``.
...
When the connection is established, set switch 2 to **N.C.** to send simulated GPS data to the nRF Connect for Cloud once every 2 seconds.
...
#. Set switch 2 in the position marked as **N.C.** and observe that simulated GPS data is sent to the nRF Connect for Cloud.
