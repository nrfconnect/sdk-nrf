...
static inline int z_zephyr_read_stdin(char * buf, int nbytes)
{
	...
	if (z_syscall_trap()) {
		return (int) z_arch_syscall_invoke2(*(u32_t *)&buf, *(u32_t *)&nbytes, K_SYSCALL_Z_ZEPHYR_READ_STDIN);
	}
#endif
	compiler_barrier();
...
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		return (int) z_arch_syscall_invoke2(*(u32_t *)&buf, *(u32_t *)&nbytes, K_SYSCALL_Z_ZEPHYR_WRITE_STDOUT);
	}
#endif
	compiler_barrier();
...
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		return (int) z_arch_syscall_invoke2(*(u32_t *)&c, *(u32_t *)&stream, K_SYSCALL_ZEPHYR_FPUTC);
	}
#endif
	compiler_barrier();
...
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		return (size_t) z_arch_syscall_invoke4(*(u32_t *)&ptr, *(u32_t *)&size, *(u32_t *)&nitems, *(u32_t *)&stream, K_SYSCALL_ZEPHYR_FWRITE);
	}
#endif
	compiler_barrier();
