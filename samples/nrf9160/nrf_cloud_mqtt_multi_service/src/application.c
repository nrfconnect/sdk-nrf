 ...
 */
static int create_timestamped_device_message(struct nrf_cloud_obj *const msg, const char *const appid,
					     const char *const msg_type)
{
	...
	if (err) {
		LOG_ERR("Failed to initialize message with appid %s and message type %s",
			appid, msg_type);
		return err;
	}
