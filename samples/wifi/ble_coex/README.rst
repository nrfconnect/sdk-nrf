...
* Test modes: Use the following Kconfig options to select the required test case:
  ...
  * :option:`CONFIG_TEST_TYPE_WLAN_ONLY` for Wi-Fi-only test.
  * :option:`CONFIG_TEST_TYPE_BLE_ONLY` for Bluetooth LE-only test.
  * :option:`CONFIG_TEST_TYPE_WLAN_BLE` for concurrent Wi-Fi and Bluetooth LE test.

  Based on the required test, set only one of these to ``y``.
* Test duration: Use the :option:`CONFIG_WIFI_TEST_DURATION` Kconfig option to set the duration of the Wi-Fi test and :option:`CONFIG_BLE_TEST_DURATION` for the Bluetooth LE test.
...
* Bluetooth LE configuration: Set the Bluetooth LE connection interval limits using the :option:`CONFIG_INTERVAL_MIN` and :option:`CONFIG_INTERVAL_MAX` Kconfig options.
...
.. note::
   Use the same test duration value for :option:`CONFIG_WIFI_TEST_DURATION`, :kconfig:option:`CONFIG_BLE_TEST_DURATION`, and :kconfig:option:`CONFIG_BT_THROUGHPUT_DURATION`.
