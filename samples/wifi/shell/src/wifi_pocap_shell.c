...
static int configure_playout_capture(uint32_t rx_mode, uint32_t tx_mode, uint32_t rx_holdoff_length,
...
static int cmd_wifi_pocap(const struct shell *sh, size_t argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "r:t:h:w:b:")) != -1) {
		...
		default:
			shell_print(
				...
				"Defaults: rx_mode=0 tx_mode=0 rx_holdoff_length=0 rx_wrap_length=0\n"
				"back_to_back_mode=0"
			);
			return -EINVAL;
		}
	...
	if (back_to_back_mode) {
		...
		shell_print(sh,
			    ...
			    back_to_back_mode);
		configure_playout_capture(0, 0, 0, 0, back_to_back_mode);
	} else {
		shell_print(
			sh,
			"Configuring with: rx_mode=%u, tx_mode=%u, rx_holdoff_length=%u, rx_wrap_length=%u "
			"back_to_back_mode=%u",
			rx_mode, tx_mode, rx_holdoff_length, rx_wrap_length,
			back_to_back_mode);
		configure_playout_capture(rx_mode, tx_mode, rx_holdoff_length, rx_wrap_length, 0);
	}
	return 0;
}
