...
To build for the Thingy:91 X to be run on the nRF5340, use the ``thingy91x/nrf5340/cpuapp`` board target with the ``SB_CONFIG_THINGY91X_STATIC_PARTITIONS_NRF53_EXTERNAL_FLASH=y`` CMake option set.
This requires an external debugger since the nRF9151 normally owns the buses. Erase the nRF9151 first to avoid conflicts. 
