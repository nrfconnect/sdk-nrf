...
The HTTP server sample demonstrates how to host an HTTP server on an Internet-connected Nordic Semiconductor's device through cellular or Wi-Fi.
...
There are slight differences in how the sample behaves that depend on the wireless technology for which the sample is built.
The following table explains some of these differences:

+------------+------------------+---------------------------------------+----------------+-------------------------------+------------------------+
|            | Network access   | LED resource URL                      | Security       | Peer verification             | IP family\***          |
+============+==================+=======================================+================+===============================+========================+
| Wi-Fi      | Local            | \http://httpserver.local/led/<led_id> | TLS (optional) | Server and client\**          | IPv4 IPv6              |
+------------+------------------+---------------------------------------+----------------+-------------------------------+------------------------+
| Cellular   | Global\*         | \http://<ip>/led/<led_id>             | TLS (optional) | Server and client\**          | IPv4 IPv6              |
+------------+------------------+---------------------------------------+----------------+-------------------------------+------------------------+

| \* - When running the server on a cellular board, you are dependent on a SIM card with a fixed/static IP to be reachable from the global internet.
       These SIM cards can be provided by Mobile Network Operators (MNO) and are often referred to as IoT SIM cards.
| \** - The sample includes pregenerated server and client certificates with the local hosthame *httpserver.local* used as the Common Name (CN).
        These ceritifcates will work out of the box when building the sample for Wi-Fi.
        On cellular, a static IP is required to reach the server.
        This means that a new set of server and client certificates needs to be generated, with a CN matching the assigned IP.
| \*** - IPv4 and/or IPv6 may not be enabled in your W-FI Access Point (AP) or cellular network.
...
.. tabs::
   ...
   .. group-tab:: Cellular
      ...
      * :kconfig:option:`CONFIG_PDN_DEFAULTS_OVERRIDE` - Used override the default PDP context configuration.
      ...
      * Run the ``AT+CGDCONT?`` AT command on a serial terminal.
      ...
      By default, the server disables both Power Saving Mode (PSM) and Extended Discontinuous Reception (eDRX) to ensure maximum availability, which increases the power consumption of the server.
...
Peer verification can be used by building the sample with the :ref:`CONFIG_HTTP_SERVER_SAMPLE_PEER_VERIFICATION_REQUIRE <CONFIG_HTTP_SERVER_SAMPLE_PEER_VERIFICATION_REQUIRE>` kconfig option, and passing in the appropriate credentials when making the HTTP calls.
These pregenerated credentials will only work when you are building for Wi-Fi and running the server on your local Wi-Fi network.
You will have to generate new credentials when building for cellular due to a CN mismatch.
...
Replace the generated header files with the pregenerated header files in the sample's credential folder.
...
CONFIG_HTTP_SERVER_SAMPLE_PEER_VERIFICATION_REQUIRE
   This configuration option enables peer verification for the HTTP server sample.
