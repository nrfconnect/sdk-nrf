...
LOG_MODULE_REGISTER(ip_addr_helper, 0);
 ...
 *	    It's expected that only one of the input arguments points to a valid
 *	    buffer when calling this function. The other buffer must be set to NULL.
 *
 * @param [out] addr4 Pointer to a NULL terminated buffer for IPv4 address.
 * @param [out] addr6 Pointer to a NULL terminated buffer for IPv6 address.
 *
 * @returns Length of address string on success, negative value on failure.
 */
static int ip_addr_get(char *addr4, char *addr6)
{
	 ...
	 * PDN type "IP": PDP_addr_1 is <IPv4>, max 16(INET_ADDRSTRLEN)
	 * PDN type "IPV6": PDP_addr_1 is <IPv6>, max 46(INET6_ADDRSTRLEN)
	...
	if ((addr6 != NULL) && (ret > 1) &&
	    (zsock_inet_pton(NET_AF_INET6, addr2, tmp) == 1)) {
		strcpy(addr6, addr2);
		return strlen(addr6);
	}
#endif
