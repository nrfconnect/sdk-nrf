...
if [[ $setup_hub -eq 1 ]]; then
    ...
    for i in {1..10}; do
        if ! az group show --name $AZ_RGROUP; then
            ...
            rgroup_status=$(az group show --name $AZ_RGROUP \
    ...
    for i in {1..10}; do
        if ! az iot hub show --name $AZ_HUB; then
            echo "IoT Hub not found, retrying"
        else
            hub_status=$(az iot hub show --name $AZ_HUB --query "properties.provisioningState" \
                --output tsv)
...
if [[ $create_ca_certs -eq 1 ]]; then
    ...
    if [[ $setup_dps -eq 1 ]]; then
        ...
        cert_etag=$(az iot dps certificate create --dps-name $AZ_DPS_NAME \
            --resource-group $AZ_RGROUP --name $AZ_CA_CERT --path ca/root-ca-cert.pem --query etag \
            --output tsv)
        ...
        verification_code=$(echo $gen_code_res | head -n 1)
        verify_etag=$(echo $gen_code_res | tail -n 1)
