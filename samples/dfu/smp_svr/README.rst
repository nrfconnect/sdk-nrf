...
.. list-table::
   ...
   * - sample.dfu.smp_svr.encryption.ecdsa_p256
     - UART
     - Swap
     - Encryption using ECDSA P-256
   * - sample.dfu.smp_svr.encryption.ed25519
     - UART
     - Swap
     - Encryption using Ed25519
...
#. Disconnect from the terminal in order to avoid conficts - the SMP communication is performed over the same serial port as the log output.
