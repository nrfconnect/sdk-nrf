...
Sample by default is building with :ref:`fw_loader_ble_mcumgr` firmware loader image.
...
#. Enter the firmware loader mode by holding the **Button 0** on your development kit while you reset the device.
   ...
   a) For Bluetooth firmware loader: Open the `nRF Connect Device Manager`_ mobile app to perform DFU over Bluetooth LE.
   ...
   b) For USB CDC ACM serial firmware loader: Use `nRF Util mcu-manager serial` to perform DFU over serial port.
