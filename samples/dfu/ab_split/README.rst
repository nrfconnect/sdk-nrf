...
This sample implements a device health check mechanism to ensure the firmware operates correctly after an update.
...
The application image is verified by checking the value of the :kconfig:option:``CONFIG_EMULATE_APP_HEALTH_CHECK_FAILURE`` symbol.
If this option is defined, the application image is considered faulty and the self-test fails.
...
Although the device exposes two images over SMP, only the manifest image (the application image) can be confirmed or reverted.
All confirmation flags set for the manifest image also apply to the other images described by the manifest.
...
This approach allows the bootloader to block all write requests to the active image partition, increasing system security and reducing permission fragmentation.
