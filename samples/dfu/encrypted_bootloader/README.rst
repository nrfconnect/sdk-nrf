...
Encrypted application and MCUboot updates
#########################################
...
The sample may be extended to support other transports by adding the appropriate overlay files.
...
* **MCUboot** — Validates and selects an appropriate destination slot of the encrypted update candidate stored instide the ``slot1_partition`` partition.
  The detection of the type of the update candidate may be based on either the load address or the UUID values in the image header.
...
Both stages use the same encryption and signature machinery: images are signed with ED25519 and encrypted for transport with X25519/AES as described in :ref:`mcuboot_wrapper` (Encrypted images).
...
The steps described above build the basic configuration with the UART transport and MCUboot using load address to determine the update candidate.
...
#. List the images on the device.
   ...
   The MCUboot image is not listed by MCUmgr commands, becasue it is not a regular MCUboot image.
   The active MCUboot version and slot can be identified based on the first stage bootloader logs:
...
#. Rebuild the sample with a new version.
   ...
   The application version is set using the :kconfig:option:`CONFIG_MCUBOOT_IMGTOOL_SIGN_VERSION` Kconfig option.
   The MCUboot version is set using the :kconfig:option:`CONFIG_FW_INFO_FIRMWARE_VERSION` Kconfig option.

   You can set both of them by providing additional arguments to the build command:
...
#. Set the confirmed flag for the second version of the image, using the acquired hash:
   ...
   If the MCUboot update candidate for an inactive slot is deteted, the slot is being overwritten with the new image.
