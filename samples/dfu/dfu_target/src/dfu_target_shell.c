...
static int cmd_dfu_target_init(const struct shell *shell, size_t argc, char **argv)
{
	...
	int img_type = (enum dfu_target_image_type) atoi(argv[1]);
	...
	enum dfu_target_image_type img_type_in_buffer = dfu_target_img_type(
...
SHELL_STATIC_SUBCMD_SET_CREATE(dfu_target_cmds,
	...
	SHELL_CMD(write, NULL,
		  "Write a chunk of data at a given offset inside the buffer in RAM "
		  "into dfu_target",
	...
	SHELL_CMD(full_update, NULL,
			  "Perform all the steps to fully update the firmware from the RAM buffer",
