...
If :kconfig:option:`CONFIG_MEMFAULT_NCS_INTERNAL_FLASH_BACKED_COREDUMP` is enabled, :kconfig:option:`CONFIG_PM_PARTITION_SIZE_MEMFAULT_STORAGE` can be used to set the flash partition size for the flash storage when Partition Manager is in use.

When building without Partition Manager (NCS 3.4+), enable the :ref:`snippet-memfault-coredump` snippet to supply the coredump partition via devicetree:
