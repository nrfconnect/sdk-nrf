...
The shell offers multiple commands to test a wide range of functionality offered by Memfault SDK.
...
*  Press **Button 1** to trigger a stack overflow
*  Press **Button 2** to trigger a division by zero
...
If nRF7002 DK is used and one wishes to automatically connect to a Wi-Fi network upon boot, the Wi-Fi credentials be configured using :kconfig:option`CONFIG_WIFI_CREDENTIALS_STATIC_SSID` and :kconfig:option:`CONFIG_WIFI_CREDENTIALS_STATIC_PASSWORD`.
...
#. When the connection is established, the following messag eis printed:
...
For Wi-Fi, the sample also uses modules that can be found in the following locations in the |NCS| folder structure:
