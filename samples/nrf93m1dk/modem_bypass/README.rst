...
* Include ``nordic-flpr`` snippet on the ``radio_test`` target.

For example, when building on the command line, enter the following command:

.. code-block:: console

   west build -p -b nrf93m1dk/nrf54l15/cpuflpr --sysbuild -- -DSB_CONFIG_APPCORE_RADIO_TEST=y -DSB_CONFIG_VPR_LAUNCHER=n -Dradio_test_SNIPPET="nordic-flpr"
