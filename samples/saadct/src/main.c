...
static struct api_saadct_config saadct_cfg = {
...
int main(void)
{
	...
	k_msleep(100);
	saadct_stop(saadct_dev, true);
	LOG_INF("Stop requested, pending series: %u", saadct_meas_pending(saadct_dev));

	while (series_cnt < NUMBER_OF_SERIES) {
		ret = saadct_meas_get(saadct_dev, &samples);
		if (ret == -EAGAIN) {
			k_msleep(1);
			continue;
		}
		...
		if (ret < 0) {
			...
			LOG_WRN("saadct_meas_get returned %d after %u series", ret, series_cnt);
			break;
		}

		LOG_INF("series[%u]: first=0x%04x last=0x%04x", series_cnt, samples[0],
			samples[samples_per_series - 1]);
		saadct_meas_free(saadct_dev, samples);
		series_cnt++;
	}
	...
	while (saadct_meas_pending(saadct_dev) > 0) {
		...
		if (ret < 0) {
			LOG_WRN("saadct_meas_get returned %d while draining", ret);
			break;
		}
