...
The Central ESL application demonstrates how to use the :ref:`esl_service_client_readme`.
...
In this application, Nordic provides full set of shell commands to control the AP and auto onboarding feature.
...
.. table-from-rows:: /includes/sample_board_rows.txt
   :header: heading
   :rows: nrf5340dk_nrf5340_cpuapp, nrf52840dk_nrf52840, thingy53_nrf5340_cpuapp

The application also requires another development kit running a compatible application (see :ref:`esl_tag`).

Building and running
********************
.. |sample path| replace:: :file:`applications/esl/central_esl`
...
The make developing process easier, this application provides auto onboarding feature. Access Point will scan and connect to ESL Tag periodically. Then configure mandatory data and send image accodring PNPID defined in application code.
...
This could involve several shell commands input. The make developing process easier, this application provides auto PAST feature. The AP will store information of associated ESL Tags into non-volatile database. Access Point will send PAST automatically if connected ESL Tag is in database.
...
After programming the application to your development kit, test this application in combination with other DevKits, which are running :ref:`esl_tag`, by the following steps:
...
#. Test the application either by by auto onboarding feature or using shell command manually:
   ...
   .. tabs::
      ...
      .. group-tab:: Shell command
         ...
         * Connect ESL Tag to connection index 0 with command:
         ...
         The application shows the following output to indicate that connection and service discovery is succeed:
         ...
         * Configure ESL Tag to esl address ``0x0101`` with command:
         ...
         The application shows the following output to indicate that ESL Tag has been configured and update complete OP code sent:
         ...
         * This command is equivalent to previous one. You can use filename without original pattern `ots_image_XX`
         ...
         The application shows the following output indicate that image has been transferred:
         ...
         * After configured all data and sent all image, use periodic advertising sync transfer procedure to let ESL Tag entering synced with command:
         ...
         The application shows the following output indicate that ESL received PAST and disconnect:
...
Transfer Images from PC to AP over USB
**************************************
...
.. note::
   ...
   Nordic provides two application files.

   * EPD format: Use this format on Access Point :download:`esl_image_bt <https://github.com/pirun/esl_sample_image/raw/main/esl_image_bt>` :download:`esl_image_empower <https://github.com/pirun/esl_sample_image/raw/main/esl_image_empower>`
   * PNG format: This format is only for reference :download:`esl_image_00_240x120_bt.png <https://github.com/pirun/esl_sample_image/blob/main/esl_image_00_240x120_bt.png>` :download:`esl_image_09_400x300_empower.png <https://github.com/pirun/esl_sample_image/blob/main/esl_image_09_400x300_empower.png>`

3. Transfer images: run from cmd /command line
...
#. If auto onboarding is performed after this, it will transfer automatically the images to the ESL Tag DevKit with EPD shield, during the associating. Shall commands can be used at any time to transfer an image from the AP to the Tag.
