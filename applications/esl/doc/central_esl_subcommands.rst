...
``esl_c acl`` is the ESL Access Point ACL related command shell command and it supports the following subcommands.
...
``esl_c pawr`` is the ESL Access Point periodic advertising with respones(PAwR) related command shell command and it supports the following subcommands.
...
This application predefined few ESL sync packet. The table shows their content.
