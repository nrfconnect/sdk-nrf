...
int lwm2m_codec_helpers_set_modem_static_data(struct cloud_data_modem_static *modem_static)
{
	...
	err = lwm2m_engine_set_res_buf(LWM2M_PATH(LWM2M_OBJECT_DEVICE_ID, 0, MODEL_NUMBER_RID),
				       CONFIG_BOARD,
				       sizeof(CONFIG_BOARD),
				       sizeof(CONFIG_BOARD),
				       LWM2M_RES_DATA_FLAG_RO);
	...
	err = lwm2m_engine_set_res_buf(LWM2M_PATH(LWM2M_OBJECT_DEVICE_ID, 0, DEVICE_SERIAL_NUMBER_ID),
				       modem_static->imei,
...
int lwm2m_codec_helpers_set_sensor_data(struct cloud_data_sensors *sensor)
{
	...
	err = lwm2m_engine_set_float(LWM2M_PATH(IPSO_OBJECT_HUMIDITY_SENSOR_ID, 0, SENSOR_VALUE_RID),
				     &sensor->humidity);
	if (err) {
		return err;
	}
