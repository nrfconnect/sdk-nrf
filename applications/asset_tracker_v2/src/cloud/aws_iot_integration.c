...
static int populate_app_endpoint_topics(void)
{
	...
	pub_topics[3].str = pgps_request_topic;
	pub_topics[3].len = PGPS_REQUEST_TOPIC_LEN;
	...
	sub_topics[1].str = pgps_response_topic;
	sub_topics[1].len = PGPS_RESPONSE_TOPIC_LEN;
...
static void incoming_message_handle(struct aws_iot_evt *event)
{
	...
	/* Check if incoming topic is equal the subscribed PGPS response topic. */
	if (strncmp(event->data.msg.topic.str, pgps_response_topic,
