...
static void update_led_pattern(enum led_states_type pattern)
{
...
static void led_pat_active_work_fn(struct k_work *work)
{
	update_led_pattern(LED_STATES_ACTIVE_MODE);
}

static void led_pat_passive_work_fn(struct k_work *work)
{
	update_led_pattern(LED_STATES_PASSIVE_MODE);
}

static void led_pat_gps_work_fn(struct k_work *work)
{
	update_led_pattern(LED_STATES_GPS_SEARCHING);
}
...
static void on_state_active_sub_state_gps_active(struct ui_msg_data *msg)
{
	...
	if ((IS_EVENT(msg, data, DATA_EVT_DATA_SEND)) ||
		...
		update_led_pattern(LED_STATES_CLOUD_PUBLISHING);
...
static void on_all_states(struct ui_msg_data *msg)
{
	...
	if (IS_EVENT(msg, util, UTIL_EVT_SHUTDOWN_REQUEST)) {
		...
		case REASON_FOTA_UPDATE:
			update_led_pattern(LED_STATES_FOTA_UPDATE_REBOOT);
			break;
		case REASON_GENERIC:
			update_led_pattern(LED_STATES_ERROR_SYSTEM_FAULT);
			break;
		default:
	...
	if (IS_EVENT(msg, modem, MODEM_EVT_LTE_CONNECTING)) {
		update_led_pattern(LED_STATES_LTE_CONNECTING);
	}
