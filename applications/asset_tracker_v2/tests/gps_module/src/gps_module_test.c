...
#if defined(CONFIG_BOARD_NATIVE_POSIX) || defined(CONFIG_BOARD_QEMU_CORTEX_M3)
...
static void setup_gps_module_in_init_state(void)
{
	...
	bool ret =
		GPS_MODULE_EVT_HANDLER((struct event_header *)app_module_event);
...
static void setup_gps_module_in_running_state(void)
{
	...
	bool ret = GPS_MODULE_EVT_HANDLER(
		(struct event_header *)data_module_event);
	TEST_ASSERT_EQUAL(0, ret);
