...
static void adjust_tail(volatile hrt_xfer_data_t *xfer_data, uint16_t frame_width, uint32_t data_length)
{
	 ...
	 * Therefore when data_length%32==FRAME_WIDTH  last word is sent shorter (24bits)
	 * and the remaining byte and FRAME_WIDTH number of bits are bit is sent together.
	 */
...
static void xfer_execute(nrfe_mspi_xfer_packet_t *xfer_packet, uint8_t *buffer)
{
	...
	xfer_params.xfer_data[HRT_FE_ADDRESS].vio_out_set = &nrf_vpr_csr_vio_out_buffered_reversed_byte_set;
