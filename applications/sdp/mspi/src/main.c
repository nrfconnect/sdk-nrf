...
static void ep_recv(const void *data, size_t len, void *priv)
{
	...
	case NRFE_MSPI_CONFIG_TIMER_PTR: {
		nrfe_mspi_flpr_timer_msg_t *timer_data = (nrfe_mspi_flpr_timer_msg_t *)data;
...
{
	volatile const uint8_t fault_opcode = NRFE_MSPI_SDP_APP_HARD_FAULT;
	volatile uint32_t cause = mcause;
	volatile uint32_t pc = mepc;
	volatile uint32_t bad_addr = mtval;
	volatile uint32_t *ctx = (volatile uint32_t *)context;
	...
	cpuflpr_error_ctx_ptr[0] = cause;
	cpuflpr_error_ctx_ptr[1] = pc;
	cpuflpr_error_ctx_ptr[2] = bad_addr;
	cpuflpr_error_ctx_ptr[3] = (uint32_t)ctx;

	ipc_service_send(&ep, (const uint8_t *)&fault_opcode, sizeof(fault_opcode));
