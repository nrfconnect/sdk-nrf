...
static void hrt_tx(volatile hrt_xfer_data_t *xfer_data, uint8_t frame_width, bool *counter_running, uint16_t counter_value)
{
	...
	for (uint32_t i = 0; i < xfer_data->word_count; i++) {
		...
		case 1: /* Last transfer */
			xfer_shift_ctrl.shift_count = xfer_data->last_word_clocks - 1;
			nrf_vpr_csr_vio_shift_ctrl_buffered_set(&xfer_shift_ctrl);

			xfer_data->vio_out_set(xfer_data->last_word);
			break;
		case 2: /* Last but one transfer.*/
			xfer_shift_ctrl.shift_count = xfer_data->penultimate_word_clocks - 1;
			nrf_vpr_csr_vio_shift_ctrl_buffered_set(&xfer_shift_ctrl);
...
void hrt_write(hrt_xfer_t *hrt_xfer_params)
{
	...
	/* Transfer command */
	hrt_tx(&hrt_xfer_params->xfer_data[HRT_FE_COMMAND], hrt_xfer_params->bus_widths.command, &counter_running, hrt_xfer_params->counter_value);
	/* Transfer address */
	hrt_tx(&hrt_xfer_params->xfer_data[HRT_FE_ADDRESS], hrt_xfer_params->bus_widths.address, &counter_running, hrt_xfer_params->counter_value);
	/* Transfer data */
	hrt_tx(&hrt_xfer_params->xfer_data[HRT_FE_DATA], hrt_xfer_params->bus_widths.data, &counter_running, hrt_xfer_params->counter_value);
