...
struct zbus_observer_node zbus_obs_node;
...
static int zbus_subscribers_create(void)
{
	...
	ret = zbus_chan_add_obs(&sdu_ref_chan, &sdu_ref_msg_listen, &zbus_obs_node,
...
static int zbus_link_producers_observers(void)
{
	...
	ret = zbus_chan_add_obs(&button_chan, &button_evt_sub, &zbus_obs_node,
				ZBUS_ADD_OBS_TIMEOUT_MS);
	if (ret) {
	...
	ret = zbus_chan_add_obs(&le_audio_chan, &le_audio_evt_sub, &zbus_obs_node,
				ZBUS_ADD_OBS_TIMEOUT_MS);
	if (ret) {
	...
	ret = zbus_chan_add_obs(&bt_mgmt_chan, &bt_mgmt_evt_listen, &zbus_obs_node,
				ZBUS_ADD_OBS_TIMEOUT_MS);
	if (ret) {
	...
	ret = zbus_chan_add_obs(&cont_media_chan, &content_control_evt_sub, &zbus_obs_node,
				ZBUS_ADD_OBS_TIMEOUT_MS);
