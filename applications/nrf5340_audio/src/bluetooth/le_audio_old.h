...
#ifndef _OLD_LE_AUDIO_H_
#define _OLD_LE_AUDIO_H_

#include <zephyr/kernel.h>
#include <zephyr/bluetooth/audio/audio.h>
#include <audio_defines.h>

#define OLD_LE_AUDIO_SDU_SIZE_OCTETS(bitrate)                                                      \
	(bitrate / (1000000 / CONFIG_AUDIO_FRAME_DURATION_US) / 8)
...
enum old_le_audio_user_defined_action {
 ...
 */
int old_le_audio_user_defined_button_press(enum le_audio_user_defined_action action);
 ...
 */
int old_le_audio_config_get(uint32_t *bitrate, uint32_t *sampling_rate, uint32_t *pres_delay);
 ...
 */
void old_le_audio_conn_set(struct bt_conn *conn);
 ...
 * @return	0 for success, error otherwise.
 */
int old_le_audio_pa_sync_set(struct bt_le_per_adv_sync *pa_sync, uint32_t broadcast_id);
 ...
 * @param[in]	conn	The connection pointer.
 */
void old_le_audio_conn_disconnected(struct bt_conn *conn);
 ...
 * @return	0 for success, error otherwise.
 */
int old_le_audio_ext_adv_set(struct bt_le_ext_adv *ext_adv);
 ...
 */
void old_le_audio_adv_get(const struct bt_data **adv, size_t *adv_size, bool periodic);
 ...
 * @retval	error	Otherwise.
 */
int old_le_audio_send(struct encoded_audio enc_audio);
 ...
 * @return	0 for success, error otherwise.
 */
int old_le_audio_enable(old_le_audio_receive_cb recv_cb, old_le_audio_timestamp_cb timestmp_cb);
 ...
 * @return	0 for success, error otherwise.
 */
int old_le_audio_disable(void);

#endif /* _OLD_LE_AUDIO_H_ */
