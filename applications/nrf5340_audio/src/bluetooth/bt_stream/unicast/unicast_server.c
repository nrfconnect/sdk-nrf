			   ...
			   struct net_buf *buf)
{
	...
	ret = bt_audio_codec_cfg_get_chan_allocation(stream->codec_cfg, &audio_frame.meta.locations,
						     true);
	if (ret < 0) {
...
int unicast_server_send(struct audio_data *audio_frame)
{
#if (CONFIG_BT_AUDIO_TX)
	int ret;
	...
	audio_frame->meta.locations = ret =
		bt_le_audio_tx_send(tx, num_active_streams, audio_frame);
	if (ret) {
		return ret;
	}
