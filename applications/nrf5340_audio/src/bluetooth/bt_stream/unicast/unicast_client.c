			   ...
			   struct net_buf *buf)
{
	...
	ret = le_audio_metadata_populate(&meta, stream, info, buf);
	if (ret) {
...
int unicast_client_send(uint8_t cig_index, struct net_buf const *const audio_frame)
{
#if (CONFIG_BT_AUDIO_TX)
	int ret;
