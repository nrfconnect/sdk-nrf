...
struct le_audio_headset {
	...
	struct bt_codec sink_codec_cap[CONFIG_CODEC_CAPAB_COUNT_MAX];
	struct bt_bap_stream source_stream;
	struct bt_bap_ep *source_ep;
	struct bt_codec source_codec_cap[CONFIG_CODEC_CAPAB_COUNT_MAX];
...
struct temp_cap_storage {
	...
	struct bt_codec codec[CONFIG_CODEC_CAPAB_COUNT_MAX];
};
			     ...
			     struct bt_bap_unicast_client_discover_params *params)
{
	...
	for (int i = 0; i < CONFIG_CODEC_CAPAB_COUNT_MAX; i++) {
