		    ...
		    void **decoded_data, size_t *decoded_size)
{
	...
	char *dec_data_stereo;
	char *dec_data_stereo_src;
	...
	case SW_CODEC_LC3: {
		...
		if (bad_frame && IS_ENABLED(CONFIG_SW_CODEC_OVERRIDE_PLC)) {
			...
			memset(dec_data_stereo, 0, PCM_NUM_BYTES_MONO * m_config.decoder.num_ch);
			...
			if (m_config.decoder.channel_mode == SW_CODEC_MONO) {
				audio_data_tx.meta.locations = 0x00000001;
			} else {
				audio_data_tx.meta.locations = 0x00000003;
			}
		...
		dec_data_stereo = decoded_data_stereo;
		dec_data_stereo_src = decoded_data_stereo_system_sample_rate;

		for (int i = 0; i < m_config.decoder.num_ch; ++i) {
			ret = sw_codec_sample_rate_convert(
				&decoder_converters[i], m_config.decoder.sample_rate_hz,
				CONFIG_AUDIO_SAMPLE_RATE_HZ, dec_data_stereo, PCM_NUM_BYTES_MONO,
				dec_data_stereo_src, &dec_data_stereo, &pcm_size_mono);
			if (ret) {
			...
			dec_data_stereo += PCM_NUM_BYTES_MONO;
			dec_data_stereo_src += PCM_NUM_BYTES_MONO;
		}
	...
	case SW_CODEC_LC3: {
		...
		if (sw_codec_cfg.decoder.enabled) {
			...
			if (sw_codec_cfg.decoder.channel_mode == SW_CODEC_MONO) {
				decoder_config.locations = 0x00000001;
			} else {
				decoder_config.locations = 0x00000003;
			}
