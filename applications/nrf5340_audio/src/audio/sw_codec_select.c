	...
	case SW_CODEC_LC3: {
		...
		while (loc_in && loc_out) {
			if ((loc_out & 0x01) && (loc_in & 0x01)) {
				...
				ERR_CHK_MSG(ret, "Dencode failed");
				...
				ERR_CHK_MSG(ret, "Dencode: Sample rate converter failed");
				...
				if (meta_out->interleaved) {
					...
					ERR_CHK_MSG(ret, "Dencode: Interleave failed");
				} else {
					if (IS_ENABLED(CONFIG_SAMPLE_RATE_CONVERTER) &&
					    meta_in->sample_rate_hz != meta_out->sample_rate_hz) {
