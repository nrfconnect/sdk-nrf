...
int sw_codec_encode(struct net_buf *audio_frame_in, struct net_buf *audio_frame_out)
{
	...
	case SW_CODEC_LC3: {
		...
		chans_in = device_location_to_chan_num(meta_in->locations);
		chans_out = device_location_to_chan_num(meta_out->locations);
		    ...
		    struct net_buf *const audio_frame_out)
{
	...
	case SW_CODEC_LC3: {
		...
		uint8_t *dec_out;
		uint8_t *conv_buf;
		uint8_t dec_pcm[PCM_NUM_BYTES_MONO];
		uint8_t conv_pcm[PCM_NUM_BYTES_MONO];
		uint8_t chans_in, chans_out;
		uint8_t *int_in;
		size_t int_in_size = 0;
		...
		if (chans_in >= chans_out) {
			LOG_DBG("Decoder input channels (%d) >= output channels (%d), "
		...
		for (uint8_t i = 0; i < chans_in; i++) {
			...
			if (ctx->config.interleaved && chans_out > 1) {
				ret = pscm_interleave(
					int_in, int_in_size, i, meta_out->carried_bits_per_sample,
		...
		meta_out->bytes_per_location = int_in_size;

		net_buf_add(audio_frame_out, int_in_size * chans_out);
#endif /* (CONFIG_SW_CODEC_LC3) */
		break;
	}
	default:
