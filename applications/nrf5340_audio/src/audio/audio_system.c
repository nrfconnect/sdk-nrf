...
/* TODO: Size these to fit use case (e.g. increase decoder pool when it is wrapped in a thread). */
...
int audio_system_decode(struct net_buf *audio_frame_in)
{
	...
	meta_out->data_len_us = CONFIG_AUDIO_FRAME_DURATION_US;
	meta_out->sample_rate_hz = CONFIG_AUDIO_SAMPLE_RATE_HZ;
	meta_out->bits_per_sample = CONFIG_AUDIO_BIT_DEPTH_BITS;
	meta_out->carried_bits_per_sample = CONFIG_AUDIO_BIT_DEPTH_BITS;
	meta_out->interleaved = true;
