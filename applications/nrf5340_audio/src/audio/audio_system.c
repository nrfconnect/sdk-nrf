...
K_MSGQ_DEFINE(queue_tx, sizeof(struct net_buf *), FIFO_TX_BLOCK_COUNT, 4);
K_MSGQ_DEFINE(queue_rx, sizeof(struct net_buf *), FIFO_RX_BLOCK_COUNT, 4);

NET_BUF_POOL_FIXED_DEFINE(queue_rx_pool, FIFO_RX_BLOCK_COUNT, FRAME_SIZE_BYTES,
			  sizeof(struct audio_metadata), NULL);
NET_BUF_POOL_FIXED_DEFINE(queue_tx_pool, FIFO_TX_BLOCK_COUNT, USB_BLOCK_SIZE_STEREO,
			  sizeof(struct audio_metadata), NULL);
...
static void encoder_thread(void *arg1, void *arg2, void *arg3)
{
	...
	while (1) {
		...
		struct net_buf *audio_frame = net_buf_alloc(&queue_rx_pool, K_NO_WAIT);
		...
		ret = k_msgq_get(&queue_rx, (void *)&audio_block, K_FOREVER);
		...
		if (debug_trans_count == DEBUG_INTERVAL_NUM) {
			queue_fill_num = k_msgq_num_used_get(&queue_rx);
			ERR_CHK(ret);
			LOG_DBG(COLOR_CYAN "RX filled: %d" COLOR_RESET, queue_fill_num);
			debug_trans_count = 0;
		} else {
			debug_trans_count++;
		}
...
int audio_system_decode(struct net_buf *audio_frame)
{
	...
	while (k_msgq_num_free_get(&queue_tx) < CONFIG_FIFO_FRAME_SPLIT_NUM) {
		struct net_buf *stale_buf;

		ret = k_msgq_get(&queue_tx, (void *)&stale_buf, K_NO_WAIT);
		if (ret == -ENOMSG) {
	...
	for (int i = 0; i < CONFIG_FIFO_FRAME_SPLIT_NUM; i++) {
		struct net_buf *audio_block = net_buf_alloc(&queue_tx_pool, K_NO_WAIT);

		net_buf_add_mem(audio_block, (char *)pcm_raw_data + (i * (BLOCK_SIZE_BYTES)),
				BLOCK_SIZE_BYTES);

		ret = k_msgq_put(&queue_tx, (void *)&audio_block, K_NO_WAIT);
		if (ret) {
	...
	if (debug_trans_count == DEBUG_INTERVAL_NUM) {
		uint32_t used_num = k_msgq_num_used_get(&queue_tx);

		LOG_DBG(COLOR_MAGENTA "TX fill grade: %d/%d" COLOR_RESET, used_num,
...
void audio_system_start(void)
{
	...
	ret = audio_usb_start(&queue_tx, &queue_rx);
	ERR_CHK(ret);
#else
	...
	ret = audio_datapath_start(&queue_rx);
	ERR_CHK(ret);
...
void audio_system_stop(void)
{
	...
	k_msgq_purge(&queue_rx);
	k_msgq_purge(&queue_tx);
}
...
int audio_system_fifo_rx_block_drop(void)
{
	...
	ret = k_msgq_get(&queue_rx, (void *)&audio_block, K_NO_WAIT);
	if (ret) {
