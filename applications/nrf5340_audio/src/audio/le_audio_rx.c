...
K_MSGQ_DEFINE(ble_queue_rx, sizeof(struct net_buf *), CONFIG_BUF_BLE_RX_PACKET_NUM, 4);
			      ...
			      uint8_t channel_index)
{
	...
	free_num = k_msgq_num_free_get(&ble_queue_rx);
	if (free_num == 0) {
		struct net_buf *stale_buf;
		/* FIFO buffer is full, swap out oldest frame for a new one */
		ret = k_msgq_get(&ble_queue_rx, (void *)&stale_buf, K_NO_WAIT);
		if (ret == -ENOMSG) {
			/* If there are no more blocks in FIFO, break */
			return;
		} else if (ret) {
			LOG_ERR("Failed to get audio frame from FIFO");
			return;
		}
	...
	if (buf->len && !meta->bad_data) {
		net_buf_add_mem(audio_frame, buf->data, buf->len);
	}

	memcpy(net_buf_user_data(audio_frame), meta, sizeof(struct audio_metadata));

	ret = k_msgq_put(&ble_queue_rx, (void *)&audio_frame, K_NO_WAIT);
	ERR_CHK_MSG(ret, "Failed to put audio frame into FIFO");
}
...
static void audio_datapath_thread(void *dummy1, void *dummy2, void *dummy3)
{
	...
	while (1) {
		ret = k_msgq_get(&ble_queue_rx, (void *)&audio_frame, K_FOREVER);
		ERR_CHK(ret);
