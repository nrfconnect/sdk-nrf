					    ...
					    uint32_t const *tx_buf_released)
{
	...
	if (IS_ENABLED(CONFIG_STREAM_BIDIRECTIONAL) || (CONFIG_AUDIO_DEV == GATEWAY)) {
		...
		uint32_t queue_in_free_num = k_msgq_num_free_get(ctrl_blk.in.queue);

		/* If RX FIFO is filled up */
		if (queue_in_free_num == 0 || pool_i2s_rx.avail_count == 0) {
			...
			ret = k_msgq_get(ctrl_blk.in.queue, (void *)&stale_i2s_data, K_NO_WAIT);
			ERR_CHK(ret);
		...
		ret = k_msgq_put(ctrl_blk.in.queue, (void *)&rx_audio_block, K_NO_WAIT);
