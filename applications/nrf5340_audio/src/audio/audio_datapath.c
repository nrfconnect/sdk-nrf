...
void audio_datapath_stream_out(struct net_buf *audio_frame_in)
{
	...
	if (meta_in->ref_ts_us == ctrl_blk.prev_pres_sdu_ref_us &&
	    meta_in->ref_ts_us != 0) {
	...
	if (ctrl_blk.pres_comp.enabled) {
		audio_datapath_presentation_compensation(meta_in->data_rx_ts_us,
							 meta_in->ref_ts_us,
	...
	meta_pcm = net_buf_user_data(audio_frame_out);
