 ...
 * Full-Speed explicit feedback endpoint wMaxPacketSize to be equal to 4, which violates the USB 2.0
 * Specification. See https://aka.ms/AArvnax for Windows Feedback Hub report. For Windows-only use
 * wMaxPacketSize can be overridden with CONFIG_USBD_UAC2_FS_WINDOWS_WORKAROUND
 */
...
struct usb_feedback_cfg {
	/* Feedback speed:
	 *   full-speed (1ms frame) = false
	 *   high-speed (125us frame) = true
	 */
	bool high_speed;
	...
	/* Number of captures between possible feedback calculation */
...
struct usb_feedback_ctx {
	...
	/* Number of samples per USB frame.
	 * e.g. For 48000 Hz sampling nominal samples is
	 *      48 per channel in 1 ms (full-speed).
	 */
	uint16_t samples_per_frame;

	/* Nominal feedback value.
	 * e.g. For 48000 Hz sampling nominal samples
	 *      is 48 per channel in 1 ms (full-speed).
	 */
	uint32_t fb_nominal;
...
/**
 * @brief Read the synchronization timer.
 *
 * @return Value of sync timer at call in us.
 */
static inline uint32_t usb_feedback_capture(void)
{
 ...
 * @param ts_us       Timestamp of the last received USB block in us.
 * @param overflow    The number of bytes remaining (overflow) after the last system audio buffer
 *                    has been filled.
 *
 * @return 0 if successful, error otherwise
 */
int usb_feedback_process(struct usb_feedback_ctx *const ctx, uint32_t ts_us, uint16_t overflow);
 ...
 */
uint32_t usb_feedback_value_get(struct usb_feedback_ctx *const fb_ctx);
 ...
 *
 * @param fb_ctx       Pointer to feedback context.
 * @param microframes  Flag to indicate if microframes (high-speed) has been selected.
