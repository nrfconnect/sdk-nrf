...
static int led_set_int(uint8_t led, enum led_color color)
{
	...
	if (led_unit->num_gpios == LED_MONOCHROME) {
		if (color) {
			return gpio_pin_set_dt(led_unit->gpio, 1);
		} else {
			return gpio_pin_set_dt(led_unit->gpio, 0);
		}
	} else {
		for (uint8_t i = 0; i < NUM_COLORS_RGB; i++) {
			if (color & BIT(i)) {
				return gpio_pin_set_dt(&led_unit->gpio[i], 1);
			} else {
				return gpio_pin_set_dt(&led_unit->gpio[i], 0);
			}
		}
	}
