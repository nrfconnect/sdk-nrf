...
static struct k_msgq *queue_tx;
static struct k_msgq *queue_rx;
...
static void data_write(const struct device *dev)
{
	...
	ret = k_msgq_get(queue_tx, &data_out, K_NO_WAIT);
	if (ret) {
		tx_num_underruns++;
...
static void data_received(const struct device *dev, struct net_buf *buffer, size_t size)
{
	...
	queue_free_num = k_msgq_num_free_get(queue_rx);

	/* RX FIFO can fill up due to re-transmissions or disconnect */
	if (queue_free_num == 0 || pool_in.avail_count == 0) {
		...
		ret = k_msgq_get(queue_rx, (void *)&stale_usb_data, K_NO_WAIT);
		ERR_CHK(ret);
	...
	ret = k_msgq_put(queue_rx, (void *)&audio_block, K_NO_WAIT);
...
{
	if (queue_tx_in == NULL || queue_rx_in == NULL) {
		return -EINVAL;
	}

	queue_tx = queue_tx_in;
	queue_rx = queue_rx_in;
