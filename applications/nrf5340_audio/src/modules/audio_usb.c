...
#if DT_NODE_EXISTS(DT_NODELABEL(hp_fb_out_terminal))
#define TERMINAL_ID_HEADPHONES_FB_OUT UAC2_ENTITY_ID(DT_NODELABEL(hp_fb_out_terminal))
#else
#define TERMINAL_ID_HEADPHONES_FB_OUT AUDIO_USB_DEV_NOT_ASSIGNED
#endif /* hp_fb_out_terminal */
...
static bool terminal_mic_in_enabled;
			       ...
			       bool microframes, void *user_data)
{
	...
	if (terminal == TERMINAL_ID_HEADSET_OUT) {
		terminal_headset_out_enabled = enabled;
		   ...
		   terminal == TERMINAL_ID_HEADPHONES_FB_OUT) {
...
static void usb_send_cb(const struct device *dev, void *user_data)
{
	...
	bool tx_ready =
		(audio_q_tx != NULL && (terminal_headset_in_enabled || terminal_mic_in_enabled) &&
		 playing_state && local_host_in);
...
overrun_cleanup:
	if (unlikely((++rx_num_overruns % 1) == 1)) {
...
int audio_usb_init(bool host_in, bool host_out)
{
	...
	if (host_out) {
		dev = DEVICE_DT_GET(DT_NODELABEL(uac2_fb_headphones));
		LOG_INF("USB initialized as unidirectional (headphones with feedback "
			"only).");
	} else {
		LOG_ERR("USB currently only supports output (host out) with feedback.");
		return -ENOTSUP;
	}
#else
	if (host_in && host_out) {
		...
		LOG_ERR("USB currently only supports output (host out) or bidirectional or "
			"microphone (host in and host out).");
		return -ENOTSUP;
	}
#endif
