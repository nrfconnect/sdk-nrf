...
#define HEADSET_OUT_TERMINAL_ID	   UAC2_ENTITY_ID(DT_NODELABEL(out_terminal))
#define HEADPHONES_OUT_TERMINAL_ID UAC2_ENTITY_ID(DT_NODELABEL(hp_out_terminal))
#define HEADSET_IN_TERMINAL_ID	   UAC2_ENTITY_ID(DT_NODELABEL(in_terminal))
...
static bool headset_out_terminal_enabled;
static bool headphones_out_terminal_enabled;
static bool headset_in_terminal_enabled;
			       ...
			       bool microframes, void *user_data)
{
	...
	if (terminal == HEADSET_OUT_TERMINAL_ID) {
		headset_out_terminal_enabled = enabled;
	} else if (terminal == HEADPHONES_OUT_TERMINAL_ID) {
		headphones_out_terminal_enabled = enabled;
	} else if (terminal == HEADSET_IN_TERMINAL_ID) {
		headset_in_terminal_enabled = enabled;
	} else {
...
static void usb_send_cb(const struct device *dev, void *user_data)
{
	int ret;
	void *pcm_buf;
	static struct net_buf *data_out;

	if (audio_q_tx == NULL || !headset_in_terminal_enabled) {
		return;
	}
	...
	ret = usbd_uac2_send(dev, HEADSET_IN_TERMINAL_ID, pcm_buf, pcm_size);
	if (ret) {
				...
				void *user_data)
{
	...
	if (terminal != HEADSET_IN_TERMINAL_ID) {
			     ...
			     void *user_data)
{
	...
	if (terminal == HEADSET_OUT_TERMINAL_ID || terminal == HEADPHONES_OUT_TERMINAL_ID) {
			 ...
			 void *user_data)
{
	...
	if (!headset_out_terminal_enabled || size == 0) {
		if (buf != NULL) {
