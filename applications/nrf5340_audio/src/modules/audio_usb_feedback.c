 ...
 * the 24-bits so it has Q10.14 format.
 *
...
int usb_feedback_process(struct usb_feedback_ctx *const fb_ctx, uint32_t ts_us, uint16_t overflow)
{
	...
	if (fb_ctx->captures_cnt == fb_ctx->fb_cfg.captures_num) {
		...
		if (!IN_RANGE(overflow, fb_ctx->fb_cfg.low_tide, fb_ctx->fb_cfg.high_tide)) {
			...
			feedback = ((uint64_t)delta << fb_ctx->speed_shift);
			feedback /= fb_ctx->fb_cfg.frame_size_us;
			feedback *= fb_ctx->samples_per_frame;
			...
			} else if (fb_ctx->fb_value >= fb_ctx->fb_max) {
				fb_ctx->fb_value = fb_ctx->fb_max;
			} else {
				fb_ctx->fb_value = (uint32_t)feedback;
			}
		}
	...
	} else if (unlikely(fb_ctx->captures_cnt == 0)) {
		/* Only do this once after initialize or reset */
		fb_ctx->start_ts_us = ts_us;
	} else {
		LOG_ERR("Unrecognized captures count: %d", fb_ctx->captures_cnt);
		return -EINVAL;
	}
 ...
 *
 */
static void nominal_feedback_values_set(struct usb_feedback_ctx *const fb_ctx, bool microframes)
{
	/* Put shift and data into correct fixed-point */
	if (microframes) {
		fb_ctx->speed_shift = FEEDBACK_HS_K + FEEDBACK_HS_SHIFT;
		fb_ctx->samples_per_frame = fb_ctx->fb_cfg.sampling_rate_hz / 8000;
		fb_ctx->fb_nominal = fb_ctx->samples_per_frame << fb_ctx->speed_shift;
		fb_ctx->fb_max = FEEDBACK_HS_MAX;
		fb_ctx->fb_min = FEEDBACK_HS_MIN;
	} else {
		fb_ctx->speed_shift = FEEDBACK_FS_K + FEEDBACK_FS_SHIFT;
		fb_ctx->samples_per_frame = fb_ctx->fb_cfg.sampling_rate_hz / 1000;
		fb_ctx->fb_nominal = fb_ctx->samples_per_frame << fb_ctx->speed_shift;
...
int usb_feedback_reset(struct usb_feedback_ctx *const fb_ctx, bool microframes)
{
	...
	if (fb_ctx->initialized) {
		nominal_feedback_values_set(fb_ctx, microframes);
	} else {
		LOG_ERR("Feedback system uninitialized");
		return -EINVAL;
	}
			    ...
			    struct usb_feedback_cfg *const fb_cfg)
{
	...
	nominal_feedback_values_set(fb_ctx, fb_cfg->high_speed);
	fb_ctx->running = false;
	fb_ctx->fb_value = fb_ctx->fb_nominal;
	fb_ctx->start_ts_us = 0;
	fb_ctx->captures_cnt = 0;
