 ...
 * bidirectional audio communication for unicast (CIS) mode. It integrates with
 * @ref nrf5340_audio_datapath for synchronized audio delivery and supports configurable buffer
 ...
 * duration can be either 10ms (standard) or 7.5ms (LC3 codec specific).
 *
 * For 7.5ms frames with LC3 codec:
 * - Uses 15/2 multiplier to represent 7.5ms (since floats aren't allowed in defines)
 * - Formula: (sample_rate_hz / 1000 * 15 / 2) * channels * bytes_per_sample
 *
 * For 10ms frames (default):
 * - Uses 10ms multiplier
