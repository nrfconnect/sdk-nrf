 ...
 */
#include "audio_nrf5340_dk_init.h"
...
LOG_MODULE_REGISTER(audio_nrf5340_dk_init, CONFIG_AUDIO_NRF5340_DK_INIT_LOG_LEVEL);
...
int audio_nrf5340_dk_init(void)
{
	int ret;
	...
	if (IS_ENABLED(CONFIG_BOARD_NRF5340_AUDIO_DK_NRF5340_CPUAPP)) {
		ret = audio_nrf5340_dk_version_valid_check();
		if (ret) {
			return ret;
		}
