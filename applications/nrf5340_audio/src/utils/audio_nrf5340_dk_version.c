...
#include "audio_nrf5340_dk_version.h"
...
LOG_MODULE_REGISTER(audio_nrf5340_dk_version, CONFIG_AUDIO_NRF5340_DK_VERSION_LOG_LEVEL);
...
int audio_nrf5340_dk_version_get(struct board_version *board_rev)
{
	int ret;
...
int audio_nrf5340_dk_version_valid_check(void)
{
	int ret;
	struct board_version board_rev;

	ret = audio_nrf5340_dk_version_get(&board_rev);
	if (ret) {
