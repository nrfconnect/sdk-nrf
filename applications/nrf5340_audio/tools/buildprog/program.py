...
def program_threads_run(devices_list: list[DeviceConf], sequential: bool = False):
    ...
    device_list_gw_first = sorted(devices_list, key=lambda dev: dev.nrf5340_audio_dk_dev != AudioDevice.gateway)
    # Halt all devices before programming
    for dev in device_list_gw_first:
        if dev.snr_connected and (dev.core_app_programmed == SelectFlags.TBD or dev.core_net_programmed == SelectFlags.TBD or dev.only_reboot == SelectFlags.TBD):
