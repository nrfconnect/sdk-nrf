...
To build the BIS gateway with the alternative name, given by ``CONFIG_BT_DEVICE_NAME_ALT``, add the option ``CONFIG_BT_DEVICE_NAME_ALT`` Kconfig option set to ``y``to the :file:`prj.conf` file for the debug version and the :file:`prj_release.conf` file for the release version.
