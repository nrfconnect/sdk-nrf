...
The watchdog timer is started when the :ref:`nrf_desktop_main` is ready (which is reported using the :c:struct:`module_state_event`).
The module periodically resets the watchdog timer using :c:struct:`k_work_delayable`.
The work resubmits itself with a delay equal to ``CONFIG_DESKTOP_WATCHDOG_TIMEOUT/3``.
In case of system hang, the work will not be processed, the watchdog timer will not be reset on time, and the system will be restarted.
