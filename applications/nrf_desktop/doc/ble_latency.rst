...
If the connected host updates transport parameters directly at the Link Layer (for example, by initiating a connection rate update without first writing to the HID Control Point characteristic), the peripheral treats this as out-of-spec behavior.
...
* Blocks the ability of the peripheral to autonomously change connection latency.
* Cancels any pending HID SCI mode or latency update requests.
* If no SCI mode matches the parameters, switches to the NONE mode.
  From this point on, the module will not attempt to update the latency until a new connection rate update is received.
...
1. Update the transport parameters to a range allowing to support all the HID SCI modes.
2. Request an SCI mode change through the HID Control Point characteristic.
