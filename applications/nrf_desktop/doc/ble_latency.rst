...
Such a situation is recognized by a :c:struct:`ble_peer_sci_conn_rate_event` event being received without a prior HID SCI mode request.
...
* Stores the updated maximum latency and supervision timeout values.
  These values are then used as upper limits for these parameters in the connection rate update requests.
  Using higher values could result in connection rate update failures.
* The module drops any pending HID SCI mode or latency update requests.
* If the peripheral is in suspended/power-down state, and the HID SCI mode resulting from the above steps is different from LOW_POWER, the module will attempt to request LOW_POWER mode parameters.
