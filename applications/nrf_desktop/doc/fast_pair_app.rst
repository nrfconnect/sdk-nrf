...
The Fast Pair module is enabled using :option:`CONFIG_DESKTOP_FAST_PAIR` Kconfig option.
...
The :option:`CONFIG_DESKTOP_FAST_PAIR_LIMIT_NORMAL_PAIRING` can be used to allow normal Bluetooth pairing only in the pairing mode.
...
With the dongle peer functionality enabled (:option:`CONFIG_DESKTOP_BLE_DONGLE_PEER_ENABLE`), the Fast Pair module selects the :option:`CONFIG_DESKTOP_BLE_DONGLE_PEER_ID_INFO` option to track the application identity of the dongle peer.
