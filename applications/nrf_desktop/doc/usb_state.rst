...
The :kconfig:option:`CONFIG_UDC_DWC2_USBHS_VBUS_READY_TIMEOUT` Kconfig option is set to non-zero value to prevent the :c:func:`usbd_enable` function from blocking the application forever when the USB cable is not connected.
Instead, the function returns error on timeout.
