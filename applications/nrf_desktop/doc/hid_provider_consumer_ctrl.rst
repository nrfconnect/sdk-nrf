...
You can enable the default implementation of the HID provider using the :ref:`CONFIG_DESKTOP_HID_REPORT_PROVIDER_CONSUMER_CTRL <config_desktop_app_options>` Kconfig option.
This option is enabled by default if the device uses HID provider events (:ref:`CONFIG_DESKTOP_HID_REPORT_PROVIDER_EVENT <config_desktop_app_options>`) and supports HID consumer control reports (:ref:`CONFIG_DESKTOP_HID_REPORT_CONSUMER_CTRL_SUPPORT <config_desktop_app_options>`).
...
The module is used by :ref:`nrf_desktop_hid_state` as a HID input report provider for the HID consumer control input report.
...
After an application-specific key ID (:c:member:`button_event.key_id`) is mapped to the HID consumer control input report ID and related HID usage ID, the HID usage ID is handled by the provider.
...
.. note::
   The module tracks and reports a single HID consumer control usage ID at a time.
