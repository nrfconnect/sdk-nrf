...
* ``CONFIG_STATUS_PENDING`` - The operation is not yet complete.
...
* Once the descriptor is read, the module options can be accessed.
  When performing a set or fetch request on the option, the Event ID contains the Option ID and the Module ID of module that owns the option.
...
For example, depending on the listener, it can provide the CPI option from :ref:`nrf_desktop_motion` or the option for searching for new peer from :ref:`nrf_desktop_ble_bond`.
