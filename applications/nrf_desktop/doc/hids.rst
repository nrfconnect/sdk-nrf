...
Right now, the only board that displays information received in the HID output report using hardware LEDs is the :ref:`nrf52840dk <nrf52840dk_nrf52840>` in ``keyboard`` build type configuration.
...
The event is submitted when the |GATT_HID| calls a callback related to enabling or disabling the notifications and the event is received only by the ``hids`` (Question: where does this "hids" refer to?) application module.
