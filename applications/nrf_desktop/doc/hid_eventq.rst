...
The utility can be internally used by an application module that handles keypresses.

Initialization
==============

Initialize an utility instance before use (:c:func:`hid_eventq_init`).
...
You can use the :c:func:`hid_eventq_keypress_get` function to get a keypress event from the queue.
The keypress events are queued in FIFO (first in first out) manner.
An ID is used to identify button related to the keypress.
...
You can use the :c:func:`hid_eventq_cleanup` to remove stale keypresses (with timestamp lower than provided minimal valid timestamp).
