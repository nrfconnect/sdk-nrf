...
* Providing HID input reports to the active HID subscriber.
  The module relies on HID report providers to aggregate the user input and form HID input reports.
...
If multiple HID subscribers are simultaneously connected, the |hid_state| selects the one with the higest priority as the active subscriber.
The |hid_state| provides HID input reports only to the active subsctiber.
...
The |hid_state| relies on the HID report providers to collect user input and form HID input reports.
...
The |hid_state| relies on the HID providers to collect user input and form HID input reports.
...
The |hid_state| relies on this event to track the number of HID reports in flight.
...
The event documentation also informs about the responsibility split.
