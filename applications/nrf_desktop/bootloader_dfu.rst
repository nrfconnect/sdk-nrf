...
nRF54L SoC Series allows achieving improved security and shorter boot times by using hardware cryptography in the MCUboot immutable bootloader.
The |NCS| allows using hardware cryptography for ED25519 signature (``SB_CONFIG_BOOT_SIGNATURE_TYPE_ED25519``) on nRF54L SoC Series.

The following sysbuild Kconfig options allow to improve security further:

* ``SB_CONFIG_BOOT_SIGNATURE_TYPE_PURE`` - This option enables using pure signature of the image (verifying signature directly on image, rather than hash of the image).
  The option cannot be used if external memory is used for the secondary image slot.
* ``SB_CONFIG_MCUBOOT_SIGNATURE_USING_KMU`` - This option enables using Key Management Unit (KMU) to store keys for signature verification instead of compiling in key data from a file.
  Using KMU requires provisioning the public key manually.
  See :ref:`ug_nrf54l_developing_provision_kmu` documentation for details.
