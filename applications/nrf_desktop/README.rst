...
.. note::
    `Selecting build type`_ is optional.
...
Before you start testing the application, you can select one of the `nRF Desktop build types`_, depending on your board and building method.
...
The application configuration files define both a set of options with which the nRF Desktop project will be created for your board and the selected `nRF Desktop build types`_.
The files that need to be included in this directory can be grouped as follows:
...
#. Copy the project files for the device that is the closest match for your hardware.
   ...
   This adds :file:`.conf` Kconfig files for your project that you will edit in the next mandatory step.
...
#. Edit the Kconfig file that you copied and that is related to your build type (:file:`app_CMAKE_BUILD_TYPE.conf`) to ensure that the following modules that are specific for gaming mouse are enabled:
