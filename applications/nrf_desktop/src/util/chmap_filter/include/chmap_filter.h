 ...
 *          affected by wifi is removed from the channel map recommendation.
 *
 * @note The library is not thread-safe. Apart from the "get" functions,
 * functions should not be called while @ref chmap_filter_process is running
 *
...
#define DEFAULT_PARAM_BLE_MIN_CHANNEL_COUNT    2
...
/**@brief Initialize channel map filter state
 *
 * @details This library version is using a single filter instance,
 * stored internally
 *
 * @return pointer to channel map instance
 */
struct chmap_instance *chmap_filter_init(void);
 ...
 * @param[in] p_inst Channel map instance pointer
 *
 * @return pointer to 5-byte channel map array
 */
u8_t const *chmap_filter_suggested_map_get(struct chmap_instance *p_inst);
 ...
 *
 * @return -1 if p_inst or chn_idx is invalid
 */
