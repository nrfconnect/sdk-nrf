...
static void consumer_ctrl_report_connection_state(uint8_t report_id,
...
static void trigger_report_transmission(void)
{
	...
	if (active_sub) {
		(void)hid_state_fops->trigger_report_send(REPORT_ID_CONSUMER_CTRL);
	} else {
...
static void init(void)
{
	...
	static const struct hid_report_provider_fops report_fops_consumer_ctrl = {
		.send_report = send_report_consumer_ctrl,
		.send_empty_report = send_empty_report,
		.connection_state = consumer_ctrl_report_connection_state,
		.notify_report_sent = consumer_ctrl_report_sent,
	};

	struct hid_report_provider_event *rp = new_hid_report_provider_event();

	rp->report_id = REPORT_ID_CONSUMER_CTRL;
	rp->p_fops = &report_fops_consumer_ctrl;
	rp->hs_fops = NULL;
...
static bool handle_hid_report_provider_event(const struct hid_report_provider_event *event)
{
	if (event->report_id == REPORT_ID_CONSUMER_CTRL) {
		__ASSERT_NO_MSG(event->p_fops->connection_state ==
				consumer_ctrl_report_connection_state);
