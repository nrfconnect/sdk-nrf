...
#define MODULE hid_provider_system_ctrl
#include <caf/events/module_state_event.h>

#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(MODULE, CONFIG_DESKTOP_HID_REPORT_PROVIDER_SYSTEM_CTRL_LOG_LEVEL);
...
{
	__ASSERT_NO_MSG(report_id == REPORT_ID_SYSTEM_CTRL);
	__ASSERT_NO_MSG(subscriber);

	size_t size = REPORT_SIZE_SYSTEM_CTRL;
...
static bool send_report_system_ctrl(uint8_t report_id, bool force)
{
	__ASSERT_NO_MSG(report_id == REPORT_ID_SYSTEM_CTRL);
	...
	/* Only one item can fit in the system control report. */
...
static void drop_stale_keypresses(struct hid_eventq *q)
{
	...
	if (!hid_eventq_is_empty(q)) {
		...
		int64_t expire =
			CONFIG_DESKTOP_HID_REPORT_PROVIDER_SYSTEM_CTRL_KEYPRESS_EXPIRATION;
...
static void system_ctrl_report_connection_state(uint8_t report_id,
...
static void system_ctrl_report_sent(uint8_t report_id, bool error)
{
	ARG_UNUSED(report_id);
...
static void trigger_report_transmission(void)
{
	...
	if (active_sub) {
		(void)hid_state_fops->trigger_report_send(REPORT_ID_SYSTEM_CTRL);
	} else {
...
static bool handle_button_event(const struct button_event *event)
{
	...
	if (map && (map->report_id == REPORT_ID_SYSTEM_CTRL)) {
...
static void init(void)
{
	hid_eventq_init(&report_data.eventq,
			CONFIG_DESKTOP_HID_REPORT_PROVIDER_SYSTEM_CTRL_EVENT_QUEUE_SIZE);
	keys_state_init(&report_data.keys_state, SYSTEM_CTRL_REPORT_KEY_COUNT_MAX);

	static const struct hid_report_provider_fops report_fops_system_ctrl = {
		.send_report = send_report_system_ctrl,
		.send_empty_report = send_empty_report,
		.connection_state = system_ctrl_report_connection_state,
		.notify_report_sent = system_ctrl_report_sent,
	};

	struct hid_report_provider_event *rp = new_hid_report_provider_event();

	rp->report_id = REPORT_ID_SYSTEM_CTRL;
	rp->p_fops = &report_fops_system_ctrl;
	rp->hs_fops = NULL;
...
static bool handle_module_state_event(const struct module_state_event *event)
{
	if (check_state(event, MODULE_ID(main), MODULE_STATE_READY)) {
		LOG_INF("Init system control report provider");
		init();
	}
...
{
	if (event->report_id == REPORT_ID_SYSTEM_CTRL) {
		__ASSERT_NO_MSG(event->p_fops->connection_state ==
				system_ctrl_report_connection_state);
