...
static void system_ctrl_report_connection_state_changed(uint8_t report_id,
...
static void init(void)
{
	...
	static const struct hid_report_provider_api provider_api_system_ctrl = {
		.send_report = send_report_system_ctrl,
		.send_empty_report = send_empty_report,
		.connection_state_changed = system_ctrl_report_connection_state_changed,
		.report_sent = system_ctrl_report_sent,
	};

	struct hid_report_provider_event *rp_event = new_hid_report_provider_event();

	rp_event->report_id = REPORT_ID_SYSTEM_CTRL;
	rp_event->provider_api = &provider_api_system_ctrl;
