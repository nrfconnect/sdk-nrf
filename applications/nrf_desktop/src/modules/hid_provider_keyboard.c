...
static void keyboard_report_connection_state(uint8_t report_id,
...
static void trigger_report_transmission(void)
{
	...
	if (active_sub) {
		(void)hid_state_fops->trigger_report_send(boot_mode ?
...
static void init(void)
{
	...
	static const struct hid_report_provider_fops report_fops_keyboard = {
		.send_report = send_report_keyboard,
		.send_empty_report = send_empty_report,
		.connection_state = keyboard_report_connection_state,
		.notify_report_sent = keyboard_report_sent,
	};

	struct hid_report_provider_event *rp;

	rp = new_hid_report_provider_event();
	rp->report_id = REPORT_ID_KEYBOARD_KEYS;
	rp->p_fops = &report_fops_keyboard;
	rp->hs_fops = NULL;
	APP_EVENT_SUBMIT(rp);

	if (IS_ENABLED(CONFIG_DESKTOP_HID_BOOT_INTERFACE_KEYBOARD)) {
		static const struct hid_report_provider_fops report_fops_boot_keyboard = {
			.send_report = send_report_keyboard,
			.send_empty_report = send_empty_report,
			.connection_state = keyboard_report_connection_state,
			.notify_report_sent = keyboard_report_sent,
		...
		rp = new_hid_report_provider_event();
		rp->report_id = REPORT_ID_BOOT_KEYBOARD;
		rp->p_fops = &report_fops_boot_keyboard;
		rp->hs_fops = NULL;
		APP_EVENT_SUBMIT(rp);
	}
}
...
static bool handle_hid_report_provider_event(const struct hid_report_provider_event *event)
{
	if (event->report_id == REPORT_ID_KEYBOARD_KEYS) {
		__ASSERT_NO_MSG(event->p_fops->connection_state ==
				keyboard_report_connection_state);
