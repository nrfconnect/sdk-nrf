...
static void hogp_out_rep_write_cb(struct bt_hogp *hogp, struct bt_hogp_rep_info *rep, uint8_t err)
{
	...
	struct enqueued_out_report *found_report = NULL;
	...
	int send_err = send_hid_out_report(hogp, found_report->data, found_report->data_size);

	if (send_err) {
		LOG_ERR("Cannot forward HID report (err: %d)", send_err);
	}

	sys_slist_find_and_remove(&per->enqueued_out_reports, &found_report->node);
	k_free(found_report->data);
	k_free(found_report);
}
