	...
	while (1) {
		...
		case LE_AUDIO_EVT_CONFIG_RECEIVED:
			...
			if (msg.dir == BT_AUDIO_DIR_SINK) {
				...
				ret = unicast_client_locations_get(&locations, &num_channels,
								   BT_AUDIO_DIR_SINK);
				ERR_CHK(ret);

				audio_system_encoder_num_ch_set(locations, num_channels);
...
static void bt_mgmt_evt_handler(const struct zbus_channel *chan)
{
	...
	case BT_MGMT_DISCONNECTED:
		...
		ret = unicast_client_locations_get(&locations, &num_channels, BT_AUDIO_DIR_SINK);
		ERR_CHK(ret);
