...
struct audio_rate_control_ops {
	 ...
	 * @return	0 if successful, error otherwise.
	 */
	int (*update)(void *const control_val_u);
};
...
/**
 * @brief	Update the error between timestamps for an audio rate control module.
 *
 * @param	type		[in]		The type of the rate control source.
 ...
 * @return	0 if successful, error otherwise.
 */
int audio_rate_control_update(uint8_t type, void *const control_val_u);
...
/**
 * @brief	Initialize the registered audio rate control module.
 *
 * @param	type	[in]		The type of the rate control source.
 *
 * @return	0 if successful, error otherwise.
