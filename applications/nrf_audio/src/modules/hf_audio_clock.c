...
int hf_audio_clock_update(void *const control_val_u, bool calibrate)
{
	...
	int32_t base_freq = calibrate ? APLL_FREQ_CENTER : current_freq;
	int32_t freq_value = base_freq + APLL_FREQ_ADJ(err_us);

	if (((freq_value > (APLL_FREQ_MAX)) || (freq_value < (APLL_FREQ_MIN))) && calibrate) {
		return -EINVAL;
	}
