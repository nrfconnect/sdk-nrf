...
static int hf_audio_clock_cfg_set(struct audio_rate_control_cfg const *const cfg)
{
	int32_t const err_us = *(int32_t const *)cfg;
	int32_t freq_value = APLL_FREQ_CENTER + APLL_FREQ_ADJ(err_us);
...
struct audio_rate_control_ops hf_audio_clock_rate_control_ops = {
	...
	.cfg_set = hf_audio_clock_cfg_set,
