	...
	case SW_CODEC_LC3: {
		...
		while (loc_in && loc_out) {
			if (!(loc_out & loc_in & 0x01)) {
				loc_in = loc_in >> in_shift;
				loc_out = loc_out >> out_shift;
				continue;
			}
		...
		meta_out->locations = meta_out->locations & meta_in->locations;
		    ...
		    struct net_buf *const audio_frame_out)
{
	...
	case SW_CODEC_LC3: {
		...
		bool dec_run_option, run_decoder = true;
		...
		while (loc_in && loc_out) {
			if (!(loc_out & loc_in & 0x01)) {
				bad_data_mask = bad_data_mask << in_shift;
				loc_in = loc_in >> in_shift;
				loc_out = loc_out >> out_shift;
				continue;
			}
			...
			if (run_decoder) {
				...
				run_decoder = dec_run_option;
				chan_in = chan_in + (loc_in & 0x01);
			}
			...
			if (meta_out->interleaved) {
				...
				if (IS_ENABLED(CONFIG_SAMPLE_RATE_CONVERTER) &&
				    meta_in->sample_rate_hz != meta_out->sample_rate_hz) {
					src_out = src_out + bytes_written;
				} else {
					dec_out = dec_out + bytes_written;
				}
			}
			...
			chan_out = chan_out + (loc_out & 0x01);
			bad_data_mask = bad_data_mask << in_shift;
			loc_in = loc_in >> in_shift;
			loc_out = loc_out >> out_shift;
		}
