...
Each nRF Audio application corresponds to one specific LE Audio role: unicast client (gateway), unicast server (headset), broadcast source (gateway), or broadcast sink (headset).

Likewise, each nRF Audio application is configured for one specific LE Audio mode: the *connected isochronous stream* (CIS, unicast) mode or in the *broadcast isochronous stream* (BIS) mode.
See :ref:`nrf_audio_app_overview_modes` for more information.
...
See :ref:`nrf_audio_app_overview_files` for more information.
...
.. _nrf_audio_app_overview_gateway_headsets:
...
You can :ref:`select gateway or headset build <nrf_audio_app_configuration_select_build>` when :ref:`nrf_audio_app_building`.

.. _nrf_audio_app_overview_modes:
...
.. figure:: /images/nrf_audio_application_topologies.png
...
Connected Isochronous Stream (CIS)
  ...
  This is the mode available for the unicast applications (:ref:`unicast client<nrf_audio_unicast_client_app>` and :ref:`unicast server<nrf_audio_unicast_server_app>`).
  In this mode, you can use the nRF5340 Audio development kit in the role of the gateway, the left headset, the right headset, or a left+right headset with :ref:`stereo support <nrf_audio_app_configuration_headset_location>`.

  In the current version of the nRF Audio unicast client, the application offers both unidirectional and bidirectional communication.
  In the bidirectional communication, the headset device will send audio from the on-board PDM microphone.
  See :ref:`nrf_audio_app_configuration_select_bidirectional` in the application description for more information.
  ...
  See :ref:`nrf_audio_app_configuration_enable_walkie_talkie` in the application description for more information.
...
Broadcast Isochronous Stream (BIS)
  ...
  This is the mode available for the broadcast applications (:ref:`broadcast source<nrf_audio_broadcast_source_app>` for gateway and :ref:`broadcast sink<nrf_audio_broadcast_sink_app>` for headset.)
  In this mode, you can use the nRF5340 Audio development kit in the role of the gateway or as one of the headsets (left, right, or left+right with :ref:`stereo support <nrf_audio_app_configuration_headset_location>`.)
...
.. _nrf_audio_app_overview_architecture:

Firmware architecture
*********************

The following figure illustrates the high-level software layout for the nRF Audio application:

.. figure:: /images/nrf_audio_structure_generic.svg
   :alt: nRF Audio high-level design (overview)

   nRF Audio high-level design (overview)
...
See :ref:`ug_ble_controller_softdevice` for more information about the controller, and :ref:`SoftDevice Controller for LE Isochronous Channels <nrfxlib:softdevice_controller_iso>` for information on how it implements ISO channels used by the nRF Audio applications.
...
.. _nrf_audio_app_overview_files:

Source file architecture
========================

The following figure illustrates the software layout for the nRF Audio application on the file-by-file level for the supported boards, regardless of the application chosen:

.. figure:: /images/nrf_audio_all_packages.svg
   :alt: nRF Audio application file-level breakdown for the nRF5340 SoC

   nRF Audio application file-level breakdown for the nRF5340 SoC
...
.. _nrf_audio_app_overview_architecture_usb:
...
.. figure:: /images/nrf_audio_broadcast_source_USB_structure.svg
   :alt: nRF Audio modules for the broadcast source using USB

   nRF Audio modules for the broadcast source using USB

Unicast client USB-based firmware
---------------------------------

.. figure:: /images/nrf_audio_unicast_client_USB_structure.svg
   :alt: nRF Audio modules for the unicast client using USB

   nRF Audio modules for the unicast client using USB

.. _nrf_audio_app_overview_architecture_i2s:
...
.. figure:: /images/nrf_audio_broadcast_source_I2S_structure.svg
   :alt: nRF Audio modules for the broadcast source using I2S

   nRF Audio modules for the broadcast source using I2S

Broadcast sink I2S-based firmware
---------------------------------

.. figure:: /images/nrf_audio_broadcast_sink_I2S_structure.svg
   :alt: nRF Audio modules for the broadcast sink using I2S

   nRF Audio modules for the broadcast sink using I2S

Unicast client I2S-based firmware
---------------------------------

.. figure:: /images/nrf_audio_unicast_client_I2S_structure.svg
   :alt: nRF Audio modules for the unicast client using I2S

   nRF Audio modules for the unicast client using I2S

Unicast server I2S-based firmware
---------------------------------

.. figure:: /images/nrf_audio_unicast_server_I2S_structure.svg
   :alt: nRF Audio modules for the unicast server using I2S

   nRF Audio modules for the unicast server using I2S

.. _nrf_audio_app_overview_architecture_sync_module:
...
.. figure:: /images/nrf_audio_structure_sync_module.svg
   :alt: nRF Audio synchronization module overview

   nRF Audio synchronization module overview

Both synchronization methods use the SDU reference timestamps (:c:type:`sdu_ref`) as the reference variable.
If the device is a gateway that is :ref:`using I2S as audio source <nrf_audio_app_overview_architecture_i2s>` and the stream is unidirectional (gateway to headsets), :c:type:`sdu_ref` is continuously being extracted from the LE Audio Controller Subsystem for nRF53 on the gateway.
...
The application core running the nRF Audio application uses its timer to generate :c:type:`cur_time` and :c:type:`frame_start_ts`.
...
.. figure:: /images/nrf_audio_sync_module_states.svg
   :alt: nRF Audio's state machine for compensation mechanisms

   nRF Audio's state machine for compensation mechanisms
...
.. _nrf_audio_app_overview_architecture_sd_card_playback:
...
The module is compatible with all nRF Audio applications.
...
For information about how to enable SD card playback, see :ref:`nrf_audio_app_configuration_sd_card_playback`.
