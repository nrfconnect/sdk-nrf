...
The Kconfig configuration file should be located in subdirectory :file:`child_image/child_image_name` and its name should match the application Kconfig file name, which contains the build type if necessary.
For example, the :file:`applications/machine_learning/configuration/thingy53_nrf5340_cpuapp/child_image/hci_ipc/prj.conf` file defines the configuration of Bluetooth HCI RPMsg for ``debug`` build type on the ``thingy53_nrf5340_cpuapp`` board, while the :file:`applications/machine_learning/configuration/thingy53_nrf5340_cpuapp/child_image/hci_ipc/prj_release.conf` file defines the configuration of Bluetooth HCI RPMsg for ``release`` build type.
...
See :ref:`modifying_build_types` for detailed steps on how to select a build type.
...
This definition is set in a similar way to the selected build type.
