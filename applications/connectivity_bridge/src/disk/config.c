...
/* All options are treated as strings. The handler functions parses according to type. */
/* Null-terminator is always added to string length. */
/* (_key, _presentation_name, _size, _default, _callback, _enabled) */
#define CONFIG_LIST \
	...
	X(cdc_force, UART_ALWAYS_ON, 1, "0", cdc_enable_opt_cb, IS_ENABLED(CONFIG_BRIDGE_CDC_ENABLE))
...
#define X(_key, _name, _size, _default, _cb, _enabled)\
		u8_t _key[sizeof(STRINGIFY(_name)) + 1];
	CONFIG_LIST
#undef X
};
...
#define X(_key, _name, _size, _default, _cb, _enabled)\
		u8_t _key[sizeof(STRINGIFY(_key)) + 1];
	CONFIG_LIST
#undef X
};
...
static bool cdc_enable_opt_cb(char *opt)
{
	enum cdc_ctrl_cmd cmd;

	if (strcmp(opt, "0") == 0) {
		cmd = CDC_CTRL_DISABLE;
	} else if (strcmp(opt, "1") == 0) {
		cmd = CDC_CTRL_ENABLE;
	} else {
		LOG_WRN("Unrecognized cdc_enable_opt: %s", log_strdup(opt));
		return false;
	}

	struct cdc_ctrl_event *event = new_cdc_ctrl_event();
...
static void save_settings(void)
{
	...
	for (int i = 0; i < ARRAY_SIZE(configs); ++i) {
		...
		if (err < 0 || err > sizeof(key)) {
			LOG_ERR("Option format err: %s", configs[i].settings_key);
			continue;
		}
			...
			settings_read_cb read_cb, void *cb_arg)
{
	for (int i = 0; i < ARRAY_SIZE(configs); ++i) {
		...
		/* SETTINGS_VALUE_MAX_LEN covers configs[i].settings_value_max_len + null terminator */
		cfg_val[len] = '\0';
	...
	while (read_line(&file, linebuf, sizeof(linebuf))) {
		for (int i = 0; i < ARRAY_SIZE(configs); ++i) {
			...
			if (strlen(cfg_val) > configs[i].settings_value_max_len) {
				/* Values exceeding expected length are ignored, not truncated */
				break;
			}
...
static bool event_handler(const struct event_header *eh)
{
	if (is_fs_event(eh)) {
		...
		if (event->req == FS_REQUEST_CREATE_FILE) {
			...
			/* Populate file with all configuration options in list */
