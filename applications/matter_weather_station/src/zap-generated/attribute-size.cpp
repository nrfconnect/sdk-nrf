...
uint16_t emberAfCopyList(ClusterId clusterId, EmberAfAttributeMetadata *am, bool write,
			 uint8_t *dest, uint8_t *src, int32_t index)
{
	if (index == -1) {
	...
	if (!CanCastTo<uint16_t>(index)) {
		ChipLogError(Zcl, "Index %" PRId32 " is invalid. Should be between 1 and 65534",
			     index);
		return 0;
	}
	...
	case 0x0030: // General Commissioning Cluster
	{
		...
		case 0x0001: // BasicCommissioningInfoList
		{
			...
			copyListMember(write ? dest : (uint8_t *)&entry->FailSafeExpiryLengthMs,
				       write ? (uint8_t *)&entry->FailSafeExpiryLengthMs : src,
				       write, &entryOffset,
	...
	case 0x0033: // General Diagnostics Cluster
	{
		...
		case 0x0000: // NetworkInterfaces
		{
			...
			if (CHIP_NO_ERROR !=
				...
				ChipLogError(Zcl,
					     "Index %" PRId32
					     " is invalid. Not enough remaining space",
					     index);
				return 0;
			}
	...
	case 0x003E: // Operational Credentials Cluster
	{
		...
		case 0x0001: // fabrics list
		{
			...
			copyListMember(write ? dest : (uint8_t *)&entry->FabricIndex,
				       write ? (uint8_t *)&entry->FabricIndex : src, write,
		...
		case 0x0004: // TrustedRootCertificates
		{
			...
			uint16_t trustedRootCertificatesRemainingSpace =
				static_cast<uint16_t>(am->size - entryOffset);
			if (CHIP_NO_ERROR !=
			    (write ? WriteByteSpan(dest + entryOffset,
						   trustedRootCertificatesRemainingSpace,
						   trustedRootCertificatesSpan) :
					   ReadByteSpan(src + entryOffset,
						  trustedRootCertificatesRemainingSpace,
						  trustedRootCertificatesSpan))) {
				ChipLogError(Zcl,
					     "Index %" PRId32
			...
			if (!CanCastTo<uint16_t>(trustedRootCertificatesSpan->size())) {
				ChipLogError(Zcl, "Span size %zu is too large",
					     trustedRootCertificatesSpan->size());
				return 0;
			}
	...
	case 0x0035: // Thread Network Diagnostics Cluster
	{
		...
		case 0x0008: // RouteTableList
		{
			...
			copyListMember(write ? dest : (uint8_t *)&entry->LQIIn,
				       write ? (uint8_t *)&entry->LQIIn : src, write, &entryOffset,
		...
		case 0x003D: // OperationalDatasetComponents
		{
			...
			_OperationalDatasetComponents *entry =
				reinterpret_cast<_OperationalDatasetComponents *>(write ? src :
												dest);
			...
			copyListMember(write ? dest : (uint8_t *)&entry->NetworkNamePresent,
				       ...
				       &entryOffset, sizeof(entry->NetworkNamePresent)); // BOOLEAN
			copyListMember(write ? dest : (uint8_t *)&entry->ExtendedPanIdPresent,
				       write ? (uint8_t *)&entry->ExtendedPanIdPresent : src, write,
				       &entryOffset,
			...
			copyListMember(write ? dest : (uint8_t *)&entry->ChannelMaskPresent,
				       ...
				       &entryOffset, sizeof(entry->ChannelMaskPresent)); // BOOLEAN
			break;
		}
...
uint16_t emberAfAttributeValueListSize(ClusterId clusterId, AttributeId attributeId,
				       const uint8_t *buffer)
{
	...
	if (!CanCastTo<uint16_t>(totalSize)) {
		ChipLogError(Zcl,
			     "Cluster " ChipLogFormatMEI ": Size of attribute " ChipLogFormatMEI
			     " is too large.",
