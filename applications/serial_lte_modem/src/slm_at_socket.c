...
static int find_opened_socket(int fd)
{
...
static int do_secure_socket_open(int peer_verify)
{
	...
	ret = zsock_setsockopt(sock.fd, SOL_TLS, TLS_SEC_TAG_LIST, sec_tag_list, sizeof(sec_tag_t));
	if (ret) {
			    ...
			    uint32_t param_count)
{
	...
	case AT_PARSER_CMD_TYPE_READ:
		for (int i = 0; i < SLM_MAX_SOCKET_COUNT; i++) {
			if (socks[i].fd != INVALID_SOCKET && socks[i].sec_tag == INVALID_SEC_TAG) {
				rsp_send("\r\n#XSOCKET: %d,%d,%d,%d,%d\r\n",
					socks[i].fd, socks[i].family, socks[i].role,
					socks[i].type, socks[i].cid);
			}
		}
		err = 0;
		break;
		...
		for (int i = 0; i < SLM_MAX_SOCKET_COUNT; i++) {
			if (socks[i].fd != INVALID_SOCKET && socks[i].sec_tag != INVALID_SEC_TAG) {
				rsp_send("\r\n#XSSOCKET: %d,%d,%d,%d,%d,%d\r\n",
					socks[i].fd, socks[i].family, socks[i].role,
					socks[i].type, socks[i].sec_tag, socks[i].cid);
			}
		}
		err = 0;
		break;
...
SLM_AT_CMD_CUSTOM(xclose, "AT#XCLOSE", handle_at_close);
			       ...
			       uint32_t param_count)
{
	...
	case AT_PARSER_CMD_TYPE_TEST:
		rsp_send("\r\n#XSOCKETOPT: <sock_handle>,(%d,%d),<name>,<value>\r\n",
				      ...
				      struct at_parser *parser, uint32_t)
{
	...
	case AT_PARSER_CMD_TYPE_TEST:
		rsp_send("\r\n#XSSOCKETOPT: <sock_handle>,(%d,%d),<name>,<value>\r\n",
			AT_SOCKETOPT_GET, AT_SOCKETOPT_SET);
		err = 0;
		break;
		...
		if (param_count == 2) {
			...
			for (int i = 0; i < SLM_MAX_SOCKET_COUNT; i++) {
				...
				if (param_count > 2 + i) {
					...
					if (find_opened_socket(handle) != handle) {
						return -EINVAL;
					}
					fds[i].fd = handle;
					fds[i].events = ZSOCK_POLLIN;
