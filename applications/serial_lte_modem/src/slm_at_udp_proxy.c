...
static int do_udp_client_connect(const char *url, uint16_t port)
{
	...
	/* Open socket */
	ret = socket(proxy.family, type, using_dtls ? IPPROTO_DTLS_1_2 : IPPROTO_UDP);
	if (ret < 0) {
		LOG_ERR("socket() failed: %d", -errno);
		return -errno;
	}
...
static void udp_thread_func(void *p1, void *p2, void *p3)
{
	...
	do {
		...
		if ((fds.revents & POLLERR) == POLLERR) {
			...
			if (ret) {
				LOG_ERR("getsockopt(SO_ERROR) error: %d", -errno);
				ret = -EIO;
				break;
			}
			if (proxy.role == UDP_ROLE_SERVER && proxy.sec_tag != INVALID_SEC_TAG &&
				...
				util_get_peer_addr((struct sockaddr *)&proxy.remote, peer_addr,
						   &peer_port);
				LOG_INF("DTLS client timed out: \"%s\",%d\r\n", peer_addr,
					peer_port);
			} else {
				LOG_WRN("POLLERR: %d", -value);
				ret = -EIO;
				break;
			}
		}
		...
		if (proxy.role == UDP_ROLE_SERVER) {
			...
			memset(&proxy.remote, 0, sizeof(proxy.remote));
			ret = recvfrom(proxy.sock, (void *)slm_data_buf, sizeof(slm_data_buf),
				       MSG_DONTWAIT, (struct sockaddr *)&proxy.remote, &size);
		} else {
		...
		if (in_datamode()) {
			...
			util_get_peer_addr((struct sockaddr *)&proxy.remote, peer_addr, &peer_port);
...
int handle_at_udp_server(enum at_cmd_type cmd_type)
{
	...
	case AT_CMD_TYPE_SET_COMMAND:
		...
		if (op == SERVER_START || op == SERVER_START6) {
			...
			if (at_params_unsigned_short_get(&slm_at_param_list, 2, &port)) {
				return -EINVAL;
			}
			proxy.sec_tag = INVALID_SEC_TAG;
			if (param_count > 3 &&
			    at_params_int_get(&slm_at_param_list, 3, &proxy.sec_tag)) {
				return -EINVAL;
			}
