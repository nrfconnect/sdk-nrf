...
static inline int kscan_config(struct device * dev, kscan_callback_t callback)
{
	...
	if (z_syscall_trap()) {
		return (int) z_arch_syscall_invoke2(*(u32_t *)&dev, *(u32_t *)&callback, K_SYSCALL_KSCAN_CONFIG);
	}
#endif
	compiler_barrier();
...
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		return (int) z_arch_syscall_invoke1(*(u32_t *)&dev, K_SYSCALL_KSCAN_ENABLE_CALLBACK);
	}
#endif
	compiler_barrier();
...
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		return (int) z_arch_syscall_invoke1(*(u32_t *)&dev, K_SYSCALL_KSCAN_DISABLE_CALLBACK);
	}
#endif
	compiler_barrier();
