...
static inline int i2s_configure(struct device * dev, enum i2s_dir dir, struct i2s_config * cfg)
{
	...
	if (z_syscall_trap()) {
		return (int) z_arch_syscall_invoke3(*(u32_t *)&dev, *(u32_t *)&dir, *(u32_t *)&cfg, K_SYSCALL_I2S_CONFIGURE);
	}
#endif
	compiler_barrier();
...
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		return (int) z_arch_syscall_invoke3(*(u32_t *)&dev, *(u32_t *)&buf, *(u32_t *)&size, K_SYSCALL_I2S_BUF_READ);
	}
#endif
	compiler_barrier();
...
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		return (int) z_arch_syscall_invoke3(*(u32_t *)&dev, *(u32_t *)&buf, *(u32_t *)&size, K_SYSCALL_I2S_BUF_WRITE);
	}
#endif
	compiler_barrier();
...
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		return (int) z_arch_syscall_invoke3(*(u32_t *)&dev, *(u32_t *)&dir, *(u32_t *)&cmd, K_SYSCALL_I2S_TRIGGER);
	}
#endif
	compiler_barrier();
