...
static int enable_settings(void)
{
	...
	struct settings_handler my_conf = { .name = "bsec",
					    .h_set = settings_set };
...
static int8_t bus_write(u8_t dev_addr, u8_t reg_addr,
			u8_t *reg_data_ptr, uint16_t data_len)
{
	u8_t buf[data_len + 1];
...
static int8_t bus_read(u8_t dev_addr, u8_t reg_addr,
		       u8_t *reg_data_ptr, uint16_t data_len)
{
	return i2c_write_read(i2c_master, dev_addr, &reg_addr, 1, reg_data_ptr,
			      data_len);
}
...
static uint32_t state_load(u8_t *state_buffer, uint32_t n_buffer)
{
...
static void state_save(const u8_t *state_buffer, uint32_t length)
{
	settings_save_one("bsec/state", state_buffer, length);
}

static uint32_t config_load(u8_t *config_buffer, uint32_t n_buffer)
{
	/*
...
int env_sensors_start_polling(void)
{
	...
	ret = bsec_iot_init(BSEC_SAMPLE_RATE_LP, 1.2f, bus_write, bus_read,
			    sleep, state_load, config_load);
