...
static inline int spi_transceive(struct device * dev, const struct spi_config * config, const struct spi_buf_set * tx_bufs, const struct spi_buf_set * rx_bufs)
{
	...
	if (z_syscall_trap()) {
		return (int) z_arch_syscall_invoke4(*(u32_t *)&dev, *(u32_t *)&config, *(u32_t *)&tx_bufs, *(u32_t *)&rx_bufs, K_SYSCALL_SPI_TRANSCEIVE);
	}
#endif
	compiler_barrier();
...
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		return (int) z_arch_syscall_invoke2(*(u32_t *)&dev, *(u32_t *)&config, K_SYSCALL_SPI_RELEASE);
	}
#endif
	compiler_barrier();
