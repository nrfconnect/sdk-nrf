...
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		return (u32_t) z_arch_syscall_invoke0(K_SYSCALL_LOG_BUFFERED_CNT);
	}
#endif
	compiler_barrier();
...
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		return (u32_t) z_arch_syscall_invoke4(*(u32_t *)&backend, *(u32_t *)&domain_id, *(u32_t *)&src_id, *(u32_t *)&level, K_SYSCALL_LOG_FILTER_SET);
	}
#endif
	compiler_barrier();
