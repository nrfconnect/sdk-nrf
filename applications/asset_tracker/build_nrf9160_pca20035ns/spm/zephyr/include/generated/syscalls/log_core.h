...
static inline void z_log_string_from_user(u32_t src_level_val, const char * str)
{
	...
	if (z_syscall_trap()) {
		z_arch_syscall_invoke2(*(u32_t *)&src_level_val, *(u32_t *)&str, K_SYSCALL_Z_LOG_STRING_FROM_USER);
		return;
	}
#endif
	compiler_barrier();
...
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		z_arch_syscall_invoke4(*(u32_t *)&src_level_val, *(u32_t *)&metadata, *(u32_t *)&data, *(u32_t *)&len, K_SYSCALL_Z_LOG_HEXDUMP_FROM_USER);
		return;
	}
#endif
	compiler_barrier();
