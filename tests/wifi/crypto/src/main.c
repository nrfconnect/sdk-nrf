...
ZTEST(wifi_crypto, test_main)
{
	 ...
	 * at the beginning.
	 */
	slot = wifi_kmu_get_next_slot(CONFIG_NRF_WIFI_KMU_NUM_SLOTS * 16);
	...
	db_id = 0;
	key_index = 0;
	key_length = wifi_kmu_get_key_size_in_bytes(type);

	dest_address = wifi_kmu_get_key_start_addr(type, db_id, key_index);
	zassert_not_equal(dest_address, wifi_kmu_key_addr_invalid);

	err = wifi_kmu_write_key(slot, dest_address, (uint8_t *)ccmp256_key, key_length);
	zassert_equal(err, 0);
