 ...
 * which is split into 2 messages.
 * Use sms_send() instead of sms_send_text() to test all API functions.
 * Also use data_len parameter to test that the buffer doesn't need to be NUL terminated.
 */
void test_send_concat_291chars_2msgs_with_length(void)
{
#if !defined(CONFIG_SMS_STATUS_REPORT)
	TEST_IGNORE();
#endif
	...
	/* The sent text is 300 bytes but length of 291 is given to verify
	 * that NUL termination is not required.
	 */
	int ret = sms_send("+123456789012",
		"",
		291, SMS_DATA_TYPE_ASCII);
