...
ZTEST(suite_contin_array_chan, test_contin_buf_api)
{
	...
	ret = contin_array_buf_create(NULL, pcm_finite, CONTIN_TEST_POOL_FINITE_SIZE, locs_out,
				      &finite_pos);
	zassert_equal(ret, -ENXIO, "Failed to recognize NULL pointer: %d", ret);

	/* Test for pcm_finite buffer pointer NULL */
	ret = contin_array_buf_create(pcm_contin, NULL, CONTIN_TEST_POOL_FINITE_SIZE, locs_out,
				      &finite_pos);
	zassert_equal(ret, -ENXIO, "Failed to recognize NULL pointer: %d", ret);

	/* Test for pcm_finite buffer pointer NULL */
	ret = contin_array_buf_create(pcm_contin, pcm_finite, CONTIN_TEST_POOL_FINITE_SIZE,
				      locs_out, NULL);
	zassert_equal(ret, -ENXIO, "Failed to recognize NULL pointer: %d", ret);
	...
	ret = contin_array_buf_create(pcm_contin, pcm_finite, CONTIN_TEST_POOL_FINITE_SIZE,
				      TEST_LOC_MONO, &finite_pos);
	zassert_equal(ret, -EPERM, "Failed to recognize channel out of range: %d", ret);

	/* Test for no channel in locations */
	ret = contin_array_buf_create(pcm_contin, pcm_finite, CONTIN_TEST_POOL_FINITE_SIZE, 10,
				      &finite_pos);
	zassert_equal(ret, -EPERM, "Failed to recognize channel out of range: %d", ret);
