...
ZTEST(suite_contin_array_chan, test_contin_buf_api)
{
	...
	ret = contin_array_buf_create(NULL, pcm_finite, ARRAY_SIZE(test_arr), locs_out,
				      &finite_pos);
	zassert_equal(ret, -ENXIO, "Failed to recognize NULL pointer: %d", ret);

	/* Test for pcm_finite buffer pointer NULL */
	ret = contin_array_buf_create(pcm_contin, NULL, ARRAY_SIZE(test_arr), locs_out,
				      &finite_pos);
	zassert_equal(ret, -ENXIO, "Failed to recognize NULL pointer: %d", ret);

	/* Test for pcm_finite buffer pointer NULL */
	ret = contin_array_buf_create(pcm_contin, pcm_finite, ARRAY_SIZE(test_arr), locs_out, NULL);
	zassert_equal(ret, -ENXIO, "Failed to recognize NULL pointer: %d", ret);
	...
	finite_pos = ARRAY_SIZE(test_arr) + 1;
	ret = contin_array_buf_create(pcm_contin, pcm_finite, ARRAY_SIZE(test_arr), locs_out,
				      &finite_pos);
	zassert_equal(ret, -EPERM, "Failed to recognize out of range finite_pos: %d", ret);

	/* Test for locations zero (mono) */
	ret = contin_array_buf_create(pcm_contin, pcm_finite, ARRAY_SIZE(test_arr), TEST_LOC_MONO,
				      &finite_pos);
	zassert_equal(ret, -EPERM, "Failed to recognize channel out of range: %d", ret);

	/* Test for no channel in locations */
	ret = contin_array_buf_create(pcm_contin, pcm_finite, ARRAY_SIZE(test_arr), 10,
				      &finite_pos);
	zassert_equal(ret, -EPERM, "Failed to recognize channel out of range: %d", ret);

	/* Test for invalid locations in the continuous buffer */
	meta_contin->locations = TEST_LOC_MONO;
	ret = contin_array_buf_create(pcm_contin, pcm_finite, ARRAY_SIZE(test_arr),
				      BT_AUDIO_LOCATION_FRONT_LEFT, &finite_pos);
	zassert_equal(ret, -EPERM, "Failed to recognize channel not present: %d", ret);

	/* Test for invalid locations in the continuous buffer */
	meta_contin->locations = TEST_LOC_R;
	ret = contin_array_buf_create(pcm_contin, pcm_finite, ARRAY_SIZE(test_arr), TEST_LOC_L,
				      &finite_pos);
	zassert_equal(ret, -EPERM, "Failed to recognize channel not present: %d", ret);
