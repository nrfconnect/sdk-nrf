			...
			uint8_t locs_out, bool interleaved, bool filled, size_t iterations)
{
	...
	uint8_t *pcm_finite_buf = test_arr;
	uint8_t num_ch;
	...
	for (size_t i = 0; i < iterations; i++) {
		...
		if (filled) {
			 ...
			 */
			memcpy(pcm_contin->data, CONTIN_TEST_BYTE, pcm_contin->size);
	...
	for (size_t i = 0; i < iterations; i++) {
		...
		meta_contin->locations = contin_locs;
		num_ch = metadata_num_ch_get(meta_contin);
		meta_contin->bytes_per_location = (pcm_contin->size / num_ch) / carrier_bytes;
