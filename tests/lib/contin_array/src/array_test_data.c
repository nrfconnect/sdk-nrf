/*
 * Copyright (c) 2024 Nordic Semiconductor ASA
 *
 * SPDX-License-Identifier: LicenseRef-Nordic-5-Clause
 */
#include <array_test_data.h>

#include <zephyr/ztest.h>
#include <errno.h>
#include <stdlib.h>
#include <stdio.h>
#include <ctype.h>
#include <zephyr/kernel.h>
#include <audio_defines.h>

const uint8_t test_arr[CONTIN_TEST_FINITE_DATA_SIZE] = {
	};

void validate_contin_single_array(struct net_buf *pcm_contin, uint8_t *pcm_finite, uint8_t channels,
				  uint8_t channel, uint16_t finite_start, uint16_t finite_end,
				  bool interleaved)
{
	uint16_t channel_step;
	uint16_t contin_end;
	struct audio_metadata *meta_contin = net_buf_user_data(pcm_contin);

	if (finite_end) {
		finite_end = finite_end - 1;
	} else {
		finite_end = CONTIN_TEST_POOL_FINITE_SIZE - 1;
	}

	if (interleaved) {
		channel_step = meta_contin->carried_bits_per_sample / 8;
		contin_end = (channels * meta_contin->bytes_per_location) -
			     (channel_step * (channels - 1)) - 1;
	} else {
		channel_step = meta_contin->bytes_per_location;
		contin_end = meta_contin->bytes_per_location - 1;
	}

	zassert_equal(pcm_contin->len, meta_contin->bytes_per_location * channels,
		      "Incorrect continuous array length: %d (%d)", pcm_contin->len,
		      meta_contin->bytes_per_location * channels);

	for (uint8_t j = 0; j < channels; j++) {
		uint8_t test_chan = j + 1;

		if (test_chan == channel) {
			zassert_equal(pcm_contin->data[channel_step * j], pcm_finite[finite_start],
				      "First value 0x%x for channel %d is not 0x%x",
				      pcm_contin->data[channel_step * j], test_chan,
				      pcm_finite[finite_start]);

			zassert_equal(pcm_contin->data[contin_end + (j * channel_step)],
				      pcm_finite[finite_end],
				      "Last value 0x%x for channel %d is not 0x%x",
				      pcm_contin->data[contin_end + (j * channel_step)], test_chan,
				      pcm_finite[finite_end]);
		} else {
			zassert_equal(pcm_contin->data[channel_step * j], CONTIN_TEST_CONTIN_BYTE,
				      "First value 0x%x for channel %d is not 0x%x",
				      pcm_contin->data[channel_step * j], test_chan,
				      CONTIN_TEST_CONTIN_BYTE);

			zassert_equal(pcm_contin->data[contin_end + (j * channel_step)],
				      CONTIN_TEST_CONTIN_BYTE,
				      "Last value 0x%x for channel %d is not 0x%x",
				      pcm_contin->data[channel_step * j], test_chan,
				      CONTIN_TEST_CONTIN_BYTE);
		}
	}
}

void validate_contin_chans_array(struct net_buf *pcm_contin, uint8_t *pcm_finite, uint8_t channels,
				 uint16_t finite_start, uint16_t finite_end, bool interleaved)
{
	uint16_t channel_step;
	uint16_t contin_end;
	struct audio_metadata *meta_contin = net_buf_user_data(pcm_contin);

	if (finite_end) {
		finite_end = finite_end - 1;
	} else {
		finite_end = CONTIN_TEST_POOL_FINITE_SIZE - 1;
	}

	if (interleaved) {
		channel_step = meta_contin->carried_bits_per_sample / 8;
		contin_end = (channels * meta_contin->bytes_per_location) -
			     (channel_step * (channels - 1)) - 1;
	} else {
		channel_step = meta_contin->bytes_per_location;
		contin_end = meta_contin->bytes_per_location - 1;
	}

	for (int j = 0; j < channels; j++) {
		uint8_t test_chan = j + 1;

		zassert_equal(pcm_contin->data[channel_step * j], pcm_finite[finite_start],
			      "First value 0x%x for channel %d is not 0x%x",
			      pcm_contin->data[channel_step * j], test_chan,
			      pcm_finite[finite_start]);

		zassert_equal(pcm_contin->data[contin_end + (j * channel_step)],
			      pcm_finite[finite_end], "Last value 0x%x for channel %d is not 0x%x",
			      pcm_contin->data[contin_end + (j * channel_step)], test_chan,
			      pcm_finite[finite_end]);
	}
}
