...
static void ntn_event_handler(const struct ntn_evt *const evt)
{
	...
	case NTN_EVT_MODEM_LOCATION_UPDATE:
		...
		TEST_ASSERT_EQUAL(test_event_data[index].modem_location.accuracy,
				  evt->modem_location.accuracy);
		break;
...
void test_ntn_modem_location_update_event(void)
{
	...
	test_event_data[0].modem_location.accuracy = 200;

	test_event_data[1].type = NTN_EVT_MODEM_LOCATION_UPDATE;
	test_event_data[1].modem_location.updates_requested = true;
	test_event_data[1].modem_location.accuracy = 2000;

	test_event_data[2].type = NTN_EVT_MODEM_LOCATION_UPDATE;
	test_event_data[2].modem_location.updates_requested = false;
	test_event_data[2].modem_location.accuracy = 0;
...
void test_ntn_modem_location_update_event_next(void)
{
	...
	test_event_data[1].modem_location.accuracy = 0;
 ...
 */
void test_ntn_modem_location_update_event_next_no_change(void)
{
	ntn_callback_count_expected = 2;
...
/* Tests that location updates are requested correctly when next_requested_accuracy is used
 * and it's needed in less than CONFIG_NTN_MODEM_LOCATION_REQ_MARGIN seconds.
 */
void test_ntn_modem_location_update_event_next_immediate(void)
{
	ntn_callback_count_expected = 2;
	 ...
	 * CONFIG_NTN_MODEM_LOCATION_REQ_MARGIN, so the event is sent immediately.
	 */
...
void test_ntn_modem_location_set(void)
{
	int ret;
	...
	test_event_data[0].type = NTN_EVT_MODEM_LOCATION_UPDATE;
	test_event_data[0].modem_location.updates_requested = true;
	test_event_data[0].modem_location.accuracy = 2000;
	...
	ret = ntn_modem_location_set(60.0, 24.0, 100.0, 43200);
...
void test_ntn_modem_location_set_error(void)
{
	int ret;
	...
	test_event_data[0].type = NTN_EVT_MODEM_LOCATION_UPDATE;
	test_event_data[0].modem_location.updates_requested = true;
	test_event_data[0].modem_location.accuracy = 500;
...
void test_ntn_modem_location_invalidate(void)
{
	int ret;
...
void test_ntn_modem_location_invalidate_error(void)
{
	int ret;
