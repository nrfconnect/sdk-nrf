...
#define TEST_CB_WAIT_TIME (CONFIG_DATE_TIME_UPDATE_INTERVAL_SECONDS * 1000 + 10000)
...
/**
 * Test initial auto update with NTP. Modem time is not queried because NTP is configured and
 * %XTIME notification hasn't been received.
 *
 ...
 */
void test_date_time_initial_auto_update_ntp(void)
{
...
{
	/* Verify that type is previously returned type, i.e., modem */
