...
This test code verifies custom LTK feature implmented in :ref:`host_extensions.h`.

Test Cases
**********

LTK test :ref:`custom_ltk.sh`
...
Test procedure:
    ...
    3. Connection is etablished and both sides set custom LTK key in the connection callback, authentication flag set to false.
...
LTK coexistence test :ref:`custom_ltk_coex.sh`

Purpose: verify that custom LTK for secure connections coexist with normal SMP workflow.

Test procedure:
    1. Ebable passkey authentication on both devices.
    2. Peripheral device starts connectable advertising.
    3. Central device starts scanning and initiates connection.
    4. Connection is etablished and both sides set custom LTK key in the connection callback with authentication flag enabled.
...
LTK invalid test :ref:`custom_ltk_invalid.sh`
