...
ZTEST(saadct_error_cases, test_meas_get_in_progress_returns_eagain)
{
	...
	zassert_equal(ret, -EAGAIN, "Expected -EAGAIN while a series is in progress, got %d", ret);
}
...
ZTEST(saadct_error_cases, test_meas_get_no_data_returns_eio)
{
	 ...
	 */
	ret = saadct_get(saadct_dev, &data, K_NO_WAIT);
	zassert_equal(ret, -EIO, "Expected -EIO when no measurement is available, got %d", ret);
}
...
ZTEST(saadct_error_cases, test_start_out_of_memory_returns_enomem)
{
	...
	zassert_equal(ret, -ENOMEM, "Expected -ENOMEM when the slab is exhausted, got %d", ret);
...
ZTEST(saadct_error_cases, test_start_null_slab)
{
	...
	zassert_equal(ret, -EINVAL, "Expected -EINVAL when slab is NULL, got %d", ret);
}
