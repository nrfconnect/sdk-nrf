 ...
 *      Read operations are performed on whole whole written area
...
ZTEST(flash_driver, test_read_unaligned_address)
{
	...
	for (off_t addr_o = 0; addr_o < 4; addr_o++) {
		rc = flash_write(flash_dev, TEST_AREA_START_ADDR + addr_o, expected, TEST_AREA_WRITE_SIZE);
		...
		if(addr_o % 4) {
			...
			for (off_t buf_o = 1; buf_o < 5; buf_o++) {
				...
				rc = flash_read(flash_dev,
					TEST_AREA_START_ADDR + addr_o, buf + buf_o, TEST_AREA_WRITE_SIZE);
				zassert_equal(rc, 0, "Cannot read flash");
				zassert_equal(memcmp(buf + buf_o, expected + addr_o, TEST_AREA_WRITE_SIZE), 0,
					"Flash read failed addr_o=%d, buf_o=%d", addr_o, buf_o);
