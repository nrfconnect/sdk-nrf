...
uint8_t test_data_pattern[FLASH_WORD_SIZE] = {0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xA, 0xB, 0xC, 0xD, 0xE, 0xF, 0x10};
#else
uint8_t test_data_pattern[FLASH_WORD_SIZE] = {0xF1, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9, 0xFA, 0xFB, 0xFC, 0xFD, 0xFE, 0xFF, 0x20};
#endif
...
static int write_soc_flash(const struct device *flash_dev, uint32_t offset, uint8_t *buffer) {
    printk("Write SoC flash, base address: %x, offset: %x\n", TEST_DATA_PARTITION_OFFSET, offset * FLASH_WORD_SIZE);
    return flash_write(flash_dev, TEST_DATA_PARTITION_OFFSET + offset * FLASH_WORD_SIZE, buffer, FLASH_WORD_SIZE);
}
#endif
...
static int read_soc_flash(const struct device *flash_dev, uint32_t offset, uint8_t *buffer) {
    ...
    return flash_read(flash_dev, TEST_DATA_PARTITION_OFFSET + offset * FLASH_WORD_SIZE, buffer, FLASH_WORD_SIZE);
}
#endif
