...
static void i2s_dir_both_transfer_long(struct i2s_config *i2s_cfg,
	uint32_t expected_divider, uint32_t expected_edges, uint32_t delta)
{
	...
	zassert_within(pulses, expected_edges, delta,
		"got %u while expected is %u with delta %u", pulses, expected_edges, delta);
}
