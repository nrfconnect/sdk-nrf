...
static const struct gpio_dt_spec gpio_mclk_spec = GPIO_DT_SPEC_GET_BY_IDX(DT_PATH(zephyr_user), gpios, 0);
static const struct gpio_dt_spec gpio_lrck_spec = GPIO_DT_SPEC_GET_BY_IDX(DT_PATH(zephyr_user), gpios, 1);

static const struct device *dev_i2s;

/* ISR that couns rising edges on the MCLK signal. */
void gpio_mclk_isr(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
...
void gpio_lrck_isr(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
...
/* Find offset. */
	...
	for (int i = 0; i < sample_no; i++) {
		if (rx_block[2 * i] != data_l[i]) {
			...
			TC_PRINT("OK: data_l match at position %d\n", i);
		}
		if (rx_block[2 * i + 1] != data_r[i]) {
			...
			TC_PRINT("OK: data_r match at position %d\n", i);
		}
	}
