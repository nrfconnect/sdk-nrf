...
#ifdef CONFIG_I2S_TEST_FRAME_CLOCK_MEASUREMENT
...
static void i2s_transfer_short(uint32_t frame_clk_freq)
{
	...
	if (IS_ENABLED(CONFIG_I2S_TEST_FRAME_CLOCK_MEASUREMENT)) {
...
static void *test_i2s_speed_rxtx_configure(void)
{
	...
	if (IS_ENABLED(CONFIG_I2S_TEST_FRAME_CLOCK_MEASUREMENT)) {
		...
		if (!IS_ENABLED(CONFIG_I2S_TEST_FRAME_CLOCK_MEASUREMENT_GPIO_POLLING)) {
			ret = gpio_pin_interrupt_configure_dt(&frame_clk_sense_gpio,
