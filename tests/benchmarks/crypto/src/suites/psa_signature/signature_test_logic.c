...
static psa_status_t setup_key(const void *context)
{
	...
	if (test->key_type == PSA_KEY_TYPE_RSA_KEY_PAIR) {
		if (test->key_bits == 2048) {
			key_data = rsa_2048_private_key;
			key_data_length = rsa_2048_private_key_len;
		} else if (test->key_bits == 4096) {
			key_data = rsa_4096_private_key;
			key_data_length = rsa_4096_private_key_len;
		} else {
...
const struct op signature_keysetup_ops[] = {{"setup", setup_key, NULL}};
