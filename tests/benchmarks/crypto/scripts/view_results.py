...
def load(path):
    ...
    if not isinstance(recs, list) or not recs:
        raise ValueError(
            "No 'operations' array — is this crypto_benchmarks "
            "JSON output? Enable CRYPTO_BENCHMARKS_OUTPUT_JSON"
        )
    clean, hidden = [], 0
    for r in recs:
...
def draw_dots(ax, rows, labels, key, log_x, xlabel, title, legend):
    ...
    for stage in stages_present(rows):
        ...
        ax.scatter(
            ...
            s=54,
            zorder=3,
            facecolor=STAGE_COLOUR.get(stage, MUTED),
            alpha=0.9,
    ...
    if legend:
        leg = ax.legend(
            loc="lower right", frameon=True, fontsize=8.5, prop={"family": MONO, "size": 8.5}
        )
...
class ScrollableFigure(ttk.Frame):
    ...
    def __init__(self, parent, fig, height_px):
        ...
        for seq, notches in (("<Button-4>", -1), ("<Button-5>", 1)):
            self.view.bind(seq, lambda e, n=notches: self._wheel(n), add="+")
        self.view.bind("<MouseWheel>", lambda e: self._wheel(-1 if e.delta > 0 else 1), add="+")
    ...
    def _render(self, width):
        self.fig.set_size_inches(width / DPI, self.height_px / DPI, forward=False)
        self.relayout(width)
        self.canvas.draw()
        rgba = np.asarray(self.canvas.buffer_rgba())
        self._image = ImageTk.PhotoImage(Image.fromarray(rgba, mode="RGBA"))
        self.view.itemconfigure(self._image_item, image=self._image)
...
class PlotTab(ScrollableFigure):
    ...
    def __init__(self, parent, specs):
        ...
        fig = Figure(figsize=(FIG_W_PX / DPI, height_px / DPI), dpi=DPI, facecolor=PLANE)
        ratios = [max(1, len(s["labels"])) for s in specs]
        axes = fig.subplots(len(specs), 1, gridspec_kw={"height_ratios": ratios}, squeeze=False)[
            :, 0
        ]

        self.picks = {}
        for ax, spec in zip(axes, specs, strict=False):
            xs, ys, recs = draw_dots(
                ax,
                spec["rows"],
                spec["labels"],
                spec["key"],
            ...
            self.picks[ax] = (np.asarray(xs, dtype=float), np.asarray(ys, dtype=float), recs)
        ...
        self.view.bind("<Motion>", self._hover, add="+")
    ...
    def _hover(self, event):
        try:
            ...
            point = (self.view.canvasx(event.x), self.height_px - self.view.canvasy(event.y))
            ax = next(
                (candidate for candidate in self.picks if candidate.bbox.contains(*point)),
                None,
            )
            if ax is None:
                return self._clear()
            ...
            d2 = (disp[:, 0] - point[0]) ** 2 + (disp[:, 1] - point[1]) ** 2
            i = int(d2.argmin())
            if d2[i] > 26**2:
                return self._clear()
            rec = recs[i]
    ...
    def _show_hover(self, text, point):
        if self._hover_text is None:
            self._hover_box = self.view.create_rectangle(0, 0, 0, 0, fill=SURFACE, outline=AXIS)
            self._hover_text = self.view.create_text(
                0, 0, anchor="nw", text=text, fill=INK, font=(MONO, 9)
            )
        else:
            self.view.itemconfigure(self._hover_text, text=text)
        bounds = self.view.bbox(self._hover_text)
        width = bounds[2] - bounds[0] + 12
        height = bounds[3] - bounds[1] + 8
        x = min(max(4, point[0] + 14), self.view.canvasx(self.view.winfo_width()) - width - 4)
        y = min(max(4, self.height_px - point[1] + 14), self.height_px - height - 4)
        self.view.coords(self._hover_box, x, y, x + width, y + height)
        self.view.coords(self._hover_text, x + 6, y + 4)

    def _clear(self):
        self._hover_state = None
        if self._hover_box is not None:
            self.view.delete(self._hover_box)
            self.view.delete(self._hover_text)
            self._hover_box = None
            self._hover_text = None
...
class TableTab(ttk.Frame):
    ...
    def _sort(self, key):
        self.sort_desc = (
            not self.sort_desc if key == self.sort_key else key in ("elapsed_us", "stack_used")
        )
        self.sort_key = key
        self._fill()
    ...
    def _fill(self):
        ...
        for r in rows:
            self.tree.insert(
                ...
                "end",
                values=(
                    str(r["group"]).replace("psa_", ""),
                    r["alg"],
                    r.get("keydesc") or "—",
                    r["stage"],
                    r["op"],
                    fmt_time(r["elapsed_us"]),
...
class App(tk.Tk):
    def __init__(self, path=None):
        ...
        ttk.Label(top, text="crypto_benchmarks results", style="H1.TLabel").pack(side="left")
    ...
    def _style(self):
        ...
        st.configure(
            "H1.TLabel", background=PLANE, foreground=INK, font=("TkDefaultFont", 15, "bold")
        )
        st.configure("Prov.TLabel", background=PLANE, foreground=INK_2, font=("TkFixedFont", 9))
        st.configure("Note.TLabel", background=PLANE, foreground=MUTED, font=("TkDefaultFont", 9))
    ...
    def show(self, path):
        try:
            recs, _summary, hidden = load(path)
        ...
        self.prov.configure(
            text=(f"{shown}  ·  {len(by_group)} families  ·  {fmt_time(total)} total  ·  {path}")
        )

        order = sorted(
            by_group, key=lambda g: -statistics.median([r["elapsed_us"] for r in by_group[g]])
        )
        for group in order:
            rows = by_group[group]
            for r in rows:
            ...
            specs = [
                dict(
                    rows=rows,
                    labels=ordered_rows(rows, "elapsed_us"),
                    key="elapsed_us",
                    log=True,
                    xlabel="elapsed (logarithmic)",
                    title=f"{name} — elapsed time",
                    legend=True,
                ),
                dict(
                    rows=rows,
                    labels=ordered_rows(rows, "stack_used"),
                    key="stack_used",
                    log=False,
                    xlabel="peak stack (bytes)",
                    title=f"{name} — peak stack",
                    legend=False,
                ),
            ]
