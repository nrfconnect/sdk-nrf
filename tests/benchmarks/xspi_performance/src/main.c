				 ...
				 const char *operation_name, flash_op_type op_type)
{
	...
	if ((required_repetitions == 0) && (flash_operation_size > MAX_TEST_BUFFER_SIZE)) {
		printk("!!!! Operation size %u exceeds test buffer %u !!!!\n",
		       (unsigned)flash_operation_size, MAX_TEST_BUFFER_SIZE);
		return;
	}
	...
	if (required_repetitions > 0) {
		...
		for (int i = 0; i < required_repetitions; i++) {
			...
			err = flash_operation(flash_dev, flash_offset, test_buffer, page_size);
			if (err != 0) {
				break;
			}

			if (IS_ENABLED(CONFIG_TEST_DATA_VERIFICATION) && (op_type == FLASH_OP_READ)) {
				err = verify_test_buffer(test_buffer, page_size, TEST_DATA_PATTERN,
							 operation_name);
				if (err != 0) {
					break;
				}
		...
		if ((err == 0) && IS_ENABLED(CONFIG_TEST_DATA_VERIFICATION) &&
		    (op_type == FLASH_OP_READ)) {
			err = verify_test_buffer(test_buffer, flash_operation_size,
						 TEST_DATA_PATTERN, operation_name);
		}
	}
