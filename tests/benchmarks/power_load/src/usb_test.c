...
static void usb_load_thread_worker(void *arg1, void *arg2, void *arg3)
{
	ARG_UNUSED(arg1);
	ARG_UNUSED(arg2);
	ARG_UNUSED(arg3);
	...
	ret = setup();
	if (ret != 0) {
		LOG_ERR("USB setup failed: %d\n", ret);
		return;
	}

	atomic_inc(&started_threads);

	LOG_INF("USB load thread started");
	...
	while (true) {
		...
		k_sleep(K_MSEC(USB_THREAD_SLEEP));
	}
...
K_THREAD_DEFINE(usb_load_thread, USB_THREAD_STACKSIZE, usb_load_thread_worker, NULL, NULL, NULL,
