...
#define TEST_CAP_STREAM(name, dir_in, pd_in, group_in, interval_in)                                \
	...
	struct bt_audio_codec_cfg name##_codec_cfg = {0};                                          \
	name##_qos.pd = pd_in;                                                                     \
	...
	name.bap_stream.codec_cfg = &name##_codec_cfg;

static struct bt_bap_ep *bt_bap_iso_get_paired_ep_fake(const struct bt_bap_ep *ep)
{
	if (ep->iso->rx.ep == ep) {
		return ep->iso->tx.ep;
	} else {
...
static int bt_bap_ep_get_info_custom_fake(const struct bt_bap_ep *ep, struct bt_bap_ep_info *info)
{
...
static int bt_cap_stream_send_ts_custom_fake(struct bt_cap_stream *stream, struct net_buf *buf,
...
static int bt_cap_stream_send_custom_fake(struct bt_cap_stream *stream, struct net_buf *buf,
					  uint16_t seq_num)
{
