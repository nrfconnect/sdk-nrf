...
#define TEST_SPIS_DEV_IDX   0
#define TEST_MEMORY_DEV_IDX 1
...
static void work_handler(struct k_work *work)
{
	...
	for (int dev_idx = 0; dev_idx < ARRAY_SIZE(mspi_devices); ++dev_idx) {
		...
		ret = mspi_dev_config(mspi_bus, &dev_id[dev_idx], MSPI_DEVICE_CONFIG_ALL,
				      &device_cfg[dev_idx]);
		zassert_equal(ret, 0, "mspi_dev_config failed.");
	}

	if (td->test_emu_spis_dev) {
		/* write or read data to/from emulated spis device */
		if (td->mtx_set) {
...
static void run_test(bool m_same_size, bool s_same_size, bool emu_spis_dev)
{
	...
	if (emu_spis_dev) {
		...
		if (tdata.srx_set) {
			...
			zassert_equal(rv, 0, "Cannot write flash");
		}
	}
