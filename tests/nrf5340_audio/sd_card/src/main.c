			...
			uint8_t subdirs)
{
	...
	f_tree[on_level].name = curr_dir_id % subdirs;

	while (on_level > 0) {
		f_tree[on_level - 1].peer_id = f_tree[on_level].peer_id / subdirs;
		f_tree[on_level - 1].name = f_tree[on_level - 1].peer_id % subdirs;
		on_level--;
	}
	...
	for (int i = 0; i < curr_level + 1; i++) {
		...
		ret = sprintf(tmp, "/dir_%d", f_tree[i].name);
		strcat(name, tmp);
	}
...
#define SD_PATHLEN_MAX	 (CONFIG_FS_FATFS_MAX_LFN - 3)
