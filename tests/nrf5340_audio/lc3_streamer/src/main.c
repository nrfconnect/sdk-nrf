...
static void test_setup(void *f)
{
	...
	zassert_equal(0, ret, "lc3_streamer_clear_all_streams should return success");
}
...
static void test_teardown(void *f)
{
	...
	ret = lc3_streamer_clear_all_streams();
	zassert_equal(0, ret, "lc3_streamer_clear_all_streams should return success");
}

ZTEST(lc3_streamer, test_lc3_streamer_register_stream_valid)
{
	int ret;
	ssize_t streamer_idx;

	ret = lc3_streamer_register_stream("test", &streamer_idx, false);
	zassert_equal(0, ret, "lc3_streamer_register_stream should return success");
	zassert_equal(0, streamer_idx, "lc3_streamer_register_stream should return index 0");
...
ZTEST(lc3_streamer, test_lc3_streamer_register_stream_invalid_nullptr)
{
	int ret;
	ssize_t streamer_idx;

	ret = lc3_streamer_register_stream(NULL, &streamer_idx, false);
	zassert_equal(-EINVAL, ret,
		      "lc3_streamer_register_stream should return -EINVAL on nullptr");
	zassert_equal(-1, streamer_idx, "lc3_streamer_register_stream should return index -1");

	ret = lc3_streamer_register_stream("test", NULL, false);
	zassert_equal(-EINVAL, ret,
		      "lc3_streamer_register_stream should return -EINVAL on nullptr");
}

ZTEST(lc3_streamer, test_lc3_streamer_register_stream_invalid_long_filename)
{
	int ret;
	ssize_t streamer_idx;
	...
	ret = lc3_streamer_register_stream(test_string, &streamer_idx, false);
	zassert_equal(-EINVAL, ret,
		      "lc3_streamer_register_stream should return -EINVAL on too long filename");
}

ZTEST(lc3_streamer, test_lc3_streamer_register_stream_invalid_too_many_streams)
{
	int ret;
	ssize_t streamer_idx;
	...
	ret = lc3_streamer_register_stream("test", &streamer_idx, false);
	zassert_equal(0, ret, "lc3_streamer_register_stream should return success");
	zassert_equal(2, streamer_idx, "lc3_streamer_register_stream should return index 2");

	int num_active = lc3_streamer_num_active_streams();

	zassert_equal(CONFIG_SD_CARD_LC3_STREAMER_MAX_NUM_STREAMS, num_active,
		      "lc3_streamer_num_active_streams should return 1");

	/* Fourth stream should fail */
	ret = lc3_streamer_register_stream("test", &streamer_idx, false);
	zassert_equal(-EAGAIN, ret, "lc3_streamer_register_stream should return success");
	zassert_equal(-1, streamer_idx, "lc3_streamer_register_stream should return index -1");
...
ZTEST(lc3_streamer, test_lc3_streamer_register_stream_invalid_file_open)
{
	int ret;
	ssize_t streamer_idx;
	...
	zassert_equal(-ENODEV, ret,
		      "lc3_streamer_register_stream should return -ENODEV from lc3_file");
	zassert_equal(-1, streamer_idx, "lc3_streamer_register_stream should return index -1");
...
ZTEST(lc3_streamer, test_lc3_streamer_register_stream_invalid_frame_get)
{
	int ret;
	ssize_t streamer_idx;
...
ZTEST(lc3_streamer, test_lc3_streamer_register_stream_invalid_no_frames)
{
	int ret;
	ssize_t streamer_idx;
	...
	ret = lc3_streamer_register_stream("test", &streamer_idx, false);
	zassert_equal(-ENODATA, ret, "lc3_streamer_register_stream should return -ENODATA");
	zassert_equal(-1, streamer_idx, "lc3_streamer_register_stream should return index -1");
	zassert_equal(1, lc3_file_frame_get_fake.call_count,
		      "lc3_file_frame_get should be called once");
	zassert_equal(1, lc3_file_open_fake.call_count, "lc3_file_open should be called once");
...
ZTEST(lc3_streamer, test_lc3_streamer_next_frame_get_valid)
{
	int ret;
	ssize_t streamer_idx;
	uint8_t *frame_buffer_1 = NULL;
	uint8_t *frame_buffer_2 = NULL;
	uint8_t *frame_buffer_3 = NULL;
...
ZTEST(lc3_streamer, test_lc3_streamer_next_frame_get_invalid_index)
{
	...
	ret = lc3_streamer_next_frame_get(-1, &frame_buffer);
...
ZTEST(lc3_streamer, test_lc3_streamer_next_frame_get_valid_loop_stream)
{
	...
	ret = lc3_streamer_register_stream(test_string, &streamer_idx, true);
	zassert_equal(0, ret, "lc3_streamer_register_stream should return success");
	zassert_equal(0, streamer_idx, "lc3_streamer_register_stream should return index 0");
	...
	/* Verify stream is looped by fething the next frame */
	ret = lc3_streamer_next_frame_get(streamer_idx, &frame_buffer_2);
	zassert_equal(0, ret, "lc3_streamer_next_frame_get should return success");
	zassert_mem_equal(fake_dataset1_valid, frame_buffer_2, fake_dataset1_valid_size,
			  "Frame data was not as expected");
...
ZTEST(lc3_streamer, test_lc3_streamer_next_frame_get_invalid_loop_stream_file_close_fail)
{
	...
	zassert_equal(-EFAULT, ret,
		      "lc3_streamer_next_frame_get should return -EINVAL as stream should not be "
		      "playing");
...
ZTEST(lc3_streamer, test_lc3_streamer_end_stream_valid)
{
	int ret;
	ssize_t streamer_idx;
	uint8_t *frame_buffer = NULL;
	...
	ret = lc3_streamer_end_stream(streamer_idx);
	zassert_equal(0, ret, "lc3_streamer_end_stream should return success");
	zassert_equal(1, lc3_file_close_fake.call_count, "lc3_file_close should be called once");
...
ZTEST(lc3_streamer, test_lc3_streamer_end_stream_invalid_index)
{
	int ret;

	ret = lc3_streamer_end_stream(-1);
	zassert_equal(-EINVAL, ret, "lc3_streamer_end_stream should return -EINVAL");

	ret = lc3_streamer_end_stream(CONFIG_SD_CARD_LC3_STREAMER_MAX_NUM_STREAMS);
	zassert_equal(-EINVAL, ret, "lc3_streamer_end_stream should return -EINVAL");
}

ZTEST_SUITE(lc3_streamer, NULL, NULL, test_setup, test_teardown, NULL);
