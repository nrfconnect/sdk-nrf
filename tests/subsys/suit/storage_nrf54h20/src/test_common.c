...
void assert_purged_reserved_uci_area_app(uint8_t *addr, size_t size)
{
	...
	uint8_t purged_uci[] = {
		/* Update Candidate Information (0xFF * 64) */
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
	...
	zassert_equal(size, sizeof(purged_reserved) + sizeof(purged_uci),
		      "Incorrect application (reserved + UCI) area size (0x%x != 0x%x)", size,
		      sizeof(purged_reserved) + sizeof(purged_uci));
	zassert_mem_equal(addr, purged_reserved, sizeof(purged_reserved),
			  "Reserved application area is not purged (addr: 0x%lx)", (uintptr_t)addr);
	zassert_mem_equal(addr + sizeof(purged_reserved), purged_uci, sizeof(purged_uci),
			  "Updtae candidate information is not purged (addr: 0x%lx)",
			  (uintptr_t)addr + sizeof(purged_reserved));
}

void assert_purged_mpi_area_app(uint8_t *addr, size_t size)
{
	/* clang-format off */
	uint8_t purged_mpi_app[] = {
		/* 0xFF * 240 */
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
	...
	zassert_equal(size, sizeof(purged_mpi_app),
		      "Incorrect application area size (0x%x != 0x%x)", size,
		      sizeof(purged_mpi_app));
	zassert_mem_equal(addr, purged_mpi_app, sizeof(purged_mpi_app),
			  "MPI application area is not purged (addr: 0x%lx)", (uintptr_t)addr);
}
...
void assert_purged_mpi_area_rad(uint8_t *addr, size_t size)
{
	...
	zassert_equal(size, sizeof(purged_mpi_rad), "Incorrect radio area size (0x%x != 0x%x)",
		      size, sizeof(purged_mpi_rad));
	zassert_mem_equal(addr, purged_mpi_rad, sizeof(purged_mpi_rad),
			  "MPI radio area is not purged (addr: 0x%lx)", (uintptr_t)addr);
}
