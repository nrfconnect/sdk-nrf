...
#if CONFIG_DFU_MULTI_IMAGE_SAVE_PROGRESS
	int err;

	err = settings_delete("dfu_multi/cbor_hdr");
	zassert_ok(err, "failed to delete settings (err=%d)", err);

	err = settings_delete("dfu_multi/img_fin");
	zassert_ok(err, "failed to delete settings (err=%d)", err);
