...
/** @brief The structure with a parameter set for a single test of ML-KEM algorithm variation. */
...
static psa_key_id_t ml_kem_derive_key_pair(const struct ml_kem_param_set *params)
{
	...
	zassert_equal(status, PSA_SUCCESS, "key derivation abort failed, got %d", status);
	status = psa_destroy_key(base_key);
	zassert_equal(status, PSA_SUCCESS, "derivation base key destroy failed, got %d", status);
			       ...
			       size_t ss_size, size_t *ss_len)
{
	...
	status = psa_destroy_key(ss_key);
	zassert_equal(status, PSA_SUCCESS, "shared secret destroy failed, got %d", status);
}
					      ...
					      const struct ml_kem_param_set *params)
{
	...
	status = psa_destroy_key(pub_key);
	zassert_equal(status, PSA_SUCCESS, "public key destroy failed, got %d", status);
}
...
ZTEST(psa_ml_kem_tests, test_ml_kem_768_imported_key)
{
	...
	zassert_equal(status, PSA_SUCCESS, "ML-KEM key pair destroy failed, got %d", status);
}
...
ZTEST(psa_ml_kem_tests, test_ml_kem_768_generated_key)
{
	...
	zassert_equal(status, PSA_SUCCESS,
		      "generated ML-KEM key pair destroy failed, got %d", status);
}
...
ZTEST(psa_ml_kem_tests, test_ml_kem_768_derived_key)
{
	...
	status = psa_destroy_key(key);
	zassert_equal(status, PSA_SUCCESS,
		      "derived ML-KEM key pair destroy failed, got %d", status);
}
...
ZTEST(psa_ml_kem_tests, test_ml_kem_768_kat_public_key)
{
	...
	for (size_t i = 0; i < params->kat_count; i++) {
		...
		status = psa_destroy_key(key);
		zassert_equal(status, PSA_SUCCESS,
			      "ML-KEM key pair destroy failed, got %d", status);
	}
}
