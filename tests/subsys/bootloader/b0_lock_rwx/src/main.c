...
#if defined(CONFIG_TEST_MCUBOOT_BOOTCONF_LOCK_WRITES)
	#define RRAMC_REGION_FOR_TEST CONFIG_TEST_IMMUTABLE_REGION
#else
	#define RRAMC_REGION_FOR_TEST CONFIG_TEST_ADDITIONAL_REGION
#endif
...
#if defined(CONFIG_TEST_MCUBOOT_ALONE)
	#define RRAMC_REGION_FOR_TEST_SIZE   PARTITION_SIZE(boot_partition)
	#define RRAM_REGION_FOR_TEST_ADDRESS PARTITION_OFFSET(boot_partition)
#if defined(CONFIG_TEST_MCUBOOT_BOOTCONF_LOCK_WRITES)
	#define RRAMC_REGION_FOR_TEST CONFIG_TEST_IMMUTABLE_REGION
#else
	#define RRAMC_REGION_FOR_TEST CONFIG_TEST_ADDITIONAL_REGION
#endif
#elif defined(CONFIG_TEST_B0_LOCK_USE_S1)
	#define RRAMC_REGION_FOR_TEST_SIZE   (PARTITION_SIZE(s1_partition) - RWX_SKIP_SIZE)
	#define RRAM_REGION_FOR_TEST_ADDRESS (PARTITION_OFFSET(s1_partition) + RWX_SKIP_SIZE)
	#define RRAMC_REGION_FOR_TEST CONFIG_TEST_ADDITIONAL_REGION
#elif defined(CONFIG_BOOTLOADER_MCUBOOT)
	#define RRAMC_REGION_FOR_TEST_SIZE   (PARTITION_SIZE(s0_partition) - RWX_SKIP_SIZE)
	#define RRAM_REGION_FOR_TEST_ADDRESS (PARTITION_OFFSET(s0_partition) + RWX_SKIP_SIZE)
	#define RRAMC_REGION_FOR_TEST CONFIG_TEST_ADDITIONAL_REGION
#else
	#define RRAMC_REGION_FOR_TEST_SIZE   PARTITION_SIZE(b0_partition)
	#define RRAM_REGION_FOR_TEST_ADDRESS PARTITION_OFFSET(b0_partition)
	#define RRAMC_REGION_FOR_TEST CONFIG_TEST_IMMUTABLE_REGION
#endif
#endif
