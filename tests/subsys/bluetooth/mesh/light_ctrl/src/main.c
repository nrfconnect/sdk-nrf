			      ...
			      bool publish)
{
	...
	/* status can not be tested as it is a dummy value for one call that can
	   be anything. */
...
static void expect_transition_start()
{
...
static void expect_turn_off_transition_start(void)
{
...
static void expect_turn_on_state_change(void)
{
	...
	expect_transition_start(expected_set);
...
static void expect_transition_complete(void)
{
	expected_msg[0] = 0;
	expected_onoff_status.present_on_off = false;
	expected_onoff_status.target_on_off = false;
...
{
	/* OFF state, the light ctrl server is disable after init */
	enum bt_mesh_light_ctrl_srv_state expected_state =
	...
	/* This may be an error. Flags should not be changed. */
	expected_flags = expected_flags | BIT(FLAG_MANUAL);
	expect_turn_off_transition_start();

	bt_mesh_light_ctrl_srv_off(&light_ctrl_srv);
	expected_statemachine_cond(true, expected_state, expected_flags);

	/* FADE_ON state, Light_On should change state */
	...
	/* FADE_STANDBY_MANUAL state, Light_Off should change state */
	...
	/* Publish when transition is completed. */
	expect_transition_complete();
}
