...
class WildcardString(str):
    def __eq__(self, other):
        return (isinstance(other, str) and len(self) == len(other) and
                all([a in ('?', b) for a, b in zip(self, other)]))
...
EXPECTED_METADATA = [
    {
        ...
        'encoded_metadata': WildcardString('0102030004000000??????01fba287050100')
    },
    {
        'sign_version': {'major': 1, 'minor': 2, 'revision': 3, 'build_number': 4},
        'binary_size': AnyInt(),
        ...
        'encoded_metadata': WildcardString('0102030004000000??????0143a1f1130200')
    }
]
    ...
    def list_diff(json, expected, path=''):
        if len(json) != len(expected):
            ...
            for i in range(len(json)):
                diff(json[i], expected[i], f"{path}[{i}]")
...
if __name__ == '__main__':
    ...
    if comp_data_layout == '--single':
        ...
        assert_metadata_equal(json_data, EXPECTED_METADATA[0])
    elif comp_data_layout == '--multiple':
        assert not isinstance(json_data, dict), "Expected multiple metadata but got one."
        assert len(json_data) == len(EXPECTED_METADATA), (f"Expected {len(EXPECTED_METADATA)} metadatas "
                                                          f"but got {len(json_data)}")
        for i, data in enumerate(json_data):
            assert_metadata_equal(data, EXPECTED_METADATA[i], f"Metadata[{i}] not equal to expected")
    else:
