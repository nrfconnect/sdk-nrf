						 ...
						 otIp6Address *address)
{
	...
	/* Must be null as iterator has not been allocated by callback before. */
	zassert_equal(iterator, NULL);
...
ZTEST(ot_rpc_meshdiag, test_otMeshDiagGetNextIp6Address)
{
	...
	mock_nrf_rpc_tr_receive(RPC_CMD(OT_RPC_CMD_MESH_DIAG_GET_NEXT_IP6_ADDRESS,
					RESOURCE_TABLE_KEY));
	mock_nrf_rpc_tr_expect_done();
...
ZTEST(ot_rpc_meshdiag, test_otMeshDiagGetNextChildInfo)
{
	...
	mock_nrf_rpc_tr_receive(RPC_CMD(OT_RPC_CMD_MESH_DIAG_GET_NEXT_CHILD_INFO,
					RESOURCE_TABLE_KEY));
	mock_nrf_rpc_tr_expect_done();
