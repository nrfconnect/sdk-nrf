...
/* Reverts to the baseline prefix set up by tether_net_fixture_setup(), without
 * the full DECT re-association tether_net_fixture_parent_restore() performs.
 */
int tether_net_fixture_revert_gua_prefix(struct tether_net_fixture *fx)
{
	return tether_assign_gua_prefix(fx->dect, tether_gua_prefix_a);
}

int tether_net_fixture_parent_restore(struct tether_net_fixture *fx)
{
	struct test_dect_association_result assoc = { 0 };
	int err;

	/* Re-associate with the same parent used in setup, no rescan needed:
	 * test_dect_association_request() issues NET_REQUEST_DECT_ASSOCIATION
	 * directly against the known long RD ID and simulates the modem's
	 * completion/joined callbacks.
	 */
	tether_dect_events_reset_flags();
	err = test_dect_association_request(fx->dect, TETHER_TEST_BEACON_LONG_RD_ID, NULL, true,
					    true, &assoc);
	...
	/* Back to the baseline prefix used by setup, so tests that run after
	 * a restore see the same state as a freshly set up suite.
	 */
	return tether_assign_gua_prefix(fx->dect, tether_gua_prefix_a);
}
