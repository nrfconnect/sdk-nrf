...
#if !defined(CONFIG_TEST_RTFW_PREINIT) && \
	!defined(CONFIG_TEST_RTFW_NO_EVENT_HANDLER)
...
#if !defined(CONFIG_TEST_RTFW_NO_EVENT_HANDLER)
...
ZTEST(rtfw_preinit, test_api_rejects_calls_before_initialization)
{
	...
	zassert_equal(rtfw_submit(&command), -EACCES);
	zassert_equal(rtfw_get_status(&status), -EACCES);
...
#elif defined(CONFIG_TEST_RTFW_NO_EVENT_HANDLER)
...
ZTEST(rtfw, test_public_command_contract)
{
	...
	zassert_equal(rtfw_submit(&too_large), -EMSGSIZE);
...
#endif /* CONFIG_TEST_RTFW_PREINIT */
