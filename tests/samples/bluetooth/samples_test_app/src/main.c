...
char *dev_names[] = {
	"Nordic_UART_Service", "Nordic_Throughput", "NCS_HIDS_keyboard",
	"NCS_HIDS_mouse",      "Nordic_LBS",	    "Test beacon",
};

char *hid_uuid = (char *)BT_UUID_HIDS;
	...
	if (conn == default_conn) {
		...
		if (err) {
			printk("Failed to start discovery process (err:%d)\n",
			       err);
			printk("FAILURE - Discovery start failed\n");
		}
	}
}
...
static void connected(struct bt_conn *conn, uint8_t conn_err)
{
	...
	if (conn_err) {
		...
		printk("FAILURE - Failed to connect\n");
		return;
	}
...
int main(void)
{
	...
	do {
		...
		err = k_sem_take(&my_sem, K_MSEC(2000));
		if (err) {
