...
This script does everything scripts/ci/check_license.py does for the "sdk-nrf" repository (it
checks the files changed by the pull request) and in addition when the pull request modifies the
west manifest (west.yml) it also checks the new files brought in by the modules:
  ...
  3. Fetches the affected repos with "west update".
  4. Collects the files to check:
       - updated project: only files added between the old and the new revision
       - added project: all files at the new revision
  5. Detects the licenses of all collected files with "west ncs-sbom" and checks them against the
     allow list.

This script is based on scripts/ci/check_license.py.
'''
...
def parse_args():
    ...
    parser.add_argument('-m', '--manifest', default='west.yml',
                        help='Path to the manifest file relative to the repository, '
                             'default is west.yml')
...
def classify_project_changes(old_items: set, new_items: set) -> 'tuple[set, set, set]':
    ...
    '''
    # Removed items (name not present in the new set).
    ritems = set(filter(lambda p: p[0] not in list(q[0] for q in new_items),
                        old_items - new_items))
    # Updated items (name present in the old set, but the tuple differs => revision changed).
    uitems = set(filter(lambda p: p[0] in list(q[0] for q in old_items),
                        new_items - old_items))
    # Added items (everything new that is not an update).
...
class PatchLicenseChecker:
    ...
    def report(self, label: str, message: str, file_name: 'str|Path' = '<none>', license: str = ''):
        ...
        if not self.args.github:
            ...
            if label in ('error', 'warning'):
                if file_name != '<none>':
                    # Files of the modules live in other repositories than the pull request, so
                    # their workspace-relative path is reported as-is.
                    try:
    ...
    def commit_range(self) -> 'tuple[str, str]':
        '''Split the "--commits" range "base..[head]" into the base and head revisions.'''
    ...
    def load_projects(self, manifest_data: str) -> 'dict':
        '''
        Parse a manifest and return a dict mapping project name to project object, keeping only
        active projects hosted under the "nrfconnect" GitHub organization.
        '''
    ...
    def ensure_revision(self, project_dir: Path, revision: str) -> bool:
        '''Make sure a revision is available locally, fetching it from origin if necessary.'''
        ...
        return self.try_run('git', 'cat-file', '-e', f'{revision}^{{commit}}',
                            cwd=project_dir).returncode == 0
    ...
    def prefix(self, project_path: str, git_output: str) -> 'list[str]':
        ...
        return [f'{project_path}/{line.strip()}'
                for line in git_output.splitlines() if line.strip()]
    ...
    def added_files(self, project, old_rev: str, new_rev: str) -> 'list[str]':
        ...
        if not (self.ensure_revision(project_dir, old_rev)
                and self.ensure_revision(project_dir, new_rev)):
            print(f'WARNING: Cannot resolve revisions of project "{project.name}" '
                  f'({old_rev}..{new_rev}); skipping.')
            return []
        out = self.run('git', 'diff', '--name-only', '--diff-filter=A',
                       f'{old_rev}..{new_rev}', cwd=project_dir)
    ...
    def all_files(self, project, new_rev: str) -> 'list[str]':
        ...
        if not self.ensure_revision(project_dir, new_rev):
            print(f'WARNING: Cannot resolve revision {new_rev} of project "{project.name}"; '
                  'skipping.')
            return []
    ...
    def collect_manifest_files(self) -> 'list[Path]':
        '''
        If the manifest changed in the patch, collect the new files of the updated and added
        "nrfconnect" projects. Returns an empty list when the manifest was not modified.
        '''
        ...
        # Fetch only the projects that changed.
        names = sorted({n for n, _ in updated} | {n for n, _ in added})
        print(f'Updating projects: {names}')
        self.try_run('west', 'update', '--narrow', *names, cwd=self.west_workspace)
        ...
        for name, _ in sorted(updated):
            ...
            print(f'Updated project "{name}": collecting files added in {old_rev[:12]}..'
                  f'{new_rev[:12]}')
