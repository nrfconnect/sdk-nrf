...
NONE_LICENSE_WARNING = (
    'Missing license information is allowed for this file, but it is recommended to add one.'
)
ANY_LICENSE_WARNING = (
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Check for allowed licenses.', allow_abbrev=False)
    parser.add_argument(
        '-c',
        '--commits',
        default='HEAD~1..',
    ...
    parser.add_argument(
        '--github', action='store_true', help='Add GitHub Actions Workflow commands to the stdout.'
    )
...
class PatchLicenseChecker:
    ...
    def run(self, program: str, *args: 'list[str|Path]', cwd=None) -> str:
        ...
        try:
            process = subprocess.Popen(
                run_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=cwd
            )
        except OSError as e:
    ...
    def report(self, label: str, message: str, file_name: 'str|Path' = '<none>', license: str = ''):
        ...
        if not self.args.github:
            ...
            if label in ('error', 'warning'):
                if file_name != '<none>':
                    ...
                    file_path = file_name
                print(
                    f'::{label} file={file_path},title=License Problem::'
        ...
        test_case = junit_xml.TestCase(
            file_name + (f':{license}' if license else ''), 'LicenseCheck'
        )
        if label == 'error':
    ...
    def generate_list_of_files(self) -> 'list[Path]':
        ...
        files = [
            (self.git_top / f.strip()).relative_to(self.west_workspace) for f in files if f.strip()
        ]
        return files
    ...
    def detect_licenses(self, files: 'list[Path]') -> 'list[dict]':
        ...
        try:
            self.run(
                ...
                tmp_list,
                '--license-detectors',
                'spdx-tag,full-text,external-file',
    ...
    def show_results(self, detected: 'list[dict]') -> bool:
        ...
        if self.total_errors > 0:
            print(
                f'License check failed with {self.total_errors} error(s) '
