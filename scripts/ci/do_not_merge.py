...
# OR: If any of those is set, DNM
DNM_LABELS = ["DNM", "DNM (manifest)", "TSC", "Architecture Review", "dev-review"]
# XNOR: If neither or both of the pair are set, DNM
DNM_LABEL_PAIRS = [("changelog", "no-changelog")]
...
def parse_args(argv):
    ...
    parser.add_argument("-o", "--org", default="nrfconnect", help="Github organization")
    parser.add_argument("-r", "--repo", default="sdk-nrf", help="Github repository")

    return parser.parse_args(argv)


WAIT_FOR_WORKFLOWS = set({"Manifest", "Pull Request Labeler"})
WAIT_FOR_DELAY_S = 60
...
def main(argv):
    ...
    ln_set = set(label_names)

    for name in label_names:
        print(f"label: {name}")

        if name in DNM_LABELS or name.startswith("block:"):
            msg = f"Pull request has label \"{name}\""

    for pair in DNM_LABEL_PAIRS:
        p = set(pair)
        d = p - ln_set
        if d == p:
            msg = f"No label from pair \"{pair}\" found in labels"
        elif len(d) == 0:
            msg = f"Both labels from pair \"{pair}\" found in labels"

    if msg:
        print(f"\nDNM conditions detected: {msg}")
        print("This workflow will fail so that the pull request cannot be merged")
        sys.exit(1)
