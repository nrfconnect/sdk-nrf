...
class PsaKeyUsage(IntEnum):
    ...
    VERIFY_MESSAGE_VERIFY_HASH_EXPORT = 0x2801
...
class PlatformKeyAttributes:
    def __init__(
        ...
        # TODO: Check key identifier is in range. Look at slots in docs.

        if location == PsaKeyLocation.LOCATION_CRACEN_KMU:
            if cracen_usage == PsaCracenUsageScheme.NONE:
                raise RuntimeError("--cracen-usage must be set to other than NONE if location is LOCATION_CRACEN_KMU")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--key-bits",
        help="Key size in bits. Note that ECC secp256r1 public keys must be set to 256 bits here, also if the key is in uncompressed 65 bytes format.",
        type=lambda number_string: int(number_string, 0),
        required=True,
    )
