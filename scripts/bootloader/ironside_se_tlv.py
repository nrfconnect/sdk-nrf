...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Binary file to write: for each ELF, 4-byte address + 4-byte size (little-endian).",
    )
        ...
        if args.periphconf_tlv_out:
            ...
            if size < 0:
                parser.error(
                    ...
                    f"{PERIPHCONF_START_SYMBOL}=0x{addr:09_x} > {PERIPHCONF_END_SYMBOL}=0x{end:09_x}"
                )
            ...
            count = size // 8
            periphconf_tlv_params.append((addr, count, fp.name))

    if args.periphconf_tlv_out:
        print("PERIPHCONF TLV contents:")
        for i, (addr, count, filename) in enumerate(periphconf_tlv_params):
            print(f"  [{i}] 0x{addr:09_x}, {count} entries (from {filename})")
            args.periphconf_tlv_out.write(addr.to_bytes(4, byteorder="little"))
