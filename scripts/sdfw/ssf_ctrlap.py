...
class Ssf:
    ...
    def request(self, service_request: bytes) -> Any:
        ...
        num_align_bytes = (4 - (len(full_request) % 4)) % 4
        full_request_aligned = full_request + bytes(num_align_bytes)

        req = sdfw_adac_cmd.Ssf(full_request_aligned).to_request()
        ...
        with io.BytesIO(rsp.data) as data_io:
            decoder = cbor2.CBORDecoder(data_io)
            ssf_response = decoder.decode()[0]
