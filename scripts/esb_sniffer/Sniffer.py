...
class Sniffer():
    def __init__(self, packet_len=45, rtt_channels={"data_down":1, "comm_down":2, "comm_up":1}, read_at_once=2000, swd_freq_khz=8000, log_lvl=logging.INFO):
        self.jlink = None
        self.buff = None
        self.remaing_data = None
    ...
    def __get_packet_length(self):
        ...
        try:
            ...
            self.logger.error(f"Failed to get packet length from sniffer: err = {err.err_code}, using default {self.packet_len}")

        if len(ret) == 4:
            length = int.from_bytes(ret, byteorder='big', signed=False)
        else:
            self.logger.error(f"Failed to get packet length from sniffer, using default {self.packet_len}")
            return -1
    ...
    def connect(self):
        ...
        try:
            ...
            self.logger.error(f"Failed to connect to device: err = {err.err_code}")
            self.jlink = None
            return -1
    ...
    def __gen_pcap_packets(self, buff):
        ...
        if self.remaing_data is not None:
            buff = self.remaing_data + buff
            self.remaing_data = None

        if len(buff) % self.packet_len != 0:
            start = int(len(buff)/self.packet_len) * self.packet_len
            self.remaing_data = buff[start:]
    ...
    def __send_command(self, command):
        ...
        try:
            ...
            self.logger.error(f"Jlink error while sending command, {err.err_code}")
            return -1
    ...
    def read(self):
        ...
        try:
            self.buff = self.jlink.rtt_read(self.rtt_ch["data_down"], self.read_at_once*self.packet_len, encoding=None)
        except APIError as err:
            self.logger.error(f"JLink connection error while reading packets: err = {err.err_code}")
            raise IOError

        packets = self.__gen_pcap_packets(self.buff)
