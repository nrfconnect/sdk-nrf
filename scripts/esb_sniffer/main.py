...
class Process():
    def __init__(self, name, target, daemon=True):
        self.name = name
...
class PipeComm(Process):
    ...
    def __init__(self, data_event, dev):
...
class Watchdog(Process):
    ...
    def stop_all(self):
        ...
        for i in self.watching_threads:
            if i.thread.is_alive():
                self.logger.debug(f"Thread: {i.name} stopped")
                i.stop_evt.set()
                i.thread.join(1)
    ...
    def start_all(self):
        ...
        for i in self.watching_threads:
            self.logger.debug(f"Thread: {i.name} started")
            i.start()
            ...
            for i in self.watching_threads:
                if i.thread.is_alive() is False:
                    if i.exit_exception is not None:
                        self.logger.info(f"Thread: {i.name} is down, reason: {i.exit_exception}")
...
def main():
    ...
    if com_path is not None:
        try:
            com = serial.Serial(com_path, 115200)
            use_shell = True
