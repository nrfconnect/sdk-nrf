    ...
    node (AGENT_LABELS) {
      stage('.'){
        docker.image("$DOCKER_REG/$IMAGE_TAG").inside {
          ...
          if (compiler == 'gnuarmemb') {
            println "entering try/catch for gnuarmemb"
            try {
            ...
            println "No error handling for zephyr compiler"
            sh FULL_SANITYCHECK_CMD
          }
        }
        cleanWs()
...
pipeline {
  ...
  stages {
    ...
    stage('Specification') { steps { script {
      ...
      // PLATFORM_LIST = [ PLATFORM_LIST[0] ]  // Only use first platform for pipeline testing

      def COMPILER_LIST = ['zephyr','gnuarmemb']
