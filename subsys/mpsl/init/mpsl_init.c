...
static int32_t mpsl_lib_init_internal(void)
{
...
#else
	clock_cfg.accuracy_ppm = DT_ENUM_IDX(CLOCK_NODE_LFCLK, k32src_accuracy_ppm);
#endif /* CONFIG_CLOCK_CONTROL_NRF */
	...
	clock_cfg.rc_ctiv = (CONFIG_MPSL_CALIBRATION_PERIOD * 4 / 1000);
#if defined(CONFIG_CLOCK_CONTROL_NRFX_CALIBRATION_MAX_SKIP)
	clock_cfg.rc_temp_ctiv = CONFIG_CLOCK_CONTROL_NRFX_CALIBRATION_MAX_SKIP + 1;
	BUILD_ASSERT(CONFIG_CLOCK_CONTROL_NRFX_CALIBRATION_TEMP_DIFF == 2,
		     "MPSL always uses a temperature diff threshold of 0.5 degrees");
#else
