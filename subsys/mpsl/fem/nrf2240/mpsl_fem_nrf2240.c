...
#if defined(CONFIG_MPSL_FEM_NRF2240)
...
#error "The nRF2240 driver must be used with CONFIG_PINCTRL! Set CONFIG_PINCTRL=y"
#endif

#if defined(CONFIG_EARLY_CONSOLE)
#error "The nRF2240 driver cannot be used with CONFIG_EARLY_CONSOLE! \
...
#define FEM_OUTPUT_POWER_ALT_DBM DT_PROP(DT_NODELABEL(nrf_radio_fem), output_power_alt_dbm)
...
#define NRF2240_OUTPUT_POWER_DBM_MIN 7
#define NRF2240_OUTPUT_POWER_DBM_MAX 21

BUILD_ASSERT(
	(FEM_OUTPUT_POWER_DBM >= NRF2240_OUTPUT_POWER_DBM_MIN) &&
	(FEM_OUTPUT_POWER_DBM <= NRF2240_OUTPUT_POWER_DBM_MAX),
	"Value of output-power-dbm property out of range.");

#if DT_NODE_HAS_PROP(DT_NODELABEL(nrf_radio_fem), pwrmd_gpios)
BUILD_ASSERT(
	(FEM_OUTPUT_POWER_ALT_DBM >= NRF2240_OUTPUT_POWER_DBM_MIN) &&
	(FEM_OUTPUT_POWER_ALT_DBM <= NRF2240_OUTPUT_POWER_DBM_MAX),
	"Value of output-power-alt-dbm property out of range.");
#endif /* DT_NODE_HAS_PROP(DT_NODELABEL(nrf_radio_fem), pwrmd_gpios) */

#if DT_NODE_HAS_PROP(DT_NODELABEL(nrf_radio_fem), twi_if)
#if DT_NODE_HAS_PROP(MPSL_FEM_TWI_IF, init_regs)
static void fem_nrf2240_twi_init_regs_configure(mpsl_fem_nrf2240_interface_config_t *cfg)
{
...
static int fem_nrf2240_configure(void)
{
	...
	mpsl_fem_nrf2240_interface_config_t cfg = {
...
#if DT_NODE_HAS_PROP(DT_NODELABEL(nrf_radio_fem), pwrmd_gpios)
			.output_power_alt_dbm = FEM_OUTPUT_POWER_ALT_DBM,
#else
			.output_power_alt_dbm = FEM_OUTPUT_POWER_DBM,
#endif
		...
		.pwrmd_pin_config = {
			.gpio_pin      = {
				.p_port   = MPSL_FEM_GPIO_PORT_REG(pwrmd_gpios),
				.port_no  = MPSL_FEM_GPIO_PORT_NO(pwrmd_gpios),
				.port_pin = MPSL_FEM_GPIO_PIN_NO(pwrmd_gpios),
			},
			.enable        = true,
			.active_high   = MPSL_FEM_GPIO_POLARITY_GET(pwrmd_gpios),
		}
#endif
	};
	...
	BUILD_ASSERT(ARRAY_SIZE(fem_twi_init_regs) % 2 == 0,
		"The number of specified nRF2240 TWI initialization registers to write differs from"
	...
	fem_nrf2240_twi_init_regs_configure(&cfg);
#endif /* DT_NODE_HAS_PROP(MPSL_FEM_TWI_IF, init_regs) */
