 ...
 * The uninitialization may be started outside of it thgourh public API.
 * To aovid interfering with other MPSL work items we need dedicated work
...
void mpsl_pm_utils_work_handler(void)
{
	...
	if (pm_state == MPSL_PM_INITIALIZED) {
		 ...
		 * There is not work queue when runnign UTs, so the mpsl_pm_utils_work_handler()
...
int32_t mpsl_pm_utils_uninit(void)
{
	...
	/* In case there is any pended MPSL work item that was not handled, we use dedicated
	 * work item to remove PM policy event and unregister latency request.
	 */
