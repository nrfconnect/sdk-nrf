...
static int32_t m_lfclk_wait(void)
{
	 ...
	 * for former requests. Due to NCSDK-31169, we allow for such such case,
	...
	if (err == -NRF_ETIMEDOUT) {
		 ...
		 * thakes priority in handling of future request and release calls until
...
#if defined(CONFIG_MPSL_EXT_CLK_CTRL_LFCLK_REQ_TIMEOUT_ALLOW)
	/* Do not attempt request LFCLK again if there was a timeout reposne
	 * for past reques. Due to NCSDK-31169, we allow for such such case
	 * assuming there is a LFCLK source selected by sysctrl with accuracy
	 * that meets radio protocols requirements.
	 */
	if (atomic_get(&m_lfclk_req_timeout) == (atomic_val_t) true) {
...
static int32_t m_lfclk_release(void)
{
	 ...
	 * Due to NCSDK-31169, we allow for such such case assuming there is
