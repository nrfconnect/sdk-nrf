...
/* Miminum allowed HFXO startup time in microseconds. Use conservative value that comes from BICR.
 *
 * The time is used by MPLS to request the clock before radio event start. If the time is too short
 ...
 */
#define MPSL_HFCLK_MINIMUM_ALLOWED_STARTUP_TIME_US 850
...
int32_t mpsl_clock_ctrl_init(void)
{
	...
	else if (startup_time_us < MPSL_HFCLK_MINIMUM_ALLOWED_STARTUP_TIME_US) {
		/* Override the startup time to the minimum allowed value. */
		startup_time_us = MPSL_HFCLK_MINIMUM_ALLOWED_STARTUP_TIME_US;
