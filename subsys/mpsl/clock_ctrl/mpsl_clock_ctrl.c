...
#define NVM_CLOCK_FREQUENCIES_SIZE DT_PROP_LEN(NVM_CLOCK_DT_LABEL, supported_clock_frequencies)

#define NVM_CLOCK_FREQUENCIES_IDX(idx) \
	DT_PROP_BY_IDX(NVM_CLOCK_DT_LABEL, supported_clock_frequencies, idx)

/* Make sure there is four entries in DTS so that we read the last one when retrieving frequency */
BUILD_ASSERT(NVM_CLOCK_FREQUENCIES_SIZE == 4);

static const struct nrf_clock_spec m_nvm_clk_spec = {
	.frequency = NVM_CLOCK_FREQUENCIES_IDX(3),
};
...
static int32_t m_clock_request_wait(struct clock_onoff_state *clock_state, uint32_t timeout_ms)
{
	...
	if (IS_ENABLED(CONFIG_CLOCK_CONTROL_NRF2) && clock_state->clk_req_rsp == -ETIMEDOUT) {
		 ...
		 */
		LOG_WRN("LFCLK could not be started: %d", clock_state->clk_req_rsp);
		return 0;
	} else if (clock_state->clk_req_rsp < 0) {
		__ASSERT(false, "LFCLK could not be started, reason: %d", clock_state->clk_req_rsp);
...
{
	return m_clock_request_wait(&m_lfclk_state,
...
static int32_t m_nvm_clock_request(void)
{
	...
	/* In the MRAM DTS node there is no:
	 * - clock that we can use
	 * - power-domain that the memory is instantiated in, that could give us the clock.
	 *
	 * We must refer to the clock explicityl. It makes the code less portable.
	 */
