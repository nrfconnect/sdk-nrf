...
#else
	err = nrf_clock_control_request(DEVICE_DT_GET_ONE(nordic_nrfx_clock_lfclk), NULL,
					&m_lfclk_state.cli);
#endif
	if (err < 0) {
		return err;
...
#else
	/* In case there is other ongoing request, cancel it. */
	err = nrf_clock_control_cancel_or_release(DEVICE_DT_GET_ONE(nordic_nrfx_clock_lfclk), NULL,
						&m_lfclk_state.cli);
#endif
	if (err < 0) {
		return err;
...
#if defined(CONFIG_CLOCK_CONCONFIG_CLOCK_CONTROL_NRF_COMMONTROL_NRFS)
