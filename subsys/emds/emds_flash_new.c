...
static void cand_put(sys_slist_t *cand_list, off_t cand_addr, struct emds_snapshot_metadata *cand)
{
	...
	LOG_DBG("Found snapshot at offset 0x%04lx with fresh_cnt %u. Metadata address: 0x%04lx",
		cand->data_instance_addr, cand->fresh_cnt, cand_addr);
			      ...
			      struct emds_snapshot_candidate *candidate)
{
	...
	do {
		...
		cand_put(&cand_list, fa->fa_off + read_addr, &cache);
		...
		if (cand_snapshot_crc_check(partition, &cand->metadata)) {
			*candidate = *cand;
			LOG_DBG("Found valid snapshot at address 0x%04lx with length %u with "
				"fresh_cnt %u",
				fa->fa_off + cand->metadata.data_instance_addr,
				 ...
				 struct emds_snapshot_candidate *allocated_snapshot)
{
	...
	off_t data_addr = freshest_snapshot
				  ? ROUND_UP(freshest_snapshot->metadata.data_instance_addr +
						     freshest_snapshot->metadata.data_instance_len,
					     fp->write_block_size)
				  : 0;
	size_t data_size = emds_store_size_get();
	int rc;

	metadata_addr -= sizeof(struct emds_snapshot_metadata);
	...
	if (flash_params_get_erase_cap(partition->fp) & FLASH_ERASE_C_EXPLICIT) {
		...
		rc = flash_area_read(fa, metadata_addr, cache,
		 ...
		 * erase partition.
		 */
		rc = flash_area_read(fa, data_addr, cache, sizeof(struct emds_data_entry));
		if (rc) {
	...
	if (REGIONS_OVERLAP(metadata_addr, sizeof(struct emds_snapshot_metadata), data_addr,
			    data_size)) {
	...
	allocated_snapshot->metadata.data_instance_addr = data_addr;
	allocated_snapshot->metadata.data_instance_len = data_size;
	allocated_snapshot->metadata.metadata_crc =
		crc32_k_4_2_update(0, (const unsigned char *)&allocated_snapshot,
				   offsetof(struct emds_snapshot_metadata, metadata_crc));

	LOG_DBG("Allocating snapshot at address 0x%04lx with length %u and fresh_cnt %u",
		fa->fa_off + data_addr, data_size, allocated_snapshot->metadata.fresh_cnt);
	LOG_DBG("Metadata address: 0x%04lx, crc: 0x%08x", fa->fa_off + metadata_addr,
		allocated_snapshot->metadata.metadata_crc);
