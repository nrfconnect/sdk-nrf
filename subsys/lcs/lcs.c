...
{
	switch (current) {
	case LCS_PSA_PROT_DEBUG:
		/* fall-through */
	case LCS_PSA_NON_PROT_DEBUG:
		/* fall-through */
	case LCS_UNKNOWN:
...
enum lcs lcs_get(void)
{
	...
	if ((provisioning_value == LCS_MEM_VALUE_NOT_ENTERED) &&
	    ...
	    (decommissioned_value == LCS_MEM_VALUE_NOT_ENTERED)) {
	...
	if (decommissioned_value != LCS_MEM_VALUE_NOT_ENTERED) {
