		       ...
		       uint32_t *raw_data_len)
{
	...
	err = nfc_ndef_msg_parser_memo_resolve(result_buf, result_buf_len, &parser_memory_helper);

	if (err != 0) {
		return err;
	}

	err = nfc_ndef_msg_parser_internal(&parser_memory_helper, raw_data, raw_data_len);
...
void nfc_ndef_msg_printout(const struct nfc_ndef_msg_desc *msg_desc)
{
	...
	LOG_INF("NDEF message contains %d record(s)", msg_desc->record_count);
