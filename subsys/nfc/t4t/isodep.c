...
#define T4T_ATS_TA_EQUAL_DEVIDER BIT(7)
#define T4T_ATS_T0_FSCI_MASK 0x0F
#define T4T_ATS_TA_DEVIDER_PL 0x07
#define T4T_ATS_TA_DEVIDER_PL_OFFSET 1
#define T4T_ATS_TA_DEVIDER_LP_OFFSET 3
#define T4T_ATS_TA_DEVIDER_LP 0x70
...
static bool did_include(u8_t *data)
{
	if (t4t_isodep.tag.did_supported && (t4t_isodep.tag.did != 0)) {
		*data |= I_BLOCK_DID_BIT;
		data++;
		*data = t4t_isodep.tag.did;
...
static int ats_parse(const u8_t *data, size_t len)
{
	...
	if (t0 & T4T_ATS_T0_TA_PRESENT) {
		...
		if (ta & T4T_ATS_TA_EQUAL_DEVIDER) {
			t4t_isodep.equal_divisor = true;
		}

		t4t_isodep.tag.pl_divisor =
			(ta & T4T_ATS_TA_DEVIDER_PL) << T4T_ATS_TA_DEVIDER_PL_OFFSET;
		t4t_isodep.tag.lp_divisor =
			(ta & T4T_ATS_TA_DEVIDER_LP) >> T4T_ATS_TA_DEVIDER_LP_OFFSET;
...
static void isodep_chunk_send(void)
{
	...
	if (t4t_isodep_cb->data_send) {
		t4t_isodep_cb->data_send(t4t_isodep.tx_data.data,
...
static void isodep_r_frame_send(bool ack)
{
	...
	if (t4t_isodep_cb->data_send) {
		t4t_isodep_cb->data_send(tx_data, data_len, fdt);
	}
}
...
static int isodep_s_frame_handle(const u8_t *data, size_t len)
{
	...
	if (t4t_isodep_cb->data_send) {
		t4t_isodep_cb->data_send(tx_data, t4t_isodep.tx_data.len, fdt);
	}
...
{
	u8_t param;
	u32_t state = atomic_get(&t4t_isodep.state);

	if ((state == ISODEP_STATE_UNINITIALIZED) ||
	    (state == ISODEP_STATE_TRANSFER)) {
		return -EACCES;
	}

	atomic_set(&t4t_isodep.state, ISODEP_STATE_TRANSFER);
