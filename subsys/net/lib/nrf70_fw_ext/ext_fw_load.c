...
#define HFCLK_MAX_FREQUENCY MHZ(128)
#define HFCLK_MIN_FREQUENCY MHZ(64)
#endif
...
static void enable_xip_and_set_cpu_freq(void)
{
	...
	if (saved_frequency == HFCLK_MIN_FREQUENCY) {
		...
		clock_control_set_rate(DEVICE_DT_GET_ONE(nordic_nrf_clock_hfclk), NULL,
				       (clock_control_subsys_rate_t)HFCLK_MIN_FREQUENCY);
...
static void disable_xip_and_restore_cpu_freq(void)
{
	...
	if (current_frequency != saved_frequency) {
		clock_control_set_rate(DEVICE_DT_GET_ONE(nordic_nrf_clock_hfclk), NULL,
				       (clock_control_subsys_rate_t)saved_frequency);
