...
static int wpa3_psa_setup_operation(struct wpa3_psa_operation *op, const u8 *addr1, const u8 *addr2,
				    const u8 *password, size_t password_len, const u8 *ssid,
				    size_t ssid_len, int group, bool h2e, bool gdh);
...
static int wpa3_psa_calculate_context(const u8 *local_scalar, const u8 *peer_scalar, u8 *context);
		       ...
		       struct sae_data *sae)
{
	...
	res = wpa3_psa_setup_operation(op, addr1, addr2, password, password_len, op->ssid,
				       op->ssid_len, op->group, op->h2e_enabled, op->gdh_enabled);
	if (res < 0) {
			  ...
			  const u8 *addr2, int *rejected_groups, const struct sae_pk *pk)
{
	...
	int res =
		wpa3_psa_setup_operation(op, addr1, addr2, op->password, op->password_len, op->ssid,
					 op->ssid_len, op->group, op->h2e_enabled, op->gdh_enabled);
	if (res < 0) {
...
size_t sae_ecc_prime_len_2_hash_len(size_t prime_len)
{
	...
	if (prime_len == 32) { /* Group 19 */
		return 32;     /* SHA-256 */
	}
	/* Unsupported groups return default */
				    ...
				    size_t ssid_len, int group, bool h2e, bool gdh)
{
	...
	wpa_printf(MSG_DEBUG, "WPA3-PSA: Group: %d, H2E: %s, GDH: %s", group, h2e ? "yes" : "no",
		   gdh ? "yes" : "no");
	...
	os_memcpy(op->own_addr, addr1, ETH_ALEN);
	os_memcpy(op->peer_addr, addr2, ETH_ALEN);
	os_memcpy(op->ssid, ssid, ssid_len);
	op->ssid_len = ssid_len;
	os_memcpy(op->password, password, password_len);
	op->password_len = password_len;
	op->group = group;
	op->h2e_enabled = h2e;
	...
	psa_set_key_bits(&attributes, password_len * 8);

	/* Set key algorithm for WPA3-SAE */
	psa_set_key_algorithm(&attributes, alg);

	/* Import password as key */
	status = psa_import_key(&attributes, password, password_len, &op->password_key);
...
static int wpa3_psa_derive_keys(struct wpa3_psa_operation *op)
{
	...
	uint8_t local_scalar[32];
	uint8_t peer_scalar[32];
	uint8_t context[32];
	size_t pmk_len;
	...
	/* Extract commit scalars for PMKID calculation */

	/* IEEE 802.11: PMKID = first 128 bits of (scalar1 + scalar2) mod r */
	if (op->local_commit_len != 98 || op->peer_commit_len != 98) {
		wpa_printf(MSG_ERROR, "WPA3-PSA: Commit messages wrong size");
		goto cleanup;
	}
	...
	os_memcpy(op->pmkid, context, SAE_PMKID_LEN);
...
/**
 * wpa3_psa_calculate_context - Calculate SAE context for PMKID
 * @local_scalar: Local commit scalar (32 bytes, big-endian)
 * @peer_scalar: Peer commit scalar (32 bytes, big-endian)
 * @context: Output buffer for context (32 bytes)
 *
 * Calculates context = (local_scalar + peer_scalar) mod r where r is the
 * NIST P-256 curve order. Used only for PMKID generation per IEEE 802.11.
 *
 * Returns: 0 on success, -1 on failure
 */
static int wpa3_psa_calculate_context(const u8 *local_scalar, const u8 *peer_scalar, u8 *context)
{
	/* NIST P-256 order r in big-endian format */
	static const uint8_t r[32] = {0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x01,
	...
	/* Add scalars with carry propagation (LSB to MSB) */
	carry = 0;
	for (i = 31; i >= 0; i--) {
		sum = local_scalar[i] + peer_scalar[i] + carry;
		context[i] = sum & 0xFF;
		carry = sum >> 8;
	}

	/* Handle overflow: if carry, add (2^256 - r) then reduce */
	if (carry) {
		static const uint8_t two_256_minus_r[32] = {
			0x01, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFE, 0xFF, 0xFF, 0xFF,
			0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01};

		carry2 = 0;
		for (i = 31; i >= 0; i--) {
			sum = context[i] + two_256_minus_r[i] + carry2;
			context[i] = sum & 0xFF;
			carry2 = sum >> 8;
		}

		/* Check if result >= r and reduce if needed */
		need_reduction = (carry2 != 0);
		if (!need_reduction) {
			for (i = 0; i < 32; i++) {
				if (context[i] > r[i]) {
					need_reduction = 1;
					break;
				} else if (context[i] < r[i]) {
					break;
				}
			}
			/* If equal to r, reduce to 0 */
			if (!need_reduction) {
				all_equal = 1;
				for (i = 0; i < 32; i++) {
					if (context[i] != r[i]) {
