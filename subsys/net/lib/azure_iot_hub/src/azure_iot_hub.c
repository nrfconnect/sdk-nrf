...
STATIC void iot_hub_state_set(enum iot_hub_state new_state)
{
	...
	if (notify_error) {
		...
		LOG_ERR("Invalid connection state transition, %s --> %s",
			log_strdup(state_name_get(iot_hub_state)),
				  ...
				  struct azure_iot_hub_buf payload)
{
	...
	LOG_DBG("Direct method name: %.*s",
		evt.data.method.name.size,
		log_strdup(evt.data.method.name.ptr));

	LOG_DBG("Direct method request ID: %.*s",
		evt.data.method.request_id.size,
		log_strdup(evt.data.method.request_id.ptr));
...
STATIC void on_connack(enum mqtt_conn_return_code return_code)
{
	...
	// TODO: Consider if the state should change here or on suback.
	//	 Possibly another state is needed to indicate subscribed
	//	 to all topics.
...
{
	size_t len = snprintk(buf, sizeof(buf_size), "%d", k_uptime_get_32());
...
int azure_iot_hub_send(const struct azure_iot_hub_msg *const msg)
{
	...
	case AZURE_IOT_HUB_TOPIC_EVENT: {
		...
		// TODO: Properties are already allocated by the caller, possible to use that buffer
		//	 instead?
...
int azure_iot_hub_method_respond(struct azure_iot_hub_result *result)
{
	...
	LOG_DBG("Publishing to topic: %.*s", len, log_strdup(param.message.topic.topic.utf8));
