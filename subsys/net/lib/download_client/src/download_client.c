...
static size_t recv_block(struct download_client *dl)
{
	int err, timeout = 0;
...
static int send_block_request(struct download_client *dl)
{
	int err, timeout = 0;
	...
	case IPPROTO_DTLS_1_2:
		if (IS_ENABLED(CONFIG_COAP)) {
			timeout = coap_get_snd_timeout(dl);
			break;
		}
	default:
		LOG_ERR("unhandled proto");
	...
	err = set_snd_socket_timeout(dl->fd, timeout);
	if (err) {
		return -1;
	}

	return request_send(dl);
}
