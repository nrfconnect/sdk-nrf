...
/* This file is only built for CONFIG_DECT_TETHER_IPV6_RA, which depends on
 * NET_L2_ETHERNET, so it is always available here.
 */
...
/* Output buffer descriptor for ra_build_minimal(). */
struct ra_build_out {
	uint8_t *buf;
	size_t cap;
};

/* Router/RDNSS lifetime and flag inputs for ra_build_minimal(), computed
 * together by the caller from the same uplink/forced-lifetime decision.
 */
struct ra_build_params {
	uint16_t router_lifetime;
	uint32_t rdnss_lifetime;
	uint8_t flags;
};

/* PIO (Prefix Information Option) inputs for ra_build_minimal(). */
struct ra_build_pio {
	const struct net_in6_addr *pfx;
	uint8_t cnt;
	uint32_t valid_lt;
...
static void bh_ra_rdnss_cfg_load(void)
{
	...
	if (sizeof(CONFIG_DECT_TETHER_IPV6_RA_RDNSS_ADDR2) > 1U &&
		...
		if (net_addr_pton(AF_INET6, CONFIG_DECT_TETHER_IPV6_RA_RDNSS_ADDR2,
			...
			/* ADDR2 duplicates ADDR1: keep the single entry, RDNSS
			 * does not need to repeat the same resolver twice.
			 */
		}
	}
...
static ssize_t ra_build_minimal(const struct ra_build_out *out, struct net_if *host,
				const struct net_in6_addr *src_ll,
				const struct net_in6_addr *ip_dst,
				const struct ra_build_params *params,
				const struct ra_build_pio *pio)
{
	uint8_t *buf = out->buf;
	size_t buf_cap = out->cap;
	uint16_t router_lifetime = params->router_lifetime;
	uint32_t rdnss_lifetime = params->rdnss_lifetime;
	uint8_t ra_flags = params->flags;
	const struct net_in6_addr *pio_pfx = pio->pfx;
	uint8_t pio_cnt = pio->cnt;
	uint32_t pio_valid_lt = pio->valid_lt;
	uint32_t pio_preferred_lt = pio->preferred_lt;
...

	{
		...
		for (uint8_t pi = 0U; pi < pio_cnt && pio_pfx != NULL; pi++) {
			struct ra_opt_pio *pio_opt = (struct ra_opt_pio *)wp;

			pio_opt->type = (uint8_t)RA_OPT_PREFIX_INFO;
			pio_opt->len = (uint8_t)(RA_PIO_SIZE / 8U);
			pio_opt->prefix_len = 64U;
			pio_opt->flags = 0U; /* L=0 (not on-link), A=0 (no SLAAC) */
			pio_opt->valid_lifetime = net_htonl(pio_valid_lt);
			pio_opt->preferred_lifetime = net_htonl(pio_preferred_lt);
			pio_opt->reserved = 0U;
			/* Copy first 64 bits; host part is already zeroed by caller. */
			memcpy(pio_opt->prefix, pio_pfx[pi].s6_addr, IPV6_ADDR_LEN);
			wp += RA_PIO_SIZE;
		}
				...
				const uint8_t *dst_mac_override, int16_t forced_rl)
{
...

	{
		...
		if (uplink && (forced_rl < 0 || forced_rl > 0)) {
			...
			bool from_dhcpv6_srv = false;

			have_ula = false;
			have_gua = false;
#if defined(CONFIG_DECT_TETHER_IPV6_DHCPV6_SERVER)
			from_dhcpv6_srv = dect_tether_ipv6_dhcpv6_srv_tether_pio_addrs(
				&ula, &gua, &have_ula, &have_gua);
#endif
			if (!from_dhcpv6_srv) {
				struct net_if *dect =
		...
		struct ra_build_out build_out = { .buf = buf, .cap = sizeof(buf) };
		struct ra_build_params build_params = {
			.router_lifetime = rl, .rdnss_lifetime = rdl, .flags = flags,
		};
		struct ra_build_pio build_pio = {
#if defined(CONFIG_DECT_TETHER_IPV6_RA_PREFIX_INFO)
			.pfx = pio_cnt > 0U ? pio_pfx : NULL,
			.cnt = pio_cnt,
			.valid_lt = CONFIG_DECT_TETHER_IPV6_RA_PIO_VALID_LIFETIME,
			.preferred_lt = CONFIG_DECT_TETHER_IPV6_RA_PIO_PREFERRED_LIFETIME,
#else
			.pfx = NULL, .cnt = 0U, .valid_lt = 0U, .preferred_lt = 0U,
#endif
		};

		pkt_len = ra_build_minimal(&build_out, host, src_ll, ip_dst, &build_params,
					   &build_pio);
	}
	if (pkt_len <= 0) {
		return;
...
{
	/* This file is only built for CONFIG_DECT_TETHER_IPV6_RA, which depends on
	 * NET_L2_DECT_MGMT, which unconditionally selects NET_MGMT_EVENT_INFO, so
	 * cb->info is always available here (unlike dect_tether_ipv6_uplink.c,
	 * which is built unconditionally and cannot make that assumption).
	 */
	...
	if (mgmt_event == NET_EVENT_IF_UP) {
		...
		/* Other events this handler is not registered/interested in
		 * (the net_mgmt_event_callback mask only requests the four
...
/* No CONFIG_DECT_TETHER_IPV6_RA guard needed here: this whole file is only
 * built for that config (see CMakeLists.txt); the guard on the declaration
 * in dect_tether_ipv6_int.h stays, since that header is also included by
 * dect_tether_ipv6_uplink.c, which is built unconditionally.
 */
