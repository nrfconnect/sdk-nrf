...
struct net_if *dect_tether_ipv6_host_eth_iface(void)
{
	...
	if (name[0] != '\0') {
		...
		LOG_WRN("host Ethernet iface %s not found (not yet up/named?)", name);
		return NULL;
	}
#else
	if (CONFIG_DECT_TETHER_IPV6_HOST_ETH_IFACE[0] != '\0') {
		LOG_ERR("CONFIG_DECT_TETHER_IPV6_HOST_ETH_IFACE set but "
			"CONFIG_NET_INTERFACE_NAME is off; enable it or clear the name");
		return NULL;
	}
#endif
