...
static void nrf_provisioning_lte_handler(const struct lte_lc_evt *const evt)
{
	...
	case LTE_LC_EVT_NW_REG_STATUS:
		...
		if ((evt->nw_reg_status != LTE_LC_NW_REG_REGISTERED_HOME) &&
			...
			if (initialized) {
				nw_connected = false;
				LOG_DBG("Disconnected from network - provisioning paused");
			}
			break;
		}

		LOG_DBG("%s - provisioning resumed",
...
int nrf_provisioning_schedule(void)
{
	...
	if (first) {
		...
		LOG_DBG("First provisioning, setting provisioning interval to %lld seconds",
