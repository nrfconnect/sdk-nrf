...
/* The whole internal API (and its PSA dependency) only exists when on-device
 * key generation is enabled. Guarding it here keeps the header safe to include
 * unconditionally from consumers that are built without PSA crypto, such as the
 * nRF Cloud unit tests on boards where PSA is not configured.
 */
