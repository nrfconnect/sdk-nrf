...
static int fota_settings_set(const char *key, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	...
	if (sz == sizeof(*pending_job)) {
		LOG_INF("Saved job: %s, type: %d, validate: %d, bl: 0x%X", pending_job->id,
			pending_job->type, pending_job->validate, pending_job->bl_flags);
	} else {
...
static enum nrf_cloud_fota_validate_status
boot_fota_validate_get(const enum nrf_cloud_fota_bootloader_status_flags bl_flags)
{
...
static enum nrf_cloud_fota_validate_status modem_full_fota_validate_get(void)
{
	...
	return (err ? NRF_CLOUD_FOTA_VALIDATE_FAIL : NRF_CLOUD_FOTA_VALIDATE_PASS);
}

static enum nrf_cloud_fota_validate_status smp_fota_validate_get(void)
{
#if defined(CONFIG_NRF_CLOUD_FOTA_SMP)
	return nrf_cloud_fota_smp_install() ? NRF_CLOUD_FOTA_VALIDATE_FAIL
...
{
	return ((type == NRF_CLOUD_FOTA_MODEM_DELTA) || (type == NRF_CLOUD_FOTA_MODEM_FULL));
}
...
int nrf_cloud_fota_smp_version_read(void)
{
	...
	for (int i = 0; i < image_list.image_list_length; ++i, ++list) {
		LOG_DBG("%s Image(%d) slot(%d)", list->flags.active ? "Primary" : "Secondary",
			list->img_num, list->slot_num);
		LOG_DBG("  Version: %s", list->version);
		LOG_DBG("  Bootable(%d) Pending(%d) Confirmed(%d)", list->flags.bootable,
