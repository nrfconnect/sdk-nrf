...
/* Base64-encoded size, including the NUL terminator, of n raw bytes. */
#define BASE64_ENCODED_SIZE(n) ((((n) + 2) / 3) * 4 + 1)

/* Shared Base64 output buffer, sized for whichever of the CSR or the public
 * key encodes larger.
 */
#define B64_BUF_SIZE MAX(BASE64_ENCODED_SIZE(CSR_DER_BUF_SIZE), \
			 BASE64_ENCODED_SIZE(PUBKEY_RAW_SIZE))

static uint8_t csr_der_buf[CSR_DER_BUF_SIZE];
static char b64_buf[B64_BUF_SIZE];
...
static int print_csr(const struct shell *sh, uint32_t sec_tag, const char *subject)
{
	...
	err = base64_encode(b64_buf, sizeof(b64_buf), &b64_len, csr_der_buf, der_len);
	if (err) {
...
static int cmd_pubkey(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = base64_encode(b64_buf, sizeof(b64_buf), &b64_len, pubkey_raw, raw_len);
	if (err) {
		shell_error(sh, "Base64 encoding failed: %d", err);
		return err;
	}
	...
	shell_print(sh, "PUBKEY: %s", b64_buf);
