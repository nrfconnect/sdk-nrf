...
static int settings_set(const char *key, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	...
	if (!strncmp(key, SETTINGS_KEY_LOCATION, strlen(SETTINGS_KEY_LOCATION)) &&
	    (len_rd == sizeof(saved_location))) {
		if (read_cb(cb_arg, (void *)&saved_location, len_rd) == len_rd) {
			LOG_DBG("Read location:%d, %d, gps sec:%d", saved_location.latitude,
				saved_location.longitude, (int32_t)saved_location.gps_sec);
			return 0;
		}
	}
	if (!strncmp(key, SETTINGS_KEY_LEAP_SEC, strlen(SETTINGS_KEY_LEAP_SEC)) &&
	    (len_rd == sizeof(gps_leap_seconds))) {
		if (read_cb(cb_arg, (void *)&gps_leap_seconds, len_rd) == len_rd) {
			LOG_DBG("Read gps to utc leap seconds offset:%d", gps_leap_seconds);
			return 0;
		}
	}
	return -ENOTSUP;
...
static int save_location(void)
{
	...
	LOG_DBG("Saving location:%d, %d; gps sec:%d", saved_location.latitude,
		saved_location.longitude, (int32_t)saved_location.gps_sec);
	ret = settings_save_one(SETTINGS_FULL_LOCATION, &saved_location, sizeof(saved_location));
	return ret;
}
...
static int save_leap_sec(void)
{
	...
	ret = settings_save_one(SETTINGS_FULL_LEAP_SEC, &gps_leap_seconds,
				sizeof(gps_leap_seconds));
	return ret;
}
...
void npgps_print_blocks(void)
{
	...
	LOG_DBG("num_blocks:%u, size:%u, first_free:%d", num_blocks, BLOCK_SIZE, pool.first_free);
	for (i = 0; i < num_blocks; i++) {
...
static int downloader_callback(const struct downloader_evt *event)
{
	...
	case DOWNLOADER_EVT_FRAGMENT:
		err = buffer_handler((uint8_t *)event->fragment.buf, event->fragment.len);
		if (!err) {
			return 0;
		}
		break;
	...
	case DOWNLOADER_EVT_ERROR: {
		...
		if ((socket_retries_left) &&
		    ((event->error == -ENOTCONN) || (event->error == -ECONNREFUSED) ||
		     (event->error == -EAGAIN) || (event->error == -ECONNRESET))) {
