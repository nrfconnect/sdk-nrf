...
static int cred_type[CRED_COUNT] = {
	...
	MODEM_KEY_MGMT_CRED_TYPE_CA_CHAIN, MODEM_KEY_MGMT_CRED_TYPE_PUBLIC_CERT,
	MODEM_KEY_MGMT_CRED_TYPE_PRIVATE_CERT
#else
	TLS_CREDENTIAL_CA_CERTIFICATE, TLS_CREDENTIAL_SERVER_CERTIFICATE, TLS_CREDENTIAL_PRIVATE_KEY
#endif
};
...
#if defined(CONFIG_NRF_CLOUD_CREDENTIALS_MGMT_MODEM)
	ret = modem_key_mgmt_write(sec_tag, (enum modem_key_mgmt_cred_type)cred_type[type], cred,
				   cred_len);
#else
	ret = tls_credential_add(sec_tag, (enum tls_credential_type)cred_type[type], cred,
				 cred_len + 1); /* Add one to include NULL */
#endif
...
static int cred_exists(uint32_t sec_tag, int type, bool *exists)
{
	...
	err = tls_credential_get(sec_tag, (enum tls_credential_type)cred_type[type], NULL,
				 &credlen);
...
static int cred_size_get(uint32_t sec_tag, int type, size_t *cred_sz)
{
	...
	*cred_sz = 0; /* We just want to determine the size */
	...
	err = modem_key_mgmt_read(sec_tag, (enum modem_key_mgmt_cred_type)cred_type[type], buf,
				  cred_sz);
	if (err && (err != -ENOMEM)) {
...
#else
	err = tls_credential_get(sec_tag, (enum tls_credential_type)cred_type[type], NULL, cred_sz);
...
int nrf_cloud_credentials_check(struct nrf_cloud_credentials_status *const cs)
{
	...
	LOG_INF("Sec Tag: %u; CA: %s, Client Cert: %s, Private Key: %s", cs->sec_tag,
		cs->ca ? "Yes" : "No", cs->client_cert ? "Yes" : "No", cs->prv_key ? "Yes" : "No");
	LOG_INF("CA Size: %zd, AWS: %s, CoAP: %s", cs->ca_size, cs->ca_aws ? "Likely" : "Unlikely",
