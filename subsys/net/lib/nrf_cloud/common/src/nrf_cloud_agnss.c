...
{
	atomic_set(&request_in_progress, in_progress ? 1 : 0);
	if (in_progress) {
...
static int agnss_send_to_modem(struct nrf_cloud_agnss_element *agnss_data)
{
	...
	case NRF_CLOUD_AGNSS_QZSS_ALMANAC: {
		...
		return send_to_modem(&almanac, sizeof(almanac), NRF_MODEM_GNSS_AGNSS_GPS_ALMANAC);
	}
	...
	case NRF_CLOUD_AGNSS_LOCATION: {
		...
		return send_to_modem(&location, sizeof(location), NRF_MODEM_GNSS_AGNSS_LOCATION);
	}
	case NRF_CLOUD_AGNSS_GPS_INTEGRITY: {
		...
		return send_to_modem(&integrity, sizeof(integrity), NRF_MODEM_GNSS_AGPS_INTEGRITY);
	}
...
int nrf_cloud_agnss_process(const char *buf, size_t buf_len)
{
	...
	LOG_DBG("Received AGNSS data. Schema version: %d, length: %d", version, buf_len);
