...
static const struct log_backend_api logger_api = {.init = logger_init,
...
static void logger_init(const struct log_backend *const backend)
{
	...
	for (i = 0; i < ARRAY_SIZE(filtered_modules); i++) {
		 ...
		 */
		actual_level =
			log_filter_set(&log_nrf_cloud_backend, Z_LOG_LOCAL_DOMAIN_ID, sid, 0);
		...
		LOG_DBG("%d. log:%s, srcid:%u, actual_level:%u", i, filtered_modules[i], sid,
			actual_level);
	}

	LOG_DBG("domain name:%s, num domains:%u, num sources:%u",
		log_domain_name_get(Z_LOG_LOCAL_DOMAIN_ID), log_domains_count(),
...
static uint32_t get_source_id(void *source_data, uint8_t dom_id)
{
	...
	if (source_data != NULL) {
		return IS_ENABLED(CONFIG_LOG_RUNTIME_FILTERING) ? log_dynamic_source_id(source_data)
...
static void logger_process(const struct log_backend *const backend, union log_msg_generic *msg)
{
	...
	if (!nrf_cloud_log_is_enabled() || (backend != &log_nrf_cloud_backend) ||
...
static void logger_notify(const struct log_backend *const backend, enum log_backend_evt event,
			  union log_backend_evt_arg *arg)
{
	if ((backend != &log_nrf_cloud_backend) || (event != LOG_BACKEND_EVT_PROCESS_THREAD_DONE) ||
	...
	if (CONFIG_NRF_CLOUD_LOG_LOG_LEVEL >= LOG_LEVEL_DBG) {
		LOG_DBG("Buffered lines:%u, bytes:%u; logged lines:%u, bytes:%u; "
			...
			stats.lines_rendered, stats.bytes_rendered, stats.lines_sent,
			stats.bytes_sent, stats.lines_dropped);
	} else {
...
static int send_ring_buffer(void)
{
	...
	if (log_rb_len != stored) {
		LOG_WRN("Capacity:%u, free:%u, stored:%u, claimed:%u",
			...
			ring_buf_space_get(&log_nrf_cloud_rb), stored, log_rb_len);
		stored = log_rb_len;
	}
	if (!log_rb_len) {
		goto cleanup;
	...
	if (IS_ENABLED(CONFIG_NRF_CLOUD_MQTT)) {
		struct nrf_cloud_tx_data output = {
			...
			.topic_type = (log_format_current == LOG_OUTPUT_TEXT) ? NRF_CLOUD_TOPIC_BULK
									      : NRF_CLOUD_TOPIC_BIN,
	...
	if (log_format_current == LOG_OUTPUT_TEXT) {
		if ((buf >= log_buf) && (&buf[size] <= &log_buf[CONFIG_NRF_CLOUD_LOG_BUF_SIZE])) {
			...
			printk("buf %p..%p is not inside our log_buf %p..%p\n", buf, &buf[size],
			       log_buf, &log_buf[CONFIG_NRF_CLOUD_LOG_BUF_SIZE]);
			return orig_size;
		}
		...
		if (log_context.src_name) {
			 ...
			 */
			if ((strncmp(log_context.src_name, (const char *)buf, MIN(size, len)) ==
			     0) &&
		...
		if (ring_buf_space_get(&log_nrf_cloud_rb) > (data.len + extra)) {
			...
			if (num_msgs == 0) {
				...
				if (log_format_current == LOG_OUTPUT_TEXT) {
					...
					ring_buf_put(&log_nrf_cloud_rb, (const uint8_t *)&hdr,
						     sizeof(hdr));
				}
