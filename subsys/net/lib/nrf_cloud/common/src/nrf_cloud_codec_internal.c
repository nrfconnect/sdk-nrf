...
static const char *const job_status_strings[] = {
	[NRF_CLOUD_FOTA_QUEUED] = "QUEUED",	  [NRF_CLOUD_FOTA_IN_PROGRESS] = "IN_PROGRESS",
	[NRF_CLOUD_FOTA_FAILED] = "FAILED",	  [NRF_CLOUD_FOTA_SUCCEEDED] = "SUCCEEDED",
	[NRF_CLOUD_FOTA_TIMED_OUT] = "TIMED_OUT", [NRF_CLOUD_FOTA_REJECTED] = "REJECTED",
	[NRF_CLOUD_FOTA_CANCELED] = "CANCELLED",  [NRF_CLOUD_FOTA_DOWNLOADING] = "DOWNLOADING",
};
#define JOB_STATUS_STRING_COUNT (sizeof(job_status_strings) / sizeof(*job_status_strings))
...
const char *nrf_cloud_get_sensor_type_str_internal(enum nrf_cloud_sensor type)
{
...
cJSON *json_create_req_obj(const char *const app_id, const char *const msg_type)
{
	...
	if (!cJSON_AddStringToObjectCS(req_obj, NRF_CLOUD_JSON_APPID_KEY, app_id) ||
	    !cJSON_AddStringToObjectCS(req_obj, NRF_CLOUD_JSON_MSG_TYPE_KEY, msg_type)) {
...
static int info_encode(cJSON *const root_obj, const struct nrf_cloud_device_status *const ds)
{
	...
	if (ds->svc) {
		cJSON *svc_inf_obj =
			cJSON_AddObjectToObjectCS(root_obj, NRF_CLOUD_JSON_KEY_SRVC_INFO);
	...
	if (ds->conn_inf == NRF_CLOUD_INFO_SET) {
		...
		ret = !cJSON_AddNullToObjectCS(root_obj, NRF_CLOUD_JSON_KEY_CONN_INFO);
	}

	return ret;
}

int nrf_cloud_device_control_encode_internal(cJSON *const obj,
					     struct nrf_cloud_ctrl_data const *const data)
{
	if (!obj) {
		return -EINVAL;
	}
	...
	if (ret == 0) {
		...
		if (!cJSON_AddItemToObjectCS(obj, NRF_CLOUD_JSON_KEY_CTRL, ctrl_obj) == 0) {
			return 0;
		}
	}
					     ...
					     bool accept, struct nrf_cloud_data *const output)
{
	...
	if (!IS_ENABLED(CONFIG_NRF_CLOUD_COAP)) {
		...
		if (!accept) {
			...
			err = nrf_cloud_device_control_encode_internal(desired_obj, NULL);
			if (err) {
				goto end;
			}
		}
	...
	err = nrf_cloud_device_control_encode_internal(reported_obj, data);
	if (err) {
		goto end;
	}
...
static int shadow_connection_info_update(cJSON *device_obj)
{
	...
	ret += !cJSON_AddStringToObjectCS(connection_obj, NRF_CLOUD_JSON_KEY_PROTOCOL,
					  NRF_CLOUD_JSON_VAL_CFGD_PROTO_VAL);
	ret += !cJSON_AddStringToObjectCS(connection_obj, NRF_CLOUD_JSON_KEY_METHOD,
					  NRF_CLOUD_JSON_VAL_CFGD_METHOD_VAL);

	return ret;
}

int nrf_cloud_pvt_data_encode(const struct nrf_cloud_gnss_pvt *const pvt, cJSON *const pvt_data_obj)
{
	if (!pvt || !pvt_data_obj) {
		return -EINVAL;
	}

	if (!cJSON_AddNumberToObjectCS(pvt_data_obj, NRF_CLOUD_JSON_GNSS_PVT_KEY_LON, pvt->lon) ||
	    !cJSON_AddNumberToObjectCS(pvt_data_obj, NRF_CLOUD_JSON_GNSS_PVT_KEY_LAT, pvt->lat) ||
	    !cJSON_AddNumberToObjectCS(pvt_data_obj, NRF_CLOUD_JSON_GNSS_PVT_KEY_ACCURACY,
				       pvt->accuracy) ||
	    (pvt->has_alt &&
	     !cJSON_AddNumberToObjectCS(pvt_data_obj, NRF_CLOUD_JSON_GNSS_PVT_KEY_ALTITUDE,
					pvt->alt)) ||
	    (pvt->has_speed &&
	     !cJSON_AddNumberToObjectCS(pvt_data_obj, NRF_CLOUD_JSON_GNSS_PVT_KEY_SPEED,
					pvt->speed)) ||
	    (pvt->has_heading &&
	     !cJSON_AddNumberToObjectCS(pvt_data_obj, NRF_CLOUD_JSON_GNSS_PVT_KEY_HEADING,
					pvt->heading))) {
					      ...
					      cJSON *const svc_inf_obj)
{
	...
	if (fota == NULL) {
		return !cJSON_AddNullToObjectCS(svc_inf_obj, NRF_CLOUD_JSON_KEY_SRVC_INFO_FOTA);
	}
					    ...
					    cJSON *const svc_inf_obj)
{
	...
	(void)!cJSON_AddNullToObjectCS(svc_inf_obj, NRF_CLOUD_JSON_KEY_SRVC_INFO_UI);
			       ...
			       cJSON *const inf_obj, cJSON *const root_obj)
{
	...
	case NRF_CLOUD_INFO_SET:
		...
		if (!cJSON_AddItemToObjectCS(root_obj, inf_name, move_obj)) {
	...
	case NRF_CLOUD_INFO_CLEAR:
		if (!cJSON_AddNullToObjectCS(root_obj, inf_name)) {
...
static int cell_info_json_encode(cJSON *const obj, const struct lte_lc_cell *const cell_inf)
{
	...
	if (!cJSON_AddNumberToObjectCS(obj, NRF_CLOUD_JSON_MCC_KEY, cell_inf->mcc) ||
	    !cJSON_AddNumberToObjectCS(obj, NRF_CLOUD_JSON_MNC_KEY, cell_inf->mnc) ||
	    !cJSON_AddNumberToObjectCS(obj, NRF_CLOUD_JSON_AREA_CODE_KEY, cell_inf->tac) ||
	    !cJSON_AddNumberToObjectCS(obj, NRF_CLOUD_JSON_CELL_ID_KEY, cell_inf->id) ||
	    !cJSON_AddNumberToObjectCS(obj, NRF_CLOUD_JSON_RSRP_KEY,
				       RSRP_IDX_TO_DBM(cell_inf->rsrp))) {
		return -ENOMEM;
	}
...
static int add_modem_info_data(struct lte_param *param, cJSON *json_obj)
{
	...
	if (data_type == MODEM_INFO_DATA_TYPE_STRING && param->type != MODEM_INFO_AREA_CODE) {
...
static int encode_modem_info_device(struct device_param *device, cJSON *json_obj)
{
	...
	if (!cJSON_AddStringToObjectCS(json_obj, "board", device->board)) {
		return -ENOMEM;
	}

	if (!cJSON_AddStringToObjectCS(json_obj, "sdkVer", SDK_VERSION)) {
		return -ENOMEM;
	}

	if (!cJSON_AddStringToObjectCS(json_obj, "appName", device->app_name)) {
		return -ENOMEM;
	}

	if (!cJSON_AddStringToObjectCS(json_obj, "zephyrVer", zver)) {
		return -ENOMEM;
	}

	ret = modem_info_get_hw_version(hw_ver, sizeof(hw_ver) - 1);
	if (!cJSON_AddStringToObjectCS(json_obj, "hwVer", ((ret == 0) ? hw_ver : "N/A"))) {
		return -ENOMEM;
	}
					 ...
					 const char *const app_ver)
{
	...
	if (IS_ENABLED(CONFIG_MODEM_INFO_ADD_NETWORK)) {
		...
		if (!ret) {
			ret = !cJSON_AddItemToObjectCS(root_obj, NRF_CLOUD_DEVICE_JSON_KEY_NET_INF,
						       network_obj);
			if (ret) {
	...
	if (IS_ENABLED(CONFIG_MODEM_INFO_ADD_SIM)) {
		...
		if (!ret) {
			ret = !cJSON_AddItemToObjectCS(root_obj, NRF_CLOUD_DEVICE_JSON_KEY_SIM_INF,
						       sim_obj);
			if (ret) {
	...
	if (IS_ENABLED(CONFIG_MODEM_INFO_ADD_DEVICE)) {
		...
		if (app_ver) {
			ret = !cJSON_AddStringToObjectCS(device_obj, NRF_CLOUD_JSON_KEY_APP_VER,
							 app_ver);
		}
		...
		if (!ret) {
			...
			if (smp_ver) {
				ret = !cJSON_AddStringToObjectCS(
					device_obj, NRF_CLOUD_JSON_KEY_SMP_APP_VER, smp_ver);
			}
		}
		...
		if (!ret) {
			ret = !cJSON_AddItemToObjectCS(root_obj, NRF_CLOUD_DEVICE_JSON_KEY_DEV_INF,
						       device_obj);
			if (ret) {
				cJSON_Delete(device_obj);
				return -ENOMEM;
				     ...
				     cJSON *const mod_inf_obj)
{
	...
	if (encode_info_item_cs(mod_inf->device, NRF_CLOUD_DEVICE_JSON_KEY_DEV_INF, tmp,
				mod_inf_obj) ||
	    encode_info_item_cs(mod_inf->network, NRF_CLOUD_DEVICE_JSON_KEY_NET_INF, tmp,
				mod_inf_obj) ||
	    encode_info_item_cs(mod_inf->sim, NRF_CLOUD_DEVICE_JSON_KEY_SIM_INF, tmp,
				mod_inf_obj)) {
...
int nrf_cloud_enabled_info_sections_json_encode(cJSON *const obj, const char *const app_ver)
{
	...
	struct nrf_cloud_modem_info mdm_inf = {.application_version = app_ver};
	...
	if ((ds.modem->device == NRF_CLOUD_INFO_SET) || (ds.modem->network == NRF_CLOUD_INFO_SET) ||
	...
	if (ds.svc->fota) {
		cJSON *svc_inf_obj =
			cJSON_AddObjectToObjectCS(device_obj, NRF_CLOUD_JSON_KEY_SRVC_INFO);
				 ...
				 struct nrf_cloud_data *output)
{
	...
	ret = !cJSON_AddItemToObjectCS(reported_obj, sensor_type_str[sensor->type], input_obj);
				     ...
				     const int64_t timestamp, cJSON *const msg_obj_out)
{
	...
	if (!cJSON_AddStringToObjectCS(msg_obj_out, NRF_CLOUD_JSON_APPID_KEY,
				       NRF_CLOUD_JSON_APPID_VAL_DEVICE) ||
	    !cJSON_AddStringToObjectCS(msg_obj_out, NRF_CLOUD_JSON_MSG_TYPE_KEY,
				       NRF_CLOUD_JSON_MSG_TYPE_VAL_DATA)) {
		return -ENOMEM;
	}

	if (timestamp > 0) {
		if (!cJSON_AddNumberToObjectCS(msg_obj_out, NRF_CLOUD_MSG_TIMESTAMP_KEY,
					       timestamp)) {
			return -ENOMEM;
		}
	}
				     ...
				     struct nrf_cloud_fota_job_update *update)
{
	...
	if (details) {
		buff_sz = sizeof(API_UPDATE_FOTA_DETAILS_TMPLT) +
			  strlen(job_status_strings[status]) + strlen(details);
	} else {
		buff_sz =
			sizeof(API_UPDATE_FOTA_BODY_TEMPLATE) + strlen(job_status_strings[status]);
	}
...
int nrf_cloud_fota_job_decode(struct nrf_cloud_fota_job_info *const job_info,
			      bt_addr_t *const ble_id, const struct nrf_cloud_data *const input)
{
	...
	if (job_id_len > (NRF_CLOUD_FOTA_JOB_ID_SIZE - 1)) {
		LOG_ERR("Job ID length: %d, exceeds allowed length: %d", job_id_len,
			NRF_CLOUD_FOTA_JOB_ID_SIZE - 1);
		goto cleanup;
	}
	...
	if (ble_id) {
		...
		if (json_array_str_get(array, RCV_ITEM_IDX_BLE_ID, &ble_str)) {
					 ...
					 const struct nrf_cloud_fota_job *const job)
{
	...
	case NRF_CLOUD_OBJ_TYPE_JSON: {
		...
		if (job->status == NRF_CLOUD_FOTA_DOWNLOADING) {
			...
			result &=
				(json_array_str_add(obj->json, get_error_string(job->error)) == 0);
		}
		      ...
		      const struct lte_lc_ncell *const neighbor_cells)
{
	...
	for (uint8_t i = 0; i < ncells_count; ++i) {
		...
		if (!cJSON_AddNumberToObjectCS(ncell_obj, NRF_CLOUD_CELL_POS_JSON_KEY_EARFCN,
					       ncell->earfcn) ||
		    !cJSON_AddNumberToObjectCS(ncell_obj, NRF_CLOUD_CELL_POS_JSON_KEY_PCI,
					       ncell->phys_cell_id)) {
			return -ENOMEM;
		}
		...
		if ((ncell->rsrp != NRF_CLOUD_LOCATION_CELL_OMIT_RSRP) &&
		    !cJSON_AddNumberToObjectCS(ncell_obj, NRF_CLOUD_CELL_POS_JSON_KEY_RSRP,
					       RSRP_IDX_TO_DBM(ncell->rsrp))) {
			return -ENOMEM;
		}
		if ((ncell->rsrq != NRF_CLOUD_LOCATION_CELL_OMIT_RSRQ) &&
		    !cJSON_AddNumberToObjectCS(ncell_obj, NRF_CLOUD_CELL_POS_JSON_KEY_RSRQ,
					       RSRQ_IDX_TO_DB(ncell->rsrq))) {
			return -ENOMEM;
		}
		if ((ncell->time_diff != LTE_LC_CELL_TIME_DIFF_INVALID) &&
		    !cJSON_AddNumberToObjectCS(ncell_obj, NRF_CLOUD_CELL_POS_JSON_KEY_TDIFF,
					       ncell->time_diff)) {
			return -ENOMEM;
		}
	}
...
static cJSON *add_lte_inf(cJSON *const lte_array, struct lte_lc_cell const *const inf)
{
	...
	/* Required parameters for the API call */
	if (!cJSON_AddNumberToObjectCS(lte_obj, NRF_CLOUD_CELL_POS_JSON_KEY_ECI, inf->id) ||
	    !cJSON_AddNumberToObjectCS(lte_obj, NRF_CLOUD_CELL_POS_JSON_KEY_MCC, inf->mcc) ||
	    !cJSON_AddNumberToObjectCS(lte_obj, NRF_CLOUD_CELL_POS_JSON_KEY_MNC, inf->mnc) ||
	    !cJSON_AddNumberToObjectCS(lte_obj, NRF_CLOUD_CELL_POS_JSON_KEY_TAC, inf->tac)) {
		return NULL;
	}

	/* Optional parameters for the API call */
	if ((inf->earfcn != NRF_CLOUD_LOCATION_CELL_OMIT_EARFCN) &&
	    !cJSON_AddNumberToObjectCS(lte_obj, NRF_CLOUD_CELL_POS_JSON_KEY_EARFCN, inf->earfcn)) {
		return NULL;
	}

	if ((inf->rsrp != NRF_CLOUD_LOCATION_CELL_OMIT_RSRP) &&
	    !cJSON_AddNumberToObjectCS(lte_obj, NRF_CLOUD_CELL_POS_JSON_KEY_RSRP,
				       RSRP_IDX_TO_DBM(inf->rsrp))) {
		return NULL;
	}

	if ((inf->rsrq != NRF_CLOUD_LOCATION_CELL_OMIT_RSRQ) &&
	    !cJSON_AddNumberToObjectCS(lte_obj, NRF_CLOUD_CELL_POS_JSON_KEY_RSRQ,
	...
	if (inf->timing_advance != NRF_CLOUD_LOCATION_CELL_OMIT_TIME_ADV) {
		...
		if (!cJSON_AddNumberToObjectCS(lte_obj, NRF_CLOUD_CELL_POS_JSON_KEY_T_ADV, t_adv)) {
			return NULL;
		}
	}
...
{
	return ((mac[0] & 0x02) || ((mac[0] == 0x00) && (mac[1] == 0x00) && (mac[2] == 0x5E)));
}
				   ...
				   cJSON *const req_obj_out)
{
	...
	const bool add_rssi =
		(add_all || IS_ENABLED(CONFIG_NRF_CLOUD_WIFI_LOCATION_ENCODE_OPT_MAC_RSSI));
	...
	for (uint8_t cnt = 0; cnt < wifi->cnt; ++cnt) {
		...
		if (is_local_mac(ap->mac)) {
			LOG_DBG("Skipping local MAC %02x:%02x:%02x:...", ap->mac[0], ap->mac[1],
				ap->mac[2]);
			continue;
		}
		...
		ret = snprintk(str_buf, sizeof(str_buf), WIFI_MAC_ADDR_TEMPLATE, ap->mac[0],
			       ap->mac[1], ap->mac[2], ap->mac[3], ap->mac[4], ap->mac[5]);
		if ((ret != WIFI_MAC_ADDR_STR_LEN) ||
		    !cJSON_AddStringToObjectCS(ap_obj, NRF_CLOUD_LOCATION_JSON_KEY_WIFI_MAC,
					       str_buf)) {
			goto cleanup;
		}
		...
		if (add_rssi && (ap->rssi != NRF_CLOUD_LOCATION_WIFI_OMIT_RSSI) &&
		    !cJSON_AddNumberToObjectCS(ap_obj, NRF_CLOUD_LOCATION_JSON_KEY_WIFI_RSSI,
					       ap->rssi)) {
			goto cleanup;
		}
		...
		if (add_all) {
			...
			if ((str_buf[0] != '\0') &&
			    !cJSON_AddStringToObjectCS(
				    ap_obj, NRF_CLOUD_LOCATION_JSON_KEY_WIFI_SSID, str_buf)) {
				goto cleanup;
			}

			if ((ap->channel != NRF_CLOUD_LOCATION_WIFI_OMIT_CHAN) &&
			    !cJSON_AddNumberToObjectCS(ap_obj, NRF_CLOUD_LOCATION_JSON_KEY_WIFI_CH,
						       ap->channel)) {
				goto cleanup;
			}
		}
			       ...
			       const char *const msg_type, enum nrf_cloud_error *const err)
{
	...
	*err = NRF_CLOUD_ERROR_NONE;
	...
	if (msg_type && !json_item_string_exists(root_obj, NRF_CLOUD_JSON_MSG_TYPE_KEY, msg_type)) {
		ret = -ENOENT;
		goto clean_up;
	}
	if (app_id && !json_item_string_exists(root_obj, NRF_CLOUD_JSON_APPID_KEY, app_id)) {
		ret = -ENOENT;
		goto clean_up;
	}
				   ...
				   cJSON *const gnss_msg_obj)
{
	...
	if (!cJSON_AddStringToObjectCS(gnss_msg_obj, NRF_CLOUD_JSON_APPID_KEY,
				       NRF_CLOUD_JSON_APPID_VAL_GNSS) ||
	    !cJSON_AddStringToObjectCS(gnss_msg_obj, NRF_CLOUD_JSON_MSG_TYPE_KEY,
				       NRF_CLOUD_JSON_MSG_TYPE_VAL_DATA) ||
	    ((gnss->ts_ms != NRF_CLOUD_NO_TIMESTAMP) &&
	     !cJSON_AddNumberToObjectCS(gnss_msg_obj, NRF_CLOUD_MSG_TIMESTAMP_KEY, gnss->ts_ms))) {
		ret = -ENOMEM;
		goto cleanup;
	}
...
int nrf_cloud_alert_encode(const struct nrf_cloud_alert_info *alert, struct nrf_cloud_data *output)
{
	...
	ret += !cJSON_AddNumberToObjectCS(root_obj, NRF_CLOUD_JSON_ALERT_TYPE, alert->type);
	if (alert->value != NRF_CLOUD_ALERT_UNUSED_VALUE) {
		ret += !cJSON_AddNumberToObjectCS(root_obj, NRF_CLOUD_JSON_ALERT_VALUE,
						  alert->value);
	}
	...
	if ((alert->ts_ms <= NRF_CLOUD_NO_TIMESTAMP) ||
		...
		ret += !cJSON_AddNumberToObjectCS(root_obj, NRF_CLOUD_JSON_ALERT_SEQUENCE,
						  alert->sequence);
	}
	if (alert->description != NULL) {
		ret += !cJSON_AddStringToObjectCS(root_obj, NRF_CLOUD_JSON_ALERT_DESCRIPTION,
						  alert->description);
	}
					 ...
					 const size_t type_count)
{
	...
	if (!err) {
		err = cJSON_AddItemToObjectCS(obj, NRF_CLOUD_JSON_KEY_AGNSS_TYPES, array) ? 0
											  : -EIO;
	}
...
int nrf_cloud_agnss_req_data_json_encode(const enum nrf_cloud_agnss_type *const types,
					 ...
					 const bool fetch_cell_inf, const bool filtered_ephem,
					 const uint8_t mask_angle, cJSON *const data_obj_out)
{
	...
	if (filtered_ephem) {
		if ((json_add_bool_cs(data_obj_out, NRF_CLOUD_JSON_FILTERED_KEY, true)) ||
		    (!cJSON_AddNumberToObjectCS(data_obj_out, NRF_CLOUD_JSON_KEY_ELEVATION_MASK,
						mask_angle))) {
			err = -ENOMEM;
			goto cleanup;
		}
				    ...
				    cJSON *const pvt_data_obj)
{
	...
	struct nrf_cloud_gnss_pvt pvt = {.lon = mdm_pvt->longitude,
				    ...
				    cJSON *const agnss_req_obj_out)
{
	...
	if (!cJSON_AddStringToObjectCS(agnss_req_obj_out, NRF_CLOUD_JSON_APPID_KEY,
				       NRF_CLOUD_JSON_APPID_VAL_AGNSS) ||
	    !cJSON_AddStringToObjectCS(agnss_req_obj_out, NRF_CLOUD_JSON_MSG_TYPE_KEY,
				       NRF_CLOUD_JSON_MSG_TYPE_VAL_DATA)) {
		err = -ENOMEM;
		goto cleanup;
	}
...
static const struct nrf_modem_gnss_agnss_system_data_need *
system_data_need_get(const struct nrf_modem_gnss_agnss_data_frame *request, uint8_t system_id)
{
				   ...
				   enum nrf_cloud_agnss_type *array, const size_t array_size)
{
	...
	if (array_size < NRF_CLOUD_AGNSS__TYPES_COUNT) {
		LOG_ERR("Array size (%d) too small, must be >= %d", array_size,
			NRF_CLOUD_AGNSS__TYPES_COUNT);
		return -ERANGE;
	}
				   ...
				   struct nrf_cloud_pgps_result *const result)
{
	...
	if ((result->host_sz <= strlen(host_ptr)) || (result->path_sz <= strlen(path_ptr))) {
		err = -ENOBUFS;
		goto cleanup;
	}
...
#define ADJ(ptr, sz)                                                                               \
	 ...
	 */
	size_t max_size = strlen(base) + 1 + sizeof(NRF_CLOUD_LOCATION_JSON_KEY_DOREPLY "=false&") +
...
bool nrf_cloud_shadow_app_send_check(struct nrf_cloud_obj_shadow_data *const input)
{
	...
	if (input->type == NRF_CLOUD_OBJ_SHADOW_TYPE_TF) {
		...
		return true;
	} else if ((input->type == NRF_CLOUD_OBJ_SHADOW_TYPE_DELTA) && input->delta->is_err) {
		/* Check delta: if anything is in error, send to app */
		return (cJSON_GetArraySize(input->delta->error.err_obj.json) > 0);
	} else if ((input->type == NRF_CLOUD_OBJ_SHADOW_TYPE_DELTA) && !input->delta->is_err) {
				      ...
				      struct nrf_cloud_obj_shadow_delta *const delta)
{
	...
	/* Check for an error code in the delta response */
	err = nrf_cloud_obj_num_get(shadow_obj, NRF_CLOUD_JSON_KEY_ERR, &num);
	if (err == 0) {
		delta->is_err = true;
		delta->error.code = (int)num;

		err = nrf_cloud_obj_str_get(shadow_obj, NRF_CLOUD_JSON_KEY_ERR_MSG,
			&delta->error.msg);
		if (err) {
			LOG_ERR("Failed to get shadow delta error message");
		}

		LOG_ERR("Shadow delta error %d : %s, ver %d, ts %lld",
			delta->error.code, delta->error.msg, delta->ver, delta->ts);

		/* Only store the JSON pointer without copying the entire structure.
		 * This is just a reference to shadow_obj data, not owned by err_obj.
		 */
		delta->error.err_obj.json = shadow_obj->json;
		/* Not returning a negative error so the application can be notified */
		return 0;
	}
					  ...
					  struct nrf_cloud_obj_shadow_transform *const tf)
{
	...
	if (err == 0) {
		 ...
		 */
		err = nrf_cloud_obj_num_get(shadow_obj, NRF_CLOUD_TRANSFORM_RSP_POS_KEY, &num);
		if (err) {
		 ...
		 * This is just a reference to shadow_obj data, not owned by err_obj.
		 */
		tf->error.err_obj.json = shadow_obj->json;
	} else {
					  ...
					  struct nrf_cloud_data *const response_out)
{
	...
	if (topic_str) {
		err = nrf_cloud_obj_str_add(&ack_obj, NRF_CLOUD_JSON_KEY_TOPIC_PRFX, topic_str,
					    false);
		if (err) {
		...
		cJSON_DeleteItemFromObject(input->delta->state.json, NRF_CLOUD_JSON_KEY_TOPIC_PRFX);
	}

	if (pair_obj.json) {
		err = nrf_cloud_obj_object_add(&ack_obj, NRF_CLOUD_JSON_KEY_PAIRING, &pair_obj,
					       false);
				     ...
				     struct nrf_cloud_data *const response_out)
{
	...
	ctrl_status = (input->type == NRF_CLOUD_OBJ_SHADOW_TYPE_DELTA) ? NRF_CLOUD_CTRL_REPLY
								       : NRF_CLOUD_CTRL_NO_REPLY;
	...
	if (response_out) {
		...
		err = nrf_cloud_shadow_control_response_encode(
			...
			(ctrl_status == NRF_CLOUD_CTRL_REPLY), response_out);
