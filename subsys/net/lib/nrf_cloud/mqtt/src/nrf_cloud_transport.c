...
/* nRF Cloud's custom delta update topic limited to 1792 KB (device SUB).
 * Functionally identical to the AWS update/delta topic.
 * Returns the delta with metadata.
 * Otherwise, an error message is published to /shadow/update/delta/full/err
 * Not currently used by the nrf_cloud library.
 */
#define NCT_SHDW_TPC_DELTA_FULL "%s/shadow/update/delta/full"

/* nRF Cloud's custom delta update topic error (device SUB).
 * If the delta exceeds the 1792 KB limit, an error message is published to this topic.
 * Not currently used by the nrf_cloud library.
 */
#define NCT_SHDW_TPC_DELTA_FULL_ERR "%s/shadow/update/delta/full/err"

/* nRF Cloud's custom trimmed delta topic limited to 1792 KB (device SUB).
 * Returns the delta without metadata.
 * Otherwise, an error message is published to /shadow/update/delta/trim/err
 */
#define NCT_SHDW_TPC_DELTA_TRIM "%s/shadow/update/delta/trim"

/* nRF Cloud's custom trimmed delta topic error (device SUB).
 * If the delta exceeds the 1792 KB limit, an error message is published to this topic.
 */
#define NCT_SHDW_TPC_DELTA_TRIM_ERR "%s/shadow/update/delta/trim/err"
...
static int nct_settings_set(const char *key, size_t len_rd, settings_read_cb read_cb, void *cb_arg);

#define SETTINGS_NAME			"nrf_cloud"
#define SETTINGS_KEY_PERSISTENT_SESSION "p_sesh"
#define SETTINGS_FULL_PERSISTENT_SESSION                                                           \
	SETTINGS_NAME                                                                              \
	"/" SETTINGS_KEY_PERSISTENT_SESSION

SETTINGS_STATIC_HANDLER_DEFINE(nrf_cloud, SETTINGS_NAME, NULL, nct_settings_set, NULL, NULL);

/* Forward declaration of the event handler registered with MQTT. */
static void nct_mqtt_evt_handler(struct mqtt_client *client, const struct mqtt_evt *evt);
...
static int endp_send(const struct nct_dc_data *dc_data, struct mqtt_utf8 *endp, enum mqtt_qos qos)
{
	...
	LOG_DBG("mqtt_publish: id = %d len = %d, topic: %.*s", publish.message_id,
		dc_data->data.len, publish.message.topic.topic.size,
...
static void nct_topic_lists_populate(void)
{
	...
	for (int idx = 0; idx < CC_RX_LIST_CNT; ++idx) {
		...
		case NCT_CC_OPCODE_UPDATE_DELTA:
			...
			break;
		case NCT_CC_OPCODE_UPDATE_DELTA_ERR:
			nct_cc_rx_list[idx].qos = MQTT_QOS_1_AT_LEAST_ONCE;
			nct_cc_rx_list[idx].topic = nct.cc.e[CC_RX_DELTA_ERR];
			break;
		default:
...
static int nct_topics_populate(void)
{
	...
	ret = allocate_and_format_topic(&nct.cc.e[CC_RX_DELTA], NCT_SHDW_TPC_DELTA_TRIM);
	if (ret) {
		goto err_cleanup;
	}
	ret = allocate_and_format_topic(&nct.cc.e[CC_RX_DELTA_ERR], NCT_SHDW_TPC_DELTA_TRIM_ERR);
	if (ret) {
		goto err_cleanup;
	}
...
int nct_save_session_state(const int session_valid)
{
	...
	ret = settings_save_one(SETTINGS_FULL_PERSISTENT_SESSION, &session_valid,
				sizeof(session_valid));
#endif
	return ret;
}
...
static void nrf_cloud_fota_cb_handler(const struct nrf_cloud_fota_evt *const evt)
{
	...
	case NRF_CLOUD_FOTA_EVT_START: {
		struct nrf_cloud_evt cloud_evt = {.type = NRF_CLOUD_EVT_FOTA_START};
	...
	case NRF_CLOUD_FOTA_EVT_ERROR: {
		...
		struct nrf_cloud_evt cloud_evt = {.type = NRF_CLOUD_EVT_FOTA_ERROR};
	...
	case NRF_CLOUD_FOTA_EVT_JOB_RCVD: {
		struct nrf_cloud_evt cloud_evt = {.type = NRF_CLOUD_EVT_FOTA_JOB_AVAILABLE};
...
static int publish_get_payload(struct mqtt_client *client, size_t length)
{
	if (length > (sizeof(nct.payload_buf) - 1)) {
		LOG_ERR("Length specified:%zd larger than payload_buf:%zd", length,
			sizeof(nct.payload_buf));
		return -EMSGSIZE;
	}
				 ...
				 const struct mqtt_evt *_mqtt_evt)
{
	...
	case MQTT_EVT_PUBLISH: {
		...
		LOG_DBG("MQTT_EVT_PUBLISH: id = %d len = %d, topic = %.*s", p->message_id,
			p->message.payload.len, p->message.topic.topic.size,
			p->message.topic.topic.utf8);

		int err = publish_get_payload(mqtt_client, p->message.payload.len);
	...
	case MQTT_EVT_SUBACK: {
		LOG_DBG("MQTT_EVT_SUBACK: id = %d result = %d", _mqtt_evt->param.suback.message_id,
			_mqtt_evt->result);
		...
		if (_mqtt_evt->param.suback.message_id == NCT_MSG_ID_DC_SUB) {
			...
			if (err) {
				LOG_ERR("Failed to save session state: %d", err);
			}
#if defined(CONFIG_NRF_CLOUD_FOTA)
	...
	case MQTT_EVT_PUBACK: {
		LOG_DBG("MQTT_EVT_PUBACK: id = %d result = %d", _mqtt_evt->param.puback.message_id,
			_mqtt_evt->result);
...
#if defined(CONFIG_NRF_CLOUD_FOTA)
	struct nrf_cloud_fota_init_param fota_init = {.evt_cb = nrf_cloud_fota_cb_handler,
...
static void nct_init_mqtt(void)
{
	if (!mqtt_client_initialized) {
		...
		LOG_DBG("MQTT clean session flag: %u", nct.client.clean_session);
...
int nct_connect(void)
{
	...
	if (IS_ENABLED(CONFIG_NRF_CLOUD_SEND_NONBLOCKING)) {
		...
		if (err == -1) {
			LOG_ERR("Failed to set socket as non-blocking, err: %d", errno);
		...
		struct timeval timeout = {.tv_sec = CONFIG_NRF_CLOUD_SEND_TIMEOUT_SEC};
...
int nct_cc_send(const struct nct_cc_data *cc_data)
{
	...
	struct mqtt_publish_param publish = {
		...
		.message.topic.topic.size = nct_cc_tx_list[cc_data->opcode].topic.size,
		.message.topic.topic.utf8 = nct_cc_tx_list[cc_data->opcode].topic.utf8,
	};
	...
	LOG_DBG("mqtt_publish: id = %d opcode = %d len = %d, topic: %.*s", publish.message_id,
		cc_data->opcode, cc_data->data.len, publish.message.topic.topic.size,
		publish.message.topic.topic.utf8);
	LOG_DBG("payload: %.*s", publish.message.payload.len,
		publish.message.payload.data ? (const char *)publish.message.payload.data : "");
...
{
	struct mqtt_topic subscribe_topic = {.topic = nct.dc.e[DC_RX],
...
int nct_dc_disconnect(void)
{
	...
	const struct mqtt_subscription_list subscription_list = {
		.list = &subscribe_topic, .list_count = 1, .message_id = NCT_MSG_ID_DC_UNSUB};
