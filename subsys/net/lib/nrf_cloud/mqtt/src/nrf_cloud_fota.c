...
static int save_validate_status(const char *const job_id, const enum nrf_cloud_fota_type job_type,
...
static struct nrf_cloud_fota_job current_fota = {.info = {.type = NRF_CLOUD_FOTA_TYPE__INVALID}};

static struct k_spinlock last_job_slock;
static uint8_t last_job[NRF_CLOUD_FOTA_JOB_ID_SIZE];
static struct nrf_cloud_settings_fota_job saved_job = {.type = NRF_CLOUD_FOTA_TYPE__INVALID};
...
int nrf_cloud_modem_fota_completed(const bool fota_success)
{
	...
	(void)save_validate_status(saved_job.id, saved_job.type,
				   fota_success ? NRF_CLOUD_FOTA_VALIDATE_PASS
...
static int build_topic(const char *const client_id, const struct mqtt_utf8 *const endpoint,
		       const char *const topic_str, struct mqtt_utf8 *const topic_out)
{
	int ret;

	char *buf;
	size_t size = endpoint->size + strlen(client_id) + strlen(topic_str) + 1;
	...
	ret = snprintk(buf, size, "%s%s%s", endpoint->utf8, client_id, topic_str);
...
static int publish_validated_job_status(void)
{
	...
	struct nrf_cloud_fota_job job = {.info = {.type = saved_job.type, .id = saved_job.id}};
	...
	case NRF_CLOUD_FOTA_VALIDATE_FAIL:
		...
		break;
	default:
		LOG_ERR("Unexpected job validation status: %d", saved_job.validate);
...
int nrf_cloud_fota_endpoint_set(struct mqtt_client *const client, const char *const client_id,
				const struct mqtt_utf8 *const endpoint)
{
	int ret;

	if (client == NULL || endpoint == NULL || endpoint->utf8 == NULL || endpoint->size == 0 ||
	    client_id == NULL) {
		return -EINVAL;
	}
	...
	ret = build_topic(client_id, endpoint, TOPIC_FOTA_UPDATE, &topic_updt.topic);
	if (ret) {
		goto error_cleanup;
	}

	ret = build_topic(client_id, endpoint, TOPIC_FOTA_REQ, &topic_req.topic);
	if (ret) {
		goto error_cleanup;
	}
	...
	ret = build_topic(client_id, endpoint, BLE_TOPIC_FOTA_UPDATE, &topic_ble_updt.topic);
	if (ret) {
		goto error_cleanup;
	}

	ret = build_topic(client_id, endpoint, BLE_TOPIC_FOTA_REQ, &topic_ble_req.topic);
	if (ret) {
		goto error_cleanup;
	}
#endif
...
{
	struct mqtt_subscription_list sub_list = {.list = sub_topics,
						  .list_count = ARRAY_SIZE(sub_topics),
						  .message_id = NCT_MSG_ID_FOTA_SUB};

	for (int i = 0; i < sub_list.list_count; ++i) {
		if (sub_list.list[i].topic.size == 0 || sub_list.list[i].topic.utf8 == NULL) {
			return -EFAULT;
		}
...
static void http_fota_handler(const struct fota_download_evt *evt)
{
	...
	case FOTA_DOWNLOAD_EVT_FINISHED:
		...
		save_validate_status(current_fota.info.id, current_fota.info.type,
	...
	case FOTA_DOWNLOAD_EVT_PROGRESS:
		...
		if ((current_fota.sent_dl_progress == 100) && (current_fota.dl_progress == 100)) {
			break;
		}

		/* Reset if new progress is less than previous */
		if (current_fota.sent_dl_progress > current_fota.dl_progress) {
		...
		if (current_fota.dl_progress != 100 &&
		    (((current_fota.dl_progress - current_fota.sent_dl_progress) <
		       ...
		       const struct nrf_cloud_fota_job *const job)
{
	...
	struct nrf_cloud_fota_evt evt = {.id = id, .status = job->status, .type = job->info.type};
...
{
	__ASSERT_NO_MSG(job != NULL);
	LOG_DBG("%s() - ID: %s", __func__, job->info.id ? job->info.id : "N/A");
...
static int publish(const struct mqtt_publish_param *const pub)
{
	...
	LOG_DBG("Payload (%d bytes): %s", pub->message.payload.len,
...
static int handle_mqtt_evt_publish(const struct mqtt_evt *evt)
{
	...
	if (strstr(sub_topics[SUB_TOPIC_IDX_BLE_RCV].topic.utf8, p->message.topic.topic.utf8) !=
	    NULL) {
	...
	if (strstr(sub_topics[SUB_TOPIC_IDX_RCV].topic.utf8, p->message.topic.topic.utf8) == NULL &&
	    !ble_id) {
		/* This MQTT event is not intended nRF Cloud FOTA */
		return 1;
	}

	LOG_DBG("MQTT_EVT_PUBLISH: id = %d len = %d", p->message_id, p->message.payload.len);
	...
	if (nrf_cloud_fota_job_decode(job_info, ble_id, &input)) {
		...
		LOG_DBG("Job ID: %s, type: %d, size: %d, file: %s/%s", job_info->id, job_info->type,
			job_info->file_size, job_info->host, job_info->path);
	}
...
int nrf_cloud_fota_mqtt_evt_handler(const struct mqtt_evt *evt)
{
	...
	case MQTT_EVT_PUBACK: {
		...
		LOG_DBG("MQTT_EVT_PUBACK: msg id %d", evt->param.puback.message_id);
