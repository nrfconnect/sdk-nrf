...
int nfsm_handle_incoming_event(const struct nct_evt *nct_evt, enum nfsm_state state)
{
	int err;

	if ((nct_evt == NULL) || (nct_evt->type >= NCT_EVT_TOTAL) || (state >= STATE_TOTAL)) {
	...
	if (state_event_handlers[state][nct_evt->type] != NULL) {
		...
		if (err) {
			LOG_ERR("Handler failed! state: %d, type: %d", state, nct_evt->type);
		}
	} else {
...
static int state_ua_pin_complete(void)
{
	...
	err = nrf_cloud_state_encode(STATE_UA_PIN_COMPLETE, c2d_topic_modified, add_shadow_info,
				     &msg.data);
	if (err) {
		LOG_ERR("nrf_cloud_state_encode failed %d", err);
		return err;
...
{
	int err;
	struct nrf_cloud_evt evt = {.type = NRF_CLOUD_EVT_TRANSPORT_CONNECTED,
	...
	if (!persistent_session) {
		...
		struct nct_evt nevt = {.type = NCT_EVT_CC_CONNECTED, .status = 0};
...
static int cc_connection_handler(const struct nct_evt *nct_evt)
{
	 ...
	 */
	int err;
	const struct nrf_cloud_evt evt = {.type = NRF_CLOUD_EVT_ERROR, .status = nct_evt->status};
	/* Transform request for desired topic prefix and pairing. */
	const char cc_req_transform_topic_pairing[] =
	"{ \"" NRF_CLOUD_JSON_KEY_TOPIC_PRFX "\": state.desired." NRF_CLOUD_JSON_KEY_TOPIC_PRFX ", "
	"\"" NRF_CLOUD_JSON_KEY_PAIRING "\": state.desired." NRF_CLOUD_JSON_KEY_PAIRING " }";
	...
	/* Only request the topic prefix and pairing state. */
	err = nrf_cloud_shadow_transform_request(cc_req_transform_topic_pairing, 0);
	if (err) {
		nfsm_set_current_state_and_notify(STATE_CONNECTED, &evt);
		return err;
	}
...
static int set_endpoint_data(const struct nrf_cloud_obj_shadow_data *const input)
{
	...
	c2d_topic_modified =
		nrf_cloud_set_wildcard_c2d_topic((char *)eps.e[DC_RX].utf8, eps.e[DC_RX].size);
...
{
	struct nct_cc_data msg = {.opcode = NCT_CC_OPCODE_UPDATE,
				  .message_id = NCT_MSG_ID_STATE_REPORT};
...
static int cc_rx_data_handler(const struct nct_evt *nct_evt)
{
	...
	LOG_DBG("CC RX on topic [%d] %.*s: %s", nct_evt->param.cc->opcode,
		nct_evt->param.cc->topic.len, (const char *)nct_evt->param.cc->topic.ptr,
	...
	} else if (nct_evt->param.cc->opcode == NCT_CC_OPCODE_UPDATE_DELTA_ERR) {
		err = nrf_cloud_obj_shadow_delta_decode(&shadow_obj, &shadow_delta);
		if (!err) {
			shadow_data.type = NRF_CLOUD_OBJ_SHADOW_TYPE_DELTA;
			shadow_data.delta = &shadow_delta;
			LOG_DBG("Delta shadow error decoded");
		}
	...
	if (shadow_data.delta->is_err == false || shadow_data.transform->is_err == false) {
	...
	if (nrf_cloud_shadow_app_send_check(&shadow_data)) {
		...
		struct nrf_cloud_evt cloud_evt = {.type = NRF_CLOUD_EVT_RX_DATA_SHADOW,
...
static int cc_tx_ack_handler(const struct nct_evt *nct_evt)
{
	...
	if (nct_evt->param.message_id == NCT_MSG_ID_STATE_REQUEST) {
		...
		if (!persistent_session) {
			...
			struct nct_evt nevt = {.type = NCT_EVT_DC_CONNECTED, .status = 0};
		...
		struct nrf_cloud_evt evt = {.type = NRF_CLOUD_EVT_SENSOR_DATA_ACK,
...
static int dc_connection_handler(const struct nct_evt *nct_evt)
{
	struct nrf_cloud_evt evt = {.type = NRF_CLOUD_EVT_READY, .status = nct_evt->status};
