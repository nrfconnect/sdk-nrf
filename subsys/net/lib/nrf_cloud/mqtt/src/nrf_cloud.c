...
void nfsm_set_current_state_and_notify(enum nfsm_state state, const struct nrf_cloud_evt *evt)
{
	bool discon_evt = (evt != NULL) && (evt->type == NRF_CLOUD_EVT_TRANSPORT_DISCONNECTED);
...
int nrf_cloud_init(const struct nrf_cloud_init_param *param)
{
	...
	if (current_state != STATE_IDLE || atomic_get(&uninit_in_progress)) {
		return -EACCES;
	}
	...
	if (param->fmfu_dev_inf || IS_ENABLED(CONFIG_DFU_TARGET_FULL_MODEM_USE_EXT_PARTITION)) {
...
{
	int err = 0;
	struct nrf_cloud_tx_data tx_data = {.topic_type = NRF_CLOUD_TOPIC_STATE,
...
int nrf_cloud_send(const struct nrf_cloud_tx_data *msg)
{
	...
	struct nrf_cloud_data send_data = {.ptr = msg->data.ptr, .len = msg->data.len};
...
int nrf_cloud_shadow_transform_request(char const *const transform, const size_t max_response_len)
{
	...
	int ret;
	struct nrf_cloud_tx_data msg = {.obj = &req_obj,
					.qos = MQTT_QOS_1_AT_LEAST_ONCE,
...
start:
	...
	while (true) {
		...
		if (ret < 0) {
			LOG_ERR("poll() returned an error: %d; revents: 0x%x", ret, fds[0].revents);
...
K_THREAD_DEFINE(nrfcloud_connection_poll_thread, CONFIG_NRF_CLOUD_CONNECTION_POLL_THREAD_STACK_SIZE,
		nrf_cloud_run, NULL, NULL, NULL, K_LOWEST_APPLICATION_THREAD_PRIO, 0, 0);
#endif
