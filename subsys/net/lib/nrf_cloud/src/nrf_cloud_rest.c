...
#define API_GET_FOTA_URL_TEMPLATE	API_VER "/fota-job-executions/%s/latest"
#define API_UPDATE_FOTA_URL_TEMPLATE	API_VER "/fota-job-executions/%s/%s"
#define API_UPDATE_FOTA_BODY_TEMPLATE	"{\"status\":\"%s\"}"
#define API_UPDATE_FOTA_DETAILS_TMPLT	"{\"status\":\"%s\", \"details\":\"%s\"}"

#define API_GET_LOCATION_TEMPLATE	API_VER "/location/locate/%s"

#define API_GET_AGPS_BASE		API_VER "/location/agps?" DEV_ID
	...
	struct nrf_cloud_cell_pos_result *const result)
{
	...
	if (result) {
		ret = nrf_cloud_parse_cell_pos(rest_ctx->response, result);
				 ...
				 struct nrf_cloud_pgps_result *const result)
{
	...
	__ASSERT_NO_MSG(request != NULL);
	__ASSERT_NO_MSG(request->device_id != NULL);
	/* result is not required, user can parse the response if they wish */
	...
	if (result) {
		ret = nrf_cloud_parse_pgps_response(rest_ctx->response,
						    result);
	}
...
int nrf_cloud_rest_jitp(const sec_tag_t nrf_cloud_sec_tag)
{
	...
	if (ret == 0) {
		if ((rest_ctx.status == 0) && (rest_ctx.response_len == 0)) {
			 ...
			 */
		} else if ((rest_ctx.status == HTTP_STATUS_FORBIDDEN) &&
			   (rest_ctx.response_len > 0) &&
