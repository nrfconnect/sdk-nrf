...
#define HDR_RANGE_BYTES_SZ                                                                         \
	(sizeof(HDR_RANGE_BYTES_TEMPLATE) + UINT32_MAX_STR_SZ + UINT32_MAX_STR_SZ)
...
static int generate_auth_header(const char *const tok, char **auth_hdr_out)
{
	...
	*auth_hdr_out = nrf_cloud_malloc(prefix_len + tok_len + postfix_len + 1);
	...
	if (!tok) {
		int err = nrf_cloud_jwt_generate(CONFIG_NRF_CLOUD_REST_AUTOGEN_JWT_VALID_TIME_S,
						 tok_ptr, tok_len + 1);
...
static void rest_error_response_log(struct nrf_cloud_rest_context *const rest_ctx)
{
	...
	if (rest_ctx->response && rest_ctx->response_len > 0) {
		LOG_ERR("Response body: %.*s", MIN(rest_ctx->response_len, 256),
			rest_ctx->response);
	}
}
				       ...
				       const char *const device_id, const char *const shadow_json)
{
	...
	char *const headers[] = {HDR_ACCEPT_APP_JSON, (char *const)auth_hdr, CONTENT_TYPE_APP_JSON,
				 NULL};
...
int nrf_cloud_rest_shadow_device_status_update(
	struct nrf_cloud_rest_context *const rest_ctx, const char *const device_id,
					      ...
					      const struct nrf_cloud_svc_info *const svc_inf)
{
	...
	const struct nrf_cloud_device_status dev_status = {
		.modem = NULL, .svc = (struct nrf_cloud_svc_info *)svc_inf};
				...
				struct nrf_cloud_fota_job_info *const job)
{
	...
	/* Format API URL with device ID */
	url_sz = sizeof(API_GET_FOTA_URL_TEMPLATE) + strlen(device_id);
	...
	char *const headers[] = {HDR_ACCEPT_APP_JSON, (char *const)auth_hdr, CONTENT_TYPE_ALL,
				 NULL};
				...
				struct nrf_cloud_location_result *const result)
{
	...
	bool do_reply =
		request->config ? request->config->do_reply : NRF_CLOUD_LOCATION_DOREPLY_DEFAULT;
	size_t url_size =
		nrf_cloud_ground_fix_url_encode(NULL, 0, API_GET_LOCATION, request->config);
	...
	ret = nrf_cloud_obj_location_request_payload_add(&payload_obj, request->cell_info,
							 request->wifi_info);
	if (ret) {
				  ...
				  struct nrf_cloud_rest_agnss_result *const result)
{
	...
	if ((request->type == NRF_CLOUD_REST_AGNSS_REQ_CUSTOM) && (request->agnss_req == NULL)) {
...
#define AGNSS_UPDATE_PERIOD ((120 - MARGIN_MINUTES) * 60 * MSEC_PER_SEC)
	...
	case NRF_CLOUD_REST_AGNSS_REQ_CUSTOM:
		type_count = nrf_cloud_agnss_type_array_get(request->agnss_req, types,
							    ARRAY_SIZE(types));
		break;
	...
	case NRF_CLOUD_REST_AGNSS_REQ_ASSISTANCE: {
		...
		assist.data_flags = NRF_MODEM_GNSS_AGNSS_GPS_UTC_REQUEST |
	...
	char *const headers[] = {HDR_ACCEPT_ALL, (char *const)auth_hdr, (char *const)range_hdr,
				 CONTENT_TYPE_APP_JSON, NULL};
	...
	do {
		/* Format range header */
		ret = format_range_header(range_hdr, sizeof(range_hdr), rcvd_bytes,
		...
		memcpy(&result->buf[pos], rest_ctx->response, rest_ctx->response_len);
...
int nrf_cloud_rest_jitp(const sec_tag_t nrf_cloud_sec_tag)
{
	...
	char *const headers[] = {HDR_ACCEPT_ALL, JITP_CONTENT_TYPE_HDR, JITP_HOST_HDR,
				 JITP_CONNECTION_HDR, NULL};
	struct rest_client_req_context req;
	struct rest_client_resp_context resp;
	...
	if (ret == 0) {
		if ((resp.http_status_code == NRF_CLOUD_HTTP_STATUS_NONE) &&
			 ...
			 */
			   ...
			   (resp.response_len > 0) && (strstr(resp.response, "\"message\":null"))) {
...
int nrf_cloud_rest_send_device_message(struct nrf_cloud_rest_context *const rest_ctx,
				       const char *const device_id, const char *const json_msg,
				       const bool bulk, const char *const topic)
{
	__ASSERT_NO_MSG(rest_ctx != NULL);
	__ASSERT_NO_MSG(device_id != NULL);
	...
	if (cJSON_AddStringToObjectCS(root_obj, NRF_CLOUD_REST_TOPIC_KEY, (d2c ? d2c : topic)) ==
	    NULL) {
		ret = -ENOMEM;
		goto clean_up;
	}
...
int nrf_cloud_rest_device_status_message_send(
	struct nrf_cloud_rest_context *const rest_ctx, const char *const device_id,
	const struct nrf_cloud_device_status *const dev_status, const int64_t timestamp_ms)
{
	__ASSERT_NO_MSG(rest_ctx != NULL);
	__ASSERT_NO_MSG(device_id != NULL);
					    ...
					    char const *const transform)
{
	...
	char *const headers[] = {HDR_ACCEPT_APP_JSON, (char *const)auth_hdr, NULL};
