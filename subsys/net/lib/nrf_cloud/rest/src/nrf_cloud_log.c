...
LOG_MODULE_REGISTER(nrf_cloud_log_rest, CONFIG_NRF_CLOUD_LOG_LOG_LEVEL);

#define REST_WAIT_MS 1000

static K_SEM_DEFINE(ncl_active, 1, 1);

int nrf_cloud_rest_log_send(struct nrf_cloud_rest_context *ctx, const char *dev_id, int log_level,
			    const char *fmt, ...)
{
	...
	err = nrf_cloud_rest_send_device_message(ctx, dev_id, output.data.ptr, false, NULL);
