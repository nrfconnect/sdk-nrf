...
static void http_fota_handler(const struct fota_download_evt *evt)
{
	...
	case FOTA_DOWNLOAD_EVT_ERROR:
		...
		/* Only reset while the connection is up. If the update could not be published
		 * because the connection is down, keep the error state, the job ID and the cause,
		 * so that on_connack_evt() retries the failed job execution update after the next
		 * reconnect. This matches how FOTA_DOWNLOAD_EVT_FINISHED handles a failed update.
		 */
		if (err < 0 && connected) {
			reset_library();
		}
