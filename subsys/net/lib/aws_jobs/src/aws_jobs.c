		     ...
		     bool subscribe)
{
	...
	if (subscribe) {
		LOG_DBG("Subscribe: %s", topic.topic.utf8);
		return mqtt_subscribe(client, &subscription_list);
	}

	LOG_DBG("Unsubscribe: %s", topic.topic.utf8);
		   ...
		   size_t payload_data_len, uint8_t *topic_buf)
{
	...
	LOG_DBG("Publish topic: %s", topic.topic.utf8);
	LOG_DBG("Publish payload %s", payload_data);
