...
{
	otError error;
	otMeshDiagRouterInfo aRouterInfo;
	uint8_t router_flags;

	error = nrf_rpc_decode_uint(ctx);
	nrf_rpc_decode_buffer(ctx, aRouterInfo.mExtAddress.m8, OT_EXT_ADDRESS_SIZE);
	aRouterInfo.mRloc16 = nrf_rpc_decode_uint(ctx);
	aRouterInfo.mRouterId = nrf_rpc_decode_uint(ctx);
	aRouterInfo.mVersion = nrf_rpc_decode_uint(ctx);
	router_flags = nrf_rpc_decode_uint(ctx);
	nrf_rpc_decode_buffer(ctx, aRouterInfo.mLinkQualities, OT_NETWORK_MAX_ROUTER_ID + 1);
	aRouterInfo.mIp6AddrIterator = (otMeshDiagIp6AddrIterator *)nrf_rpc_decode_uint(ctx);
	aRouterInfo.mChildIterator = (otMeshDiagChildIterator *)nrf_rpc_decode_uint(ctx);

	aRouterInfo.mIsThisDevice = (bool)(router_flags & BIT(0));
	aRouterInfo.mIsThisDeviceParent = (bool)(router_flags & BIT(1));
	aRouterInfo.mIsLeader = (bool)(router_flags & BIT(2));
	aRouterInfo.mIsBorderRouter = (bool)(router_flags & BIT(3));
	...
	if (mesh_diag_discover_cb != NULL) {
		mesh_diag_discover_cb(error, &aRouterInfo, mesh_diag_discover_cb_context);
	}
