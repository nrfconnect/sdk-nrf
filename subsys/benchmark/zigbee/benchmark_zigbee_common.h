...
enum zigbee_benchmark_ctrl {
	/**< Notify the receiver about the intention to start the test. */
	TEST_START_REQUEST = 0x00,
	/**< Notify the receiver about the intention to stop the test. */
	TEST_STOP_REQUEST = 0x01,
	/**< Notify the receiver about the intention to receive test results. */
	TEST_RESULTS_REQUEST = 0x02,
	/**< Notify the receiver that the payload contains remote peer's test results. */
	TEST_RESULTS_RESPONSE = 0x03,
	/**< Notify the received about the intention to change its tx power. */
	TEST_SET_TX_POWER = 0x04,
	/**< Notify the received about the intention to get its tx power. */
	TEST_GET_TX_POWER = 0x05,
	/**< Provide the receiver with a current configuration of sender's tx power. */
	TEST_TX_POWER_RESPONSE = 0x06,
	/**<
	 * Request the receiver to open the network for joining, doesn't have any effect
	 * if the receiver isn't the network coordinator
	 */
	TEST_OPEN_NETWORK_REQUEST = 0x07,
	/**< Request the receiver to perform device reset. */
	TEST_RESET_REQUEST = 0x08,
};

struct benchmark_address_context {
	/**<
	 * Remember only network address. EUI64 address is stored as device_id
	 * inside global device table.
	 */
 ...
 */
void benchmark_results_received(struct benchmark_result *p_result);
