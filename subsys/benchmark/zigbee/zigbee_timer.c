...
void timer_set(uint32_t timeout_ms, timer_cb_t p_callback)
{
	...
	if (m_enabled == ZB_TRUE) {
		m_timeout_beacon_int =
			ZB_TIMER_GET() + ZB_MILLISECONDS_TO_BEACON_INTERVAL(m_timeout_ms);
		zb_err_code =
			ZB_SCHEDULE_APP_ALARM(timer_timeout_zboss_cb, 0,
					      ZB_MILLISECONDS_TO_BEACON_INTERVAL(m_timeout_ms));
		m_timeout_ms = 0;
	}
}
...
void timer_stop(void)
{
	if (m_enabled == ZB_TRUE) {
		...
		if (m_timeout_beacon_int > ZB_TIMER_GET()) {
			m_timeout_ms = ZB_TIME_BEACON_INTERVAL_TO_MSEC(m_timeout_beacon_int -
								       ZB_TIMER_GET());
		} else {
			m_timeout_ms = 0;
		}

		(void)ZB_SCHEDULE_APP_ALARM_CANCEL(timer_timeout_zboss_cb, ZB_ALARM_ANY_PARAM);
	}
}
