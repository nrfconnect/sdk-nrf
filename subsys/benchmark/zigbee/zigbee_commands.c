...
void protocol_cmd_peer_db_get(const struct shell *shell, const struct benchmark_peer_db *p_peers)
{
	...
	for (uint16_t i = 0; i < p_peers->peer_count; i++) {
		peers_info_len += sprintf(peers_info + peers_info_len, "\r\n[%d]:  %08x%08x  %04x",
					  i, DEVICE_ID_HI(p_peers->peer_table[i].device_id),
...
void protocol_cmd_config_get(const struct shell *shell)
{
	...
	shell_info(shell, "Device ID:   %08x%08x", DEVICE_ID_HI(device_id),
...
static void tx_power_cmd_local(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (argc == 1) {
		...
		shell_error(
			shell,
			"Invalid value (%s) The tx power can be set to value between: <-127, 127>",
			argv[1]);
	}
}
...
static void device_reset_request(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (parse_hex_u16(argv[1], &peer_addr)) {
		shell_info(shell, "Resetting remote peer %.4x", peer_addr);
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	bm_zigbee_extension,
	SHELL_CMD_ARG(tx_power_local, NULL, "Get/Set tx power of the local device",
		      tx_power_cmd_local, 1, 1),
	SHELL_CMD_ARG(tx_power_get, NULL, "Get radio power of the remote peer", tx_power_get_remote,
		      2, 0),
	SHELL_CMD_ARG(tx_power_set, NULL, "Set radio power of the remote peer", tx_power_set_remote,
		      3, 0),
	SHELL_CMD_ARG(open_network, NULL, "Request the network coordinator to open the network",
		      open_network_request, 1, 0),
