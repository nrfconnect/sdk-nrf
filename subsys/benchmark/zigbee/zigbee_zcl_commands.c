...
/**
 * @brief ZCL Frame control field of Zigbee benchmark commands that expects command
 *        execution result.
 */
#define ZIGBEE_BENCHMARK_FRAME_CONTROL_FIELD                                                       \
...
/**
 * @brief ZCL Frame control field of Zigbee benchmark commands, that does not expect command
 *        execution result.
 */
#define ZIGBEE_BENCHMARK_FRAME_CONTROL_FIELD_NORESP                                                \
	ZB_ZCL_CONSTRUCT_FRAME_CONTROL(ZB_ZCL_FRAME_TYPE_CLUSTER_SPECIFIC,                         \
				       ZB_ZCL_NOT_MANUFACTURER_SPECIFIC,                           \
				       ZB_ZCL_FRAME_DIRECTION_TO_SRV,                              \
...
static zb_uint8_t m_seq_num; /* ZCL sequence number for benchmark commands. */

/**@brief Forward declaration of function that sends local test results over Zigbee network. */
static zb_zcl_status_t
zigbee_benchmark_peer_results_response_send(zb_bufid_t bufid, zb_uint16_t peer_addr,
					    struct benchmark_result *p_results);
...
static zb_zcl_status_t zigbee_benchmark_send_tx_power_response(zb_bufid_t bufid,
							       zb_uint16_t remote_addr);
...
zb_uint8_t zigbee_benchmark_ep_handler(zb_bufid_t bufid)
{
	...
	case TEST_RESULTS_REQUEST:
		...
		zb_zcl_status = zigbee_benchmark_peer_results_response_send(
			bufid, remote_short_addr, zigbee_benchmark_local_result_get());
	...
	case TEST_RESULTS_RESPONSE:
		...
		if (zb_buf_len(bufid) != sizeof(struct benchmark_result)) {
			zigbee_benchmark_test_abort();
		} else {
			zigbee_benchmark_results_received(
				(struct benchmark_result *)zb_buf_begin(bufid));
		}
		break;
	...
	case TEST_SET_TX_POWER:
		if (bufid && (zb_buf_len(bufid) == sizeof(struct benchmark_tx_power))) {
			zb_zcl_status = zigbee_benchmark_set_tx_power(
				(struct benchmark_tx_power *)zb_buf_begin(bufid));
		} else {
	...
	case TEST_TX_POWER_RESPONSE:
		if (zb_buf_len(bufid) == sizeof(struct benchmark_tx_power)) {
			LOG_INF("Tx power of a remote peer (0x%04x) is %d", remote_short_addr,
	...
	case TEST_OPEN_NETWORK_REQUEST:
		if (zb_get_network_role() == ZB_NWK_DEVICE_TYPE_COORDINATOR) {
			LOG_INF("Remote peer 0x%04x requested to open the network",
				remote_short_addr);
			...
			LOG_WRN("Remote peer 0x%04x requested to open the network, but "
				"the device is not the network coordinator",
				remote_short_addr);
			zb_zcl_status = ZB_ZCL_STATUS_FAIL;
		}
		send_response = ZB_TRUE;
	...
	case ZB_ZCL_CMD_DEFAULT_RESP: {
		...
		zigbee_benchmark_command_response_handler(
			(enum zigbee_benchmark_ctrl)p_def_resp->command_id,
	...
	if (bufid) {
		if (send_response) {
			...
			LOG_DBG("Send (benchmark) default response. Command: %u, status: %u",
				p_cmd_info->cmd_id, zb_zcl_status);
			ZB_ZCL_SEND_DEFAULT_RESP(bufid, remote_short_addr,
						 ZB_APS_ADDR_MODE_16_ENDP_PRESENT, cli_ep, cli_ep,
						 ZB_AF_HA_PROFILE_ID, BENCHMARK_CUSTOM_CLUSTER,
						 m_seq_num++, cmd_id, zb_zcl_status);
		} else {
			zb_buf_free(bufid);
		}
	}
 ...
 */
static zb_ret_t zigbee_benchmark_ctrl_request_send(zb_bufid_t bufid, zb_uint16_t peer_addr,
						   enum zigbee_benchmark_ctrl ctrl_cmd)
{
	zb_uint8_t cli_ep = zb_shell_get_endpoint();
	...
	*(p_cmd_buf++) = ctrl_cmd; /* Command ID field */
	...
	zb_err_code =
		zb_zcl_finish_and_send_packet(bufid, p_cmd_buf, &remote_addr,
					      ZB_APS_ADDR_MODE_16_ENDP_PRESENT, cli_ep, cli_ep,
					      ZB_AF_HA_PROFILE_ID, BENCHMARK_CUSTOM_CLUSTER,
 ...
 * @param[in] p_result   Pointer to the benchmark test results, that will be copied
 *                       to the Zigbee command frame.
 *
...
static zb_zcl_status_t
zigbee_benchmark_peer_results_response_send(zb_bufid_t bufid, zb_uint16_t peer_addr,
					    struct benchmark_result *p_result)
{
	zb_uint8_t cli_ep = zb_shell_get_endpoint();
	zb_ret_t zb_err_code;
	zb_uint8_t *p_cmd_buf;
	...
	*(p_cmd_buf++) = TEST_RESULTS_RESPONSE;
	memcpy(p_cmd_buf, p_result, sizeof(struct benchmark_result));
	p_cmd_buf += sizeof(struct benchmark_result);
	m_seq_num++;
...
{
	zb_ret_t err_code =
		zigbee_benchmark_ctrl_request_send(bufid, peer_addr, TEST_RESULTS_REQUEST);

	if (err_code != RET_OK) {
		LOG_DBG("Sending results request (ID: %u) to the remote peer (nwk_addr: 0x%04x) "
			"failed with error: %d",
...
void zigbee_benchmark_get_tx_power_remote(zb_bufid_t bufid, zb_uint16_t peer_addr)
{
	...
	if (err_code != RET_OK) {
		LOG_ERR("Sending GET TX POWER request (ID: %u) to the remote peer"
			" (nwk_addr: 0x%04x) failed with error: %d",
...
{
	zb_ret_t err_code =
		zigbee_benchmark_ctrl_request_send(bufid, peer_addr, TEST_RESET_REQUEST);

	if (err_code != RET_OK) {
		LOG_ERR("Sending RESET DEVICE request (ID: %u) to the remote peer "
			"(nwk_addr: 0x%04x) failed with error: %d",
...
{
	zb_ret_t err_code =
		zigbee_benchmark_ctrl_request_send(bufid, 0x0000, TEST_OPEN_NETWORK_REQUEST);
...
{
	zb_ret_t err_code =
		zigbee_benchmark_ctrl_request_send(bufid, peer_addr, TEST_START_REQUEST);

	if (err_code != RET_OK) {
		LOG_DBG("Sending test start request (ID: %u) to the remote peer (nwk_addr: 0x%04x) "
			"failed with error: %d",
...
void zigbee_benchmark_peer_stop_request_send(zb_bufid_t bufid, zb_uint16_t peer_addr)
{
	...
	if (err_code != RET_OK) {
		LOG_DBG("Sending test stop request (ID: %u) to the remote peer (nwk_addr: 0x%04x) "
			"failed with error: %d",
