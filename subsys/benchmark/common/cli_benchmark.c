...
static void print_test_results(union benchmark_event_context *p_context);

struct shell *p_shell;
static struct benchmark_peer_db *mp_peer_db;
static struct benchmark_configuration m_test_configuration = {
	.length = 64,
	.ack_timeout = 200,
	.count = 1000,
...
static const char *configuration_mode_name_get(enum benchmark_mode *p_mode)
{
	switch (*(p_mode)) {
...
static void discovered_peers_print(const struct shell *shell)
{
	if (mp_peer_db != NULL) {
		...
		print_error(shell,
			    "The list of known peers is empty. Discover peers and try again.");
	}
}

static void benchmark_evt_handler(struct benchmark_evt *p_evt)
{
	uint16_t peer_count;
	...
	case BENCHMARK_TEST_STOPPED:
		...
		if (!protocol_is_error(p_evt->context.error)) {
			...
			shell_info(p_shell, "Test stopped with errors. Error code: %u",
				   p_evt->context.error);
			if (p_shell) {
				shell_error(p_shell,
					    "Error: Test stopped with errors. Error code: %u\r\n",
					    p_evt->context.error);
			}
		}
		break;
	...
	default:
		cli_suppress_disable();
		shell_info(p_shell, "Unknown benchmark_evt.");
		break;
	};
}
...
void cmd_config_get(const struct shell *shell, size_t argc, char **argv)
{
	shell_info(shell,
		   ...
		   m_test_configuration.ack_timeout, m_test_configuration.count,
...
static void parse_decimal(char *buf_out, const char *p_description, const char *p_unit,
			  uint32_t value)
{
	if (value != BENCHMARK_COUNTERS_VALUE_NOT_SUPPORTED) {
		...
		sprintf(buf_out, "%s: %u.%02u%s", p_description, value_int, value_remainder,
			p_unit);
	} else {
...
static void dump_config(struct benchmark_configuration *p_config)
{
	...
	shell_info(p_shell,
		   ...
		   "\r\n        Count: %u"
		   "\r\n        Mode: %s",
		   p_config->length, p_config->ack_timeout, p_config->count, modes[p_config->mode]);
}

static void dump_status(struct benchmark_status *p_status)
{
	shell_info(p_shell,
		   "\r\n        Test in progress: %s"
		   "\r\n        Reset counters: %s"
		   ...
		   p_status->reset_counters ? "True" : "False", p_status->acks_lost,
		   p_status->waiting_for_ack, p_status->packets_left_count, p_status->frame_number);
	...
	if (m_test_configuration.mode == BENCHMARK_MODE_ECHO) {
		...
		if (p_status->latency.cnt > 0) {
			avg = (uint32_t)(p_status->latency.sum * DECIMAL_PRECISION /
					 (p_status->latency.cnt));
		}
...
static void dump_result(struct benchmark_result *p_result)
{
	shell_info(p_shell,
		   "\r\nDuration: %u ms"
		   "\r\nApp counters:"
...
static void foreach_callback(const struct k_thread *thread, void *user_data)
{
	...
	((k_thread_runtime_stats_t *)user_data)->execution_cycles += stats.execution_cycles;

	printk("Thread: \"%s\" used %llu (%f.2\%)\n", k_thread_name_get((k_tid_t)thread),
	       stats.execution_cycles, stats.execution_cycles * 100.0 / all_ticks);
}
...
static void print_test_results(union benchmark_event_context *p_context)
{
	struct benchmark_evt_results *p_results = &p_context->results;
	...
	if ((p_results->p_local_status != NULL) && (p_results->p_local_result != NULL) &&
	    (p_results->p_local_result->duration != 0)) {
		...
		if (m_test_configuration.mode == BENCHMARK_MODE_ECHO) {
			...
			if (p_results->p_local_status->latency.cnt > 0) {
				avg = (uint32_t)(p_results->p_local_status->latency.sum *
						 DECIMAL_PRECISION /
						 p_results->p_local_status->latency.cnt);
			}
			...
			shell_info(p_shell,
				   ...
				   "\r\n    Min: %u"
				   "\r\n    Max: %u",
				   avg_str, p_results->p_local_status->latency.min,
		...
		if (m_test_configuration.mode == BENCHMARK_MODE_UNIDIRECTIONAL) {
			...
			if (packets_sent != 0) {
				per = (uint32_t)((DECIMAL_PRECISION * 100ULL *
		...
		if (m_test_configuration.mode == BENCHMARK_MODE_UNIDIRECTIONAL) {
			...
			if (mac_tx_attempts != 0) {
				mac_per = (uint32_t)((DECIMAL_PRECISION * 100ULL * mac_tx_errors) /
						     mac_tx_attempts);
			}
			...
			if (p_results->p_remote_result != NULL) {
				uint32_t mac_tx_attempts =
					p_results->p_local_result->mac_tx_counters.total +
					p_results->p_remote_result->mac_tx_counters.total;
				uint32_t mac_tx_errors =
					p_results->p_local_result->mac_tx_counters.error +
					p_results->p_remote_result->mac_tx_counters.error;
				uint32_t mac_per = UINT32_MAX;
				char mac_per_str[40] = "";

				if (mac_tx_attempts != 0) {
					mac_per = (uint32_t)(
						(DECIMAL_PRECISION * 100ULL * mac_tx_errors) /
						mac_tx_attempts);
				}
...
static void cmd_test_start(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (mp_peer_db == NULL) {
		shell_error(shell, "No peer selected; run: \"test peer discover\"\n"
						   " to find peers\n");
		return;
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(test_configure_mode,
			       ...
			       SHELL_CMD_ARG(unidirectional, NULL,
					     "Transmission in the single direction",
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(count, NULL, "Set number of packets to be sent", cmd_config_packet_count, 1,
		      1),
	SHELL_CMD_ARG(length, NULL, "Set UDP payload length in bytes", cmd_config_packet_length, 1,
		      1),
	SHELL_CMD_ARG(mode, &test_configure_mode, "Set test type", cmd_config_mode_get, 2, 0),
	SHELL_SUBCMD_SET_END);

SHELL_STATIC_SUBCMD_SET_CREATE(
	benchmark_cmds,
	SHELL_CMD_ARG(configure, &test_configure_cmds, "Configure the test", cmd_default, 1, 1),
	SHELL_CMD_ARG(info, NULL, "Print current configuration", cmd_info_get, 1, 0),
	SHELL_CMD_ARG(peer, &test_configure_peer, "Configure the peer receiving data", cmd_default,
		      1, 0),
	SHELL_CMD_ARG(start, NULL, "Start the test", cmd_test_start, 1, 0),
	SHELL_CMD_ARG(stop, NULL, "Stop the test", cmd_test_stop, 1, 0), SHELL_SUBCMD_SET_END);
