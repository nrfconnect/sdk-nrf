				  ...
				  void *cb_arg)
{
	...
	if (strncmp(key, FW_LOADER_SETTINGS_ADV_NAME_KEY, key_len) != 0) {
		return -ENOENT;
	}
...
static int fw_loader_settings_get(const char *key, char *val, int val_len_max)
{
	...
	if (!(settings_name_steq(key, FW_LOADER_SETTINGS_ADV_NAME_KEY, &next) && !next)) {
		return -ENOENT;
	}
