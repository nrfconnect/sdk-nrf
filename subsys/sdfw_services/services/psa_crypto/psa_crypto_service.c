 ...
 *
 */
static void *prepare_out_buffer(const void *original_buffer, size_t size)
{
	void *out_buffer;

	if ((IS_ALIGNED(original_buffer, CACHE_GRANULARITY)) &&
	    (IS_ALIGNED(size, CACHE_GRANULARITY))) {
		out_buffer = original_buffer;
	} else {
		out_buffer = k_malloc(size);
		if (out_buffer != NULL) {
 ...
 * @param size Size of the buffer
 */
static void release_out_buffer(void *original_buffer, const void *out_buffer, size_t size)
{
	if (out_buffer == NULL) {
		return;
	}
