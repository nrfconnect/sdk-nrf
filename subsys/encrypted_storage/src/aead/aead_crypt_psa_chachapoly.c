...
psa_status_t encrypted_storage_aead_init(void)
{
	return psa_crypto_init();
}

size_t encrypted_storage_aead_get_encrypted_size(size_t data_size)
{
	return PSA_AEAD_ENCRYPT_OUTPUT_SIZE(PSA_KEY_TYPE_CHACHA20, PSA_ALG_CHACHA20_POLY1305,
					    data_size);
}

static psa_status_t encrypted_storage_aead_psa_crypt(psa_key_usage_t key_usage, const void *key_buf,
...
psa_status_t encrypted_storage_aead_encrypt(const void *key_buf, size_t key_len, const void *nonce_buf,
					 size_t nonce_len, const void *add_buf, size_t add_len,
					 const void *input_buf, size_t input_len, void *output_buf,
					 size_t output_size, size_t *output_len)
{
	return encrypted_storage_aead_psa_crypt(PSA_KEY_USAGE_ENCRYPT, key_buf, key_len, nonce_buf,
					     nonce_len, add_buf, add_len, input_buf, input_len,
					     output_buf, output_size, output_len);
}

psa_status_t encrypted_storage_aead_decrypt(const void *key_buf, size_t key_len, const void *nonce_buf,
					 size_t nonce_len, const void *add_buf, size_t add_len,
					 const void *input_buf, size_t input_len, void *output_buf,
					 size_t output_size, size_t *output_len)
{
	return encrypted_storage_aead_psa_crypt(PSA_KEY_USAGE_DECRYPT, key_buf, key_len, nonce_buf,
					     nonce_len, add_buf, add_len, input_buf, input_len,
					     output_buf, output_size, output_len);
}
