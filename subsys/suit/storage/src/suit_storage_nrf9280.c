...
struct suit_storage_oem_exposed_config {
	...
	uint8_t nordic_exposed_suit_config_version;
	uint8_t reserved_0[15];
	uint8_t modem_manifest_class_id[16];
...
struct suit_storage_nordic {
	...
	union {
		struct suit_storage_area_nordic {
			...
			struct suit_storage_oem_exposed_config
				oem_exposed_config_bak; /* 96 bytes */
...
struct suit_storage_nordic_cell {
	/* Area accessible by the SECURE core debugger. */
	union {
		struct suit_storage_area_nordic_cell {
			/* Installed envelope: Nordic Top */
			uint8_t top[EB_ALIGN(1536)];
			/* Installed envelope: LTE/DECT Firmware */
			uint8_t cellfw[EB_ALIGN(1536)];
			/* Installed envelope: iSIM Firmware*/
			uint8_t isimfw[EB_ALIGN(1024)];
		} nordic_cell;
		uint8_t nordic_cell_area[ACCESS_SIZE(struct suit_storage_area_nordic_cell)];
	};
};
					  ...
					  size_t *size)
{
	...
	struct suit_storage_nordic_cell *nordic_storage_cell =
		(struct suit_storage_nordic_cell *)SUIT_STORAGE_NORDIC_CELL_ADDRESS;
	...
	case SUIT_MANIFEST_SEC_TOP:
		*addr = nordic_storage_cell->nordic_cell.top;
...
suit_plat_err_t suit_storage_purge(suit_manifest_domain_t domain)
{
	...
	struct suit_storage_rad *rad_storage = (struct suit_storage_rad *)SUIT_STORAGE_RAD_ADDRESS;
	struct suit_storage_app *app_storage = (struct suit_storage_app *)SUIT_STORAGE_APP_ADDRESS;
	struct suit_storage_nordic_cell *cell_storage =
		(struct suit_storage_nordic_cell *)SUIT_STORAGE_NORDIC_CELL_ADDRESS;
	const struct device *fdev = SUIT_PLAT_INTERNAL_NVM_DEV;
	suit_plat_err_t ret = SUIT_PLAT_SUCCESS;
	int err = 0;
