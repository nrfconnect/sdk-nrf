						    ...
						    size_t size)
{
	...
	for (size_t i = 0; i < entries_len; i++) {
		...
		if (entries[i].role == role) {
			LOG_ERR("Manifest with role 0x%x%s already configured.", role,
				suit_role_name_get(role));
			LOG_ERR("Current : " SUIT_MANIFEST_CLASS_ID_LOG_FORMAT,
				SUIT_MANIFEST_CLASS_ID_LOG_ARGS(new_class_id));
			LOG_ERR("Previous: " SUIT_MANIFEST_CLASS_ID_LOG_FORMAT,
				SUIT_MANIFEST_CLASS_ID_LOG_ARGS(ex_class_id));
			return SUIT_PLAT_ERR_EXISTS;
		}

		if (suit_metadata_uuid_compare(new_class_id, ex_class_id) == SUIT_PLAT_SUCCESS) {
			LOG_ERR("Manifest class ID assigned for multiple roles (current: 0x%x%s, "
				"previous: 0x%x%s)",
				role, suit_role_name_get(role), entries[i].role,
				suit_role_name_get(entries[i].role));
			LOG_ERR("Class ID: " SUIT_MANIFEST_CLASS_ID_LOG_FORMAT,
				SUIT_MANIFEST_CLASS_ID_LOG_ARGS(new_class_id));
			return SUIT_PLAT_ERR_EXISTS;
		}
	}
