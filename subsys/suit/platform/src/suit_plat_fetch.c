...
static int external_streamer(const uint8_t *src, size_t len, struct stream_sink *sink)
{
#ifndef CONFIG_SUIT_STREAM_SOURCE_CACHE
	return SUIT_ERR_UNSUPPORTED_COMMAND;
#else
	suit_plat_err_t plat_ret = suit_dfu_cache_streamer_stream(src, len, sink);
...
static inline int internal_streamer(const uint8_t *src, size_t len, struct stream_sink *sink)
{
#ifndef CONFIG_SUIT_STREAM_SOURCE_MEMPTR
	return SUIT_ERR_UNSUPPORTED_COMMAND;
#else
	return suit_memptr_streamer_stream(src, len, sink);
#endif /* CONFIG_SUIT_STREAM_SOURCE_MEMPTR */
}
