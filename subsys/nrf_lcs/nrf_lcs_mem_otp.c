...
static int bl_storage_lcs_set(uint32_t address, uint32_t value)
{
#if defined(CONFIG_NRFX_NVMC)
...
{
	uint32_t provisioning = bl_storage_lcs_get((uint32_t)&LCS_OTP_AREA->provisioning);
...
{
	uint32_t secured = bl_storage_lcs_get((uint32_t) &LCS_OTP_AREA->secured);
...
{
	uint32_t decommissioned = bl_storage_lcs_get((uint32_t)&LCS_OTP_AREA->decommissioned);
