...
# Copy check_config.h from Mbed TLS to prevent Oberons PSA core version to be used
# The current version in Oberon PSA core does not resolve legacy vs PSA configs
configure_file(${ARM_MBEDTLS_PATH}/include/mbedtls/check_config.h
  ${generated_include_path}/mbedtls/check_config.h
)
...
if(CONFIG_GENERATE_MBEDTLS_CFG_FILE AND MBEDTLS_PSA_CRYPTO_SPM)
  ...
  # Inside The TF-M secure image:
  # Copy an empty Mbed TLS user config file (default nrf-config-user.h)
  configure_file(${NRF_SECURITY_ROOT}/configs/nrf-config-user-empty.h
    ${generated_include_path}/${CONFIG_MBEDTLS_USER_CONFIG_FILE}
  )

elseif(CONFIG_GENERATE_MBEDTLS_CFG_FILE)

  # NS image or single image builds (i.e. TrustZone not supported/enabled)
  # Generate the mbed TLS config file (default: nrf-config.h)
  configure_file(${NRF_SECURITY_ROOT}/configs/nrf-config.h.template
    ${generated_include_path}/${CONFIG_MBEDTLS_CFG_FILE}
  )

  # Generate the Mbed TLS userconfig file (default nrf-config.h)
  configure_file(${NRF_SECURITY_ROOT}/configs/psa_crypto_config.h.template
    ${generated_include_path}/${CONFIG_MBEDTLS_USER_CONFIG_FILE}
  )
  ...
  message(FATAL_ERROR "GENERATE_MBEDTLS_CFG_FILE is required")
