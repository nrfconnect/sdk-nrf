...
- **nrf_security_utils** (when not BUILD_INSIDE_TFM) links to the kernel by
  path to **libkernel.a** to avoid a cyclic dependency with the kernel target;
  see comment in `src/utils/nrf_security_utils.cmake`.

- **cracen_psa_driver** (when not BUILD_INSIDE_TFM and not OpenThread) depends
  on **zephyr_generated_headers**; when OpenThread is in the build this
  dependency is omitted to avoid a cyclic dependency; see
