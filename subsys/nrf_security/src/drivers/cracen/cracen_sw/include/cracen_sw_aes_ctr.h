 ...
 * @param alg PSA_ALG_CTR, or PSA_ALG_CCM_STAR_NO_TAG which runs as CTR over the
 *            CCM* counter blocks and differs only in cracen_sw_aes_ctr_set_iv().
 *
 ...
 *
 * @param operation Pointer to the cipher operation structure.
 * @param iv 16-octet counter block for CTR, or the 13-octet nonce for CCM*.
 * @param iv_length Length of IV.
 *
 * @retval psa_status_t PSA_SUCCESS on success, error code otherwise
 */
 ...
 * @param key_buffer Key material.
 * @param key_buffer_size Size of the key material.
 * @param alg PSA_ALG_CTR or PSA_ALG_CCM_STAR_NO_TAG.
 * @param iv IV (Counter) value: 16 octets for CTR, the 13-octet nonce for CCM*.
 * @param iv_length Length of IV.
