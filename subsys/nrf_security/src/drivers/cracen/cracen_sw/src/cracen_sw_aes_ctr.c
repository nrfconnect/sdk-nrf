				     ...
				     psa_algorithm_t alg)
{
	...
	/* CCM* shares this path; only set_iv() differs. */
	operation->alg = alg;
				      ...
				      size_t *output_length)
{
	...
	*output_length = 0;
	...
	status = cracen_ccm_star_add_length(operation, input_length);
	if (status != PSA_SUCCESS) {
		return status;
	}
