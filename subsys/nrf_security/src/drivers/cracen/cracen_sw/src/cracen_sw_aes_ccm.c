...
#define CCM_AAD_LONG32_MAX		0xFFFFFFFFu
...
static size_t encode_ccm_ad_length(uint8_t *output, size_t ad_length)
{
	...
	if (ad_length <= CCM_AAD_LONG32_MAX) {
	...
	/* RFC 3610: AAD > 2^32-1 uses markers 0xFF, 0xFF and 8-byte length */
	output[0] = CCM_AAD_MARKER0;
	output[1] = CCM_AAD_MARKER0;
	encode_big_endian_length(output + 2, 8, ad_length, 8);
	return 10;
}
			  ...
			  size_t key_buffer_size, psa_algorithm_t alg)
{
	...
	operation->dir = dir;
	operation->tag_size =
		PSA_AEAD_TAG_LENGTH(PSA_KEY_TYPE_AES, PSA_BYTES_TO_BITS(key_buffer_size), alg);
	return PSA_SUCCESS;
}
					 ...
					 size_t input_length)
{
	...
	status = initialize_cbc_mac(operation);
	if (status != PSA_SUCCESS) {
		return status;
	}
	/* Complete any partial AD block from previous call */
	if (ccm_ctx->has_partial_ad_block) {
		size_t space_in_block = SX_BLKCIPHER_AES_BLK_SZ - ccm_ctx->total_ad_fed;
		size_t to_copy = MIN(input_length, space_in_block);

		memcpy(&ccm_ctx->partial_block[ccm_ctx->total_ad_fed], input, to_copy);
		processed += to_copy;
		ccm_ctx->total_ad_fed += to_copy;
		if (ccm_ctx->total_ad_fed == SX_BLKCIPHER_AES_BLK_SZ) {
...
static psa_status_t ctr_xor(cracen_aead_operation_t *operation, const uint8_t *input,
			    uint8_t *output, size_t length, size_t counter_size)
{
	cracen_sw_ccm_context_t *ccm_ctx = &operation->sw_ccm_ctx;
	struct sxblkcipher cipher;
	psa_status_t status;
				      ...
				      size_t *output_length)
{
	...
	if (operation->dir == CRACEN_ENCRYPT) {
		...
		while (processed < input_length) {
			...
			status = ctr_xor(operation, &input[processed], &output[processed],
				      ...
				      uint8_t *tag, size_t tag_size, size_t *tag_length)
{
	...
	status = generate_tag(operation, tag);
	if (status != PSA_SUCCESS) {
		return status;
	}
				      ...
				      const uint8_t *tag, size_t tag_length)
{
	...
	status = generate_tag(operation, computed_tag);
	if (status != PSA_SUCCESS) {
		goto exit;
	}
