					 ...
					 size_t output_size, size_t *output_length)
{
	...
	return verify_and_remove_pkcs7_padding(decrypted_block, output, output_size, output_length);
}
			      ...
			      enum cipher_operation dir)
{
	...
	*output_length = 0;
	...
	status = cracen_sw_aes_cbc_setup(&operation, attributes, key_buffer, key_buffer_size, dir);
	if (status != PSA_SUCCESS) {
		return status;
	}
