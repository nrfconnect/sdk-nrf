...
#if defined(PSA_NEED_CRACEN_AES_MMO_ZIGBEE)
	if (is_aes_mmo(alg)) {
		cracen_aes_mmo_operation_t operation = {0};
		psa_status_t psa_status = cracen_aes_mmo_update(&operation, input, input_length);

		if (psa_status == PSA_SUCCESS) {
			psa_status =
				cracen_aes_mmo_finish(&operation, hash, hash_size, hash_length);
		}
...
#if defined(PSA_NEED_CRACEN_AES_MMO_ZIGBEE)
	if (is_aes_mmo(alg)) {
		/* The PSA core has already zeroed the operation, which is all the
		 * state AES-MMO needs to start.
		 */
