	...
	*output_length = 0;
	...
	while (remaining_input > 0) {
		...
		if (keystream_used == 0) {
			...
			status = cracen_aes_ecb_encrypt(&operation->keyref, current_ctr,
							keystream_block);
			if (status != PSA_SUCCESS) {
				return status;
			}
		...
		size_t avail = SX_BLKCIPHER_AES_BLK_SZ - keystream_used;
		size_t bytes_to_process = MIN(remaining_input, avail);

		/* XOR keystream slice with input into output */
		memcpy(current_output, current_input, bytes_to_process);
		cracen_xorbytes((char *)current_output,
				(const char *)(keystream_block + keystream_used), bytes_to_process);
...
psa_status_t cracen_sw_aes_ctr_finish(cracen_cipher_operation_t *operation, uint8_t *output,
...
psa_status_t cracen_sw_aes_ctr_encrypt(const psa_key_attributes_t *attributes,
				       const uint8_t *key_buffer, size_t key_buffer_size,
				       ...
				       size_t *output_length)
{
	...
	*output_length = 0;
	...
	status = cracen_sw_aes_ctr_finish(&operation, output + *output_length,
					  output_size - *output_length, &finish_length);
	if (status != PSA_SUCCESS) {
		return status;
	}
...
psa_status_t cracen_sw_aes_ctr_decrypt(const psa_key_attributes_t *attributes,
				       const uint8_t *key_buffer, size_t key_buffer_size,
				       const uint8_t *iv, size_t iv_length, const uint8_t *input,
				       size_t input_length, uint8_t *output, size_t output_size,
				       size_t *output_length)
{
	...
	return cracen_sw_aes_ctr_encrypt(attributes, key_buffer, key_buffer_size, iv, iv_length,
