...
int cracen_rsa_pkcs1v15_encrypt(struct cracen_rsa_key *rsa_key,
			    struct cracen_crypt_text *text, uint8_t *output, size_t output_sz)
{
	int sx_status;
	size_t modulussz = CRACEN_RSA_KEY_OPSZ(rsa_key);
	size_t workmemsz = (PSA_BITS_TO_BYTES(PSA_MAX_RSA_KEY_BITS) + PSA_HASH_MAX_SIZE + 4);
