...
static psa_status_t handle_cracen_key_location(const uint8_t *key_buffer, size_t key_buffer_size,
						       ...
						       size_t data_size, size_t *data_length)
{
	...
	if (PSA_KEY_LIFETIME_GET_LOCATION(psa_get_key_lifetime(attributes)) ==
		...
		return handle_cracen_key_location(key_buffer, key_buffer_size, sx_curve, data,
						  &priv_key, &pub_key);
	} else {
