...
static const cracen_builtin_ikg_key_policy_t g_builtin_ikg_policy[] = {
#ifdef TFM_SP_ITS
	{.owner = TFM_SP_ITS, .key_slot = TFM_BUILTIN_KEY_ID_HUK, .usage = PSA_KEY_USAGE_ALL},
#endif /* TFM_SP_ITS */
#ifdef TFM_SP_PS
	{.owner = TFM_SP_PS, .key_slot = TFM_BUILTIN_KEY_ID_HUK, .usage = PSA_KEY_USAGE_ALL},
#endif /* TFM_SP_PS */
#ifdef TFM_SP_PS_TEST
	{.owner = TFM_SP_PS_TEST, .key_slot = TFM_BUILTIN_KEY_ID_HUK, .usage = PSA_KEY_USAGE_ALL},
	...
	{.owner = MAPPED_TZ_NS_AGENT_DEFAULT_CLIENT_ID,
	 ...
	 .usage = PSA_KEY_USAGE_DERIVE | PSA_KEY_USAGE_VERIFY_DERIVATION},
};
					    ...
					    psa_key_usage_t usage)
{
	for (uint32_t i = 0; i < NRFX_ARRAY_SIZE(g_builtin_ikg_policy); i++) {
		if (g_builtin_ikg_policy[i].owner == owner &&
			...
			return (g_builtin_ikg_policy[i].usage & usage) == usage;
		}
	}
...
bool cracen_builtin_key_user_allowed(const psa_key_attributes_t *attributes, psa_key_usage_t usage)
{
	...
	case PSA_KEY_LOCATION_CRACEN:
		slot_id = key_id;
		return cracen_builtin_ikg_user_allowed(owner, slot_id, usage);

#ifdef PSA_NEED_CRACEN_KMU_DRIVER
	case PSA_KEY_LOCATION_CRACEN_KMU:
		slot_id = CRACEN_PSA_GET_KMU_SLOT(key_id);
		return cracen_builtin_kmu_user_allowed(owner, slot_id);
#endif /* PSA_NEED_CRACEN_KMU_DRIVER */
