...
static int mgf1xor_hash(uint8_t *mgfctr, size_t workmemsz, const struct sxhashalg *hashalg, uint8_t *seed,
			size_t digestsz)
{
	uint8_t *mask_segment = mgfctr + MGF1_COUNTER_SZ;

	if (sx_hash_get_alg_digestsz(hashalg) > workmemsz - MGF1_COUNTER_SZ) {
	...
	return cracen_hash_all_inputs(hash_array, hash_array_lengths, input_count, hashalg, mask_segment);
}

int cracen_run_mgf1xor(uint8_t *workmem, size_t workmemsz, const struct sxhashalg *hashalg,
			uint8_t *seed, size_t digestsz, uint8_t *out, size_t masksz)
{

	if (workmemsz < sx_hash_get_alg_digestsz(hashalg) + MGF1_COUNTER_SZ) {
	...
	*/
	...
	while (masksz > 0){
		...
		cracen_be_add((unsigned char *)mgfctr, MGF1_COUNTER_SZ, 1);
