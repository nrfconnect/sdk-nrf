 ...
 */
const char dom2[34] = {0x53, 0x69, 0x67, 0x45, 0x64, 0x32, 0x35, 0x35, 0x31, 0x39, 0x20, 0x6e,
...
int ed25519_calculate_k(struct sxhash hashopctx, char *workmem, char *pointr, const char *message,
			size_t message_length, int prehash)
{
			  ...
			  size_t message_length, int prehash)
{
	...
	status = sx_ed25519_ptmult(
		(const struct sx_ed25519_dgst *)(workmem),
		(struct sx_ed25519_pt *)(struct sx_ed25519_pt *)((char *)workmem +
								 SX_ED25519_PT_SZ));
	...
	status = sx_ed25519_sign((const struct sx_ed25519_dgst *)(workmem + SX_ED25519_SZ),
...
int ed25519_sign(const uint8_t *priv_key, char *signature, const uint8_t *message,
		 size_t message_length)
{
	return ed25519_sign_internal(priv_key, signature, message, message_length, 0);
}

int ed25519ph_sign(const uint8_t *priv_key, char *signature, const uint8_t *message,
...
int ed25519_verify_internal(const uint8_t *pubkey, const char *message, size_t message_length,
			    const char *signature, int prehash)
{
	...
	status =
		sx_ed25519_verify((struct sx_ed25519_dgst *)workmem, (struct sx_ed25519_pt *)pubkey,
...
{
	return ed25519_verify_internal(pubkey, message, message_length, signature, 0);
}
		     ...
		     const char *signature, int ismessage)
{
	...
	if (ismessage) {
		...
		return ed25519_verify_internal(pubkey, hashedmessage, SX_ED25519_DGST_SZ, signature,
					       1);
	}
