...
struct rsa_pss_workmem {
	...
	uint8_t *EM;   /** Encoded message (EM) after modexp */
	uint8_t *DB;   /** DataBlock (masked) */
	uint8_t *seed; /** Seed for MGF1 */
	uint8_t *H;    /** Location of recovered H */
	uint8_t *H1;   /** Location of recomputed H' */
};
			       ...
			       size_t digest_length, size_t saltsz)
{
	...
	const size_t wmem_sz = get_workmem_size(WORKMEM_SIZE, digestsz);
	...
	safe_memset(wmem + padding_ps_offset, wmem_sz - padding_ps_offset, 0, masksz - saltsz - 1);
	...
	*(salt - 1) = 1;
	...
	sx_status = cracen_hash_all_inputs(hash_array, hash_array_lengths, input_count, hashalg, H);
	...
	sx_status =
		cracen_run_mgf1xor(workmem, WORKMEM_SIZE, hashalg, seed, digestsz, xorinout, masksz);
	if (sx_status != SX_OK) {
		return sx_status;
	}
	...
	((uint8_t *)(wmem))[keysz - 1] = PSS_TRAILER_BYTE;
	...
	sx_status = cracen_rsa_modexp(&pkreq, inputs, rsa_key, wmem, keysz, sizes);
	if (sx_status != SX_OK) {
		return sx_status;
	}
	...
	safe_memzero(workmem, WORKMEM_SIZE);
				 ...
				 size_t digest_length, size_t saltsz)
{
	...
	/* modular exponentiation m^d mod n (RSASP1 sign primitive) */
	sx_status = cracen_rsa_modexp(&pkreq, inputs, rsa_key, signature->r, signature->sz, sizes);
	if (sx_status != SX_OK) {
		return sx_status;
	}
	 ...
	 */
	if ((wmem[CRACEN_RSA_KEY_OPSZ(rsa_key) - 1]) != PSS_TRAILER_BYTE) {
