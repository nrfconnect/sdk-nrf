					     ...
					     size_t key_buffer_size, size_t *key_buffer_length)
{
	...
	if (type == SICR) {
		...
		uint32_t mkek_plat_key_id = HALTIUM_PLATFORM_PSA_KEY_ID(ACCESS_INTERNAL, domain, USAGE_MKEK, 0);
		/* The opaque key is populated by the PSA core when the IKG key is accessed from the top level PSA APIs,
		* now that we use the PSA driver APIs directly we need to populate it manually.
		*/
		ikg_opaque_key mkek_ikg_opaque_key = {.slot_number = CRACEN_INTERNAL_HW_KEY1_ID, .owner_id = domain} ;
