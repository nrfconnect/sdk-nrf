...
static psa_status_t get_kmu_slot_id_and_metadata(psa_key_id_t key_id,
...
psa_status_t cracen_kmu_block(const psa_key_attributes_t *key_attr)
{
	...
	status = get_kmu_slot_count(metadata, key_attr, &slot_count);
	if (status != PSA_SUCCESS) {
		return status;
	}
...
psa_status_t cracen_kmu_block_raw(unsigned int slot_id, unsigned int slot_count)
{
