				     ...
				     size_t data_length)
{
	psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
	size_t hmac_length = 0;
	size_t hash_len = PSA_HASH_BLOCK_LENGTH(PSA_ALG_GET_HASH(operation->alg));
	...
	/** Operation must be initialized to a HKDF state and
