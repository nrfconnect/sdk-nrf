...
struct pqc_bit_reader {
	...
	uint64_t acc;	     /* Accumulated, but not yet consumed bits, LSB first */
	uint32_t acc_bits;   /* Number of valid bits in @p acc */
};
...
struct pqc_bit_writer {
	...
	uint64_t acc;	   /* Accumulated, but not yet stored bits, LSB first */
	uint32_t acc_bits; /* Number of valid bits in @p acc */
};
