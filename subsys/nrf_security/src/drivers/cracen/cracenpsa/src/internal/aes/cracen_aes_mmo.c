...
static uint8_t aes_mmo_ciphertext[ROUND_UP(SX_BLKCIPHER_AES_BLK_SZ, AES_MMO_DMA_ALIGNMENT)]
...
static psa_status_t cracen_aes_mmo_compress(cracen_aes_mmo_operation_t *operation)
{
	...
	sx_status = sx_blkcipher_ecb_simple(operation->digest, sizeof(operation->digest),
					    operation->block, SX_BLKCIPHER_AES_BLK_SZ,
					    aes_mmo_ciphertext, SX_BLKCIPHER_AES_BLK_SZ);
	if (sx_status != SX_OK) {
				   ...
				   size_t input_length)
{
	 ...
	 * pads them.
	 */
	if (operation->block_length + input_length < SX_BLKCIPHER_AES_BLK_SZ) {
	...
	while (input_length > 0) {
		size_t chunk_length =
			MIN(SX_BLKCIPHER_AES_BLK_SZ - operation->block_length, input_length);
		...
		if (operation->block_length == SX_BLKCIPHER_AES_BLK_SZ) {
			psa_status = cracen_aes_mmo_compress(operation);
				   ...
				   size_t hash_size, size_t *hash_length)
{
	...
	if (hash_size < SX_BLKCIPHER_AES_BLK_SZ) {
	...
	memset(operation->block + operation->block_length, 0,
	       SX_BLKCIPHER_AES_BLK_SZ - operation->block_length);
	operation->block[operation->block_length] = 0x80;

	if (operation->block_length >= SX_BLKCIPHER_AES_BLK_SZ - 2) {
		psa_status = cracen_aes_mmo_compress(operation);
		if (psa_status != PSA_SUCCESS) {
			goto exit;
		}
		memset(operation->block, 0, SX_BLKCIPHER_AES_BLK_SZ);
	}

	operation->block[SX_BLKCIPHER_AES_BLK_SZ - 2] = (uint8_t)(message_bits >> 8);
	operation->block[SX_BLKCIPHER_AES_BLK_SZ - 1] = (uint8_t)message_bits;
	...
	memcpy(hash, operation->digest, SX_BLKCIPHER_AES_BLK_SZ);
	*hash_length = SX_BLKCIPHER_AES_BLK_SZ;
