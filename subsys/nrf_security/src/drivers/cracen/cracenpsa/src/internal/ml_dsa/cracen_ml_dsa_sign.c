					   ...
					   const uint8_t *mu, uint8_t *priv_rand_seed)
{
	...
	return cracen_pqc_xof_compute(PSA_ALG_SHAKE256,
				      ...
				      priv_rand_seed, ML_DSA_PRIV_SEED_BYTES);
}
		       ...
		       const ml_dsa_poly_vector_t *z, const uint8_t *hint, uint8_t *sig)
{
	...
	uint32_t bitlen = cracen_pqc_bit_length(2u * alg_params->gamma1 - 1u);
				 ...
				 uint16_t counter, uint8_t *sig, bool *signature_produced)
{
	...
	uint32_t bitlen = cracen_pqc_bit_length(alg_params->w1_max);
	/* w1_bytes */
