				 ...
				 enum cipher_operation dir)
{
	...
	if (IS_ENABLED(PSA_NEED_CRACEN_RSA_OAEP)) {
		if (PSA_ALG_IS_RSA_OAEP(alg)) {
			...
			if (dir == CRACEN_ENCRYPT) {
				sx_status =
					cracen_rsa_oaep_encrypt(hashalg, &pubkey, &text, &label, output, output_length);
			} else {
				sx_status =
					cracen_rsa_oaep_decrypt(hashalg, &pubkey, &text, &label, output, output_length);
			}
		}
	}
	...
	if (IS_ENABLED(PSA_NEED_CRACEN_RSA_PKCS1V15_CRYPT)) {
		if (alg == PSA_ALG_RSA_PKCS1V15_CRYPT) {
			if (dir == CRACEN_ENCRYPT) {
				sx_status = cracen_rsa_pkcs1v15_encrypt(&pubkey, &text, output, output_length);
			} else {
				sx_status = cracen_rsa_pkcs1v15_decrypt(&pubkey, &text, output, output_length);
			}
		}
	}
