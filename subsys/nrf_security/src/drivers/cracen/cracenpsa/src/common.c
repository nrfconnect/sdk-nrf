					     ...
					     bool *is_qr)
{
	...
	uint8_t tmp_buf1[PSA_BITS_TO_BYTES(PSA_VENDOR_ECC_MAX_CURVE_BITS)] = {};
	uint8_t tmp_buf2[PSA_BITS_TO_BYTES(PSA_VENDOR_ECC_MAX_CURVE_BITS)] = {};
	...
	/* p_m_1 = (p - 1) */
	cracen_be_add(tmp_buf1, op_size, 1); /* 1 */
	if (cracen_be_sub(curve_prime->bytes, tmp_buf1, tmp_buf2, op_size) != 0) {
		return PSA_ERROR_INVALID_ARGUMENT;
	}
	sx_op p_m_1 = {.sz = op_size, .bytes = tmp_buf2};

	/* (p - 1) / 2 */
	cracen_be_rshift(p_m_1.bytes, 1, tmp_buf1, op_size);
	sx_const_op pm1_div_2 = {.sz = op_size, .bytes = tmp_buf1};

	/* tmp = value^((p-1)/2) mod p */
	sx_op tmp = {.sz = op_size, .bytes = tmp_buf2};
	...
	safe_memzero(tmp_buf1, op_size);
	cracen_be_add(tmp_buf1, op_size, 1); /* 1 */

	*is_qr = constant_memcmp_is_zero(tmp.bytes, tmp.sz);
	*is_qr = *is_qr || (constant_memcmp(tmp.bytes, tmp_buf1, tmp.sz) == 0);
				       ...
				       const sx_const_op *modulo, const sx_const_op *x, sx_op *gx)
{
	...
	uint8_t tmp_buf1[PSA_BITS_TO_BYTES(PSA_VENDOR_ECC_MAX_CURVE_BITS)] = {};
	uint8_t tmp_buf2[PSA_BITS_TO_BYTES(PSA_VENDOR_ECC_MAX_CURVE_BITS)] = {};
	uint8_t tmp_buf3[PSA_BITS_TO_BYTES(PSA_VENDOR_ECC_MAX_CURVE_BITS)] = {};
	...
	/* gx = x^3 + a*x + b */
	/* x_3 = x^3 */
	cracen_be_add(tmp_buf1, op_size, 3); /* 3 */
	sx_const_op exp = {.sz = op_size, .bytes = tmp_buf1};
	sx_op x_3 = {.sz = op_size, .bytes = tmp_buf2};
	...
	/* ax = a*x */
	sx_op ax = {.sz = op_size, .bytes = tmp_buf3};
	...
	sx_op x_3_ax = {.sz = op_size, .bytes = tmp_buf1};
				      ...
				      sx_op *zu_sqr, sx_op *m)
{
	...
	/* The following buffer may be reused */
	uint8_t tmp_buf[max_key_size];
	...
	/* u_sqr = u^2 */
	sx_op u_sqr = {.sz = curve_opsize, .bytes = tmp_buf};
	...
	sx_op tmp2 = {.sz = curve_opsize, .bytes = tmp_buf};
	...
	/* m = tmp2 + zu_sqr */
	sx_const_op tmp2_const;

	sx_get_const_op(&tmp2, &tmp2_const);
	sx_status = sx_mod_primitive_cmd(NULL, cmd_add, modulo, &tmp2_const, &zu_sqr_const, m);
				       ...
				       sx_op *x1)
{
	...
	/* The following buffer may be reused */
	uint8_t tmp_buf1[max_key_size];
	uint8_t tmp_buf2[max_key_size];
	uint8_t tmp_buf3[max_key_size];
	uint8_t tmp_buf4[max_key_size];
	uint8_t tmp_buf5[max_key_size];
	...
	/* (z * a) */
	sx_op za = {.sz = curve_op_size, .bytes = tmp_buf2};
	...
	sx_op x1a = {.sz = curve_op_size, .bytes = tmp_buf3};
	...
	/* x1b calculation */
	/* p_m_b = p - b */
	if (cracen_be_sub(modulo->bytes, b->bytes, tmp_buf2, modulo->sz) != 0) {
		return SX_ERR_INVALID_ARG;
	}
	sx_const_op p_m_b = {.sz = modulo->sz, .bytes = tmp_buf2};

	/* inv_a = 1/a mod p */
	sx_op inv_a = {.sz = curve_op_size, .bytes = tmp_buf4};
	...
	sx_op m_b_inv_a = {.sz = curve_op_size, .bytes = tmp_buf5};
	...
	/* one_p_t = (1 + t) mod p */
	safe_memzero(tmp_buf2, curve_op_size);
	cracen_be_add(tmp_buf2, curve_op_size, 1);

	sx_const_op one = {.sz = curve_op_size, .bytes = tmp_buf2};
	sx_op one_p_t = {.sz = curve_op_size, .bytes = tmp_buf4};
	...
	sx_op x1b = {.sz = curve_op_size, .bytes = tmp_buf1};
				 ...
				 const sx_const_op *u, const sx_op *result)
{
	...
	/* The following buffers may be reused */
	uint8_t tmp_parambuf[max_key_size];
	uint8_t tmp_parambuf2[max_key_size];
	uint8_t tmp_buf1[max_key_size];
	uint8_t tmp_buf2[max_key_size];
	uint8_t tmp_buf3[max_key_size];
	uint8_t tmp_buf4[max_key_size];
	...
	if (z_int < 0) {
		safe_memzero(tmp_buf1, sx_pk_curve_opsize(sx_curve));
		cracen_be_add(tmp_buf1, sx_pk_curve_opsize(sx_curve), abs(z_int));
		cracen_be_sub(sx_pk_curve_prime(sx_curve), tmp_buf1, z_buf,
			      sx_pk_curve_opsize(sx_curve));
	} else {
		cracen_be_add(z_buf, sx_pk_curve_opsize(sx_curve), abs(z_int));
	}

	/* Note: the following const cast will be removed */
	sx_const_op modulo = {.sz = sx_pk_curve_opsize(sx_curve),
			      .bytes = sx_pk_curve_prime(sx_curve)
	};

	/* Note: The implementation of the SSWU algorithm shall be done in constant time */
	...
	sx_const_op zu_sqr_const;
	sx_op zu_sqr = {.sz = sx_pk_curve_opsize(sx_curve), .bytes = tmp_buf2};
	sx_op m = {.sz = sx_pk_curve_opsize(sx_curve), .bytes = tmp_buf3};
	 ...
	 * t = m^(p-2) modulo p;
	 */
	safe_memzero(tmp_parambuf, sx_pk_curve_opsize(sx_curve));
	cracen_be_add(tmp_parambuf, sx_pk_curve_opsize(sx_curve), 2);
	cracen_be_sub(sx_pk_curve_prime(sx_curve), tmp_parambuf, tmp_parambuf,
		      sx_pk_curve_opsize(sx_curve));

	sx_const_op m_const;
	sx_const_op p_decreased = {.sz = sx_pk_curve_opsize(sx_curve), .bytes = tmp_parambuf};
	sx_op t = {.sz = sx_pk_curve_opsize(sx_curve), .bytes = tmp_buf1};
	...
	sx_op x1 = {.sz = sx_pk_curve_opsize(sx_curve), .bytes = tmp_buf3};
	sx_const_op t_const;
	...
	sx_op gx1 = {.sz = sx_pk_curve_opsize(sx_curve), .bytes = tmp_buf1};
	...
	sx_op x2 = {.sz = sx_pk_curve_opsize(sx_curve), .bytes = tmp_buf4};
	...
	sx_op gx2 = {.sz = sx_pk_curve_opsize(sx_curve), .bytes = tmp_buf2};
	...
	/* v = CSEL(l, gx1, gx2) */
	sx_op v = {.sz = sx_pk_curve_opsize(sx_curve), .bytes = tmp_buf5};

	constant_select_bin(is_gx1_qr, gx1.bytes, gx2.bytes, v.bytes, v.sz);

	/* x = CSEL(l, x1, x2 ) */
	sx_op x = {.sz = sx_pk_curve_opsize(sx_curve), .bytes = tmp_buf1};

	constant_select_bin(is_gx1_qr, x1.bytes, x2.bytes, x.bytes, x.sz);

	/* y = sqrt(v) */
	sx_const_op v_const;
	sx_op y = {.sz = sx_pk_curve_opsize(sx_curve), .bytes = tmp_buf2};
	...
	/* tmp_parambuf2 = p - y */
	cracen_be_sub(sx_pk_curve_prime(sx_curve), y.bytes, tmp_parambuf2,
		      sx_pk_curve_opsize(sx_curve));
	constant_select_bin(lsb_eq, y.bytes, tmp_parambuf2,
