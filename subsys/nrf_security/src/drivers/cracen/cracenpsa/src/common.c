...
int cracen_hash_all_inputs(const uint8_t *inputs[], const size_t inputs_lengths[],
			   size_t input_count, const struct sxhashalg *hashalg, uint8_t *digest)
{
	struct sxhash hashopctx;

	return cracen_hash_all_inputs_with_context(&hashopctx, inputs, inputs_lengths, input_count,
						   hashalg, digest);
}
