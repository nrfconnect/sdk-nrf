...
static psa_status_t calculate_v_from_k(const uint8_t *k, size_t k_size, uint8_t *v, size_t v_size)
{
	...
	sx_status = sx_sync_mod_exp(&pkreq, NULL, &g, &a, &modulo, &result);
exit:
							  ...
							  size_t *output_length)
{
	...
	sx_status = sx_sync_mod_exp(&pkreq, NULL, &g, &b, &modulo, &temp_b);
	status = silex_statuscodes_to_psa(sx_status);
	if (status != PSA_SUCCESS) {
		goto error;
	}
						  ...
						  const size_t S_length)
{
	...
	sx_status = sx_sync_mod_exp(&pkreq, NULL, &g, &x, &modulo, &temp_g);
	status = silex_statuscodes_to_psa(sx_status);
	if (status != PSA_SUCCESS) {
		goto exit;
	}
	...
	sx_get_const_op(&temp_b, &c_temp_b);
	sx_status = sx_sync_mod_exp(&pkreq, NULL, &c_temp_b, &a, &modulo, &temp_1);
	status = silex_statuscodes_to_psa(sx_status);
	if (status != PSA_SUCCESS) {
		goto exit;
	}
	...
	sx_status = sx_sync_mod_exp(&pkreq, NULL, &c_temp_b, &u, &modulo, &temp_2);
	status = silex_statuscodes_to_psa(sx_status);
	if (status != PSA_SUCCESS) {
		goto exit;
	}
	...
	sx_status = sx_sync_mod_exp(&pkreq, NULL, &c_temp_2, &x, &modulo, &temp_3);
	status = silex_statuscodes_to_psa(sx_status);
	if (status != PSA_SUCCESS) {
		goto exit;
	}
						  ...
						  const size_t S_length)
{
	...
	sx_status = sx_sync_mod_exp(&pkreq, NULL, &v, &u, &modulo, &temp_1);
	status = silex_statuscodes_to_psa(sx_status);
	if (status != PSA_SUCCESS) {
		goto exit;
	}
	...
	sx_get_const_op(&temp_2, &c_temp_2);
	sx_status = sx_sync_mod_exp(&pkreq, NULL, &c_temp_2, &b, &modulo, &result);
	status = silex_statuscodes_to_psa(sx_status);
