...
static psa_status_t check_key_attributes(const psa_key_attributes_t *attributes,
					 size_t key_buffer_size, const struct sx_pk_ecurve **ecurve)
{
...
static psa_status_t verify_signing_conditions(bool is_message, psa_algorithm_t alg,
					      const psa_key_attributes_t *attributes)
{
...
static psa_status_t handle_eddsa_sign(bool is_message, psa_algorithm_t alg,
				      const psa_key_attributes_t *attributes,
				      const uint8_t *key_buffer, uint8_t *signature,
...
static psa_status_t handle_ecdsa_sign(bool is_message, psa_algorithm_t alg,
				      const uint8_t *key_buffer, const uint8_t *input,
					      ...
					      size_t signature_size, size_t *signature_length)
{
	...
	status = check_key_attributes(attributes, key_buffer_size, &ecurve);
	if (status != PSA_SUCCESS) {
		return status;
	}
	...
	status = verify_signing_conditions(is_message, alg, attributes);
	if (status != PSA_SUCCESS) {
		return status;
	}
	...
	if (alg == PSA_ALG_PURE_EDDSA && (IS_ENABLED(CONFIG_PSA_WANT_ALG_ED25519PH) ||
						     IS_ENABLED(CONFIG_PSA_WANT_ALG_PURE_EDDSA))) {
		return handle_eddsa_sign(is_message, alg, attributes, key_buffer, signature, input,
					 input_length, ecurve, signature_length);
	} else if (PSA_ALG_IS_ECDSA(alg) && (IS_ENABLED(CONFIG_PSA_WANT_ALG_ECDSA) ||
					     IS_ENABLED(CONFIG_PSA_WANT_ALG_DETERMINISTIC_ECDSA))) {
		return handle_ecdsa_sign(is_message, alg, key_buffer, input, input_length,
					 signature, ecurve, signature_length);
	}
						...
						size_t signature_length)
{
	...
	si_status = cracen_signature_prepare_ec_pubkey((const char *)key_buffer, key_buffer_size,
	...
	if (alg == PSA_ALG_ED25519PH) {
		...
		if (is_message) {
			si_status =
				cracen_ecdsa_verify_message(pubkey_buffer, hashalgpointer, input,
							    input_length, curve, signature);
		} else {
			si_status = cracen_ecdsa_verify_digest(pubkey_buffer, input, input_length,
							       curve, signature);
		}
		return silex_statuscodes_to_psa(si_status);
	}
