					      ...
					      size_t signature_size, size_t *signature_length)
{
	...
	if (PSA_ALG_IS_RSA_PSS(alg) && IS_ENABLED(CONFIG_PSA_NEED_CRACEN_RSA_PSS)) {
	...
	} else if (PSA_ALG_IS_RSA_PKCS1V15_SIGN(alg) && IS_ENABLED(PSA_NEED_CRACEN_RSA_PKCS1V15_SIGN)) {
		if (is_message) {
