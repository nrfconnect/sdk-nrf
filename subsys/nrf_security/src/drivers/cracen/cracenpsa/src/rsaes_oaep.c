			    ...
			    size_t *output_length)
{
	...
	sx_status = cracen_run_mgf1xor(workmem, workmemsz, hashalg, xorinout,
				       modulussz - digestsz - 1, seed, digestsz);
	if (sx_status != SX_OK) {
	...
	memcpy(output, datab, datablockend - datab);
	*output_length = datablockend - datab;
	safe_memzero(workmem, workmemsz);
	return SX_OK;
}
			    ...
			    size_t *output_length)
{
	...
	*datab++ = 1;
	...
	sx_status = cracen_run_mgf1xor(workmem, workmemsz, hashalg, datablock,
				       modulussz - digestsz - 1, wmem + 1, digestsz);
	if (sx_status != SX_OK) {
		safe_memzero(workmem, workmemsz);
		return sx_status;
	}
