...
static void rsa_oaep_decrypt_init(struct rsa_oaep_workmem *workmem, size_t digestsz,
				  size_t modulussz)
{
	workmem->wmem = get_workmem_pointer(workmem->workmem, digestsz);
			    ...
			    size_t *output_length)
{
	...
	sx_status = cracen_run_mgf1xor(workmem.workmem, WORKMEM_SIZE, hashalg, xorinout,
	...
	r |= cracen_memdiff(workmem.wmem + 1, datab, digestsz);
	datab += digestsz;
...
{
	workmem->datablock = get_workmem_pointer(workmem->workmem, digestsz) + digestsz + 1;
	workmem->wmem = get_workmem_pointer(workmem->workmem, digestsz);
			    ...
			    size_t *output_length)
{
	...
	size_t digestsz = sx_hash_get_alg_digestsz(hashalg);
	size_t modulussz = CRACEN_RSA_KEY_OPSZ(rsa_key);
	const size_t wmem_size = get_workmem_size(WORKMEM_SIZE, digestsz);
	...
	*datab++ = 1;
	...
	sx_status = cracen_run_mgf1xor(workmem.workmem, WORKMEM_SIZE, hashalg, workmem.datablock,
	...
	sx_status = cracen_rsa_modexp(&pkreq, inputs, rsa_key, workmem.wmem, modulussz, sizes);
	if (sx_status != SX_OK) {
		safe_memzero(workmem.workmem, WORKMEM_SIZE);
		sx_pk_release_req(pkreq.req);
		return sx_status;
