					...
					size_t *output_length)
{
	...
	sx_status = cracen_rsa_modexp(&pkreq, inputs, rsa_key, text->addr, modulussz, sizes);
	uint8_t *wmem = get_workmem_pointer(workmem, digestsz);
	uint8_t *xorinout = wmem + 1 + digestsz;
	uint8_t *seed     = wmem + 1;
	...
	sx_status = cracen_run_mgf1xor(workmem, workmemsz, hashalg, xorinout, modulussz - digestsz - 1, seed,
				       digestsz);
	if (sx_status != SX_OK) {
		return sx_status;
	}
		/* Possible source of error */
	sx_status = cracen_run_mgf1xor(workmem, workmemsz, hashalg, seed, digestsz,
				       xorinout, modulussz - digestsz - 1);
	if (sx_status != SX_OK) {
		return sx_status;
	}
	...
	if (label) {
		sx_status = cracen_hash_input(label->bytes, label->sz, hashalg, wmem + 1);
		if (sx_status != SX_OK) {
			return sx_status;
		}
	} else {
		sx_status = cracen_hash_input(0, 0, hashalg, wmem + 1);
		if (sx_status != SX_OK) {
			return sx_status;
		}
	}
	...
	int r = 0;
	uint8_t *const datablockstart = wmem + digestsz + 1;
	uint8_t *const datablockend = datablockstart + modulussz - digestsz - 1;
					...
					size_t *output_length)
{
	...
	uint8_t *datablock = get_workmem_pointer(workmem, digestsz) + digestsz + 1;

	/* hash the label */
	if (label) {
		sx_status = cracen_hash_input(label->bytes, label->sz, hashalg, datablock);
		if (sx_status != SX_OK) {
			return sx_status;
		}
	} else {
		sx_status = cracen_hash_input(0, 0, hashalg, datablock);
		if (sx_status != SX_OK) {
			return sx_status;
		}
	}
	...
	uint8_t *wmem = get_workmem_pointer(workmem, digestsz);
	const size_t wmem_size = get_workmem_size(workmemsz, digestsz);
	const size_t datablockstart_offset = digestsz + 1;
	uint8_t *const datablockstart = wmem + datablockstart_offset;
	 ...
	 */
	uint8_t *datab = datablockstart + digestsz;
	...
	*datab++ = 1;
	...
	uint8_t *xorinout = wmem + digestsz + 1; /* same as start of DB */

	sx_status = cracen_run_mgf1xor(workmem, workmemsz, hashalg, seed, digestsz, datablock,
	...
	xorinout = wmem + 1;
	sx_status = cracen_run_mgf1xor(workmem, workmemsz, hashalg, datablock, modulussz - digestsz - 1,
				       wmem + 1, digestsz);
	if (sx_status != SX_OK) {
		return sx_status;
	}
	...
	sx_status = cracen_rsa_modexp(&pkreq, inputs, rsa_key, wmem, modulussz, sizes);
	if (sx_status != SX_OK) {
		sx_pk_release_req(pkreq.req);
		return sx_status;
	}
