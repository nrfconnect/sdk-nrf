...
{
	char *wmem = workmem + GENPRIVKEY_WORKMEM_OFST(keysz);
...
static int miller_rabin_get_random(uint8_t *workmem, struct cracen_rsacheckpq *rsacheckpq)
{
	...
	char *wmem = workmem + CHECKPQ_WORKMEM_OFST(candidatesz);
...
static int rsagenpq_get_random(uint8_t *workmem, size_t workmemsz, struct cracen_rsagenpq *rsagenpq)
{
	int sx_status;
	 ...
	 * be zero.
	 */
	if (((rsagenpq->pubexp[rsagenpq->pubexpsz - 1] & 1) == 0) || (rsagenpq->pubexpsz > (2 * rsagenpq->candidatesz)) ||
	    (rsagenpq->pubexp[0] == 0)) {
	 ...
	 * even.
	 */
	if ((!(rsagenpq->p[rsagenpq->candidatesz - 1] & 1)) || ((rsagenpq->q != NULL) && !(rsagenpq->q[rsagenpq->candidatesz - 1] & 1))) {
	...
	static const uint8_t compareval[8] = "\xB5\x04\xF3\x33\xF9\xDE\x64\x85";
	uint8_t *candprime = get_candidate_prime(&rsacheckpq);
	int r;
	...
	if (rsacheckpq.q != NULL) {
		 ...
		 */
		r = check_abs_diff((uint8_t *)rsacheckpq.p, (uint8_t *)candprime, rsacheckpq.candidatesz);
	 ...
	 */
	size_t candidatesz = rsacheckpq.candidatesz;
	sx_status = cracen_coprime_check(workmem, workmemsz,
	candprime, candidatesz, product_small_primes,
	 ...
	 */
	sx_status = cracen_coprime_check( workmem, workmemsz,
		wmem, candidatesz, (const uint8_t *)rsacheckpq.pubexp, rsacheckpq.pubexpsz);
	 ...
	 */
	be_sub((uint8_t *)wmem, candidatesz, 1);
				 ...
				 struct cracen_rsa_key *privkey)
{
	...
	size_t workmemsz = PSA_BITS_TO_BYTES(PSA_MAX_RSA_KEY_BITS) * 2;
	uint8_t workmem[workmemsz];
	if (workmemsz < 2 * keysz) {
	...
	if (CRACEN_RSA_KEY_CRT(privkey)) {
		sx_status = rsagenpriv_crt_finish(&pkreq, keysz, workmem, privkey);
	} else {
		sx_status = rsagenpriv_finish(&pkreq, privkey, keysz);
	}
	if (sx_status != SX_OK) {
		return sx_status;
	}

	return rsagenpq_get_random(workmem, workmemsz, &rsagenpq);
}
