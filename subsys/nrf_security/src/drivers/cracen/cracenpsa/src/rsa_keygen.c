				 ...
				 struct cracen_rsa_key *priv_key)
{
	...
	// 1) p and q come out of the earlier key-gen call in workmem:
	memcpy(priv_key->elements[0]->bytes,  wmem, primefactorsz); // p
...
static int rsagenpq_get_random(uint8_t *workmem, size_t workmemsz, struct cracen_rsagenpq *rsagenpq)
{
	...
	if (rsacheckpq.q != NULL) {
		 ...
		 */
		r = check_abs_diff((uint8_t *)rsacheckpq.p, (uint8_t *)candprime,
				   rsacheckpq.candidatesz);
	...
	sx_status = cracen_coprime_check(workmem, workmemsz, candprime, candidatesz,
	 ...
	 */
	sx_status = cracen_coprime_check(workmem, workmemsz, wmem, candidatesz,
					 (const uint8_t *)rsacheckpq.pubexp, rsacheckpq.pubexpsz);
				...
				struct cracen_rsa_key *privkey)
{
	...
	do {
		sx_status = rsagenpq_get_random(workmem, workmemsz, &rsagenpq);
