...
static int rsagenpriv_crt_finish(struct sx_pk_acq_req pkreq, size_t keysz, uint8_t *workmem, struct si_rsa_key *key)
{
	...
	if (pkreq.status != SX_OK) {
		sx_pk_release_req(pkreq.status);
		return pkreq.status;
	}

	/* get p and q from the workmem */
	memcpy(key->elements[0]->bytes, wmem, primefactorsz);
	memcpy(key->elements[1]->bytes, wmem + primefactorsz,
	       primefactorsz);

	/* get dp, dq and qinv from device memory slots (take the last
	 * primefactorsz bytes of each slot)
	 */
	sx_rdpkmem(key->elements[2]->bytes, outputs[0] + primefactorsz,
		   primefactorsz);
	sx_rdpkmem(key->elements[3]->bytes, outputs[1] + primefactorsz,
		   primefactorsz);
	sx_rdpkmem(key->elements[4]->bytes, outputs[2] + primefactorsz,
		   primefactorsz);
...
static int rsagenpriv_finish(struct sx_pk_acq_req *pkreq, struct si_rsa_key *key, size_t keysz)
{
	int sx_status;
	const char **outputs = sx_pk_get_output_ops(pkreq.req);
 ...
 */
void rsa_create_genprivkey(const char *pubexp, size_t pubexpsz, size_t keysz,
			      struct si_rsa_key *privkey)
{
	...
	pubexp.bytes = (char *)rsagenprivkey.pubexp;
	pubexp.sz = rsagenprivkey.pubexpsz;
	...
	if (IS_RSA_KEY_CRT(rsagenprivkey.key)) {
		si_wq_run_after(t, &rsagenprivkey.wq, rsagenpriv_crt_finish);
	} else {
		si_wq_run_after(t, &rsagenprivkey.wq, rsagenpriv_finish);
	}
