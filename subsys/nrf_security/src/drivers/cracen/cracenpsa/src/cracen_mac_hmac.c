				      ...
				      psa_algorithm_t alg)
{
	...
	/* State is saved and HW released after mac_create_hmac */
				       ...
				       size_t input_length)
{
	...
	if (operation->has_saved_state) {
		sx_hw_reserve(&operation->hmac.hashctx.dma, SX_HW_RESERVE_DEFAULT);
...
exit:
	sx_cmdma_release_hw(&operation->hmac.hashctx.dma);
	if (sx_status != SX_OK) {
		return silex_statuscodes_to_psa(sx_status);
	}
...
psa_status_t cracen_hmac_finish(cracen_mac_operation_t *operation)
{
	...
	/* Generate the MAC. hmac_produce handles HW release internally. */
	return silex_statuscodes_to_psa(
		hmac_produce(&operation->hmac.hashctx, sx_hash_algo, operation->input_buffer,
