/* HMAC implementation, based on FIPS 198-1.
 *
 * Copyright (c) 2023 Nordic Semiconductor ASA
 *
 * SPDX-License-Identifier: LicenseRef-Nordic-5-Clause
 *
 * Workmem layout for the HMAC task:
 *      1. Area used to store values obtained from processing the key.
 *         Size: hash algorithm block size.
 *      2. Area used to store the intermediate hash value.
 *         Size: hash algorithm digest size.
 */

#include <string.h>
#include <sxsymcrypt/hash.h>
#include <cracen/statuscodes.h>
#include <cracen/mem_helpers.h>
#include "final.h"

/* xor bytes in 'buf' with a constant 'v', in place */
static void xorbuf(char *buf, char v, size_t sz)
{
	for (size_t i = 0; i < sz; i++) {
		*buf = *buf ^ v;
		buf++;
	}
}

static int finish_hmac_computation(struct sxhashalg hashalg, char *out, char *workmem)
{
	size_t blocksz, digestsz;
	struct sxhash hashopctx;
	int status;

	/* task for computing the 2nd hash of the HMAC algorithm */
	status = sx_hash_create(&hashopctx, &hashalg, sizeof(hashopctx));

	if (status != 0) {
		return status;
	}

	blocksz = sx_hash_get_alg_blocksz(hashalg);
	digestsz = sx_hash_get_alg_digestsz(hashalg);

	/* compute K0 xor opad (0x36 ^ 0x5C = 0x6A) */
	xorbuf(workmem, 0x6A, blocksz);

	/* feed and start the hash task, result at location provided by the user
	 */
	status = sx_hash_feed(&hashopctx, workmem, blocksz + digestsz);
	if (status != 0) {
		return status;
	}
	status = sx_hash_digest(&hashopctx, out);
	if (status != 0) {
		return status;
	}
	status = sx_hash_wait(&hashopctx);

	return status;
}

int hmac_produce(struct sxhash hashctx, struct sxhashalg hashalg, char *out, size_t sz, char *workmem)
{
	int status;
	size_t blocksz, digestsz;

	digestsz = sx_hash_get_alg_digestsz(hashalg);
	if (sz < digestsz) {
		sx_hash_free(&hashctx);
		return SX_ERR_OUTPUT_BUFFER_TOO_SMALL;
	}

	/* store pointer for final HMAC result, we need it later */

	blocksz = sx_hash_get_alg_blocksz(hashalg);

	/* run hash (1st hash of HMAC algorithm), result inside workmem */
	status = sx_hash_digest(&hashctx, workmem + blocksz);
	return finish_hmac_computation(hashalg, out, workmem);
}

static int start_hmac_computation(struct sxhash hashopctx, struct sxhash hashalg, char *workmem)
{
	size_t blocksz;
	int status;

	/* task for computing the 1st hash in the HMAC algorithm */
	status = sx_hash_create(&hashopctx, &hashalg, sizeof(hashopctx));

	if (status != 0) {
		return status;
	}

	/* override the produce function pointer */
	/*Change produce to direct calls to hmac_produce I guess*/
	/*t->actions.produce = hmac_produce;*/

	blocksz = sx_hash_get_alg_blocksz(hashalg);

	/* The "prepared" key (called K0 in FIPS 198-1) is available and
	 * t->workmem points to it. Here we compute K0 xor ipad.
	 */
	xorbuf(workmem, 0x36, blocksz);

	/* start feeding the hash task */
	status = sx_hash_feed(&hashopctx, workmem, blocksz);
	if (status != 0) {
		return status;
	}

	return status;
}

int mac_create_hmac(const struct sxhashalg *hashalg, struct sxhash hashopctx, const char *key,
			size_t keysz)
{
	size_t digestsz, blocksz;

	digestsz = sx_hash_get_alg_digestsz(hashalg);
	blocksz = sx_hash_get_alg_blocksz(hashalg);
	int status;
	size_t workmemsz = blocksz + digestsz;
	char *workmem[workmemsz];

	/* a key longer than hash block size needs an additional preparation
	 * step
	 */
	if (keysz > blocksz) {
		/* hash the key */
		status = sx_hash_create(&hashopctx, hashalg, sizeof(hashopctx));
		if (status != 0) {
			return status;
		}
		status = sx_hash_feed(&hashopctx, key, keysz);
		if (status != 0) {
			return status;
		}
		status = sx_hash_digest(&hashopctx, workmem);
		if (status != 0) {
			return status;
		}
		status = sx_hash_wait(&hashopctx);
		if (status != 0) {
			return status;
		}

		/* zero padding */
		safe_memset(workmem + digestsz, workmemsz - digestsz, 0, blocksz - digestsz);
		status = start_hmac_computation(hashalg, hashopctx, workmem);
	} else {
		memcpy(workmem, key, keysz);
		/* zero padding */
		safe_memset(workmem + keysz, workmemsz - keysz, 0, blocksz - keysz);

		status = start_hmac_computation(hashalg, hashopctx, workmem);
	}
	return status;
}
