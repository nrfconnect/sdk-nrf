...
static int finish_hmac_computation(const struct sxhashalg *hashalg, char *out, char *workmem)
{
...
static int start_hmac_computation(struct sxhash *hashopctx, const struct sxhashalg *hashalg,
				  char *workmem)
{
	size_t blocksz;
	int status;
		    ...
		    size_t keysz, char *workmem, size_t workmemsz)
{
	...
	if (keysz > blocksz) {
		...
		status = start_hmac_computation(hashopctx, hashalg, workmem);
	} else {
