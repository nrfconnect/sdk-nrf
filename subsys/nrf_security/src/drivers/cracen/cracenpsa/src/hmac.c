...
static int finish_hmac_computation(const struct sxhashalg *hashalg, char *digest, char *workmem)
{
	...
	return hash_input(workmem, blocksz + digestsz, hashalg, digest);
}
		 ...
		 char *workmem)
{
	...
	return finish_hmac_computation(hashalg, digest, workmem);
}
		    ...
		    size_t keysz, char *workmem, size_t workmemsz)
{
	...
	if (keysz > blocksz) {
		/* hash the key */
		status = hash_input(key, keysz, hashalg, workmem);
		if (status != SX_OK) {
			return status;
		}
