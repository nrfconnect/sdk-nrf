...
static int modular_inversion_start(uint8_t *workmem, struct cracen_coprimecheck *coprimecheck)
{
	int sx_status;
				    ...
				    struct cracen_coprimecheck *coprimecheck)
{
	...
	return modular_inversion_start(workmem, coprimecheck);
}
...
int cracen_coprime_check(uint8_t *workmem, uint8_t workmemsz, const uint8_t *a, size_t asz,
			 const uint8_t *b, size_t bsz)
{
	int sx_status;
	/* we might be able to reuse memory, that would be good */
	int a_is_odd;
	int b_is_odd;
	size_t minworkmemsz;
	struct cracen_coprimecheck coprimecheck;
	...
	if (asz > bsz) {
		...
		sx_status = modular_inversion_start(workmem, &coprimecheck);
	}
	return sx_status;
}
