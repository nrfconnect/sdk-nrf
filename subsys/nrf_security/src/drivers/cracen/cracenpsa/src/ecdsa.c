 ...
 */
void bits2octets(const uint8_t *data, size_t data_len, uint8_t *out_data, const uint8_t *order, size_t order_len)
{
				    ...
				    uint8_t *hmac)
{
	...
	if (internal_octet != INTERNAL_OCTET_NOT_USED) {
		...
		status = sx_hash_feed(&hashopctx, &value,
			     sizeof(value));
	}
	if (sk) {
			       ...
			       const struct sx_pk_ecurve *curve, const uint8_t *signature)
{
	...
	internal_signature.r = (const char *)signature;
	internal_signature.s = (const char *)signature + opsz;
