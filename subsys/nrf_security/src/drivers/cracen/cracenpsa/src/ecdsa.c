...
LOG_MODULE_REGISTER(ecdsa_internal, LOG_LEVEL_DBG);
			      ...
			      size_t signature_size, size_t *signature_length)
{
	...
	return cracen_ecdsa_sign_digest(privkey, privkey_size, hashalg, curve, digest, digestsz,
					   ...
					   size_t signature_size, size_t *signature_length)
{
	...
	char workmem[workmem_requirement]__attribute__((aligned(16)));
	...
	} while (((pkreq.status == SX_ERR_INVALID_SIGNATURE) ||
					    ...
					    size_t *signature_length)
{
	...
	return cracen_ecdsa_sign_digest_deterministic(privkey, privkey_size, hashalg, curve, digest,
						      digestsz, signature, signature_size,
						      signature_length);
}
				...
				const struct sx_pk_ecurve *curve, const uint8_t *signature)
{
	...
	return cracen_ecdsa_verify_digest(pubkey, hashalg, digest, digestsz, curve, signature);
}
