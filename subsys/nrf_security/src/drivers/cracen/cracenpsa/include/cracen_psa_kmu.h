 ...
 *
 * @note Applications should use the PSA Crypto API (psa_* functions) instead
...
/** @brief A slot number identifying a key in a driver.
 *
...
enum cracen_kmu_metadata_key_usage_scheme {
	...
	/** CRACEN's IKG seed uses 3 key slots.
	 *
 ...
 *       it, and a `PUSHBLOCK` on those that don't.
 *
 * @retval PSA_SUCCESS              The operation completed successfully.
 * @retval PSA_ERROR_INVALID_HANDLE The key handle is invalid.
 * @retval PSA_ERROR_DOES_NOT_EXIST The key does not exist.
