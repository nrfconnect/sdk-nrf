 ...
 *  @{
 */
psa_status_t cc3xx_key_agreement(const psa_key_attributes_t *attributes, const uint8_t *priv_key,
				 ...
				 size_t *output_length, psa_algorithm_t alg)
{
	...
	*output_length = output_size;
	...
	case PSA_ALG_ECDH:
		...
		if (curve == PSA_ECC_FAMILY_MONTGOMERY && key_bits == 255) {
			ret = cc3xx_ecdh_calc_secret_mont(priv_key, priv_key_size, publ_key,
							  publ_key_size, output, output_length);
		} else {
			ret = cc3xx_ecdh_calc_secret_wrst(curve, key_bits, priv_key, priv_key_size,
