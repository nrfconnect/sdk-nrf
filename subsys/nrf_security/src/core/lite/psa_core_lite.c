...
psa_status_t silex_statuscodes_to_psa(int ret);
	...
	defined(PSA_WANT_ECC_SECP_256_R1)
	...
	const uint8_t *signature, size_t signature_length)
{
	...
	/* TODO: Check for BL config, not PSA want (filter not accurate)*/
	VERIFY_PSA_TYPE_AT_LEAST_ONE_RETURNS(
		alg,
#if defined(PSA_WANT_ALG_ED25519PH)
		PSA_ALG_ED25519PH,
#endif
#if defined(PSA_WANT_ALG_ECDSA) && defined(PSA_WANT_ECC_SECP_256_R1)
		PSA_ALG_ECDSA(PSA_ALG_SHA_256),
#endif
#if defined(PSA_WANT_ALG_DETERMINISTIC_ECDSA) && defined(PSA_WANT_ECC_SECP_256_R1)
	...
	uint8_t *hash, size_t hash_size, size_t *hash_length)
{
	...
	/* Return if no supported alg-type */
	VERIFY_PSA_TYPE_AT_LEAST_ONE_RETURNS(
		alg,
