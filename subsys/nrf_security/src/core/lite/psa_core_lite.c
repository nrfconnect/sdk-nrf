...
static psa_status_t get_key_attributes(
	psa_key_attributes_t *attributes, mbedtls_svc_key_id_t key_id)
{
	psa_status_t status;
	...
	const uint8_t *signature, size_t signature_length)
{
	...
	/* TODO: Will be changed to a single macro */
	if (IS_ENABLED_ALL(PSA_WANT_ALG_ED25519PH,
			   PSA_WANT_ECC_TWISTED_EDWARDS_255) &&
			   alg == PSA_ALG_ED25519PH) {
		/** Do nothing  */
	} else if (IS_ENABLED_ALL(PSA_WANT_ALG_ECDSA, PSA_WANT_ECC_SECP_256_R1,
				  PSA_WANT_ALG_SHA_256) &&
				  PSA_ALG_IS_ECDSA(alg)) {
		/** Do nothing  */
	} else if (IS_ENABLED_ALL(PSA_WANT_ALG_DETERMINISTIC_ECDSA,
				  PSA_WANT_ECC_SECP_256_R1, PSA_WANT_ALG_SHA_256) &&
				  PSA_ALG_DETERMINISTIC_ECDSA(alg)) {
		/** Do nothing  */
	} else {
	...
	uint8_t *hash, size_t hash_size, size_t *hash_length)
{
	...
	/* TODO: Will be changed to a single macro */
	if (IS_ENABLED(PSA_WANT_ALG_SHA_256) && alg == PSA_ALG_SHA_256) {
		/* Do nothing */
	} else if (IS_ENABLED(PSA_WANT_ALG_SHA_512) && alg == PSA_ALG_SHA_512) {
		/* Do nothing */
	} else {
		return PSA_ERROR_NOT_SUPPORTED;
	}
...
psa_status_t psa_destroy_key(mbedtls_svc_key_id_t key_id)
{
	...
	return psa_driver_wrapper_destroy_key(&attr);
}
...
psa_status_t psa_lock_key(mbedtls_svc_key_id_t key_id)
{
	...
	return psa_driver_wrapper_lock_key(&attr);
}
