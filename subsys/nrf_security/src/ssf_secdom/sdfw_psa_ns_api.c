...
#include <tfm/sdfw_psa_ipc_service.h>

#include "sdfw_psa_ns_ipc.h"
						  ...
						  psa_outvec *out_vec, size_t out_len)
{
	...
	psa_status_t ipc_status = sdfw_psa_ns_ipc_setup();
	...
	ipc_service_buf[INDEX_HANDLE] = handle; /* i.e. TFM_CRYPTO_HANDLE defined to 0x40000100U */
	ipc_service_buf[INDEX_IN_VEC] = (uint32_t)in_vec;
	ipc_service_buf[INDEX_IN_LEN] = in_len;
	ipc_service_buf[INDEX_OUT_VEC] = (uint32_t)out_vec;
	ipc_service_buf[INDEX_OUT_LEN] = out_len;
	ipc_service_buf[INDEX_STATUS_PTR] = (uint32_t)&status;

	int32_t ret = sdfw_psa_ns_ipc_send(ipc_service_buf, sizeof(ipc_service_buf));
