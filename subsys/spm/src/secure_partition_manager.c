...
/*
 * Example code for a Secure Partition Manager
...
static void spm_config_flash(void)
{
	...
	for (i = SECURE_FLASH_REGION_FIRST;
		...
		printk("SPM: SPU: set region %u as Secure\n", i);
	}
	...
	for (i = NONSECURE_FLASH_REGION_FIRST;
		...
		NRF_SPU->FLASHREGION[i].PERM =
			...
			((SPU_FLASHREGION_PERM_LOCK_Locked <<
					SPU_FLASHREGION_PERM_LOCK_Pos)
				& SPU_FLASHREGION_PERM_LOCK_Msk);
		printk("SPM: SPU: set Flash region %u as Non-Secure\n",
			i);
	}
}
...
static void spm_config_sram(void)
{
	...
	for (i = SECURE_SRAM_REGION_FIRST;
		...
		printk("SPM: SPU: set SRAM region %u as Secure\n", i);
	}
	...
	for (i = NONSECURE_SRAM_REGION_FIRST;
		...
		printk("SPM: SPU: set SRAM region %u as Non-Secure\n",
			i);
	}
}

static void spm_config_peripheral(u8_t id, u32_t dma_present)
{
...
static void spm_config_peripherals(void)
{
	 ...
	 */
	printk("SPM: configure peripherals\n");

	/* Configure GPIO pins to be non-Secure */
	NRF_SPU->GPIOPORT[0].PERM = 0;

	/* Configure Clock Control as Non-Secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_CLOCK), 0);
	/* Configure RTC1 as Non-Secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_RTC1), 0);
	/* Configure IPC as Non-Secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_IPC_S), 0);
	/* Configure NVMC as Non-Secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_NVMC), 0);
	/* Configure VMC as Non-Secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_VMC_S), 0);
	/* Configure GPIO as Non-Secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_P0), 0);
	/* Make GPIOTE1 interrupt available in Non-Secure domain */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_GPIOTE1_NS), 0);
	/* Configure UARTE1 as Non-Secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_UARTE1), 0);
	/* Configure UARTE2 as Non-Secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_UARTE2), 0);
	/* Configure EGU1 as Non-Secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_EGU1_S), 0);
	/* Configure EGU2 as Non-Secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_EGU2_S), 0);
	/* Configure FPU as non-secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_FPU_S), 0);
	/* Configure TWIM2 as non-secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_TWIM2_S), 0);
	/* Configure SPIM3 as non-secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_SPIM3_S), 0);
	/* Configure TIMER0 as non-secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_TIMER0_S), 0);
	/* Configure TIMER1 as non-secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_TIMER1_S), 0);
	/* Configure TIMER2 as non-secure */
	spm_config_peripheral(NRFX_PERIPHERAL_ID_GET(NRF_TIMER2_S), 0);
}
...
static void spm_configure_ns(const tz_nonsecure_setup_conf_t *spm_ns_conf)
{
...
static void spm_jump(void)
{
	...
	if (TZ_NONSECURE_FUNC_PTR_IS_NS(reset_ns)) {
		printk("SPM: prepare to jump to Non-Secure image\n");
		...
		printk("SPM: wrong pointer type: 0x%x\n",
			(u32_t)reset_ns);
	}
}
