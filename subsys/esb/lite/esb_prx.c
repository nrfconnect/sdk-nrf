...
static void normal_ack_radio_irq(void)
{
	...
	if (skip_ack) {
		...
		/* Start radio if timer has reached the compare value
		 * (we are after inter-frame delay).
		 */
...
static void fast_sw_normal_ack_graph(bool enable_graph, uint32_t flags)
{
	 ...
	 * if sending ACK: DISABLE_GROUP(0), ENABLE_GROUP(1)
	 *     if group 1 enabled too late: RADIO(TXEN)
	 *     if not: nothing to do
	 * if skipping ACK: TIMER(STOP), RADIO(START)
	 */
...
static void normal_sw_normal_ack_graph(bool enable_graph, uint32_t flags)
{
	...
	/* Simplified normal_ack_radio_irq logic:
	 * if sending ACK: DISABLE_GROUP(0), ENABLE_GROUP(1)
	 *     if group 1 enabled too late: RADIO(DISABLE)
	 *     if not: nothing to do
	 * if skipping ACK: TIMER(STOP), RADIO(START)
	 */
	RADIO_IRQ(normal_ack_radio_irq, CRCOK);
...
static void fast_sw_fast_ack_radio_irq(void)
{
	...
	if (skip_ack) {
		...
		if (nrf_timer_cc_get(ESB_TIMER, 5) < nrf_timer_cc_get(ESB_TIMER, 0)) {
			...
			esb_enable_group(0);
			nrf_radio_task_trigger(NRF_RADIO, NRF_RADIO_TASK_START);
		} else if (initial_state == NRF_RADIO_STATE_TXIDLE
			|| nrf_radio_event_check(NRF_RADIO, NRF_RADIO_EVENT_READY)) {

			/* TX ramp-up is done, we need to switch back to RX manually. */
			esb_enable_group(0);
			nrf_radio_task_trigger(NRF_RADIO, NRF_RADIO_TASK_RXEN);
		} else {
			/* TX ramp-up is still ongoing, so group 1 will transition us
			 * automatically to RX.
			 */
			DISABLE_ALL_IRQS();
			esb_enable_group(1);
			/* There is a small chance that READY event occurred somewhere in
			 * the middle of above operation. We can't say if before or after,
			 * so we need to handle it manually here.
			 */
			if (nrf_radio_event_check(NRF_RADIO, NRF_RADIO_EVENT_READY)) {
				esb_disable_group(1);
				esb_enable_group(0);
				/* If we are still in TxIdle, we know that the READY event
				 * happened before enabling group 1, so we need to start RX.
				 */
				if (nrf_radio_state_get(NRF_RADIO) == NRF_RADIO_STATE_TXIDLE) {
					nrf_radio_task_trigger(NRF_RADIO, NRF_RADIO_TASK_RXEN);
				}
			}
			ENABLE_ALL_IRQS();
		...
		DISABLE_ALL_IRQS();
		esb_enable_group(0);
		if (initial_state == NRF_RADIO_STATE_TXIDLE
			|| nrf_radio_event_check(NRF_RADIO, NRF_RADIO_EVENT_READY)) {
...
static void fast_sw_fast_ack_graph(bool enable_graph, uint32_t flags)
{
	 ...
	 *     if group 0 enabled too late: RADIO(START)
	 *     if not: nothing to do
	 * if skipping ACK: TIMER(STOP)
	 *     if stopped before COMPARE0: ENABLE_GROUP(0), RADIO(START)
	 *     if not:
	 *         if TX ramp-up done: ENABLE_GROUP(0), RADIO(RXEN)
	 *         if TX ramp-up ongoing: ENABLE_GROUP(1)
	 *             if group 1 enabled too late: DISABLE_GROUP(1), ENABLE_GROUP(0), RADIO(RXEN)
	 */
	...
	GROUP_BEGIN(0);
	CONNECT_2_TO_1(true,
		REUSABLE(RADIO(READY), reuse_ready_event),
		RADIO(CRCERROR),                           /* --> */ RADIO(START));
	GROUP_END();

	GROUP_BEGIN(1);
	CONNECT_1_TO_3(false,
		REUSE(RADIO(READY), reuse_ready_event), /* --> */ RADIO(RXEN),
								  DISABLE_GROUP_DECL(1),
								  ENABLE_GROUP(0));
	GROUP_END();
	DISABLE_GROUP_IMPL(1);
	...
	CONNECT_1_TO_2(false,
		RADIO(PHYEND), /* --> */ RADIO(RXEN),
					 DISABLE_GROUP_DECL(2));
	GROUP_END();
	DISABLE_GROUP_IMPL(2);
