...
#define ESB_MAX_PRX_CHANNELS 13
#define ESB_MAX_PRX_GROUPS 4
...
/** Number of EGU channels used directly in graphs. Other EGU channels may be
 *  used in DPPI variant internally by graph-building macros.
 */
#define ESB_RESERVED_EGU_CHANNELS 2
...
/** Macro that can be used only as argument of CONNECT_x_TO_y, REUSABLE, or REUSE macros.
 *  It represents a EGU event or task of specific number.
 */
#define EGU(number) \
	nrf_egu_publish_set(ESB_EGU, _ESB_EGU_EVENT_TRIGGERED(number), current_channel),           \
	nrf_egu_publish_clear(ESB_EGU, _ESB_EGU_EVENT_TRIGGERED(number)),                          \
	nrf_egu_subscribe_set(ESB_EGU, _ESB_EGU_TASK_TRIGGER(number), current_channel),            \
	nrf_egu_subscribe_clear(ESB_EGU, _ESB_EGU_TASK_TRIGGER(number))
#define _ESB_EGU_EVENT_TRIGGERED(n) (NRF_EGU_EVENT_TRIGGERED0 + (n) *                              \
				     (NRF_EGU_EVENT_TRIGGERED1 - NRF_EGU_EVENT_TRIGGERED0))
#define _ESB_EGU_TASK_TRIGGER(n) (NRF_EGU_TASK_TRIGGER0 + (n) *                                    \
				  (NRF_EGU_TASK_TRIGGER1 - NRF_EGU_TASK_TRIGGER0))
 ...
 *  REUSE macro. In PPI, this macro does nothing. Uses 1 DPPI channel.
 */
...
#define GRAPH_BEGIN(this_function_name)                                                            \
	...
	uint8_t egu_event = ESB_RESERVED_EGU_CHANNELS;                                             \
...
/** Special type of connection useful to control event flow from software when we not sure if
 *  the software can handle it on time. It avoids race conditions that may happen in that case.
 *  The event is accepted only if "accepting_group" is enabled, otherwise it is ignored.
 *  If event was accepted, but "passing_group" is disabled, the event stays within the connection
 *  and waits. We have two options here, we can enable "passing_group" it will pass the event
 *  and disable both groups to ensure one-shot behavior. Or we can disable "accepting_group"
 *  to discard the event that is waiting to be passed. To do another one-shot event, we need to
 *  enable "accepting_group" again.
 *  The "DISABLE_GROUP(passing_group)" task is used internally and cannot be used in other
 *  connections. You can use them only in software.
 *  Uses 2 DPPI channels and 2 EGU channels.
 */
#define CONNECT_CONTROLLED_ONE_SHOT_1_TO_1(accepting_group, passing_group, initial_accept,         \
					   initial_pass, from1, to1)                               \
	do {                                                                                       \
		uint32_t accepting_mask;	                                                   \
		uint8_t holding_egu = egu_event;                                                   \
		uint8_t passing_egu = egu_event + 1;                                               \
		egu_event += 2;                                                                    \
		GROUP_BEGIN(accepting_group);                                                      \
		CONNECT_2_TO_2(initial_accept,                                                     \
			_REGROUP(from1),                                                           \
			EGU(holding_egu), /* -> */ EGU(holding_egu),                               \
						   EGU(passing_egu));                              \
		accepting_mask = used_mask;                                                        \
		GROUP_END();                                                                       \
		GROUP_BEGIN(passing_group);                                                        \
		CONNECT_1_TO_2(initial_pass,                                                       \
			EGU(passing_egu), /* -> */ DISABLE_GROUP_DECL(passing_group),              \
						   _REGROUP(to1));                                 \
		used_mask |= accepting_mask;                                                       \
		GROUP_END();                                                                       \
                DISABLE_GROUP_IMPL(passing_group);                                                 \
	} while(0);                                                                                \

#define _REGROUP(a, b, c, d) a, b, c, d

/** Works similar to CONNECT_CONTROLLED_ONE_SHOT_1_TO_1, but allows to select between two
 *  output tasks. You can select where to pass the event by enabling one of
 *  "passing_group1" or "passing_group2".
 *  Uses 3 DPPI channels and 3 EGU channels.
 */
#define CONNECT_CONTROLLED_ONE_SHOT_1_TO_1_AND_1(accepting_group, passing_group1, passing_group2,  \
						 initial_accept, initial_pass1, initial_pass2,     \
						 from1, to1_1, to1_2)                              \
	do {                                                                                       \
		uint32_t accepting_mask;	                                                   \
		uint8_t holding_egu = egu_event;                                                   \
		uint8_t passing1_egu = egu_event + 1;                                              \
		uint8_t passing2_egu = egu_event + 2;                                              \
		egu_event += 3;                                                                    \
		GROUP_BEGIN(accepting_group);                                                      \
		CONNECT_2_TO_3(initial_accept,                                                     \
			_REGROUP(from1),                                                           \
			EGU(holding_egu), /* -> */ EGU(holding_egu),                               \
						   EGU(passing1_egu),                              \
						   EGU(passing2_egu));                             \
		accepting_mask = used_mask;                                                        \
		GROUP_END();                                                                       \
		GROUP_BEGIN(passing_group1);                                                       \
		CONNECT_1_TO_2(initial_pass1,                                                      \
			EGU(passing1_egu), /* -> */ DISABLE_GROUP_DECL(passing_group1),            \
						    _REGROUP(to1_1));                              \
		used_mask |= accepting_mask;                                                       \
		GROUP_END();                                                                       \
                DISABLE_GROUP_IMPL(passing_group1);                                                \
		GROUP_BEGIN(passing_group2);                                                       \
		CONNECT_1_TO_2(initial_pass2,                                                      \
			EGU(passing2_egu), /* -> */ DISABLE_GROUP_DECL(passing_group2),            \
						    _REGROUP(to1_2));                              \
		used_mask |= accepting_mask;                                                       \
		GROUP_END();                                                                       \
                DISABLE_GROUP_IMPL(passing_group2);                                                \
	} while(0);                                                                                \
