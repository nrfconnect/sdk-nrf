...
static void invalidate_ping_entry_cb(zb_uint8_t index)
{
	...
	if (ping_entry) {
		...
		if (ping_entry->zcl_data.ping_req.cb) {
			ping_entry->zcl_data.ping_req.cb(PING_EVT_FRAME_TIMEOUT, delay_ms,
							 ping_entry);
		}
...
static struct ctx_entry *find_ping_entry_by_short(zb_uint16_t addr_short)
{
	...
	for (i = 0; i < CONFIG_ZIGBEE_SHELL_CTX_MGR_ENTRIES_NBR; i++) {
		...
		if ((ping_entry->type == CTX_MGR_PING_REQ_ENTRY_TYPE) && ping_entry->taken) {
			req_remote_addr = ping_entry->zcl_data.pkt_info.dst_addr;
		} else {
			continue;
		}
		...
		if (ping_entry->zcl_data.pkt_info.dst_addr_mode ==
			...
			if (zb_address_short_by_ieee(req_remote_addr.addr_long) == addr_short) {
				return ping_entry;
			}
		}
	}
...
static void zb_zcl_send_ping_frame(zb_uint8_t index)
{
	...
	if (ping_entry->type == CTX_MGR_PING_REQ_ENTRY_TYPE) {
		packet_info = &(ping_entry->zcl_data.pkt_info);

		/* Capture the sending time. */
		ping_entry->zcl_data.ping_req.sent_time = k_uptime_ticks();
	} else {
		packet_info = &(ping_entry->zcl_data.pkt_info);
	}
	...
	zb_err_code = zb_zcl_finish_and_send_packet_new(
		packet_info->buffer, packet_info->ptr, &(packet_info->dst_addr),
		packet_info->dst_addr_mode, packet_info->dst_ep, packet_info->ep,
		packet_info->prof_id, packet_info->cluster_id, packet_info->cb, 0,
		packet_info->disable_aps_ack, 0);
	...
	if (ping_entry->type == CTX_MGR_PING_REQ_ENTRY_TYPE) {
		...
		zb_err_code =
			ZB_SCHEDULE_APP_ALARM(invalidate_ping_entry_cb, index,
					      ZB_MILLISECONDS_TO_BEACON_INTERVAL(
						      ping_entry->zcl_data.ping_req.timeout_ms));

		if (zb_err_code != RET_OK) {
			zb_shell_print_error(ping_entry->shell, "Can not schedule timeout alarm.",
					     ZB_FALSE);
			ctx_mgr_delete_entry(ping_entry);
			return;
		}
		...
		if (ping_entry->zcl_data.ping_req.cb) {
			...
			ping_entry->zcl_data.ping_req.cb(PING_EVT_FRAME_SCHEDULED, time_diff,
							 ping_entry);
		}
	} else {
		if (zb_err_code != RET_OK) {
...
static zb_uint32_t get_request_duration(struct ctx_entry *req_data)
{
	 ...
	 */
	time_diff_ticks = k_uptime_ticks() - req_data->zcl_data.ping_req.sent_time;
...
static void dispatch_user_callback(zb_bufid_t bufid)
{
	...
	ping_cmd_status = ZB_BUF_GET_PARAM(bufid, zb_zcl_command_send_status_t);

	if (ping_cmd_status->dst_addr.addr_type == ZB_ZCL_ADDR_TYPE_SHORT) {
		short_addr = ping_cmd_status->dst_addr.u.short_addr;
	} else if (ping_cmd_status->dst_addr.addr_type == ZB_ZCL_ADDR_TYPE_IEEE) {
		short_addr = zb_address_short_by_ieee(ping_cmd_status->dst_addr.u.ieee_addr);
	} else {
	...
	if (req_data != NULL) {
		...
		if (ping_cmd_status->status == RET_OK) {
			...
			if (req_data->zcl_data.ping_req.cb) {
				if (req_data->zcl_data.ping_req.request_ack == 0) {
					req_data->zcl_data.ping_req.cb(PING_EVT_FRAME_SENT,
								       delay_ms, req_data);
				} else {
					req_data->zcl_data.ping_req.cb(PING_EVT_ACK_RECEIVED,
								       delay_ms, req_data);
				}
			}
			...
			LOG_ERR("Ping request returned error status: %d", ping_cmd_status->status);
		}
	} else {
		LOG_ERR("Couldn't find entry with ping request sent to %#.4x", short_addr);
	}
				   ...
				   struct ctx_entry *req_data)
{
	...
	case PING_EVT_FRAME_TIMEOUT:
		shell_error(req_data->shell, "Error: Request timed out after %u ms.", delay_ms);
		break;
...
static void ping_request_send(struct ctx_entry *ping_entry)
{
	...
	if (ping_entry->zcl_data.ping_req.count > PING_MAX_LENGTH) {
		if (ping_entry->zcl_data.ping_req.cb) {
			ping_entry->zcl_data.ping_req.cb(PING_EVT_ERROR, 0, ping_entry);
		}
		return;
	}
	...
	*(cmd_buf_ptr++) = ping_seq_num;
	...
	/* Fill Command Field. */
	if ((ping_entry->zcl_data.ping_req.request_echo) &&
	    (ping_entry->zcl_data.ping_req.request_ack)) {
		*(cmd_buf_ptr++) = PING_ECHO_REQUEST;
	} else if ((ping_entry->zcl_data.ping_req.request_echo) &&
		   (!ping_entry->zcl_data.ping_req.request_ack)) {
	...
	memset(cmd_buf_ptr, PING_ECHO_REQUEST_BYTE, ping_entry->zcl_data.ping_req.count);
	cmd_buf_ptr += ping_entry->zcl_data.ping_req.count;
	ping_entry->zcl_data.ping_req.ping_seq = ping_seq_num;
	...
	packet_info->disable_aps_ack =
		(ping_entry->zcl_data.ping_req.request_ack ? ZB_FALSE : ZB_TRUE);
...
static void ping_reply_send(struct ctx_entry *ping_entry)
{
	...
	cmd_buf_ptr = ZB_ZCL_START_PACKET(bufid);
	*(cmd_buf_ptr++) = ZIGBEE_PING_FRAME_CONTROL_FIELD;
	*(cmd_buf_ptr++) = ping_entry->zcl_data.ping_reply.ping_seq;
	*(cmd_buf_ptr++) = PING_ECHO_REPLY;
	memset(cmd_buf_ptr, PING_ECHO_REPLY_BYTE, ping_entry->zcl_data.ping_reply.count);
	cmd_buf_ptr += ping_entry->zcl_data.ping_reply.count;
	...
	packet_info->disable_aps_ack =
		(ping_entry->zcl_data.ping_reply.send_ack ? ZB_FALSE : ZB_TRUE);
...
static void ping_req_indicate(zb_bufid_t zcl_cmd_bufid)
{
	...
	zb_zcl_parsed_hdr_t *zcl_hdr = ZB_BUF_GET_PARAM(zcl_cmd_bufid, zb_zcl_parsed_hdr_t);
	...
	case PING_ECHO_REQUEST:
		tmp_request.zcl_data.ping_req.request_echo = 1;
		tmp_request.zcl_data.ping_req.request_ack = 1;
		break;
	...
	tmp_request.zcl_data.ping_req.ping_seq = zcl_hdr->seq_number;
	tmp_request.zcl_data.ping_req.count = zb_buf_len(zcl_cmd_bufid);
	tmp_request.zcl_data.ping_req.sent_time = k_uptime_ticks();
	...
	tmp_request.zcl_data.pkt_info.dst_addr_mode = ZB_APS_ADDR_MODE_16_ENDP_PRESENT;
	tmp_request.zcl_data.pkt_info.dst_addr.addr_short = remote_node_addr.u.short_addr;

	ping_ind_cb(PING_EVT_REQUEST_RECEIVED,
		    k_ticks_to_us_near32(tmp_request.zcl_data.ping_req.sent_time), &tmp_request);
}
...
zb_uint8_t zb_shell_ep_handler_ping(zb_bufid_t bufid)
{
	...
	if (zcl_hdr->cmd_id == PING_ECHO_REPLY) {
		...
		struct ctx_entry *req_data =
			ctx_mgr_find_ctx_entry(zcl_hdr->seq_number, CTX_MGR_PING_REQ_ENTRY_TYPE);

		if (req_data == NULL) {
			return ZB_FALSE;
		}

		packet_info = &(req_data->zcl_data.pkt_info);

		if (packet_info->dst_addr_mode == ZB_APS_ADDR_MODE_16_ENDP_PRESENT) {
			remote_short_addr = packet_info->dst_addr.addr_short;
		} else {
			remote_short_addr =
				zb_address_short_by_ieee(packet_info->dst_addr.addr_long);
		}
		...
		zb_ret_t zb_err_code = ZB_SCHEDULE_APP_ALARM_CANCEL(
			invalidate_ping_entry_cb, ctx_mgr_get_index_by_entry(req_data));
		ZB_ERROR_CHECK(zb_err_code);
		 ...
		 */
		if (req_data->zcl_data.ping_req.cb) {
			req_data->zcl_data.ping_req.cb(PING_EVT_ECHO_RECEIVED, time_diff, req_data);
		}
		...
		struct zcl_packet_info *packet_info;
		struct ctx_entry *ping_entry = ctx_mgr_new_entry(CTX_MGR_PING_REPLY_ENTRY_TYPE);

		if (ping_entry == NULL) {
			LOG_WRN("Request pool empty - wait for ongoing command to finish");
			return ZB_FALSE;
		}
		packet_info = &(ping_entry->zcl_data.pkt_info);
		 ...
		 * a sending function.
		 */
		ping_entry->zcl_data.ping_reply.count = len;
		ping_entry->zcl_data.ping_reply.ping_seq = zcl_hdr->seq_number;

		if (zcl_hdr->cmd_id == PING_ECHO_REQUEST) {
			LOG_DBG("PING echo request with APS ACK received");
			ping_entry->zcl_data.ping_reply.send_ack = 1;
		} else {
			LOG_DBG("PING echo request without APS ACK received");
			ping_entry->zcl_data.ping_reply.send_ack = 0;
		}

		if (remote_node_addr.addr_type == ZB_ZCL_ADDR_TYPE_SHORT) {
			packet_info->dst_addr.addr_short = remote_node_addr.u.short_addr;
		} else {
		...
		LOG_WRN("Unsupported Ping message received, cmd_id %d", zcl_hdr->cmd_id);
	}
...
int cmd_zb_ping(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (!ping_entry) {
		zb_shell_print_error(
			shell, "Request pool empty - wait for ongoing command to finish", ZB_FALSE);
		return -ENOEXEC;
	}
	...
	for (i = 1; i < (argc - 2); i++) {
		if (strcmp(argv[i], "--aps-ack") == 0) {
			ping_entry->zcl_data.ping_req.request_ack = 1;
		} else if (strcmp(argv[i], "--no-echo") == 0) {
			ping_entry->zcl_data.ping_req.request_echo = 0;
		}
	}

	ping_entry->zcl_data.pkt_info.dst_addr_mode =
		parse_address(argv[argc - 2], &(ping_entry->zcl_data.pkt_info.dst_addr), ADDR_ANY);
	...
	if (!zb_shell_sscan_uint(argv[argc - 1], (uint8_t *)&ping_entry->zcl_data.ping_req.count, 2,
				 10)) {
...
int cmd_zb_ping_generic(const struct shell *shell, struct ping_req *ping_req,
			struct zcl_packet_info *pkt_info)
{
	struct ctx_entry *ping_entry = ctx_mgr_new_entry(CTX_MGR_PING_REQ_ENTRY_TYPE);

	if (!ping_entry || !ping_req) {
		zb_shell_print_error(shell, "Couldn't get free entry to store data", ZB_FALSE);
		return -ENOEXEC;
	}
	...
	if (ping_req->count > PING_MAX_LENGTH) {
		shell_print(shell, "Note: Ping payload size exceeds maximum possible size, "
			   "assuming maximum size of %u bytes", PING_MAX_LENGTH);
		ping_entry->zcl_data.ping_req.count = PING_MAX_LENGTH;
	}
