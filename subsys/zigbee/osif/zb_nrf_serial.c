 ...
 */
static u8_t uart_rx_buf_mem[CONFIG_ZIGBEE_UART_RX_BUF_LEN +
 ...
 */
static void rx_buf_push(u8_t *buf, size_t len)
{
	size_t cpy_len = 0;
 ...
 */
static size_t rx_buf_pop(u8_t *buf, size_t len)
{
	size_t copied = 0;
	size_t cpy_len = 0;
 ...
 */
static u8_t *rx_buf_get_next(void)
{
...
static void uart_evt_handler(struct uart_event *evt, void *user_data)
{
	...
	case UART_RX_BUF_RELEASED:
		if (uart_rx_buf && (uart_rx_buf_offset == uart_rx_buf_len)) {
			...
			if (zigbee_schedule_work(uart_rx_notify, 0)) {
				uart_rx_buf_offset = 0;
				uart_rx_buf = NULL;
				k_sem_give(&rx_fill_buffer_sem);
			}
