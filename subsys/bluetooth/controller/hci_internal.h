...
#ifdef CONFIG_BT_HCI_CUSTOM_EXTENSION
 ...
 * It is suggested to call this function from a SYS_INIT with a
 * HCI_INTERNAL_HCI_EXTENSION_INIT_PRIORITY priority.
 * The passed handler struct needs to have static lifetime.
 *
 * Multiple handlers can be registered. The order in which the handlers
 * are called is not defined.
 *
 * @param[in] handler A pointer to a statically allocated extension handler struct.
 */
void hci_internal_user_extension_handler_register(
