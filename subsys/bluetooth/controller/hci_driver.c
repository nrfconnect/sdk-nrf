...
/* The Controller needs at least SDC_DEFAULT_RX_PACKET_COUNT RX buffers. */
#define SDC_ACL_RX_BUFFER_COUNT MAX(CONFIG_BT_CTLR_RX_BUFFERS, SDC_DEFAULT_RX_PACKET_COUNT)

#define MASTER_MEM_SIZE (SDC_MEM_PER_MASTER_LINK( \
	MAX_TX_PACKET_SIZE, \
	MAX_RX_PACKET_SIZE, \
	CONFIG_BT_BUF_ACL_TX_COUNT, \
	SDC_ACL_RX_BUFFER_COUNT) \
...
static int configure_memory_usage(void)
{
	...
	cfg.buffer_cfg.rx_packet_count = SDC_ACL_RX_BUFFER_COUNT;
