...
/*
 * Workaround for mobile apps (like nRF Connect) that don't properly close
 * PB-GATT connection after provisioning, especially after DFU. We force
 * disconnect mesh connections after provisioning to ensure the app
 * reconnects via Proxy service.
 */
...
#ifdef CONFIG_BT_MESH_PB_GATT
	/* Schedule disconnect after 500ms to allow provisioning completion
	 * message to be sent, then force transition from PB-GATT to Proxy.
	 * This works around mobile apps that don't properly close PB-GATT.
	 */
	k_work_schedule(&prov_disconnect_work, K_MSEC(500));
#endif
}
