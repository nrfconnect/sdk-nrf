...
void bt_rpc_encode_bt_conn(struct nrf_rpc_cbor_ctx *encoder, const struct bt_conn *conn)
{
	...
	if (CONFIG_BT_MAX_CONN > 1) {
		...
		nrf_rpc_encode_uint(encoder, index);
	}
}
						    ...
						    void *handler_data)
{
	...
	callback_slot = (bt_conn_foreach_cb)nrf_rpc_decode_callback_call(ctx);
					...
					void *handler_data)
{
	...
	nrf_rpc_decode_buffer(ctx, res->dst, sizeof(bt_addr_le_t));
}
...
{
	data->tx_phy = nrf_rpc_decode_uint(ctx);
	data->rx_phy = nrf_rpc_decode_uint(ctx);
}
#endif /* defined(CONFIG_BT_USER_PHY_UPDATE) */
					 ...
					 struct bt_conn_le_data_len_info *data)
{
	...
	data->rx_max_len = nrf_rpc_decode_uint(ctx);
	data->rx_max_time = nrf_rpc_decode_uint(ctx);
}
#endif /* defined(CONFIG_BT_USER_DATA_LEN_UPDATE) */
...
{
	info->type = nrf_rpc_decode_uint(ctx);
	info->role = nrf_rpc_decode_uint(ctx);
	...
	if (info->type == BT_CONN_TYPE_LE) {
		...
		LOCK_CONN_INFO();
		info->le.src = nrf_rpc_decode_buffer(ctx, &conn->src, sizeof(bt_addr_le_t));
		info->le.dst = nrf_rpc_decode_buffer(ctx, &conn->dst, sizeof(bt_addr_le_t));
		info->le.local = nrf_rpc_decode_buffer(ctx, &conn->local, sizeof(bt_addr_le_t));
		info->le.remote = nrf_rpc_decode_buffer(ctx, &conn->remote, sizeof(bt_addr_le_t));
#if defined(CONFIG_BT_USER_PHY_UPDATE)
		...
		info->state = nrf_rpc_decode_uint(ctx);
...
{
	remote_info->type = nrf_rpc_decode_uint(ctx);
	remote_info->version = nrf_rpc_decode_uint(ctx);
	remote_info->manufacturer = nrf_rpc_decode_uint(ctx);
	remote_info->subversion = nrf_rpc_decode_uint(ctx);

	if (remote_info->type == BT_CONN_TYPE_LE && conn != NULL) {
		LOCK_CONN_INFO();
		remote_info->le.features =
			nrf_rpc_decode_buffer(ctx, &conn->features, sizeof(conn->features));
		UNLOCK_CONN_INFO();
	} else {
		/* non-LE connection types are not supported. */
		nrf_rpc_decoder_invalid(ctx, ZCBOR_ERR_UNKNOWN);
				     ...
				     const struct bt_conn_le_phy_param *data)
{
	...
	nrf_rpc_encode_uint(encoder, data->pref_tx_phy);
	nrf_rpc_encode_uint(encoder, data->pref_rx_phy);
}
...
int bt_conn_set_security(struct bt_conn *conn, bt_security_t sec)
{
	...
	bt_rpc_encode_bt_conn(&ctx, conn);
	nrf_rpc_encode_uint(&ctx, (uint32_t)sec);
					 ...
					 void *handler_data)
{
	...
	res->result = (bt_security_t)nrf_rpc_decode_uint(ctx);
}
...
uint8_t bt_conn_enc_key_size(const struct bt_conn *conn)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_CONN_ENC_KEY_SIZE_RPC_CMD,
				&ctx, nrf_rpc_rsp_decode_u8, &result);
						 ...
						 void *handler_data)
{
	...
	return;
decoding_error:
	report_decoding_error(BT_CONN_CB_RECYCLED_CALL_RPC_CMD, handler_data);
}
							 ...
							 void *handler_data)
{
	...
	latency = nrf_rpc_decode_uint(ctx);
	timeout = nrf_rpc_decode_uint(ctx);
							  ...
							  void *handler_data)
{
	...
	conn = bt_rpc_decode_bt_conn(ctx);
	rpa = nrf_rpc_decode_buffer(ctx, &rpa_data, sizeof(bt_addr_le_t));
	identity = nrf_rpc_decode_buffer(ctx, &identity_data, sizeof(bt_addr_le_t));
							 ...
							 void *handler_data)
{
	...
	conn = bt_rpc_decode_bt_conn(ctx);
	level = (bt_security_t)nrf_rpc_decode_uint(ctx);
	err = (enum bt_security_err)nrf_rpc_decode_uint(ctx);
...
int bt_le_oob_set_legacy_tk(struct bt_conn *conn, const uint8_t *tk)
{
	...
	scratchpad_size += NRF_RPC_SCRATCHPAD_ALIGN(tk_size);

	NRF_RPC_CBOR_ALLOC(&bt_rpc_grp, ctx, buffer_size_max);
	nrf_rpc_encode_uint(&ctx, scratchpad_size);

	bt_rpc_encode_bt_conn(&ctx, conn);
	nrf_rpc_encode_buffer(&ctx, tk, tk_size);
...
{
	data->io_capability = nrf_rpc_decode_uint(ctx);
	data->oob_data_flag = nrf_rpc_decode_uint(ctx);
	data->auth_req = nrf_rpc_decode_uint(ctx);
	data->max_enc_key_size = nrf_rpc_decode_uint(ctx);
	data->init_key_dist = nrf_rpc_decode_uint(ctx);
	data->resp_key_dist = nrf_rpc_decode_uint(ctx);
}
							...
							void *handler_data)
{
	...
	conn = bt_rpc_decode_bt_conn(ctx);
	info = nrf_rpc_decode_buffer(ctx, &info_data, sizeof(struct bt_conn_oob_info));
							     ...
							     void *handler_data)
{
	...
	conn = bt_rpc_decode_bt_conn(ctx);
	bonded = nrf_rpc_decode_bool(ctx);
							   ...
							   void *handler_data)
{
	...
	conn = bt_rpc_decode_bt_conn(ctx);
	reason = (enum bt_security_err)nrf_rpc_decode_uint(ctx);
