					  ...
					  struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	nrf_rpc_decode_buffer(ctx, res->data, sizeof(uint8_t) * (res->size));
}
...
static void bt_rpc_get_check_list(uint8_t *data, size_t size)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_RPC_GET_CHECK_LIST_RPC_CMD, &ctx,
				bt_rpc_get_check_list_rpc_rsp, &result);
}
...
static void validate_config(void)
{
	...
	if (!validated) {
		...
		if (!bt_rpc_validate_check_list(data, size)) {
			nrf_rpc_err(-EINVAL, NRF_RPC_ERR_SRC_RECV, &bt_rpc_grp, BT_ENABLE_RPC_CMD,
				    NRF_RPC_PACKET_TYPE_CMD);
		}
	}
}
					       ...
					       struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	err = nrf_rpc_decode_int(ctx);
	callback_slot = (bt_ready_cb_t)nrf_rpc_decode_callback_call(ctx);
...
int bt_enable(bt_ready_cb_t cb)
{
	...
	if (IS_ENABLED(CONFIG_SETTINGS)) {
		...
		nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_SETTINGS_LOAD_RPC_CMD, &ctx,
...
bool bt_is_ready(void)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_IS_READY_RPC_CMD, &ctx, nrf_rpc_rsp_decode_bool,
				&result);
...
int bt_set_name(const char *name)
{
	...
	scratchpad_size += NRF_RPC_SCRATCHPAD_ALIGN(name_strlen + 1);

	NRF_RPC_CBOR_ALLOC(&bt_rpc_grp, ctx, buffer_size_max);
	nrf_rpc_encode_uint(&ctx, scratchpad_size);

	nrf_rpc_encode_str(&ctx, name, name_strlen);

	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_SET_NAME_RPC_CMD, &ctx, nrf_rpc_rsp_decode_i32,
				&result);
...
static void bt_get_name_out_rpc_rsp(const struct nrf_rpc_group *group, struct nrf_rpc_cbor_ctx *ctx,
				    void *handler_data)
{
	struct bt_get_name_out_rpc_res *res = (struct bt_get_name_out_rpc_res *)handler_data;

	res->result = nrf_rpc_decode_bool(ctx);
	(void)nrf_rpc_decode_str(ctx, res->name, (res->size));
}
...
static bool bt_get_name_out(char *name, size_t size)
{
	...
	scratchpad_size += NRF_RPC_SCRATCHPAD_ALIGN(size);
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_GET_NAME_OUT_RPC_CMD, &ctx, bt_get_name_out_rpc_rsp,
				&result);
...
int bt_set_appearance(uint16_t new_appearance)
{
	...
	nrf_rpc_encode_uint(&ctx, new_appearance);

	nrf_rpc_cbor_cmd(&bt_rpc_grp, BT_SET_APPEARANCE_RPC_CMD, &ctx, nrf_rpc_rsp_decode_u16,
			 &result);
...
static uint16_t bt_get_appearance_from_remote(void)
{
	...
	nrf_rpc_cbor_cmd(&bt_rpc_grp, BT_GET_APPEARANCE_RPC_CMD, &ctx, nrf_rpc_rsp_decode_u16,
			 &appearance);
...
static void bt_id_get_rpc_rsp(const struct nrf_rpc_group *group, struct nrf_rpc_cbor_ctx *ctx,
			      void *handler_data)
{
	struct bt_id_get_rpc_res *res = (struct bt_id_get_rpc_res *)handler_data;

	*(res->count) = nrf_rpc_decode_uint(ctx);
	nrf_rpc_decode_buffer(ctx, res->addrs, *(res->count) * sizeof(bt_addr_le_t));
}
...
void bt_id_get(bt_addr_le_t *addrs, size_t *count)
{
	...
	scratchpad_size += NRF_RPC_SCRATCHPAD_ALIGN(*count * sizeof(bt_addr_le_t));

	NRF_RPC_CBOR_ALLOC(&bt_rpc_grp, ctx, buffer_size_max);
	nrf_rpc_encode_uint(&ctx, scratchpad_size);

	nrf_rpc_encode_uint(&ctx, *count);

	result.count = count;
	result.addrs = addrs;

	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_ID_GET_RPC_CMD, &ctx, bt_id_get_rpc_rsp, &result);
}
...
static void bt_id_create_rpc_rsp(const struct nrf_rpc_group *group, struct nrf_rpc_cbor_ctx *ctx,
				 void *handler_data)
{
	struct bt_id_create_rpc_res *res = (struct bt_id_create_rpc_res *)handler_data;

	res->result = nrf_rpc_decode_int(ctx);
	nrf_rpc_decode_buffer(ctx, res->addr, sizeof(bt_addr_le_t));
	nrf_rpc_decode_buffer(ctx, res->irk, sizeof(uint8_t) * 16);
}
...
int bt_id_create(bt_addr_le_t *addr, uint8_t *irk)
{
	...
	scratchpad_size += NRF_RPC_SCRATCHPAD_ALIGN(irk_size);

	NRF_RPC_CBOR_ALLOC(&bt_rpc_grp, ctx, buffer_size_max);
	nrf_rpc_encode_uint(&ctx, scratchpad_size);

	nrf_rpc_encode_buffer(&ctx, addr, sizeof(bt_addr_le_t));
	nrf_rpc_encode_buffer(&ctx, irk, irk_size);

	result.addr = addr;
	result.irk = irk;

	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_ID_CREATE_RPC_CMD, &ctx, bt_id_create_rpc_rsp,
				&result);
...
static void bt_id_reset_rpc_rsp(const struct nrf_rpc_group *group, struct nrf_rpc_cbor_ctx *ctx,
				void *handler_data)
{
	struct bt_id_reset_rpc_res *res = (struct bt_id_reset_rpc_res *)handler_data;
...
int bt_id_reset(uint8_t id, bt_addr_le_t *addr, uint8_t *irk)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_ID_RESET_RPC_CMD, &ctx, bt_id_reset_rpc_rsp,
				&result);
...
int bt_id_delete(uint8_t id)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_ID_DELETE_RPC_CMD, &ctx, nrf_rpc_rsp_decode_i32,
				&result);
...
static void bt_data_enc(struct nrf_rpc_cbor_ctx *encoder, const struct bt_data *data)
{
	...
	nrf_rpc_encode_uint(encoder, data->data_len);
...
{
	nrf_rpc_encode_uint(encoder, data->type);
	nrf_rpc_encode_uint(encoder, data->options);
	nrf_rpc_encode_uint(encoder, data->interval);
	nrf_rpc_encode_uint(encoder, data->window);
	nrf_rpc_encode_uint(encoder, data->timeout);
	nrf_rpc_encode_uint(encoder, data->interval_coded);
	nrf_rpc_encode_uint(encoder, data->window_coded);
}
#endif /* defined(CONFIG_BT_OBSERVER) */

static void net_buf_simple_dec(struct nrf_rpc_scratchpad *scratchpad, struct net_buf_simple *data)
{
	size_t len;

	data->data = nrf_rpc_decode_buffer_into_scratchpad(scratchpad, &len);
	data->len = len;
	data->size = data->len;
						 ...
						 struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	rssi = nrf_rpc_decode_int(ctx);
	adv_type = nrf_rpc_decode_uint(ctx);
	net_buf_simple_dec(&scratchpad, &buf);
	callback_slot = (bt_le_scan_cb_t *)nrf_rpc_decode_callback_call(ctx);
...
static size_t bt_le_adv_param_sp_size(const struct bt_le_adv_param *data)
{
	...
	scratchpad_size += !data->peer ? 0 : NRF_RPC_SCRATCHPAD_ALIGN(sizeof(bt_addr_le_t));
...
{
	nrf_rpc_encode_uint(encoder, data->id);
	nrf_rpc_encode_uint(encoder, data->sid);
	nrf_rpc_encode_uint(encoder, data->secondary_max_skip);
	nrf_rpc_encode_uint(encoder, data->options);
	nrf_rpc_encode_uint(encoder, data->interval_min);
	nrf_rpc_encode_uint(encoder, data->interval_max);
	nrf_rpc_encode_buffer(encoder, data->peer, sizeof(bt_addr_le_t));
}
		    ...
		    const struct bt_data *sd, size_t sd_len)
{
	...
	for (size_t i = 0; i < ad_len; i++) {
		...
		scratchpad_size += NRF_RPC_SCRATCHPAD_ALIGN(sizeof(struct bt_data));
	...
	nrf_rpc_encode_uint(&ctx, ad_len);

	for (size_t i = 0; i < ad_len; i++) {
		bt_data_enc(&ctx, &ad[i]);
	}

	nrf_rpc_encode_uint(&ctx, sd_len);

	for (size_t i = 0; i < sd_len; i++) {
		bt_data_enc(&ctx, &sd[i]);
	}

	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_ADV_START_RPC_CMD, &ctx, nrf_rpc_rsp_decode_i32,
				&result);
			  ...
			  size_t sd_len)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_ADV_UPDATE_DATA_RPC_CMD, &ctx,
...
int bt_le_adv_stop(void)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_ADV_STOP_RPC_CMD, &ctx, nrf_rpc_rsp_decode_i32,
				&result);
...
static void bt_le_oob_dec(struct nrf_rpc_cbor_ctx *ctx, struct bt_le_oob *data)
{
	...
	nrf_rpc_decode_buffer(ctx, data->le_sc_data.r, 16 * sizeof(uint8_t));
	nrf_rpc_decode_buffer(ctx, data->le_sc_data.c, 16 * sizeof(uint8_t));
}
...
{
	data->num_sent = nrf_rpc_decode_uint(ctx);
}

static void bt_le_ext_adv_scanned_info_dec(struct nrf_rpc_scratchpad *scratchpad,
					   struct bt_le_ext_adv_scanned_info *data)
{
	data->addr = nrf_rpc_decode_buffer_into_scratchpad(scratchpad, NULL);
}
						       ...
						       void *handler_data)
{
	...
	callback_slot = (bt_le_ext_adv_cb_sent)nrf_rpc_decode_callback_call(ctx);
							    ...
							    void *handler_data)
{
	...
	callback_slot = (bt_le_ext_adv_cb_connected)nrf_rpc_decode_callback_call(ctx);
							  ...
							  void *handler_data)
{
	...
	callback_slot = (bt_le_ext_adv_cb_scanned)nrf_rpc_decode_callback_call(ctx);
...
{
	nrf_rpc_encode_callback(encoder, data->sent);
	nrf_rpc_encode_callback(encoder, data->connected);
	nrf_rpc_encode_callback(encoder, data->scanned);
}
					 ...
					 struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	res->result = nrf_rpc_decode_int(ctx);
	*(res->adv) = (struct bt_le_ext_adv *)(uintptr_t)nrf_rpc_decode_uint(ctx);
}
			 ...
			 struct bt_le_ext_adv **adv)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_EXT_ADV_CREATE_RPC_CMD, &ctx,
				bt_le_ext_adv_create_rpc_rsp, &result);
...
int bt_le_ext_adv_start(struct bt_le_ext_adv *adv, struct bt_le_ext_adv_start_param *param)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_EXT_ADV_START_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_ext_adv_stop(struct bt_le_ext_adv *adv)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_EXT_ADV_STOP_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
			   ...
			   const struct bt_data *sd, size_t sd_len)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_EXT_ADV_SET_DATA_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_ext_adv_update_param(struct bt_le_ext_adv *adv, const struct bt_le_adv_param *param)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_EXT_ADV_UPDATE_PARAM_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_ext_adv_delete(struct bt_le_ext_adv *adv)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_EXT_ADV_DELETE_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
uint8_t bt_le_ext_adv_get_index(struct bt_le_ext_adv *adv)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_EXT_ADV_GET_INDEX_RPC_CMD, &ctx,
...
int bt_le_ext_adv_get_info(const struct bt_le_ext_adv *adv, struct bt_le_ext_adv_info *info)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_EXT_ADV_GET_INFO_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_ext_adv_oob_get_local(struct bt_le_ext_adv *adv, struct bt_le_oob *oob)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_EXT_ADV_OOB_GET_LOCAL_RPC_CMD, &ctx,
				bt_le_ext_adv_oob_get_local_rpc_rsp, &result);
...
int bt_le_per_adv_set_param(struct bt_le_ext_adv *adv, const struct bt_le_per_adv_param *param)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_SET_PARAM_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_per_adv_set_data(const struct bt_le_ext_adv *adv, const struct bt_data *ad, size_t ad_len)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_SET_DATA_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_per_adv_start(struct bt_le_ext_adv *adv)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_START_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_per_adv_stop(struct bt_le_ext_adv *adv)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_STOP_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
				    ...
				    uint16_t service_data)
{
	...
	nrf_rpc_encode_uint(&ctx, service_data);

	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_SET_INFO_TRANSFER_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
uint8_t bt_le_per_adv_sync_get_index(struct bt_le_per_adv_sync *per_adv_sync)
{
	...
	nrf_rpc_encode_uint(&ctx, (uintptr_t)per_adv_sync);

	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_SYNC_GET_INDEX_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_u8, &result);
				  ...
				  const struct bt_le_per_adv_sync_param *data)
{
	...
	nrf_rpc_encode_uint(encoder, data->sid);
	nrf_rpc_encode_uint(encoder, data->options);
	nrf_rpc_encode_uint(encoder, data->skip);
	nrf_rpc_encode_uint(encoder, data->timeout);
}
					      ...
					      struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	res->result = nrf_rpc_decode_int(ctx);
	*(res->out_sync) = (struct bt_le_per_adv_sync *)(uintptr_t)nrf_rpc_decode_uint(ctx);
}
			      ...
			      struct bt_le_per_adv_sync **out_sync)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_SYNC_CREATE_RPC_CMD, &ctx,
				bt_le_per_adv_sync_create_rpc_rsp, &result);
...
int bt_le_per_adv_sync_delete(struct bt_le_per_adv_sync *per_adv_sync)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_SYNC_DELETE_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_per_adv_sync_recv_enable(struct bt_le_per_adv_sync *per_adv_sync)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_SYNC_RECV_ENABLE_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_per_adv_sync_recv_disable(struct bt_le_per_adv_sync *per_adv_sync)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_SYNC_RECV_DISABLE_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
				...
				const struct bt_conn *conn, uint16_t service_data)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_SYNC_TRANSFER_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
					  ...
					  const struct bt_le_per_adv_sync_transfer_param *param)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_SYNC_TRANSFER_SUBSCRIBE_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_per_adv_sync_transfer_unsubscribe(const struct bt_conn *conn)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_SYNC_TRANSFER_UNSUBSCRIBE_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_per_adv_list_add(const bt_addr_le_t *addr, uint8_t sid)
{
	...
	nrf_rpc_encode_buffer(&ctx, addr, sizeof(bt_addr_le_t));
	nrf_rpc_encode_uint(&ctx, sid);

	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_LIST_ADD_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_per_adv_list_remove(const bt_addr_le_t *addr, uint8_t sid)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_LIST_REMOVE_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_per_adv_list_clear(void)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_PER_ADV_LIST_CLEAR_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);

	return result;
}

static void bt_le_per_adv_sync_synced_info_dec(struct nrf_rpc_scratchpad *scratchpad,
					       struct bt_le_per_adv_sync_synced_info *data)
{
	...
	data->phy = nrf_rpc_decode_uint(ctx);
	data->recv_enabled = nrf_rpc_decode_bool(ctx);
	data->service_data = nrf_rpc_decode_uint(ctx);
#if defined(CONFIG_BT_CONN)
	data->conn = bt_rpc_decode_bt_conn(ctx);
#else
	data->conn = 0;
					       ...
					       struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	sync = (struct bt_le_per_adv_sync *)nrf_rpc_decode_uint(ctx);
...
static void bt_le_per_adv_sync_term_info_dec(struct nrf_rpc_scratchpad *scratchpad,
...
static void bt_le_per_adv_sync_recv_info_dec(struct nrf_rpc_scratchpad *scratchpad,
					     struct bt_le_per_adv_sync_recv_info *data)
{
	...
	data->rssi = nrf_rpc_decode_int(ctx);
	data->cte_type = nrf_rpc_decode_uint(ctx);
}
...
int bt_le_scan_start(const struct bt_le_scan_param *param, bt_le_scan_cb_t cb)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_SCAN_START_RPC_CMD, &ctx, nrf_rpc_rsp_decode_i32,
				&result);
...
int bt_le_scan_stop(void)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_SCAN_STOP_RPC_CMD, &ctx, nrf_rpc_rsp_decode_i32,
				&result);

	return result;
}

static void bt_le_scan_recv_info_dec(struct nrf_rpc_scratchpad *scratchpad,
				     struct bt_le_scan_recv_info *data)
{
	...
	data->rssi = nrf_rpc_decode_int(ctx);
	data->tx_power = nrf_rpc_decode_int(ctx);
	data->adv_type = nrf_rpc_decode_uint(ctx);
	data->adv_props = nrf_rpc_decode_uint(ctx);
	data->interval = nrf_rpc_decode_uint(ctx);
	data->primary_phy = nrf_rpc_decode_uint(ctx);
	data->secondary_phy = nrf_rpc_decode_uint(ctx);
}
...
static void bt_le_scan_cb_register_on_remote(void)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_SCAN_CB_REGISTER_ON_REMOTE_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_void, NULL);
}
...
int bt_le_filter_accept_list_add(const bt_addr_le_t *addr)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_FILTER_ACCEPT_LIST_ADD_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_filter_accept_list_remove(const bt_addr_le_t *addr)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_FILTER_ACCEPT_LIST_REMOVE_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_filter_accept_list_clear(void)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_ACCEPT_LIST_CLEAR_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_set_chan_map(uint8_t chan_map[5])
{
	...
	scratchpad_size += NRF_RPC_SCRATCHPAD_ALIGN(chan_map_size);
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_SET_CHAN_MAP_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_i32, &result);
...
int bt_le_oob_get_local(uint8_t id, struct bt_le_oob *oob)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_OOB_GET_LOCAL_RPC_CMD, &ctx,
				bt_le_oob_get_local_rpc_rsp, &result);
...
int bt_unpair(uint8_t id, const bt_addr_le_t *addr)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_UNPAIR_RPC_CMD, &ctx, nrf_rpc_rsp_decode_i32,
				&result);
						    ...
						    void *handler_data)
{
	...
	callback_slot = (bt_foreach_bond_cb)nrf_rpc_decode_callback_call(ctx);
		     ...
		     void *user_data)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_FOREACH_BOND_RPC_CMD, &ctx, nrf_rpc_rsp_decode_void,
				NULL);
}
