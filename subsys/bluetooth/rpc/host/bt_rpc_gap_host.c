					      ...
					      struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	data = nrf_rpc_scratchpad_add(&scratchpad, sizeof(uint8_t) * size);
...

	{
		...
		nrf_rpc_encode_buffer(&ectx, data, sizeof(uint8_t) * size);
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_RPC_GET_CHECK_LIST_RPC_CMD);
}
...
static inline void bt_ready_cb_t_callback(int err, uint32_t callback_slot)
{
	...
	nrf_rpc_encode_int(&ctx, err);
	nrf_rpc_encode_callback_call(&ctx, callback_slot);

	nrf_rpc_cbor_evt_no_err(&bt_rpc_grp, BT_READY_CB_T_CALLBACK_RPC_EVT, &ctx);
}

NRF_RPC_CBKPROXY_HANDLER(bt_ready_cb_t_encoder, bt_ready_cb_t_callback, (int err), (err));
				  ...
				  void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_ENABLE_RPC_CMD);
}

NRF_RPC_CBOR_CMD_DECODER(bt_rpc_grp, bt_enable, BT_ENABLE_RPC_CMD, bt_enable_rpc_handler, NULL);
...
decoding_error:
	bt_rpc_report_decoding_error(BT_DISABLE_RPC_CMD);
}

NRF_RPC_CBOR_CMD_DECODER(bt_rpc_grp, bt_disable, BT_DISABLE_RPC_CMD, bt_disable_rpc_handler, NULL);
...
decoding_error:
	bt_rpc_report_decoding_error(BT_IS_READY_RPC_CMD);
}

NRF_RPC_CBOR_CMD_DECODER(bt_rpc_grp, bt_is_ready, BT_IS_READY_RPC_CMD, bt_is_ready_rpc_handler,
			 NULL);
				    ...
				    void *handler_data)
{
	...
	name = nrf_rpc_decode_str_into_scratchpad(&scratchpad, NULL);
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_SET_NAME_RPC_CMD);
}

NRF_RPC_CBOR_CMD_DECODER(bt_rpc_grp, bt_set_name, BT_SET_NAME_RPC_CMD, bt_set_name_rpc_handler,
			 NULL);
					...
					struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	name = nrf_rpc_scratchpad_add(&scratchpad, size);
...

	{
		...
		nrf_rpc_encode_bool(&ectx, result);
		nrf_rpc_encode_str(&ectx, name, name_strlen);
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_GET_NAME_OUT_RPC_CMD);
}
...
static void bt_get_appearance_rpc_handler(const struct nrf_rpc_group *group,
					  struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	uint16_t appearance;
	...
	nrf_rpc_rsp_send_uint(group, appearance);

	return;

decoding_error:
	bt_rpc_report_decoding_error(BT_GET_APPEARANCE_RPC_CMD);
}

NRF_RPC_CBOR_CMD_DECODER(bt_rpc_grp, bt_get_appearance, BT_GET_APPEARANCE_RPC_CMD,
			 bt_get_appearance_rpc_handler, NULL);

static void bt_set_appearance_rpc_handler(const struct nrf_rpc_group *group,
					  struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	int result;
	uint16_t appearance;

	appearance = nrf_rpc_decode_uint(ctx);
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_SET_APPEARANCE_RPC_CMD);
}
				  ...
				  void *handler_data)
{
	...
	*count = nrf_rpc_decode_uint(ctx);
	...
	addrs = nrf_rpc_scratchpad_add(&scratchpad, *count * sizeof(bt_addr_le_t));
...

	{
		...
		nrf_rpc_encode_uint(&ectx, *count);
		nrf_rpc_encode_buffer(&ectx, addrs, *count * sizeof(bt_addr_le_t));
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_ID_GET_RPC_CMD);
}

NRF_RPC_CBOR_CMD_DECODER(bt_rpc_grp, bt_id_get, BT_ID_GET_RPC_CMD, bt_id_get_rpc_handler, NULL);
				     ...
				     struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	irk = nrf_rpc_decode_buffer_into_scratchpad(&scratchpad, NULL);
...

	{
		...
		nrf_rpc_encode_buffer(&ectx, addr, sizeof(bt_addr_le_t));
		nrf_rpc_encode_buffer(&ectx, irk, sizeof(uint8_t) * 16);
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_ID_CREATE_RPC_CMD);
}

NRF_RPC_CBOR_CMD_DECODER(bt_rpc_grp, bt_id_create, BT_ID_CREATE_RPC_CMD, bt_id_create_rpc_handler,
			 NULL);
				    ...
				    void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_ID_RESET_RPC_CMD);
}

NRF_RPC_CBOR_CMD_DECODER(bt_rpc_grp, bt_id_reset, BT_ID_RESET_RPC_CMD, bt_id_reset_rpc_handler,
			 NULL);
				     ...
				     struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_ID_DELETE_RPC_CMD);
}
...
{
	data->type = nrf_rpc_decode_uint(ctx);
	data->options = nrf_rpc_decode_uint(ctx);
	data->interval = nrf_rpc_decode_uint(ctx);
	data->window = nrf_rpc_decode_uint(ctx);
	data->timeout = nrf_rpc_decode_uint(ctx);
	data->interval_coded = nrf_rpc_decode_uint(ctx);
	data->window_coded = nrf_rpc_decode_uint(ctx);
}
#endif /* defined(CONFIG_BT_OBSERVER) */

static size_t net_buf_simple_sp_size(struct net_buf_simple *data)
{
	return NRF_RPC_SCRATCHPAD_ALIGN(data->len);
}

static size_t net_buf_simple_buf_size(struct net_buf_simple *data)
{
	return 3 + data->len;
}

static void net_buf_simple_enc(struct nrf_rpc_cbor_ctx *encoder, struct net_buf_simple *data)
{
	nrf_rpc_encode_buffer(encoder, data->data, data->len);
}

static void bt_le_scan_cb_t_callback(const bt_addr_le_t *addr, int8_t rssi, uint8_t adv_type,
				     struct net_buf_simple *buf, uint32_t callback_slot)
{
	struct nrf_rpc_cbor_ctx ctx;
	...
	nrf_rpc_encode_int(&ctx, rssi);
	nrf_rpc_encode_uint(&ctx, adv_type);
	net_buf_simple_enc(&ctx, buf);
	nrf_rpc_encode_callback_call(&ctx, callback_slot);

	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_SCAN_CB_T_CALLBACK_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_void, NULL);
}

NRF_RPC_CBKPROXY_HANDLER(bt_le_scan_cb_t_encoder, bt_le_scan_cb_t_callback,
			 (const bt_addr_le_t *addr, int8_t rssi, uint8_t adv_type,
			  struct net_buf_simple *buf),
			 (addr, rssi, adv_type, buf));

#if defined(CONFIG_BT_BROADCASTER)

static void bt_le_adv_param_dec(struct nrf_rpc_scratchpad *scratchpad, struct bt_le_adv_param *data)
{
	struct nrf_rpc_cbor_ctx *ctx = scratchpad->ctx;

	data->id = nrf_rpc_decode_uint(ctx);
	data->sid = nrf_rpc_decode_uint(ctx);
	data->secondary_max_skip = nrf_rpc_decode_uint(ctx);
	data->options = nrf_rpc_decode_uint(ctx);
	data->interval_min = nrf_rpc_decode_uint(ctx);
	data->interval_max = nrf_rpc_decode_uint(ctx);
	data->peer = nrf_rpc_decode_buffer_into_scratchpad(scratchpad, NULL);
}
					...
					struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	ad_len = nrf_rpc_decode_uint(ctx);
	ad = nrf_rpc_scratchpad_add(&scratchpad, ad_len * sizeof(struct bt_data));
	if (ad == NULL) {
		goto decoding_error;
	}
	...
	sd_len = nrf_rpc_decode_uint(ctx);
	sd = nrf_rpc_scratchpad_add(&scratchpad, sd_len * sizeof(struct bt_data));
	if (sd == NULL) {
		goto decoding_error;
	}
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_ADV_START_RPC_CMD);
}
					      ...
					      struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_ADV_UPDATE_DATA_RPC_CMD);
}
...
static size_t bt_le_oob_buf_size(const struct bt_le_oob *data)
{
	size_t buffer_size_max = 13;
...
static void bt_le_oob_enc(struct nrf_rpc_cbor_ctx *encoder, const struct bt_le_oob *data)
{
	nrf_rpc_encode_buffer(encoder, &data->addr, sizeof(bt_addr_le_t));
	nrf_rpc_encode_buffer(encoder, data->le_sc_data.r, 16 * sizeof(uint8_t));
	nrf_rpc_encode_buffer(encoder, data->le_sc_data.c, 16 * sizeof(uint8_t));
}

#if defined(CONFIG_BT_EXT_ADV)

K_MEM_SLAB_DEFINE(bt_rpc_ext_adv_cb_cache, sizeof(struct bt_le_ext_adv_cb),
		  CONFIG_BT_EXT_ADV_MAX_ADV_SET, sizeof(void *));
static struct bt_le_ext_adv_cb *ext_adv_cb_cache_map[CONFIG_BT_EXT_ADV_MAX_ADV_SET];

static void bt_le_ext_adv_sent_info_enc(struct nrf_rpc_cbor_ctx *encoder,
					const struct bt_le_ext_adv_sent_info *data)
{
	nrf_rpc_encode_uint(encoder, data->num_sent);
}

static void bt_le_ext_adv_connected_info_enc(struct nrf_rpc_cbor_ctx *encoder,
					     const struct bt_le_ext_adv_connected_info *data)
{
	bt_rpc_encode_bt_conn(encoder, data->conn);
}

static size_t bt_le_ext_adv_scanned_info_sp_size(const struct bt_le_ext_adv_scanned_info *data)
{
	size_t scratchpad_size = 0;

	scratchpad_size += NRF_RPC_SCRATCHPAD_ALIGN(sizeof(bt_addr_le_t));

	return scratchpad_size;
}

static size_t bt_le_ext_adv_scanned_info_buf_size(const struct bt_le_ext_adv_scanned_info *data)
{
...
static void bt_le_ext_adv_scanned_info_enc(struct nrf_rpc_cbor_ctx *encoder,
					   const struct bt_le_ext_adv_scanned_info *data)
{
	nrf_rpc_encode_buffer(encoder, data->addr, sizeof(bt_addr_le_t));
}

static void bt_le_ext_adv_cb_sent_callback(struct bt_le_ext_adv *adv,
					   ...
					   uint32_t callback_slot)
{
	...
	nrf_rpc_encode_uint(&ctx, (uintptr_t)adv);
	bt_le_ext_adv_sent_info_enc(&ctx, info);
	nrf_rpc_encode_callback_call(&ctx, callback_slot);

	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_EXT_ADV_CB_SENT_CALLBACK_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_void, NULL);
}

NRF_RPC_CBKPROXY_HANDLER(bt_le_ext_adv_cb_sent_encoder, bt_le_ext_adv_cb_sent_callback,
			 (struct bt_le_ext_adv *adv, struct bt_le_ext_adv_sent_info *info),
			 (adv, info));

static void bt_le_ext_adv_cb_connected_callback(struct bt_le_ext_adv *adv,
						...
						uint32_t callback_slot)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_EXT_ADV_CB_CONNECTED_CALLBACK_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_void, NULL);
}

NRF_RPC_CBKPROXY_HANDLER(bt_le_ext_adv_cb_connected_encoder, bt_le_ext_adv_cb_connected_callback,
			 (struct bt_le_ext_adv *adv, struct bt_le_ext_adv_connected_info *info),
			 (adv, info));

static void bt_le_ext_adv_cb_scanned_callback(struct bt_le_ext_adv *adv,
					      ...
					      uint32_t callback_slot)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_EXT_ADV_CB_SCANNED_CALLBACK_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_void, NULL);
}

NRF_RPC_CBKPROXY_HANDLER(bt_le_ext_adv_cb_scanned_encoder, bt_le_ext_adv_cb_scanned_callback,
			 (struct bt_le_ext_adv *adv, struct bt_le_ext_adv_scanned_info *info),
			 (adv, info));

static void bt_le_ext_adv_cb_dec(struct nrf_rpc_cbor_ctx *ctx, struct bt_le_ext_adv_cb *data)
{
	data->sent =
		(bt_le_ext_adv_cb_sent)nrf_rpc_decode_callbackd(ctx, bt_le_ext_adv_cb_sent_encoder);
	data->connected = (bt_le_ext_adv_cb_connected)nrf_rpc_decode_callbackd(
		ctx, bt_le_ext_adv_cb_connected_encoder);
	data->scanned = (bt_le_ext_adv_cb_scanned)nrf_rpc_decode_callbackd(
		ctx, bt_le_ext_adv_cb_scanned_encoder);
}
					     ...
					     struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	if (!nrf_rpc_decode_is_undefined(ctx)) {
...

	{
		...
		nrf_rpc_encode_int(&ectx, result);
		nrf_rpc_encode_uint(&ectx, (uintptr_t)(*adv));
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_EXT_ADV_CREATE_RPC_CMD);
...
static void bt_le_ext_adv_start_param_dec(struct nrf_rpc_cbor_ctx *ctx,
					  struct bt_le_ext_adv_start_param *data)
{
	data->timeout = nrf_rpc_decode_uint(ctx);
	data->num_events = nrf_rpc_decode_uint(ctx);
}
					    ...
					    struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	adv = (struct bt_le_ext_adv *)nrf_rpc_decode_uint(ctx);
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_EXT_ADV_START_RPC_CMD);
}
					   ...
					   struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_EXT_ADV_STOP_RPC_CMD);
}
					       ...
					       struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_EXT_ADV_SET_DATA_RPC_CMD);
}
						   ...
						   struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_EXT_ADV_UPDATE_PARAM_RPC_CMD);
}
					     ...
					     struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_EXT_ADV_DELETE_RPC_CMD);
}
...
static void bt_le_ext_adv_info_dec(struct nrf_rpc_cbor_ctx *ctx, struct bt_le_ext_adv_info *data)
{
	data->id = nrf_rpc_decode_uint(ctx);
	data->tx_power = nrf_rpc_decode_int(ctx);
}
					       ...
					       struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	adv = (const struct bt_le_ext_adv *)nrf_rpc_decode_uint(ctx);
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_EXT_ADV_GET_INFO_RPC_CMD);
}
						    ...
						    void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_EXT_ADV_OOB_GET_LOCAL_RPC_CMD);
}
					 ...
					 struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	cb = (bt_le_scan_cb_t *)nrf_rpc_decode_callbackd(ctx, bt_le_scan_cb_t_encoder);
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_SCAN_START_RPC_CMD);
}
...
static size_t bt_le_scan_recv_info_sp_size(const struct bt_le_scan_recv_info *data)
{
	size_t scratchpad_size = 0;
...
static size_t bt_le_scan_recv_info_buf_size(const struct bt_le_scan_recv_info *data)
{
...
static void bt_le_scan_recv_info_enc(struct nrf_rpc_cbor_ctx *encoder,
				     const struct bt_le_scan_recv_info *data)
{
	nrf_rpc_encode_buffer(encoder, data->addr, sizeof(bt_addr_le_t));
	nrf_rpc_encode_uint(encoder, data->sid);
	nrf_rpc_encode_int(encoder, data->rssi);
	nrf_rpc_encode_int(encoder, data->tx_power);
	nrf_rpc_encode_uint(encoder, data->adv_type);
	nrf_rpc_encode_uint(encoder, data->adv_props);
	nrf_rpc_encode_uint(encoder, data->interval);
	nrf_rpc_encode_uint(encoder, data->primary_phy);
	nrf_rpc_encode_uint(encoder, data->secondary_phy);
}

static void bt_le_scan_cb_recv(const struct bt_le_scan_recv_info *info, struct net_buf_simple *buf)
{
	struct nrf_rpc_cbor_ctx ctx;
	size_t scratchpad_size = 0;
	size_t buffer_size_max = 5;
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_SCAN_CB_RECV_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_void, NULL);
}
...
static void bt_le_scan_cb_timeout(void)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_LE_SCAN_CB_TIMEOUT_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_void, NULL);
}
						     ...
						     void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_FILTER_ACCEPT_LIST_ADD_RPC_CMD);
}
					   ...
					   struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	chan_map = nrf_rpc_decode_buffer_into_scratchpad(&scratchpad, NULL);
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_SET_CHAN_MAP_RPC_CMD);
}
					    ...
					    struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_OOB_GET_LOCAL_RPC_CMD);
}
				  ...
				  void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_UNPAIR_RPC_CMD);
}

NRF_RPC_CBOR_CMD_DECODER(bt_rpc_grp, bt_unpair, BT_UNPAIR_RPC_CMD, bt_unpair_rpc_handler, NULL);
#endif /* defined(CONFIG_BT_CONN) */

#if (defined(CONFIG_BT_CONN) && defined(CONFIG_BT_SMP))
static size_t bt_bond_info_buf_size(const struct bt_bond_info *data)
{
	size_t buffer_size_max = 3;
...
static void bt_foreach_bond_cb_callback(const struct bt_bond_info *info, void *user_data,
					uint32_t callback_slot)
{
	struct nrf_rpc_cbor_ctx ctx;
	size_t buffer_size_max = 8;
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, BT_FOREACH_BOND_CB_CALLBACK_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_void, NULL);
}

NRF_RPC_CBKPROXY_HANDLER(bt_foreach_bond_cb_encoder, bt_foreach_bond_cb_callback,
					...
					struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	id = nrf_rpc_decode_uint(ctx);
	func = (bt_foreach_bond_cb)nrf_rpc_decode_callbackd(ctx, bt_foreach_bond_cb_encoder);
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_FOREACH_BOND_RPC_CMD);
}
					       ...
					       struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_PER_ADV_LIST_ADD_RPC_CMD);
}
						  ...
						  struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_PER_ADV_LIST_REMOVE_RPC_CMD);
}
...
static void bt_le_per_adv_param_dec(struct nrf_rpc_cbor_ctx *ctx, struct bt_le_per_adv_param *data)
{
	data->interval_min = nrf_rpc_decode_uint(ctx);
	data->interval_max = nrf_rpc_decode_uint(ctx);
	data->options = nrf_rpc_decode_uint(ctx);
						...
						struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_PER_ADV_SET_PARAM_RPC_CMD);
}
					       ...
					       struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_PER_ADV_SET_DATA_RPC_CMD);
}
					    ...
					    struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_PER_ADV_START_RPC_CMD);
}
							...
							void *handler_data)
{
	...
	service_data = nrf_rpc_decode_uint(ctx);
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_PER_ADV_SET_INFO_TRANSFER_RPC_CMD);
}
						     ...
						     void *handler_data)
{
	...
	per_adv_sync = (struct bt_le_per_adv_sync *)nrf_rpc_decode_uint(ctx);
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_PER_ADV_SYNC_GET_INDEX_RPC_CMD);
}
...
static void bt_le_per_adv_sync_param_dec(struct nrf_rpc_cbor_ctx *ctx,
					 struct bt_le_per_adv_sync_param *data)
{
	nrf_rpc_decode_buffer(ctx, &data->addr, sizeof(bt_addr_le_t));
	data->sid = nrf_rpc_decode_uint(ctx);
	data->options = nrf_rpc_decode_uint(ctx);
	data->skip = nrf_rpc_decode_uint(ctx);
	data->timeout = nrf_rpc_decode_uint(ctx);
}
						  ...
						  struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
...

	{
		...
		nrf_rpc_encode_int(&ectx, result);
		nrf_rpc_encode_uint(&ectx, (uintptr_t)(*out_sync));
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_PER_ADV_SYNC_CREATE_RPC_CMD);
}
						  ...
						  struct nrf_rpc_cbor_ctx *ctx, void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_PER_ADV_SYNC_DELETE_RPC_CMD);
}
						       ...
						       void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_PER_ADV_SYNC_RECV_ENABLE_RPC_CMD);
}
							...
							void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_PER_ADV_SYNC_RECV_DISABLE_RPC_CMD);
}
						    ...
						    void *handler_data)
{
	...
	per_adv_sync = (const struct bt_le_per_adv_sync *)nrf_rpc_decode_uint(ctx);
	conn = bt_rpc_decode_bt_conn(ctx);
	service_data = nrf_rpc_decode_uint(ctx);
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_PER_ADV_SYNC_TRANSFER_RPC_CMD);
}
								...
								void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_PER_ADV_SYNC_TRANSFER_UNSUBSCRIBE_RPC_CMD);
}
...
static void bt_le_per_adv_sync_transfer_param_dec(struct nrf_rpc_cbor_ctx *ctx,
							      ...
							      void *handler_data)
{
	...
	return;
decoding_error:
	bt_rpc_report_decoding_error(BT_LE_PER_ADV_SYNC_TRANSFER_SUBSCRIBE_RPC_CMD);
}
...
static size_t
bt_le_per_adv_sync_synced_info_sp_size(const struct bt_le_per_adv_sync_synced_info *data)
{
	size_t scratchpad_size = 0;
...
static size_t
bt_le_per_adv_sync_synced_info_buf_size(const struct bt_le_per_adv_sync_synced_info *data)
{
...
static void bt_le_per_adv_sync_synced_info_enc(struct nrf_rpc_cbor_ctx *encoder,
					       const struct bt_le_per_adv_sync_synced_info *data)
{
	nrf_rpc_encode_buffer(encoder, data->addr, sizeof(bt_addr_le_t));
	nrf_rpc_encode_uint(encoder, data->sid);
	nrf_rpc_encode_uint(encoder, data->interval);
	nrf_rpc_encode_uint(encoder, data->phy);
	nrf_rpc_encode_bool(encoder, data->recv_enabled);
	nrf_rpc_encode_uint(encoder, data->service_data);
	bt_rpc_encode_bt_conn(encoder, data->conn);
}
				   ...
				   struct bt_le_per_adv_sync_synced_info *info)
{
	...
	nrf_rpc_encode_uint(&ctx, (uintptr_t)sync);
	bt_le_per_adv_sync_synced_info_enc(&ctx, info);

	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, PER_ADV_SYNC_CB_SYNCED_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_void, NULL);
}

static size_t bt_le_per_adv_sync_term_info_sp_size(const struct bt_le_per_adv_sync_term_info *data)
{
	size_t scratchpad_size = 0;
...
static size_t bt_le_per_adv_sync_term_info_buf_size(const struct bt_le_per_adv_sync_term_info *data)
{
	size_t buffer_size_max = 5;
...
static void bt_le_per_adv_sync_term_info_enc(struct nrf_rpc_cbor_ctx *encoder,
				 ...
				 const struct bt_le_per_adv_sync_term_info *info)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, PER_ADV_SYNC_CB_TERM_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_void, NULL);
}

static size_t bt_le_per_adv_sync_recv_info_sp_size(const struct bt_le_per_adv_sync_recv_info *data)
{
	size_t scratchpad_size = 0;
...
static size_t bt_le_per_adv_sync_recv_info_buf_size(const struct bt_le_per_adv_sync_recv_info *data)
{
	size_t buffer_size_max = 11;
...
static void bt_le_per_adv_sync_recv_info_enc(struct nrf_rpc_cbor_ctx *encoder,
					     const struct bt_le_per_adv_sync_recv_info *data)
{
	...
	nrf_rpc_encode_int(encoder, data->tx_power);
	nrf_rpc_encode_int(encoder, data->rssi);
	nrf_rpc_encode_uint(encoder, data->cte_type);
}
				 ...
				 struct net_buf_simple *buf)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, PER_ADV_SYNC_CB_RECV_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_void, NULL);
}

static void bt_le_per_adv_sync_state_info_enc(struct nrf_rpc_cbor_ctx *encoder,
					  ...
					  const struct bt_le_per_adv_sync_state_info *info)
{
	...
	nrf_rpc_cbor_cmd_no_err(&bt_rpc_grp, PER_ADV_SYNC_CB_STATE_CHANGED_RPC_CMD, &ctx,
				nrf_rpc_rsp_decode_void, NULL);
}

static struct bt_le_per_adv_sync_cb per_adv_sync_cb = {.synced = per_adv_sync_cb_synced,
						       .term = per_adv_sync_cb_term,
						       .recv = per_adv_sync_cb_recv,
						       .state_changed =
							       per_adv_sync_cb_state_changed};
