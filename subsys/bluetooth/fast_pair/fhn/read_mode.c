...
LOG_MODULE_REGISTER(fp_fhn_read_mode, CONFIG_BT_FAST_PAIR_LOG_LEVEL);
...
/* Timeout for the FHN recovery mode */
#define FHN_RECOVERY_TIMEOUT (CONFIG_BT_FAST_PAIR_FHN_READ_MODE_FHN_RECOVERY_TIMEOUT)

BUILD_ASSERT(FHN_RECOVERY_TIMEOUT > 0);

static const struct bt_fast_pair_fhn_read_mode_cb *read_mode_cb;

static void fhn_recovery_timeout_handle(struct k_work *w);

static K_WORK_DELAYABLE_DEFINE(fhn_recovery_timeout_work,
			       fhn_recovery_timeout_handle);

static void read_mode_exited(enum bt_fast_pair_fhn_read_mode mode)
{
	__ASSERT_NO_MSG(
		(mode == BT_FAST_PAIR_FHN_READ_MODE_FHN_RECOVERY) ||
		(mode == BT_FAST_PAIR_FHN_READ_MODE_DULT_ID));
...
static void fhn_recovery_timeout_handle(struct k_work *w)
{
	...
	read_mode_exited(BT_FAST_PAIR_FHN_READ_MODE_FHN_RECOVERY);
}
...
static int dult_id_payload_get(uint8_t *buf, size_t *len)
{
	...
	int err;
	uint8_t eid[FP_FHN_STATE_EID_LEN];
	...
	if (*len < payload_len) {
		LOG_ERR("FHN Read Mode: Insufficient Identifier buffer length");
		return -EINVAL;
	}

	err = fp_fhn_state_eid_read(eid);
	if (err) {
		LOG_ERR("FHN Read Mode: fp_fhn_state_eid_read failed: %d", err);
		return err;
	}
	...
	err = fp_crypto_hmac_sha256(local_sha,
				    ...
				    sizeof(recovery_key));
	if (err) {
		LOG_ERR("FHN Read Mode: fp_crypto_hmac_sha256 failed: %d", err);
		return err;
	}
...
{
	read_mode_exited(BT_FAST_PAIR_FHN_READ_MODE_DULT_ID);
}
...
static int read_mode_fhn_recovery_enter(void)
{
	(void) k_work_reschedule(&fhn_recovery_timeout_work,
				 K_MINUTES(FHN_RECOVERY_TIMEOUT));
...
static int read_mode_dult_id_enter(void)
{
	...
	err = dult_id_read_state_enter(fp_fhn_dult_integration_user_get());
	if (err) {
		return err;
	}
...
int bt_fast_pair_fhn_read_mode_enter(enum bt_fast_pair_fhn_read_mode mode)
{
	...
	if ((mode != BT_FAST_PAIR_FHN_READ_MODE_FHN_RECOVERY) &&
	    (mode != BT_FAST_PAIR_FHN_READ_MODE_DULT_ID)) {
		return -EINVAL;
	}
	...
	case BT_FAST_PAIR_FHN_READ_MODE_FHN_RECOVERY:
		return read_mode_fhn_recovery_enter();
...
int fp_fhn_read_mode_recovery_mode_request(bool *accepted)
{
	...
	*accepted = k_work_delayable_is_pending(&fhn_recovery_timeout_work);
...
{
	if (k_work_delayable_is_pending(&fhn_recovery_timeout_work)) {
		(void) k_work_cancel_delayable(&fhn_recovery_timeout_work);

		fhn_recovery_timeout_handle(NULL);
	}
}
...
static int init(void)
{
	...
	err = fp_fhn_callbacks_info_cb_register(&fhn_info_cb);
	if (err) {
		LOG_ERR("FHN: fp_fhn_callbacks_info_cb_register returned error: %d", err);
		return err;
	}

	if (IS_ENABLED(CONFIG_BT_FAST_PAIR_FHN_DULT)) {
		err = dult_id_read_state_cb_register(fp_fhn_dult_integration_user_get(),
						     &dult_id_read_state_cb);
		if (err) {
			LOG_ERR("FHN: dult_id_read_state_cb_register returned error: %d", err);
			return err;
		}
	}
...
#define FP_FHN_READ_MODE_INIT_PRIORITY	FP_ACTIVATION_INIT_PRIORITY_DEFAULT

#if CONFIG_BT_FAST_PAIR_FHN_DULT_INTEGRATION
BUILD_ASSERT(CONFIG_BT_FAST_PAIR_FHN_DULT_INTEGRATION_INIT_PRIORITY <
	     FP_FHN_READ_MODE_INIT_PRIORITY);
BUILD_ASSERT(FP_FHN_READ_MODE_INIT_PRIORITY <
	     CONFIG_BT_FAST_PAIR_FHN_STATE_INIT_PRIORITY_DULT);
#endif

FP_ACTIVATION_MODULE_REGISTER(fp_fhn_read_mode,
			      FP_FHN_READ_MODE_INIT_PRIORITY,
			      init,
			      uninit);
