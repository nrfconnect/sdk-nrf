...
#ifndef _FP_FHN_PF_OOB_MSG_H_
#define _FP_FHN_PF_OOB_MSG_H_
...
/**
 * @defgroup fp_fhn_pf_oob_msg OOB message encoders and decoders for Android ranging
 * @brief Internal API for encoding and decoding out-of-band (OOB) messages defined in
 *        the Android Ranging specification (used in the Fast Pair FHN Precision Finding
...
#define FP_FHN_PF_OOB_MSG_RANGING_TECH_PAYLOAD_MAX_NUM (4U)

/** ID of the out-of-band message (OOB) that is described by the @ref fp_fhn_pf_oob_msg
 *  structure.
 */
enum fp_fhn_pf_oob_msg_id {
	/* ID of the Ranging Capability Request message. */
	FP_FHN_PF_OOB_MSG_ID_RANGING_CAPABILITY_REQ = 0x00,

	/* ID of the Ranging Capability Response message. */
	FP_FHN_PF_OOB_MSG_ID_RANGING_CAPABILITY_RSP = 0x01,

	/* ID of the Ranging Configuration message. */
	FP_FHN_PF_OOB_MSG_ID_RANGING_CONFIG_REQ = 0x02,

	/* ID of the Ranging Configuration Response message. */
	FP_FHN_PF_OOB_MSG_ID_RANGING_CONFIG_RSP = 0x03,

	/* ID of the Stop Ranging message. */
	FP_FHN_PF_OOB_MSG_ID_STOP_RANGING_REQ = 0x06,

	/* ID of the Stop Ranging Response message. */
	FP_FHN_PF_OOB_MSG_ID_STOP_RANGING_RSP = 0x07,
};

/** Structure representing the out-of-band (OOB) message that respects the format
 *  from the Android Ranging specification.
 */
struct fp_fhn_pf_oob_msg {
	/* The version from the message header. */
	uint8_t version;

	/* The message ID from the message header. */
	enum fp_fhn_pf_oob_msg_id id;
 ...
 *  that originate from the Initiator device.
 *
...
/** Encode the provided out-of-band (OOB) message that respects the format from the
 ...
 *  can be used to define the output buffer for the @ref fp_fhn_pf_oob_msg_encode
