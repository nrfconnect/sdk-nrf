						...
						struct fp_fhn_pf_oob_msg *msg_desc)
{
	...
	if (msg_desc->ranging_config_req.ranging_tech_bm != rfu) {
		LOG_WRN("Precision Finding: OOB message: Ranging Configuration: "
			"RFU does not match the ranging technologies bitmask (0x%04X != 0x%04X)",
	...
	if (rfu != 0) {
		LOG_WRN("Precision Finding: OOB message: Ranging Configuration: "
			"missing configuration payloads for some ranging technologies (0x%04X)",
			rfu);
			     ...
			     struct net_buf_simple *msg_buf)
{
	...
	default:
		/* Unsupported or unknown message ID. */
		LOG_ERR("Precision Finding: OOB message: unsupported response message ID: 0x%02X",
			msg_desc->id);
