...
struct ranging_comm_chan {
	...
	/* Flags for the state machine of the communication channel. */
	...
	/* Bitmask of ranging technologies that were most recently requested. */
	uint16_t last_request_ranging_tech_bm;
};
...
static bool pf_ranging_req_handle(struct ranging_comm_chan *chan,
					     ...
					     struct net_buf_simple *req_buf)
{
	...
	chan->last_request_ranging_tech_bm = req.ranging_capability_req.ranging_tech_bm;
					 ...
					 struct net_buf_simple *req_buf)
{
	...
	chan->last_request_ranging_tech_bm = req.ranging_config_req.ranging_tech_bm;
				       ...
				       struct net_buf_simple *req_buf)
{
	...
	chan->last_request_ranging_tech_bm = req.stop_ranging_req.ranging_tech_bm;
				  ...
				  struct net_buf_simple *rsp_buf)
{
	...
	if (!atomic_test_bit(chan->flags, req_flag)) {
		LOG_ERR("Precision Finding: Ranging Response (0x%02X): "
			"cannot send response without the preceding request", rsp->id);
		return -ESRCH;
	}

	if ((ranging_tech_bm & ~chan->last_request_ranging_tech_bm) > 0) {
		LOG_ERR("Precision Finding: Ranging Response (0x%02X): "
			"response cannot refer to unrequested ranging technologies: 0x%04X",
			rsp->id, ranging_tech_bm & ~chan->last_request_ranging_tech_bm);
		return -EINVAL;
	}
...
void fp_fhn_pf_ranging_rsp_sent_notify(struct bt_conn *conn, uint16_t ranging_tech_bm)
{
	...
	ATOMIC_DEFINE(chan_flags, RANGING_COMM_CHAN_FLAGS);

	BUILD_ASSERT(sizeof(chan_flags) == sizeof(chan->flags));

	memcpy(chan_flags, chan->flags, sizeof(chan_flags));
	...
	if (active_req_flag_cnt != 1) {
		LOG_ERR("Precision Finding: fp_fhn_pf_ranging_rsp_sent_notify: "
			"incorrect number of pending requests: %d", active_req_flag_cnt);
		__ASSERT_NO_MSG(0);
	}

	if (atomic_test_bit(chan_flags, RANGING_COMM_CHAN_FLAG_STOP_REQ_PENDING)) {
		/* Clear the successfully stopped ranging technologies from the bitmask. */
		chan->ranging_tech_bm &= ~ranging_tech_bm;
	} else {
		/* Update the bitmask with the ranging technologies that are accepted by
		 * the local device.
		 */
		chan->ranging_tech_bm = ranging_tech_bm;
	}
...
static void pf_oob_msg_len_validate(void)
{
	...
	max_len = fp_fhn_pf_oob_msg_max_len_get(FP_FHN_PF_OOB_MSG_ID_STOP_RANGING_RSP);
