...
LOG_MODULE_REGISTER(fp_fhn_motion_detector, CONFIG_BT_FAST_PAIR_LOG_LEVEL);
...
{
	int err;
	const struct dult_user *user = fp_fhn_dult_integration_user_get();

	__ASSERT_NO_MSG(user->accessory_capabilities &
			BIT(DULT_ACCESSORY_CAPABILITY_MOTION_DETECTOR_UT_BIT_POS));

	if (!dult_motion_detector_cb_registered) {
		LOG_ERR("FHN: Motion detector callbacks not registered");
		return -EACCES;
	}

	err = dult_motion_detector_cb_register(user, &dult_motion_detector_cb);
	if (err) {
		LOG_ERR("FHN: dult_motion_detector_cb_register returned error: %d", err);
		return err;
	}
...
#define FP_FHN_MOTION_DETECTOR_INIT_PRIORITY	FP_ACTIVATION_INIT_PRIORITY_DEFAULT

#if CONFIG_BT_FAST_PAIR_FHN_DULT_INTEGRATION
BUILD_ASSERT(CONFIG_BT_FAST_PAIR_FHN_DULT_INTEGRATION_INIT_PRIORITY <
	     FP_FHN_MOTION_DETECTOR_INIT_PRIORITY);
BUILD_ASSERT(FP_FHN_MOTION_DETECTOR_INIT_PRIORITY <
	     CONFIG_BT_FAST_PAIR_FHN_STATE_INIT_PRIORITY_DULT);
#endif

FP_ACTIVATION_MODULE_REGISTER(fp_fhn_motion_detector,
			      FP_FHN_MOTION_DETECTOR_INIT_PRIORITY,
			      init,
			      uninit);
