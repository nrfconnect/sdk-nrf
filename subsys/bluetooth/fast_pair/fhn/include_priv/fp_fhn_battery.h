...
#ifndef _FP_FHN_BATTERY_H_
#define _FP_FHN_BATTERY_H_

#include <stdint.h>
#include <stddef.h>

/**
 * @defgroup fp_fhn_battery Fast Pair FHN Battery
 * @brief Internal API for Fast Pair FHN Battery
 *
 * @{
 */
...
/** Battery levels */
enum fp_fhn_battery_level {
	/* Battery level indication unsupported. */
	FP_FHN_BATTERY_LEVEL_NONE = 0x00,

	/* Normal battery level. */
	FP_FHN_BATTERY_LEVEL_NORMAL = 0x01,

	/* Low battery level. */
	FP_FHN_BATTERY_LEVEL_LOW = 0x02,

	/* Critically low battery level (battery replacement needed soon). */
	FP_FHN_BATTERY_LEVEL_CRITICAL = 0x03,
};

/** @brief Get the current battery level.
 *
 *  This function gets the current battery level as quantified by the FHN
 *  Accessory specification.
 *
 *  @return Battery level. See the @ref fp_fhn_battery_level for the detailed
 *  description.
 */
enum fp_fhn_battery_level fp_fhn_battery_level_get(void);

/** Battery callback structure.  */
struct fp_fhn_battery_cb {
	/** @brief Indicate a change in battery level.
	 *
	 *  Indicate that battery level as defined by the @ref fp_fhn_battery_level
