...
static enum bt_gatt_ots_olcp_res_code obj_manager_to_olcp_err_code_map(int err)
{
	switch (-err) {
	case EINVAL:
	...
	struct bt_gatt_ots *ots)
{
	...
	err = bt_gatt_ots_obj_manager_first_obj_get(ots->obj_manager,
						    &first_obj);
	if (err) {
		return obj_manager_to_olcp_err_code_map(err);
	}
...
static enum bt_gatt_ots_olcp_res_code olcp_goto_proc_execute(
	struct bt_gatt_ots *ots, u64_t id)
{
	int err;
...
static enum bt_gatt_ots_olcp_res_code olcp_proc_execute(
	struct bt_gatt_ots *ots, struct bt_gatt_ots_olcp_proc *proc)
{
...
static enum bt_gatt_ots_olcp_res_code olcp_command_decode(
	const u8_t *buf, struct bt_gatt_ots_olcp_proc *proc)
{
	...
	case BT_GATT_OTS_OLCP_PROC_GOTO:
	{
		...
		id = (struct bt_gatt_ots_obj_id *) &proc->goto_params.id;
		memcpy(id->ba, buf, sizeof(id->ba));
		break;
	}
	default:
...
static bool olcp_command_len_verify(enum bt_gatt_ots_olcp_proc_type type,
				    u16_t len)
{
	u16_t ref_len = OLCP_PROC_TYPE_SIZE;
	...
	case BT_GATT_OTS_OLCP_PROC_CLEAR_MARKING:
		break;
	case BT_GATT_OTS_OLCP_PROC_GOTO:
		ref_len += sizeof(((struct bt_gatt_ots_obj_id *) 0)->ba);
		break;
	case BT_GATT_OTS_OLCP_PROC_ORDER:
		ref_len += sizeof(u8_t);
		break;
...
{
	u8_t olcp_res[OLCP_RES_MAX_SIZE];
	u16_t olcp_res_len = 0;
	struct bt_gatt_ots *ots = (struct bt_gatt_ots *) olcp_attr->user_data;
			       ...
			       u8_t flags)
{
	...
	if (olcp_status != BT_GATT_OTS_OLCP_RES_SUCCESS) {
		...
		id = (struct bt_gatt_ots_obj_id *) &ots->cur_obj->metadata.id;
		LOG_HEXDUMP_DBG(id->ba, sizeof(id->ba),
			"Selecting a new Current Object with id:");

		if (ots->cb->obj_selected) {
			ots->cb->obj_selected(ots, conn,
					      ots->cur_obj->metadata.id);
		}
	}
...
void bt_gatt_ots_olcp_cfg_changed(const struct bt_gatt_attr *attr, u16_t value)
{
