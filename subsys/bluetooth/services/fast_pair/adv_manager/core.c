...
static uint32_t fp_adv_request_pairing_mode_bm;
static uint32_t fp_adv_request_suspend_rpa_bm;
...
static int bt_stack_advertising_start(void)
{
	...
	if (err) {
		LOG_ERR("Fast Pair Adv Manager: bt_le_ext_adv_start returned error: %d", err);
		return err;
	}
...
{
	if (err) {
		LOG_WRN("Fast Pair Adv Manager: connection failed (err %" PRIu8 ")", err);

		(void) k_work_submit(&fp_adv_restart_work);
	} else {
		LOG_INF("Fast Pair Adv Manager: connected");
	}
}

static void disconnected(struct bt_conn *conn, uint8_t reason)
{
	LOG_INF("Fast Pair Adv Manager: disconnected (reason %" PRIu8 ")", reason);
...
{
	return (fp_adv_request_pairing_mode_bm != 0);
}

static bool fp_adv_request_pairing_mode_bm_set(uint8_t idx, bool enabled)
{
	bool pairing_mode_to_be_activated;
	bool pairing_mode_active = bt_fast_pair_adv_manager_is_pairing_mode();

	WRITE_BIT(fp_adv_request_pairing_mode_bm, idx, enabled);
...
static bool fp_adv_manager_is_rpa_suspended(void)
{
	return (fp_adv_request_suspend_rpa_bm != 0);
}

static void fp_adv_request_suspend_rpa_bm_set(uint8_t idx, bool enabled)
{
	bool rpa_to_be_suspended;
	bool rpa_suspended = fp_adv_manager_is_rpa_suspended();

	WRITE_BIT(fp_adv_request_suspend_rpa_bm, idx, enabled);
	...
	struct bt_fast_pair_adv_manager_trigger *trigger, bool enable)
{
	...
	if (trigger->config) {
		if (trigger->config->pairing_mode) {
			force_update = fp_adv_request_pairing_mode_bm_set(idx, enable);
		}

		if (trigger->config->suspend_rpa) {
			fp_adv_request_suspend_rpa_bm_set(idx, enable);
		}
	}
