...
struct bt_gatt_ots_obj_manager {
	sys_dlist_t list;
	struct bt_gatt_ots_pool_item pool[CONFIG_BT_GATT_OTS_MAX_OBJ_CNT];
	bool is_assigned;
};
...
{
	u64_t i = OTS_OBJ_ID_TO_INDEX(id);
...
void *bt_gatt_ots_obj_manager_assign(void)
{
	static struct bt_gatt_ots_obj_manager
		obj_manager[CONFIG_BT_GATT_OTS_MAX_INST_CNT];
	struct bt_gatt_ots_obj_manager *cur_manager;


	for (cur_manager = obj_manager;
	     cur_manager != obj_manager + CONFIG_BT_GATT_OTS_MAX_INST_CNT;
	     cur_manager++) {
