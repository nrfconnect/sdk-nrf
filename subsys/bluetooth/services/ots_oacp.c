...
LOG_MODULE_DECLARE(bt_gatt_ots, CONFIG_BT_GATT_OTS_LOG_LEVEL);
	...
	struct bt_gatt_ots_oacp_proc *proc)
{
	...
	if (!BT_GATT_OTS_OBJ_GET_PROP_READ(ots->cur_obj->metadata.props)) {
	...
	if ((params->offset + (u64_t) params->len) >
...
static enum bt_gatt_ots_oacp_res_code oacp_command_decode(
	const u8_t *buf, u16_t len, struct bt_gatt_ots_oacp_proc *proc)
{
	const u8_t *end_buf = (buf + len);

	proc->type = *buf++;
	switch (proc->type) {
	case BT_GATT_OTS_OACP_PROC_CREATE:
		proc->create_params.size = sys_get_le32(buf);
		buf += sizeof(proc->create_params.size);
		if ((end_buf - buf) ==
			sizeof(proc->create_params.type.uuid_16)) {
			proc->create_params.type.uuid.type = BT_UUID_TYPE_16;
			proc->create_params.type.uuid_16.val =
				sys_get_le16(buf);
		} else if ((end_buf - buf) ==
			sizeof(proc->create_params.type.uuid_128)) {
			proc->create_params.type.uuid.type = BT_UUID_TYPE_128;
			memcpy(proc->create_params.type.uuid_128.val, buf,
				sizeof(proc->create_params.type.uuid_128.val));
		}
		break;
	case BT_GATT_OTS_OACP_PROC_DELETE:
		break;
	case BT_GATT_OTS_OACP_PROC_CHECKSUM_CALC:
		proc->cs_calc_params.offset = sys_get_le32(buf);
		buf += sizeof(proc->cs_calc_params.offset);
		proc->cs_calc_params.len = sys_get_le32(buf);
		break;
	case BT_GATT_OTS_OACP_PROC_EXECUTE:
		break;
	case BT_GATT_OTS_OACP_PROC_READ:
		proc->read_params.offset = sys_get_le32(buf);
		buf += sizeof(proc->read_params.offset);
		proc->read_params.len = sys_get_le32(buf);
		return BT_GATT_OTS_OACP_RES_SUCCESS;
	case BT_GATT_OTS_OACP_PROC_WRITE:
		proc->write_params.offset = sys_get_le32(buf);
		buf += sizeof(proc->write_params.offset);
		proc->write_params.len = sys_get_le32(buf);
		buf += sizeof(proc->write_params.len);
		proc->write_params.mode = *buf;
		break;
	case BT_GATT_OTS_OACP_PROC_ABORT:
	default:
		break;
...
static bool oacp_command_len_verify(struct bt_gatt_ots_oacp_proc *proc,
				    u16_t len)
{
	u16_t ref_len = OACP_PROC_TYPE_SIZE;

	switch (proc->type) {
	case BT_GATT_OTS_OACP_PROC_CREATE:
		ref_len += sizeof(proc->create_params.size);
		if (proc->create_params.type.uuid.type == BT_UUID_TYPE_16) {
			ref_len += sizeof(proc->create_params.type.uuid_16.val);
		} else if (proc->create_params.type.uuid.type == BT_UUID_TYPE_128) {
			ref_len += sizeof(proc->create_params.type.uuid_128.val);
		} else {
			return true;
		}
		break;
			      ...
			      struct bt_conn *conn)
{
	...
	ots     = CONTAINER_OF(l2cap_ctx, struct bt_gatt_ots, l2cap);
	...
	if (read_op->sent_len >= read_op->oacp_params.len) {
		...
		if (read_op->sent_len > read_op->oacp_params.len) {
			LOG_ERR("More bytes sent that the client requested");
		}

		ots->cur_obj->state.type = BT_GATT_OTS_OBJECT_IDLE_STATE;
		ots->cb->obj_read(ots, conn, ots->cur_obj->metadata.id, NULL,
				  0, offset);
		return;
	}

	len = read_op->oacp_params.len - read_op->sent_len;
	len = ots->cb->obj_read(ots, conn, ots->cur_obj->metadata.id,
...
static void oacp_read_proc_execute(struct bt_gatt_ots *ots,
				   struct bt_conn *conn)
{
...
{
	u8_t oacp_res[OACP_RES_MAX_SIZE];
	u16_t oacp_res_len = 0;
	struct bt_gatt_ots *ots = (struct bt_gatt_ots *) oacp_attr->user_data;
...
void bt_gatt_ots_oacp_cfg_changed(const struct bt_gatt_attr *attr, u16_t value)
{
