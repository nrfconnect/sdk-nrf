...
struct bt_gatt_ots_obj_size {
	u32_t cur;
	u32_t alloc;
};

struct bt_gatt_ots_obj_metadata {
	u64_t id;
	char *name;
	struct bt_gatt_ots_obj_type type;
	struct bt_gatt_ots_obj_size size;
	u32_t props;
};
...
struct bt_gatt_ots_object {
	struct bt_gatt_ots_obj_metadata metadata;
