...
NRF_RPC_CBOR_CMD_DECODER(rpc_utils_group, get_server_version, DEV_INFO_RPC_GET_VERSION,
...
NRF_RPC_CBOR_CMD_DECODER(rpc_utils_group, remote_shell_cmd, DEV_INFO_RPC_INVOKE_SHELL_CMD,
			    ...
			    void *handler_data)
{
	...
	crash_command = CRASH_GEN_RPC_ASSERT;
			    ...
			    void *handler_data)
{
	...
	crash_command = CRASH_GEN_RPC_HARD_FAULT;
	k_work_reschedule(&crash_work, K_MSEC(timeout_ms));
}
			    ...
			    void *handler_data)
{
	...
	crash_command = CRASH_GEN_RPC_STACK_OVERFLOW;
	k_work_reschedule(&crash_work, K_MSEC(timeout_ms));
}
