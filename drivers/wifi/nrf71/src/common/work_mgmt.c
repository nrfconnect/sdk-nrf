/*
 * Copyright (c) 2026 Nordic Semiconductor ASA
 *
 * SPDX-License-Identifier: LicenseRef-Nordic-5-Clause
 */

/**
 * @brief Work management functions for the nRF71 driver.
 */

#include <common/work_mgmt.h>

#include <zephyr/init.h>
#include <zephyr/logging/log.h>

LOG_MODULE_DECLARE(wifi_nrf, CONFIG_WIFI_NRF71_LOG_LEVEL);

K_THREAD_STACK_DEFINE(bh_wq_stack_area, CONFIG_NRF71_BH_WQ_STACK_SIZE);
struct k_work_q zep_wifi_bh_q;

#ifdef CONFIG_NRF71_TX_DONE_WQ_ENABLED
K_THREAD_STACK_DEFINE(tx_done_wq_stack_area, CONFIG_NRF71_TX_DONE_WQ_STACK_SIZE);
struct k_work_q zep_wifi_tx_done_q;
#endif /* CONFIG_NRF71_TX_DONE_WQ_ENABLED */

#ifdef CONFIG_NRF71_RX_WQ_ENABLED
K_THREAD_STACK_DEFINE(rx_wq_stack_area, CONFIG_NRF71_RX_WQ_STACK_SIZE);
struct k_work_q zep_wifi_rx_q;
#endif /* CONFIG_NRF71_RX_WQ_ENABLED */

struct zep_work_item zep_work_item[CONFIG_NRF71_WORKQ_MAX_ITEMS];

static int get_free_work_item_index(void)
{
	int i;

	for (i = 0; i < CONFIG_NRF71_WORKQ_MAX_ITEMS; i++) {
		if (zep_work_item[i].in_use) {
			continue;
		}
		return i;
	}

	return -1;
}

static void workqueue_callback(struct k_work *work)
{
	struct zep_work_item *item = CONTAINER_OF(work, struct zep_work_item, work);

	item->callback(item->data);
}

struct zep_work_item *nrf_wifi_work_alloc(enum zep_work_type type)
{
	int free_work_index = get_free_work_item_index();

	if (free_work_index < 0) {
		LOG_ERR("%s: Reached maximum work items", __func__);
		return NULL;
	}

	zep_work_item[free_work_index].in_use = true;
	zep_work_item[free_work_index].type = type;

	return &zep_work_item[free_work_index];
}

static int workqueue_init(void)
{
	k_work_queue_init(&zep_wifi_bh_q);

	k_work_queue_start(&zep_wifi_bh_q,
			   bh_wq_stack_area,
			   K_THREAD_STACK_SIZEOF(bh_wq_stack_area),
			   CONFIG_NRF71_BH_WQ_PRIORITY,
			   NULL);

	k_thread_name_set(zep_wifi_bh_q.thread_id, "nrf71_bh_wq");

#ifdef CONFIG_NRF71_TX_DONE_WQ_ENABLED
	k_work_queue_init(&zep_wifi_tx_done_q);

	k_work_queue_start(&zep_wifi_tx_done_q,
			   tx_done_wq_stack_area,
			   K_THREAD_STACK_SIZEOF(tx_done_wq_stack_area),
			   CONFIG_NRF71_TX_DONE_WQ_PRIORITY,
			   NULL);

	k_thread_name_set(zep_wifi_tx_done_q.thread_id, "nrf71_tx_done_wq");
#endif /* CONFIG_NRF71_TX_DONE_WQ_ENABLED */

#ifdef CONFIG_NRF71_RX_WQ_ENABLED
	k_work_queue_init(&zep_wifi_rx_q);

	k_work_queue_start(&zep_wifi_rx_q,
			   rx_wq_stack_area,
			   K_THREAD_STACK_SIZEOF(rx_wq_stack_area),
			   CONFIG_NRF71_RX_WQ_PRIORITY,
			   NULL);

	k_thread_name_set(zep_wifi_rx_q.thread_id, "nrf71_rx_wq");
#endif /* CONFIG_NRF71_RX_WQ_ENABLED */

	return 0;
}

void nrf_wifi_work_init(struct zep_work_item *item, void (*callback)(unsigned long),
			unsigned long data)
{
	item->callback = callback;
	item->data = data;

	k_work_init(&item->work, workqueue_callback);
}

void nrf_wifi_work_schedule(struct zep_work_item *item)
{
	if (item->type == ZEP_WORK_TYPE_BH) {
		k_work_submit_to_queue(&zep_wifi_bh_q, &item->work);
	}
#ifdef CONFIG_NRF71_TX_DONE_WQ_ENABLED
	else if (item->type == ZEP_WORK_TYPE_TX_DONE) {
		k_work_submit_to_queue(&zep_wifi_tx_done_q, &item->work);
	}
#endif /* CONFIG_NRF71_TX_DONE_WQ_ENABLED */
#ifdef CONFIG_NRF71_RX_WQ_ENABLED
	else if (item->type == ZEP_WORK_TYPE_RX) {
		k_work_submit_to_queue(&zep_wifi_rx_q, &item->work);
	}
#endif /* CONFIG_NRF71_RX_WQ_ENABLED */
}

void nrf_wifi_work_kill(struct zep_work_item *item)
{
	/* TODO: Based on context, use _sync version */
	k_work_cancel(&item->work);
}

void nrf_wifi_work_free(struct zep_work_item *item)
{
	item->in_use = 0;
}

SYS_INIT(workqueue_init, POST_KERNEL, 0);
