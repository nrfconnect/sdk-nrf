				     ...
				     const unsigned char *addr, int key_idx, uint32_t db_id)
{
	...
	attr = wifi_keys_key_attributes_init(max_size);
	...
	wifi_keys_reverse_byte_order_in_place(key_buf, max_size);

	uint32_t dest_address = wifi_keys_get_key_start_addr(type, db_id, crypto_key_index);

	if (dest_address == WIFI_KEYS_ADDR_INVALID) {
		LOG_ERR("%s: Invalid dest address for type %d, db %u, idx %u",
			__func__, type, db_id, crypto_key_index);
		return -EINVAL;
	}

	kmu_push_key_buffer_t kmu_push_buffer = {
		.usage_mask = KMU_PUSH_USAGE_MASK_PUSH_IMMEDIATELY |
			      KMU_PUSH_USAGE_MASK_DESTROY_AFTER,
		.dest_address = dest_address,
		.key_buffer = key_buf,
		.buffer_size = max_size,
	};

	status = psa_import_key(&attr, (const uint8_t *)&kmu_push_buffer,
				sizeof(kmu_push_buffer), &key_id);
