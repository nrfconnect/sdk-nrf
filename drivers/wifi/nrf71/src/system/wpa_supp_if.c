				     ...
				     const unsigned char *addr, int key_idx, uint32_t db_id)
{
	...
	dest_address = wifi_kmu_get_key_start_addr(type, db_id, crypto_key_index);
	...
	slot = wifi_kmu_get_next_slot(max_size);
			...
			unsigned int flags_and)
{
	...
	if (!net_eth_is_addr_valid((struct net_eth_addr *)&chg_sta.mac_addr)) {
		LOG_ERR("%s: Invalid peer MAC address: %s", __func__,
			net_sprint_ll_addr_buf(chg_sta.mac_addr, NET_ETH_ADDR_LEN, buf, sizeof(buf));
		goto out;
	}
