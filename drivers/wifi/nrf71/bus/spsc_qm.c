...
/**
 * @brief SPSC queue management layer for nRF71 Wi-Fi IPC.
 */
...
bool spsc32_pop(spsc_queue_t *queue, uint32_t *out_value)
{
	...
	memcpy(out_value, buf, sizeof(*out_value));
...
{
	/* Consume head: claim, copy, free. Same as pop to avoid stalling the queue. */
	return spsc32_pop(queue, out_value);
}
