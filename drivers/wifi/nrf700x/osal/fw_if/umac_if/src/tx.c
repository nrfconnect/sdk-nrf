		  ...
		  int queue)
{
	...
	if (desc >= (fpriv->num_tx_tokens_per_ac * NRF_WIFI_FMAC_AC_MAX)) {
			 ...
			 int queue)
{
	...
	if (cnt == fpriv->num_tx_tokens_per_ac) {
		for (desc = fpriv->num_tx_tokens_per_ac * NRF_WIFI_FMAC_AC_MAX;
		     desc < fpriv->num_tx_tokens;
		     desc++) {
		   ...
		   int peer_id)
{
	...
	struct nrf_wifi_fmac_vif_ctx *vif_ctx = fmac_dev_ctx->vif_ctx[vif_id];
			      ...
			      unsigned char *ac)
{
	...
	/* Reserved desc */
	if (desc < (fmac_dev_ctx->fpriv->num_tx_tokens_per_ac * NRF_WIFI_FMAC_AC_MAX)) {
...
enum nrf_wifi_status tx_init(struct nrf_wifi_fmac_dev_ctx *fmac_dev_ctx)
{
	 ...
	 */
	fmac_dev_ctx->tx_config.pkt_info_p = nrf_wifi_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv,
	...
	for (i = 0; i < fpriv->num_tx_tokens; i++) {
		fmac_dev_ctx->tx_config.pkt_info_p[i].pkt = nrf_wifi_utils_list_alloc(fpriv->opriv);
	...
	fmac_dev_ctx->tx_config.tx_lock = nrf_wifi_osal_spinlock_alloc(fmac_dev_ctx->fpriv->opriv);
	...
	fmac_dev_ctx->tx_config.wakeup_client_q = nrf_wifi_utils_q_alloc(fpriv->opriv);
	...
	fmac_dev_ctx->twt_sleep_status = NRF_WIFI_FMAC_TWT_STATE_AWAKE;

#ifdef CONFIG_NRF700X_TX_DONE_WQ_ENABLED
	fmac_dev_ctx->tx_done_tasklet = nrf_wifi_osal_tasklet_alloc(fpriv->opriv,
	...
	fmac_dev_ctx->tx_config.tx_done_tasklet_event_q = nrf_wifi_utils_q_alloc(fpriv->opriv);
...
wakeup_client_q_free:
	nrf_wifi_utils_q_free(fpriv->opriv, fmac_dev_ctx->tx_config.wakeup_client_q);
#endif /* CONFIG_NRF700X_TX_DONE_WQ_ENABLED */
tx_spin_lock_free:
					      ...
					      void *nbuf)
{
	...
	ra = nrf_wifi_util_get_ra(fmac_dev_ctx->vif_ctx[if_idx], nbuf);
