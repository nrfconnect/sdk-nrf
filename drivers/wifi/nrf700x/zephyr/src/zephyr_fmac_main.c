...
enum nrf_wifi_status nrf_wifi_fmac_dev_add_zep(struct nrf_wifi_drv_priv_zep *drv_priv_zep)
{
	...
	fw_info.lmac_patch_pri.data = nrf_wifi_lmac_patch_pri_bimg;
	fw_info.lmac_patch_pri.size = sizeof(nrf_wifi_lmac_patch_pri_bimg);
	fw_info.lmac_patch_sec.data = nrf_wifi_lmac_patch_sec_bin;
	fw_info.lmac_patch_sec.size = sizeof(nrf_wifi_lmac_patch_sec_bin);
	fw_info.umac_patch_pri.data = nrf_wifi_umac_patch_pri_bimg;
	fw_info.umac_patch_pri.size = sizeof(nrf_wifi_umac_patch_pri_bimg);
	fw_info.umac_patch_sec.data = nrf_wifi_umac_patch_sec_bin;
	...
	if (status != NRF_WIFI_STATUS_SUCCESS) {
		LOG_ERR("%s: nrf_wifi_fmac_fw_load failed\n", __func__);
		goto out;
	}
...
static int nrf_wifi_drv_main_zep(const struct device *dev)
{
	...
	rpu_drv_priv_zep.fmac_priv = nrf_wifi_fmac_init();
#endif /* CONFIG_NRF700X_RADIO_TEST */
