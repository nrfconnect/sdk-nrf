...
{
	(void)mpsl_clock_hfclk_src_request(MPSL_CLOCK_HF_SRC_XO, mpsl_hfclk_src_callback);
}

void nrfx_clock_hfclk_stop(void)
{
	(void)mpsl_clock_hfclk_src_release(MPSL_CLOCK_HF_SRC_XO);
}
...
{
	(void)mpsl_clock_hfclk_src_request(MPSL_CLOCK_HF_SRC_HFCLK24M, mpsl_hfclk_src_callback);
}

void nrfx_clock_xo24m_stop(void)
{
	(void)mpsl_clock_hfclk_src_release(MPSL_CLOCK_HF_SRC_HFCLK24M);
}
#endif /* NRF_CLOCK_HAS_HFCLK24M */
 ...
 * per CLOCK IRQ via CLOCK_CONTROL_NRFX_IRQ_HANDLERS_ITERABLE below.
 */
...
#if !defined(CONFIG_CLOCK_CONTROL_NRF)
CLOCK_CONTROL_NRFX_IRQ_HANDLERS_ITERABLE(clock_control_nrfx_mpsl, &nrfx_clock_irq_handler);
