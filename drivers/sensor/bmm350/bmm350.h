...
typedef int (*bmm350_reg_read_fn)(const struct bmm350_bus *bus,
			uint8_t start, uint8_t *buf, int size);
typedef int (*bmm350_reg_write_fn)(const struct bmm350_bus *bus,
...
#define DT_DRV_COMPAT bosch_bmm350
#define BMM350_USE_FIXED_POINT                1
#define BMM350_OK                       (0)
...
int bmm350_trigger_set(const struct device *dev,
	const struct sensor_trigger *trig,
	sensor_trigger_handler_t handler);
int bmm350_reg_write(const struct device *dev,
	uint8_t reg,
	uint8_t val);
