...
static inline int bmm350_reg_read(const struct device *dev,
...
int bmm350_reg_write(const struct device *dev, uint8_t reg,
			uint8_t val)
{
	const struct bmm350_config *cfg = dev->config;
...
static void bmm350_update_mag_off_sens(struct bmm350_data *data)
{
	...
	data->mag_comp.dut_offset_coef.t_offs = fix_sign(t_off, BMM350_SIGNED_8_BIT) * BMM350_MAG_COMP_COEFF_SCALING;
...
#ifdef BMM350_USE_FIXED_POINT
	data->mag_comp.dut_sensit_coef.sens_x = fix_sign(sens_x, BMM350_SIGNED_8_BIT) * BMM350_MAG_COMP_COEFF_SCALING;
	...
	data->mag_comp.dut_sensit_coef.t_sens = fix_sign(t_sens, BMM350_SIGNED_8_BIT) * BMM350_MAG_COMP_COEFF_SCALING;
...
#ifdef BMM350_USE_FIXED_POINT
	data->mag_comp.dut_tco.tco_x = fix_sign(tco_x, BMM350_SIGNED_8_BIT) * BMM350_MAG_COMP_COEFF_SCALING;
	data->mag_comp.dut_tco.tco_y = fix_sign(tco_y, BMM350_SIGNED_8_BIT) * BMM350_MAG_COMP_COEFF_SCALING;
	data->mag_comp.dut_tco.tco_z = fix_sign(tco_z, BMM350_SIGNED_8_BIT) * BMM350_MAG_COMP_COEFF_SCALING;
...
#ifdef BMM350_USE_FIXED_POINT
	data->mag_comp.dut_tcs.tcs_x = fix_sign(tcs_x, BMM350_SIGNED_8_BIT) * BMM350_MAG_COMP_COEFF_SCALING;
	data->mag_comp.dut_tcs.tcs_y = fix_sign(tcs_y, BMM350_SIGNED_8_BIT) * BMM350_MAG_COMP_COEFF_SCALING;
...
#ifdef BMM350_USE_FIXED_POINT
	data->mag_comp.dut_t0 = (fix_sign(data->otp_data[BMM350_MAG_DUT_T_0], BMM350_SIGNED_16_BIT) / 512) + 23;
#endif
...
#ifdef BMM350_USE_FIXED_POINT
	data->mag_comp.cross_axis.cross_x_y = fix_sign(cross_x_y, BMM350_SIGNED_8_BIT) * BMM350_MAG_COMP_COEFF_SCALING;
	data->mag_comp.cross_axis.cross_y_x = fix_sign(cross_y_x, BMM350_SIGNED_8_BIT) * BMM350_MAG_COMP_COEFF_SCALING;
	data->mag_comp.cross_axis.cross_z_x = fix_sign(cross_z_x, BMM350_SIGNED_8_BIT) * BMM350_MAG_COMP_COEFF_SCALING;
	data->mag_comp.cross_axis.cross_z_y = fix_sign(cross_z_y, BMM350_SIGNED_8_BIT) * BMM350_MAG_COMP_COEFF_SCALING;
...
static int bmm350_otp_dump_after_boot(const struct device *dev)
{
	...
	data->enable_auto_br = ((data->var_id > BMM350_CURRENT_SHUTTLE_VARIANT_ID) ? BMM350_DISABLE : BMM350_ENABLE);
 ...
 */
static int8_t bmm350_get_pmu_cmd_status_0(
	struct bmm350_pmu_cmd_status_0 *pmu_cmd_stat_0, const struct device *dev)
{
	...
	if (pmu_cmd_stat_0 != NULL) {
		...
		if (ret == BMM350_OK) {
			pmu_cmd_stat_0->pmu_cmd_busy = BMM350_GET_BITS_POS_0(rx_buf[2], BMM350_PMU_CMD_BUSY);
			...
			pmu_cmd_stat_0->pwr_mode_is_normal = BMM350_GET_BITS(rx_buf[2], BMM350_PWR_MODE_IS_NORMAL);
...
static int8_t bmm350_set_powermode(enum bmm350_power_modes powermode, const struct device *dev)
{
	...
	if (ret == BMM350_OK) {
		...
		if (ret == BMM350_OK) {
			...
			if ((ret == BMM350_OK) &&
				((rx_buf[2] == BMM350_PMU_CMD_NM) || (rx_buf[2] == BMM350_PMU_CMD_UPD_OAE))) {
...
static int8_t bmm350_magnetic_reset_and_wait(const struct device *dev)
{
	...
	if (ret == BMM350_OK) {
		...
		if ((ret == BMM350_OK) && (pmu_cmd_stat_0.pmu_cmd_value != BMM350_PMU_CMD_STATUS_0_BR)) {
	...
	if (ret == BMM350_OK) {
		...
		if ((ret == BMM350_OK) && (pmu_cmd_stat_0.pmu_cmd_value != BMM350_PMU_CMD_STATUS_0_FGR))
			ret = BMM350_E_PMU_CMD_VALUE;
	}
 ...
 */
int8_t bmm350_read_uncomp_mag_temp_data(struct bmm350_raw_mag_data *raw_data, const struct device *dev)
{
	struct bmm350_data *data = dev->data;
	...
	if (raw_data != NULL) {
		...
		if (rslt == BMM350_OK) {
			raw_mag_x = (uint32_t)mag_data[2] + ((uint32_t)mag_data[3] << 8) + ((uint32_t)mag_data[4] << 16);
			raw_mag_y = (uint32_t)mag_data[5] + ((uint32_t)mag_data[6] << 8) + ((uint32_t)mag_data[7] << 16);
			raw_mag_z = (uint32_t)mag_data[8] + ((uint32_t)mag_data[9] << 8) + ((uint32_t)mag_data[10] << 16);
			raw_temp = (uint32_t)mag_data[11] + ((uint32_t)mag_data[12] << 8) + ((uint32_t)mag_data[13] << 16);
...
static int8_t read_out_raw_data(int32_t *out_data, const struct device *dev)
{
	...
	if (out_data != NULL) {
		...
		if (rslt == BMM350_OK) {
			...
			out_data[0] = ((raw_data.raw_xdata * BMM350_LSB_TO_UT_XY_COEFF) / BMM350_LSB_TO_UT_COEFF_DIV);
			out_data[1] = ((raw_data.raw_ydata * BMM350_LSB_TO_UT_XY_COEFF) / BMM350_LSB_TO_UT_COEFF_DIV);
			out_data[2] = ((raw_data.raw_zdata * BMM350_LSB_TO_UT_Z_COEFF) / BMM350_LSB_TO_UT_COEFF_DIV);
			out_data[3] = ((raw_data.raw_data_t * BMM350_LSB_TO_UT_TEMP_COEFF) / BMM350_LSB_TO_UT_COEFF_DIV);
...
int8_t bmm350_get_compensated_mag_xyz_temp_data_fixed(
	...
	const struct device *dev)
{
	...
	if (mag_temp_data != NULL) {
		...
		if (rslt == BMM350_OK) {
			...
			out_data[3] =
			(((BMM350_MAG_COMP_COEFF_SCALING + data->mag_comp.dut_sensit_coef.t_sens) * out_data[3]) +
			data->mag_comp.dut_offset_coef.t_offs) / BMM350_MAG_COMP_COEFF_SCALING;
			...
			for (indx = 0; indx < 3; indx++) {
				out_data[indx] = (out_data[indx] * (BMM350_MAG_COMP_COEFF_SCALING + dut_sensit_coef[indx])) /
					BMM350_MAG_COMP_COEFF_SCALING;
				out_data[indx] = (out_data[indx] + dut_offset_coef[indx]);
				out_data[indx] =
					((out_data[indx] * BMM350_MAG_COMP_COEFF_SCALING) +
					(dut_tco[indx] * (out_data[3] - data->mag_comp.dut_t0))) / BMM350_MAG_COMP_COEFF_SCALING;
			...
			cr_ax_comp_x =
				((((out_data[0] * BMM350_MAG_COMP_COEFF_SCALING) - (data->mag_comp.cross_axis.cross_x_y * out_data[1])) *
				BMM350_MAG_COMP_COEFF_SCALING) /
				((BMM350_MAG_COMP_COEFF_SCALING * BMM350_MAG_COMP_COEFF_SCALING) -
				(data->mag_comp.cross_axis.cross_y_x * data->mag_comp.cross_axis.cross_x_y)));

			cr_ax_comp_y =
				((((out_data[1] * BMM350_MAG_COMP_COEFF_SCALING) - (data->mag_comp.cross_axis.cross_y_x * out_data[0])) *
				BMM350_MAG_COMP_COEFF_SCALING) /
				((BMM350_MAG_COMP_COEFF_SCALING * BMM350_MAG_COMP_COEFF_SCALING) -
				(data->mag_comp.cross_axis.cross_y_x * data->mag_comp.cross_axis.cross_x_y)));
			...
			cr_ax_comp_z =
				(out_data[2] +
				(((out_data[0] *
				((data->mag_comp.cross_axis.cross_y_x * data->mag_comp.cross_axis.cross_z_y) -
				(data->mag_comp.cross_axis.cross_z_x * BMM350_MAG_COMP_COEFF_SCALING))) -
				(out_data[1] *
				((data->mag_comp.cross_axis.cross_z_y * BMM350_MAG_COMP_COEFF_SCALING) -
				(data->mag_comp.cross_axis.cross_x_y * data->mag_comp.cross_axis.cross_z_x))))) /
				(((BMM350_MAG_COMP_COEFF_SCALING * BMM350_MAG_COMP_COEFF_SCALING) -
...
static int bmm350_sample_fetch(const struct device *dev,
...
static int bmm350_channel_get(const struct device *dev,
			enum sensor_channel chan,
			...
			const struct sensor_value *osr)
{
	...
	if (odr) {
		...
		if (bmm350_set_odr_performance((enum bmm350_data_rates)odr_bits, BMM350_AVERAGING_2, dev) < 0) {
...
static int bmm350_attr_set(const struct device *dev,
		enum sensor_channel chan,
...
static const struct sensor_driver_api bmm350_api_funcs = {
...
static int bmm350_init_chip(const struct device *dev)
{
	...
	if (ret != 0) {
		LOG_ERR("bmm350_init_chip %d", ret);
	}
