...
#include <drivers/ppi_seq/ppi_seq_i2c_spi.h>
...
static void ppi_seq_i2c_spi_internal_cb(struct ppi_seq *seq, bool last)
{
	...
	if (IS_SPI(dev)) {
		...
		if (last == true) {
			...
			nrf_spim_rx_ptr_set(config->nrfx_dev.spim->p_reg,
					    data->primary_buf ?
					    job->rx_second_buf : job->desc.spim.p_rx_buffer);
			if (job->tx_postinc) {
				nrf_spim_tx_ptr_set(config->nrfx_dev.spim->p_reg,
						    data->primary_buf
							? job->tx_second_buf
		...
		if (last == true) {
			...
			nrf_twim_rx_ptr_set(config->nrfx_dev.twim->p_twim,
					    data->primary_buf ?
					    job->rx_second_buf : job->desc.twim.p_secondary_buf);
			if (job->tx_postinc) {
				nrf_twim_tx_ptr_set(config->nrfx_dev.twim->p_twim,
						    data->primary_buf
							   ? job->tx_second_buf
