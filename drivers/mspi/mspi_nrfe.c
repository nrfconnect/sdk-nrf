...
static nrfe_mspi_pinctrl_soc_pin_t mspi_pin_config;
static nrfe_mspi_xfer_config_t mspi_xfer_config;
...
static void ipc_recv_clbk(const void *data, size_t len)
{
	...
	case NRFE_MSPI_RX: {
		...
		k_sem_give(&ipc_sem_xfer);
#else
		atomic_set_bit(&ipc_atomic_sem, NRFE_MSPI_RX);
#endif
		break;
	}
	default: {
...
static int send_config(nrfe_mspi_opcode_t opcode, const void *config, size_t len)
{
	...
	if (rc < 0) {
		LOG_ERR("%d: Configuration send failed: %d", __LINE__, rc);
		return rc;
	}

	rc = nrfe_mspi_wait_for_response(opcode, CONFIG_TIMEOUT_MS);
	if (rc < 0) {
		LOG_ERR("%d: FLPR config: %d response timeout: %d!", __LINE__, opcode, rc);
	}
...
static int api_config(const struct mspi_dt_spec *spec)
{
	...
	if (spec->config.op_mode != MSPI_OP_MODE_CONTROLLER) {
		LOG_ERR("%u, only support MSPI controller mode.", __LINE__);
		return -ENOTSUP;
	}

	if (spec->config.dqs_support) {
		LOG_ERR("%u, only support non-DQS mode.", __LINE__);
		return -ENOTSUP;
	}

	if (spec->config.max_freq > drv_cfg->mspicfg.max_freq) {
		LOG_ERR("%u, max_freq too large.", __LINE__);
		return -ENOTSUP;
	}
	...
	if (drv_cfg->pcfg->states[state_id].pin_cnt > NRFE_MSPI_PINS_MAX) {
		LOG_ERR("%u, too much pins defined. Max: %d", __LINE__, NRFE_MSPI_PINS_MAX);
		return -ENOTSUP;
	}

	if (drv_cfg->pcfg->states[state_id].id != PINCTRL_STATE_DEFAULT) {
		LOG_ERR("%u, pins default state not found.", __LINE__);
		return -ENOTSUP;
	}
	...
	mspi_xfer_config.duplex = spec->config.duplex;
	memset(mspi_xfer_config.ce_polarities, NRFE_MSPI_POL_UNDEFINED, NRFE_MSPI_CE_PINS_MAX*sizeof(nrfe_mspi_polarity_t));

	/* Send pinout configuration to FLPR */
	return send_config(NRFE_MSPI_CONFIG_PINS, (const void *)&mspi_pin_config, sizeof(nrfe_mspi_pinctrl_soc_pin_t));
}
			  ...
			  const enum mspi_dev_cfg_mask param_mask, const struct mspi_dev_cfg *cfg)
{
	...
	mspi_xfer_config.ce_polarities[cfg->ce_num] = (cfg->ce_polarity == MSPI_CE_ACTIVE_LOW) ? NRFE_MSPI_POL_ACTIVE_LOW : NRFE_MSPI_POL_ACTIVE_HIGH;
        mspi_xfer_config.ce_num = cfg->ce_num; 
        mspi_xfer_config.io_mode = cfg->io_mode;
        mspi_xfer_config.cpp = cfg->cpp;
        mspi_xfer_config.endian = cfg->endian;
        mspi_xfer_config.freq = cfg->freq;
...
{
	int rc;
	nrfe_mspi_xfer_packet_t xfer_packet = {
		.opcode = (packet->dir == MSPI_RX) ? NRFE_MSPI_RX : NRFE_MSPI_TX,
        	.command = packet->cmd,
        	.address = packet->address,
        	.num_bytes = packet->num_bytes,
	};

	uint32_t len = sizeof(nrfe_mspi_xfer_packet_t)+packet->num_bytes;
	uint8_t buffer[len];
	memcpy((void *)buffer, (void *)&xfer_packet, sizeof(nrfe_mspi_xfer_packet_t));
	memcpy((void *)(buffer + sizeof(nrfe_mspi_xfer_packet_t)), (void *)packet->data_buf, packet->num_bytes);

	rc = mspi_ipc_data_send(xfer_packet.opcode, buffer, len);
	if (rc < 0) {
		LOG_ERR("%d: Packet transfer error: %d", __LINE__, rc);
	}

	rc = nrfe_mspi_wait_for_response(xfer_packet.opcode, timeout);
	if (rc < 0) {
		LOG_ERR("%d: FLPR Xfer response timeout: %d", __LINE__, rc);
		return rc;
	}
			  ...
			  const struct mspi_xfer *req)
{
	...
	mspi_xfer_config.opcode = NRFE_MSPI_CONFIG_XFER;
        mspi_xfer_config.command_length = req->cmd_length;
        mspi_xfer_config.address_length = req->addr_length;
        mspi_xfer_config.hold_ce = req->hold_ce;
        mspi_xfer_config.tx_dummy = req->tx_dummy;
        mspi_xfer_config.rx_dummy = req->rx_dummy;

	drv_data->xfer = *req;

	rc = send_config(NRFE_MSPI_CONFIG_XFER, (void *)&mspi_xfer_config, sizeof(nrfe_mspi_xfer_config_t));
	if (rc < 0) {
...
/**
 * @brief Check the configuration of pins for the MSPI controller.
 *
 * This function verifies that the specified pins in the pinctrl state match the
 * expected MSPI configuration for the given device. It checks that the pins are
 * in the correct GPIO port and have the correct pin numbers for each MSPI
 * function (e.g., CS0, SCK, DQ0, etc.). If any pin is incorrectly configured,
 * the function logs an error and returns an error code.
 *
 * @param config Pointer to the pinctrl device configuration structure.
 * @param id Identifier of the pinctrl state to check.
 *
 * @retval 0 If the pin configuration is correct.
 * @retval -ENOTSUP If the pin is not supported or in the wrong port.
 */
static int check_pins_config(const struct pinctrl_dev_config *config, uint8_t id)
{
	int ret;
	...
	pins = state->pins;
	pin_cnt = state->pin_cnt;

	for (uint8_t i = 0U; i < pin_cnt; i++) {
		uint32_t psel = NRF_GET_PIN(pins[i]);
		uint32_t fun = NRF_GET_FUN(pins[i]);
		NRF_GPIO_Type *reg = nrf_gpio_pin_port_decode(&psel);

		if (reg != (NRF_GPIO_Type *)(NRFX_CONCAT(NRF_P, NRFE_MSPI_PORT_NUMBER))) {
			LOG_ERR("Wrong GPIO port number. Needs to be port %d!",
				NRFE_MSPI_PORT_NUMBER);
			return -ENOTSUP;
		} else if (reg == NULL) {
			LOG_ERR("Cannot obtain GPIO register for GPIO pin %d!", psel);
			return -ENOTSUP;
		}

		switch (fun) {
		case NRF_FUN_SDP_MSPI_CS0: {
			if (psel < NRFE_MSPI_CS0_PIN_NUMBER) {
				LOG_ERR("Wrong CS0 GPIO pin number: %d! Needs to be >= %d.", psel,
		...
		case NRF_FUN_SDP_MSPI_SCK: {
			if (psel != NRFE_MSPI_SCK_PIN_NUMBER) {
				LOG_ERR("Wrong SCK GPIO pin number: %d! Needs to be = %d.", psel,
					NRFE_MSPI_SCK_PIN_NUMBER);
				return -ENOTSUP;
			}
			break;
		}
		case NRF_FUN_SDP_MSPI_DQ0:
			if (psel != NRFE_MSPI_DQ0_PIN_NUMBER) {
				LOG_ERR("Wrong DQ0 GPIO pin number: %d! Needs to be = %d.", psel,
					NRFE_MSPI_DQ0_PIN_NUMBER);
				return -ENOTSUP;
			}
			break;
		case NRF_FUN_SDP_MSPI_DQ1:
			if (psel != NRFE_MSPI_DQ1_PIN_NUMBER) {
				LOG_ERR("Wrong DQ1 GPIO pin number: %d! Needs to be = %d.", psel,
					NRFE_MSPI_DQ1_PIN_NUMBER);
				return -ENOTSUP;
			}
			break;
		case NRF_FUN_SDP_MSPI_DQ2:
			if (psel != NRFE_MSPI_DQ2_PIN_NUMBER) {
				LOG_ERR("Wrong DQ2 GPIO pin number: %d! Needs to be = %d.", psel,
					NRFE_MSPI_DQ2_PIN_NUMBER);
				return -ENOTSUP;
			}
			break;
		case NRF_FUN_SDP_MSPI_DQ3:
			if (psel != NRFE_MSPI_DQ3_PIN_NUMBER) {
				LOG_ERR("Wrong DQ3 GPIO pin number: %d! Needs to be = %d.", psel,
					NRFE_MSPI_DQ3_PIN_NUMBER);
				return -ENOTSUP;
			}
			break;
...
static int nrfe_mspi_init(const struct device *dev)
{
	...
	const struct pinctrl_state *state = &drv_cfg->pcfg->states[0];

	while (state < &drv_cfg->pcfg->states[drv_cfg->pcfg->state_cnt]) {
		ret = check_pins_config(drv_cfg->pcfg, state->id);
		if (ret < 0) {
			return ret;
		}
		state++;
