...
#define NRFE_MSPI_DATA_LINE_CNT_MAX 4
#else
...
static int check_pin_assignments(const struct pinctrl_state *state)
{
	...
	for (uint8_t i = 0; i < state->pin_cnt; i++) {
		...
		if (NRF_PIN_NUMBER_TO_PORT(psel) != NRFE_MSPI_PORT_NUMBER) {
			LOG_ERR("Wrong port number. Only %d port is suppported.", NRFE_MSPI_PORT_NUMBER);
			return -ENOTSUP;
		}
		...
		case NRF_FUN_SDP_MSPI_DQ7:
			LOG_ERR("Too many data lines defined, max supported: 4.");
			return -ENOTSUP;
		...
		case NRF_FUN_SDP_MSPI_SCK:
			if (NRF_PIN_NUMBER_TO_PIN(psel) != NRFE_MSPI_SCK_PIN_NUMBER) {
				LOG_ERR("Clock signal only supported on pin %d.%d", NRFE_MSPI_PORT_NUMBER, NRFE_MSPI_SCK_PIN_NUMBER);
				return -ENOTSUP;
			}
			break;
		default:
			  ...
			  const enum mspi_dev_cfg_mask param_mask, const struct mspi_dev_cfg *cfg)
{
	...
	if (param_mask & MSPI_DEVICE_CONFIG_IO_MODE) {
		...
		rc = check_pins_for_io_mode(&drv_cfg->pcfg->states[PINCTRL_STATE_DEFAULT], cfg->io_mode);
		if (rc < 0) {
			return rc;
		}
	}
