...
static int nrfe_mspi_init(const struct device *dev)
{
	...
	ret = mspi_ipc_data_send(NRFE_MSPI_CONFIG_TIMER_PTR, &timer_data, sizeof(timer_data));
	if (ret < 0) {
		LOG_ERR("Send timer configuration failure");
		return ret;
	}

	/* Wait for timer configuration confirmation */
#if defined(CONFIG_MULTITHREADING)
	k_sem_take(&ipc_sem, K_FOREVER);
#else
	while (!atomic_test_and_clear_bit(&ipc_atomic_sem, NRFE_MSPI_CONFIG_TIMER_PTR)) {
	}
#endif
