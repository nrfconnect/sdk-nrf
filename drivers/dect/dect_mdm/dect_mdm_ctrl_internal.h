...
typedef struct {
	...
	uint32_t long_rd_id;
	uint32_t network_id;
} dect_mdm_ctrl_configure_params_t;
...
struct dect_mdm_ctrl_data {
	...
	dect_mdm_ctrl_configure_params_t configure_params;
...
#if defined(CONFIG_DECT_MDM_MDM_BUNDLED_TX_RESPS)
	struct dect_mdm_ctrl_dlc_data_tx_evt_data_item
		acked_data[DECT_MDM_DLC_DATA_INFO_MAX_COUNT];
#else
...
int dect_mdm_ctrl_internal_msgq_data_op_add(
	dect_mdm_ctrl_op_t event_id, void *data, size_t data_size);
