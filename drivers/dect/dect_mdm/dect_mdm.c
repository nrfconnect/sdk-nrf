					     ...
					     struct dect_status_info *status_info_out)
{
	...
	status_info.mdm_activated = dect_mdm_ctrl_api_mdm_activated();
	...
	status_info.nw_beacon_running = dect_mdm_ctrl_api_nw_beacon_running();
...
static int dect_mdm_hal_settings_read(const struct device *dev,
...
static int dect_mdm_hal_settings_write(const struct device *dev,
...
static int dect_mdm_hal_driver_send(const struct device *dev, struct net_pkt *pkt)
{
	...
	dect_mdm_ctrl_api_tx_cmd_params_t tx_params;
...
void dect_mdm_parent_association_created(
...
void dect_mdm_parent_association_removed(
	uint32_t long_rd_id,
...
void dect_mdm_child_association_removed(
