...
#define TIMER_PERIOD_US(timer_freq, sample_rate) ((uint32_t)((timer_freq) / (sample_rate)))
...
struct saadct_drv_data {
	...
	uint32_t ppi_end_evt;
	uint32_t ppi_end_tsk;
#endif
...
static int saadct_start_sampling(void)
{
...
#if !NRF_SAADC_HAS_SHORTS
	nrfx_gppi_conn_free(p_data->ppi_end_evt, p_data->ppi_end_tsk, p_data->ppi_end_start);
#endif
...
static int timer_init(const struct device *dev)
{
	...
	desired_ticks = nrfx_timer_us_to_ticks(timer, TIMER_PERIOD_US(timer_cfg.frequency,
								     sample_rate));
...
static int gppi_init(const struct device *dev)
{
	...
	p_data->ppi_sample_tsk = nrfy_saadc_task_address_get(NRF_SAADC, NRF_SAADC_TASK_SAMPLE);
...
#if !NRF_SAADC_HAS_SHORTS
	p_data->ppi_end_evt = nrf_saadc_event_address_get(NRF_SAADC, NRF_SAADC_EVENT_END);
	p_data->ppi_end_tsk = nrf_saadc_task_address_get(NRF_SAADC, NRF_SAADC_TASK_START);

	ret = nrfx_gppi_conn_alloc(p_data->ppi_end_evt, p_data->ppi_end_tsk,
				   &p_data->ppi_end_start);
	if (ret < 0) {
...
static int validate_config(struct api_saadct_config *cfg)
{
	if (cfg == NULL || cfg->channels_config == NULL) {
		return -EINVAL;
	}

	if (cfg->num_of_channels == 0U || cfg->num_of_meas == 0U || cfg->sample_rate == 0U) {
		return -EINVAL;
	}
...
static void saadct_discard_active_buffers(struct saadct_drv_data *p_data)
{
...
static int saadct_nrfx_start(const struct device *dev, struct k_mem_slab *slab)
{
	...
	saadct_discard_active_buffers(p_data);
...
static bool saadct_measurement_active(struct saadct_drv_data *p_data)
{
	return p_data->state == SAADCT_STATE_RUNNING ||
	       p_data->state == SAADCT_STATE_STOPPING;
}

static int saadct_nrfx_meas_get(const struct device *dev, nrf_saadc_value_t **data)
{
	struct saadct_drv_data *p_data = dev->data;
	struct saadct_meas_block *block;
	...
	if (block == NULL) {
		if (atomic_get(&p_data->pending_series) > 0 ||
		    k_mem_slab_num_used_get(p_data->mem_slab) > 0 ||
		    saadct_measurement_active(p_data)) {
			return -EAGAIN;
		}
...
static void saadct_nrfx_meas_free(const struct device *dev, nrf_saadc_value_t *data)
{
	struct saadct_drv_data *p_data = dev->data;
	struct saadct_meas_block *block =
...
static DEVICE_API(saadct, saadct_nrf_drv_api) = {
...
#define NRFX_SAADCT_DEVICE(inst)                                                                   \
	...
	{                                                                                          \
		...
		IRQ_CONNECT(DT_IRQN(TIMER_NODE(inst)), IRQ_PRIO_LOWEST, nrfx_timer_irq_handler,    \
			    &drv_data_##inst.timer, 0);                                            \
	...
	DEVICE_DT_INST_DEFINE(inst, saadct_##inst##_init, PM_DEVICE_DT_INST_GET(inst),             \
			      &drv_data_##inst, NULL, POST_KERNEL, 50, &saadct_nrf_drv_api);
