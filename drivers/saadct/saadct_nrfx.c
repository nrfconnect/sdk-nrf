...
#define TIMER_PERIOD_US(sample_rate) ((uint32_t)(USEC_PER_SEC / (sample_rate)))
...
static int timer_init(const struct device *dev)
{
	...
	desired_ticks = nrfx_timer_us_to_ticks(timer, TIMER_PERIOD_US(sample_rate));
...
int saadct_get(const struct device *dev, nrf_saadc_value_t **data)
{
	struct saadct_drv_data *p_data = dev->data;
	struct saadct_meas_block *block;
