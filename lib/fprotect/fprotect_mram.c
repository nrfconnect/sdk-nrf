...
#define NRF_MRAMC_REGION_SIZE_UNIT         0x400
#define NRF_MRAMC_REGION_ADDRESS_RESOLUTION 0x400
	...
	BUILD_ASSERT(CONFIG_NRF_MRAMC_REGION_ADDRESS_RESOLUTION ==
		NRF_MRAMC_REGION_ADDRESS_RESOLUTION,
		"NRF_MRAMC_REGION_ADDRESS_RESOLUTION has wrong value");
	BUILD_ASSERT(CONFIG_NRF_MRAMC_REGION_SIZE_UNIT == NRF_MRAMC_REGION_SIZE_UNIT,
		"NRF_MRAMC_REGION_SIZE_UNIT has wrong value");
#else
	#error Unsupported SOC
#endif

#define SINGLE_REGION_SIZE ((MRAMC_REGION_CONFIG_SIZE_Msk >> MRAMC_REGION_CONFIG_SIZE_Pos) * \
			NRF_MRAMC_REGION_SIZE_UNIT)
...
{
	if (start % NRF_MRAMC_REGION_ADDRESS_RESOLUTION ||
	    size % NRF_MRAMC_REGION_SIZE_UNIT) {
		return -EINVAL;
	}
	...
	if (size > SINGLE_REGION_SIZE) {
		...
		if (size > 2 * SINGLE_REGION_SIZE) {
			...
			configure_region(MRAMC_REGION_FOR_COMBINED_FPROTECT_MID,
					 SINGLE_REGION_SIZE,
					 SINGLE_REGION_SIZE / NRF_MRAMC_REGION_SIZE_UNIT);
