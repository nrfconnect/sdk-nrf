...
static uint16_t on_vendor_specific_cmd(uint8_t channel, uint8_t length, struct net_buf *hci_cmd)
{
	...
	case CARRIER_TEST:
		...
		cmd_hdr->opcode = SDC_HCI_OPCODE_CMD_VS_TRANSMITTER_CARRIER_TEST;
		cmd_hdr->param_len = sizeof(sdc_hci_cmd_vs_transmitter_carrier_test_t);
		sdc_hci_cmd_vs_transmitter_carrier_test_t *cp = net_buf_add(
			hci_cmd, sizeof(sdc_hci_cmd_vs_transmitter_carrier_test_t));
