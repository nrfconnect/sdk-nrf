...
/* Length of uint64_t_max written as a string + 1 string
			     ...
			     char *uuid_str_out, const int32_t uuid_str_out_size)
{
	...
	/* for some reason, sprintf will not respect the amount */
	/* of characters to print that is specified in the */
	/* formatting string. use this cascade of sprintf to */
	/* overcome this issue. */
...
static int bytes_to_str(const uint8_t *const bytes_buf, const size_t bytes_len, char *string_buff,
			const size_t string_buff_size, size_t *string_len)
{
	int err = 0;

	if ((NULL != bytes_buf) && (0 != bytes_len) && (NULL != string_buff) &&
	    (0 != string_buff_size) && (NULL != string_len)) {
		uint32_t str_pos = 0;
		...
		for (uint32_t count = 0; count < bytes_len; count++) {
			...
			if (byte_h < 10) {
				/* transpose from 0-9 integer numbers */
				/* to 0x30('0') - 0x31('9') ASCII characters */
				byte_h += '0';
			} else {
				/* transpose from 10-15 integer numbers */
				/* to 0x61('a') - 0x66('f') ASCII characters */
				byte_h += ('a' - 10);
			}
				  ...
				  size_t key_hash_buffer_size, size_t *key_hash_Length)
{
	...
	err = bytes_to_str(pubkey_der, pubkey_der_len, pubkey_der_str, sizeof(pubkey_der_str),
			   &pubkey_der_str_len);
	if (err == 0) {
