...
static char *jwt_header_create(struct app_jwt_data *const jwt)
{
	...
	if (jwt->sec_tag != 0) {
		 ...
		 */
		LOG_WRN("sec_tag provided, value = 0x%08X", jwt->sec_tag);
		key_id = jwt->sec_tag;
	} else {
...
static char *jwt_payload_create(struct app_jwt_data *const jwt)
{
	...
	if (err) {
		/* clock_gettime error, use DEFAULT_TIMESTAMP value */
		LOG_WRN("clock_gettime error, use %d", CONFIG_APP_JWT_DEFAULT_TIMESTAMP);
	} else {
...
finish_crypto_exit:
	...
	if (err) {
		LOG_ERR("Failed to cleanup PSA Crypto, Error : %d", err);
		return err;
	}
