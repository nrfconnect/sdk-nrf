...
typedef union {
	...
	struct {
		uint32_t metadata_version
			: 4; /**!< Metadata in the same location as CRACEN KMU metadata. */
			   ...
			   const uint8_t *key_buffer, size_t key_size)
{
	...
	for (size_t i = slot_id; i < (slot_id + num_slots); i++) {
		...
		block_size = size_left > KMU_KEYSLOTBYTES ? KMU_KEYSLOTBYTES : size_left;
		       ...
		       size_t key_size)
{
	...
	num_slots = DIV_ROUND_UP(key_size, KMU_KEYSLOTBYTES);

	if (slot_id < CONFIG_NRF_WIFI_KMU_SLOT_MIN ||
	    slot_id + num_slots > CONFIG_NRF_WIFI_KMU_SLOT_MIN + CONFIG_NRF_WIFI_KMU_NUM_SLOTS) {
		return -EINVAL;
	}

	if (slot_id + num_slots > KMU_RESERVED_SLOT_MIN) {
		return -EINVAL;
	}
	...
	/* Erase even on error, to clean out any partial keys */
	err_erase = erase_slots(slot_id, num_slots);

	/* Disable write and erase from KMU to SICR in MRAM */
	nrfx_mramc_confignvr_perm_set(false, MRAM_CONFIGNVR_SICR_PAGE);

	return err ? err : err_erase;
#endif
}
