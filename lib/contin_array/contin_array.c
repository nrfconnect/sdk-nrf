...
/** @brief This is used to mask the input locations to test if this is to have the finite array
 * written to it.
 */
#define PCM_CONT_CHANNEL_MASK 0x01
			    ...
			    uint16_t *const finite_pos)
{
	...
	if (meta_contin->locations == 0 && locations == 0) {
		out_locs = PCM_CONT_CHANNEL_MASK;
	} else {
	...
	while (out_locs) {
		if (out_locs & PCM_CONT_CHANNEL_MASK) {
...
int contin_array_net_buf_create(struct net_buf *pcm_contin, struct net_buf const *const pcm_finite,
				uint32_t locations, uint16_t const *const finite_pos)
{
	struct audio_metadata *meta_contin;
