...
static void copy_samples(uint8_t *pcm_contin, uint32_t const pcm_contin_size,
			 uint8_t const *const pcm_finite, uint32_t const pcm_finite_size,
			 uint16_t *const finite_pos, uint16_t const step, uint8_t carrier_bytes)
{
	for (size_t j = 0; j < pcm_contin_size; j += carrier_bytes) {
		for (size_t k = 0; k < carrier_bytes; k++) {
			*pcm_contin++ = pcm_finite[(*finite_pos)++];
		...
		if (out_locs & 0x01) {
			...
			*finite_pos = finite_start_pos;
			...
			copy_samples(output, meta_contin->bytes_per_location, (uint8_t *)pcm_finite,
				     pcm_finite_size, finite_pos, step, carrier_bytes);
