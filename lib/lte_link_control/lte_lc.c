...
int lte_lc_conn_eval_params_get(struct lte_lc_conn_eval_params *params)
{
	...
	char plmn_str[LTE_LC_PLMN_MCC_MNC_MAX + 1] = {0};
...
int lte_lc_plmn_access_list_write(struct lte_lc_plmn_entry *entries, size_t size, bool allowed)
{
	...
	entries_str_format = (char *)k_malloc(format_size);
	if (!entries_str_format) {
		LOG_ERR("Failed to allocate PLMN access list AT command parameters.");
		return -ENOMEM;
	}
	/* Always zero the malloc'd buffer. */
	memset(entries_str_format, 0, format_size);

	/* Populate the format string. */
	for (int i = 0; i < size; i++) {
		snprintf(entry_str_temp, sizeof(entry_str_temp), ",\"%s\",%d", entries[i].mcc_mnc,
			 entries[i].act_bitmask);
		strcat(entries_str_format, entry_str_temp);
	}

	err = nrf_modem_at_printf("AT%%PALL=0,%d%s", allowed ? 0 : 1, entries_str_format);
	if (err) {
		LOG_ERR("AT command failed, returned error code: %d", err);
		err = -EFAULT;
		goto clean_exit;
...
int lte_lc_plmn_access_list_read(struct lte_lc_plmn_entry *list, size_t *size, bool allowed)
{
	...
	err = nrf_modem_at_cmd(buffer, buffer_size, "AT%%PALL=1");
	if (err) {
		LOG_ERR("AT command failed, returned error code: %d", err);
