...
{
	if (gHTTP.csBuffer) {
		LOG_WRN("CS Buffer %p already initialized", gHTTP.csBuffer);
...
static otdoa_api_error_codes_t handle_encryption_parameters(tOTDOA_HTTP_MEMBERS *pG, char *pValue, int iTokenCount)
{
	...
	if (!bDisableEncryption) {
		...
		len = strlen(pValue);
		LOG_INF("pubkey length: %zu", len);
		LOG_INF("pubkey: %s", pValue);
		...
		if (len == 2 * PUBKEY_DER_LEN) {
			...
			LOG_HEXDUMP_INF(pG->pubkey, PUBKEY_LMAX, "pubkey: ");
		} else {
...
void otdoa_http_h1_init_config_dl(void)
{
	...
	LOG_DBG("csBuffer: %p", gHTTP.csBuffer);
}
...
int otdoa_http_h1_send_get_ubsa_auth_req(tOTDOA_HTTP_MEMBERS *pG)
{
	...
	LOG_INF("ubsa_auth_req: %s", pG->csBuffer);
...
int otdoa_http_h1_send_get_cfg_req(tOTDOA_HTTP_MEMBERS *pG)
{
	...
	if (NULL == pG->csBuffer) {
		LOG_ERR("csBuffer is NULL:h %p", pG->csBuffer);
		iRC = -1;
		goto exit;
	}
