	...
	if (tlv) {
		...
		if (strstr(tlv->value, "WPA-PSK")) {
			...
			struct {
				...
				{ TLV_EAP,           "SET_NETWORK 0 eap %s",
						     NULL, true  },
				{ TLV_IDENTITY,      "SET_NETWORK 0 identity \"%s\"",
						     NULL, true  },
				{ TLV_CA_CERT,       "SET_NETWORK 0 ca_cert \"%s\"",
						     "blob://ca_cert", false },
				{ TLV_CLIENT_CERT,   "SET_NETWORK 0 client_cert \"%s\"",
						     "blob://client_cert", false },
				{ TLV_PRIVATE_KEY,   "SET_NETWORK 0 private_key \"%s\"",
