...
enum lte_lc_nw_reg_status {
	/** @brief Not registered. UE is not currently searching for an operator to register to. */
	LTE_LC_NW_REG_NOT_REGISTERED			= 0,

	/** @brief Registered, home network. */
	LTE_LC_NW_REG_REGISTERED_HOME			= 1,

	/**
	 * @brief Not registered, but UE is currently trying to attach or searching for an operator
	 * to register to.
	 */
	...
	/** @brief Unknown, for example out of LTE coverage. */
	LTE_LC_NW_REG_UNKNOWN				= 4,

	/** @brief Registered, roaming. */
	...
	/**
	 * @brief Registered in stored cellular profile, home network. Device in receive only mode.
	 *
	 * @note This is only supported by the following modem firmware:
	 *       - mfw_nrf9151-ntn
	 */
	...
	/**
	 * @brief Registration denied. Device in receive only mode.
	 *
	 * @note This is only supported by the following modem firmware:
	 *       - mfw_nrf9151-ntn
	 */
	...
	/**
	 * @brief Unknown, for example, out of LTE coverage. Device in receive only mode.
	 *
	 * @note This is only supported by the following modem firmware:
	 *       - mfw_nrf9151-ntn
	 */
	...
	/**
	 * @brief Registered in stored cellular profile, roaming. Device in receive only mode.
	 *
	 * @note This is only supported by the following modem firmware:
	 *       - mfw_nrf9151-ntn
	 */
	...
	/** @brief Not registered due to UICC failure. */
	LTE_LC_NW_REG_UICC_FAIL				= 90,

	/**
	 * @brief The modem has completed searches, but no suitable cell for normal service was
	 * found.
...
enum lte_lc_system_mode {
	...
	/** @brief LTE-M + NB-IoT + GNSS. */
	LTE_LC_SYSTEM_MODE_LTEM_NBIOT_GPS,

	/**
	 * @brief NTN NB-IoT only.
	 *
	 * @note This is only supported by the following modem firmware:
	 *       - mfw_nrf9151-ntn
	 */
...
enum lte_lc_system_mode_preference {
	/** @brief No LTE preference, automatically selected by the modem. */
	LTE_LC_SYSTEM_MODE_PREFER_AUTO = 0,

	/**
	 * @brief LTE-M is preferred over PLMN selection.
	 *
	...
	/**
	 * @brief NB-IoT is preferred over PLMN selection.
	 *
	...
	/**
	 * @brief LTE-M is preferred, but PLMN selection is more important.
	 *
	...
	/**
	 * @brief NB-IoT is preferred, but PLMN selection is more important.
	 *
...
enum lte_lc_func_mode {
	/**
	 * @brief Sets the device to minimum functionality.
	 *
	...
	/**
	 * @brief Sets the device to full functionality.
	 *
	...
	/**
	 * @brief Sets the device to receive only functionality.
	 *
	...
	/**
	 * @brief Sets the device to flight mode.
	 *
	 * Disables both transmit and receive RF circuits and deactivates LTE and GNSS services.
	 */
	LTE_LC_FUNC_MODE_OFFLINE			= 4,

	/** @brief Deactivates LTE without shutting down GNSS services. */
	LTE_LC_FUNC_MODE_DEACTIVATE_LTE			= 20,

	/** @brief Activates LTE without changing GNSS. */
	LTE_LC_FUNC_MODE_ACTIVATE_LTE			= 21,

	/** @brief Deactivates GNSS without shutting down LTE services. */
	LTE_LC_FUNC_MODE_DEACTIVATE_GNSS		= 30,

	/** @brief Activates GNSS without changing LTE. */
	LTE_LC_FUNC_MODE_ACTIVATE_GNSS			= 31,

	/** @brief Deactivates UICC. */
	LTE_LC_FUNC_MODE_DEACTIVATE_UICC		= 40,

	/** @brief Activates UICC. */
	LTE_LC_FUNC_MODE_ACTIVATE_UICC			= 41,

	/** @brief Sets the device to flight mode without shutting down UICC. */
	LTE_LC_FUNC_MODE_OFFLINE_UICC_ON		= 44,

	/**
	 * @brief Sets the device to flight mode while preserving the LTE registration context.
	 *
	...
	/**
	 * @brief Sets the device to flight mode while preserving the LTE registration context and
...
enum lte_lc_evt_type {
	...
	/**
	 * @brief PSM parameters provided by the network.
	 *
	...
	/**
	 * @brief eDRX parameters provided by the network.
	 *
	...
	/**
	 * @brief Neighbor cell measurement results.
	 *
	...
	/**
	 * @brief Information about modem operation.
	 *
	...
	/**
	 * @brief Information about RAI (Release Assistance Indication) configuration.
	 *
	...
	/**
	 * @brief Environment evaluation result.
	 *
	...
	/**
	 * @brief A cellular profile is activated for the current access technology.
	 *
...
enum lte_lc_modem_sleep_type {
	/** @brief Power Saving Mode (PSM). */
	LTE_LC_MODEM_SLEEP_PSM			= 1,

	/** @brief RF inactivity, for example eDRX. */
	LTE_LC_MODEM_SLEEP_RF_INACTIVITY	= 2,

	/** @brief Limited service or out of coverage. */
...
enum lte_lc_tau_triggered {
	/**
	 * @brief The evaluated cell is in the Tracking Area Identifier list.
	 *
	...
	/**
	 * @brief The evaluated cell is not in the Tracking Area Identifier list.
	 *
...
enum lte_lc_ce_level {
	/** @brief No repetitions or a small number of repetitions. */
	LTE_LC_CE_LEVEL_0	= 0,

	/** Medium number of repetitions. */
	LTE_LC_CE_LEVEL_1	= 1,

	/** @brief Large number of repetitions. */
	LTE_LC_CE_LEVEL_2	= 2,

	/** @brief Very large number of repetitions. */
	LTE_LC_CE_LEVEL_3	= 3,
...
enum lte_lc_modem_evt_type {
	/**
	 * @brief Indicates that a light search has been performed.
	 *
	...
	/**
	 * @brief Indicates that a network search has been performed.
	 *
	...
	/**
	 * @brief The modem has detected a reset loop.
	 *
	...
	/** @brief Battery voltage is low and the modem is therefore deactivated. */
	LTE_LC_MODEM_EVT_BATTERY_LOW,

	/** @brief The device is overheated and the modem is therefore deactivated. */
	LTE_LC_MODEM_EVT_OVERHEATED,

	/** @brief The modem does not have an IMEI. */
	LTE_LC_MODEM_EVT_NO_IMEI,

	/**
	 * @brief Selected CE level in RACH procedure. See 3GPP TS 36.331 for details.
	 *
	...
	/**
	 * @brief RF calibration or power class selection not done.
	 *
	 * @note This is only supported by the following modem firmware:
	 *       - mfw_nrf91x1
	 *       - mfw_nrf9151-ntn
	...
	/**
	 * @brief Invalid band configuration.
	 *
	...
	/**
	 * @brief Current country the device is operating in.
	 *
...
enum lte_lc_band_conf_status {
	/** @brief Usable bands available in system. */
	LTE_LC_BAND_CONF_STATUS_OK = 0,

	/** @brief No usable bands available in system. */
	LTE_LC_BAND_CONF_STATUS_INVALID = 1,

	/** @brief System support not configured. */
...
enum lte_lc_neighbor_search_type {
	/**
	 * @brief The modem searches the network it is registered to (RPLMN) based on previous cell
	 * history.
	 */
	LTE_LC_NEIGHBOR_SEARCH_TYPE_DEFAULT = 1,

	/**
	 * @brief The modem starts with the same search method as the default type. If needed, it
	...
	/**
	 * @brief The modem follows the same procedure as for
	...
	/**
	 * @brief GCI search, option 1. Modem searches EARFCNs based on previous cell history.
	 */
	LTE_LC_NEIGHBOR_SEARCH_TYPE_GCI_DEFAULT = 4,

	/**
	 * @brief GCI search, option 2. Modem starts with the same search method as in
	...
	/**
	 * @brief GCI search, option 3. Modem starts with the same search method as in
	 * @ref LTE_LC_NEIGHBOR_SEARCH_TYPE_GCI_DEFAULT. If less than gci_count cells were found,
...
enum lte_lc_env_eval_type {
	...
	/** @brief PLMN search is stopped after light search even if no PLMNs to evaluate
	 * were found.
	 */
	LTE_LC_ENV_EVAL_TYPE_LIGHT = 1,

	/** @brief PLMN search covers all channels in all supported frequency bands. */
...
enum lte_lc_periodic_search_pattern_type {
	/** @brief Range search pattern. */
	LTE_LC_PERIODIC_SEARCH_PATTERN_RANGE = 0,

	/** @brief Table search pattern. */
...
enum lte_lc_pdn_evt_type {
	/** @brief PDN connection activated.
	 *
	 * The associated payload is the @c lte_lc_evt.pdn member of type
	 * @ref lte_lc_pdn_evt in the event.
	 */
	...
	/**
	 * @brief PDN connection deactivated.
	 *
	 * The associated payload is the @c lte_lc_evt.pdn member of type
	 * @ref lte_lc_pdn_evt in the event.
	 */
	...
	/**
	 * @brief PDN has IPv6 connectivity.
	 *
	 * The associated payload is the @c lte_lc_evt.pdn member of type
	 * @ref lte_lc_pdn_evt in the event.
	 */
	...
	/**
	 * @brief PDN has lost IPv6 connectivity.
	 *
	 * The associated payload is the @c lte_lc_evt.pdn member of type
	 * @ref lte_lc_pdn_evt in the event.
	 */
	...
	/**
	 * @brief APN rate control is ON for the given PDN.
	 *
	 * The associated payload is the @c lte_lc_evt.pdn member of type
	 * @ref lte_lc_pdn_evt in the event.
	 */
	...
	/**
	 * @brief APN rate control is OFF for the given PDN.
	 *
	 * The associated payload is the @c lte_lc_evt.pdn member of type
	 * @ref lte_lc_pdn_evt in the event.
	 */
	...
	/**
	 * @brief PDP context is destroyed for the given PDN.
	 *
	 * The associated payload is the @c lte_lc_evt.pdn member of type
	 * @ref lte_lc_pdn_evt in the event.
	 */
	...
	/**
	 * @brief ESM error notification for PDN.
	 *
	 * The associated payload is the @c lte_lc_evt.pdn member of type
...
enum lte_lc_pdn_auth {
	...
	/** @brief Password Authentication Protocol (PAP). */
	LTE_LC_PDN_AUTH_PAP  = 1,
	/** @brief Challenge-Handshake Authentication Protocol (CHAP). */
	LTE_LC_PDN_AUTH_CHAP = 2,
};

/** @brief UICC configuration type, used for cellular profile selection */
enum lte_lc_uicc {
	/** @brief Physical SIM or eSIM */
...
/**
 * @brief Enable modem domain events.
 *
