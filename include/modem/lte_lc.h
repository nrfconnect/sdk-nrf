...
enum lte_lc_evt_type {
	 ...
	 * @ref lte_lc_pdn_evt in the event.
	 */
	LTE_LC_EVT_PDN_NETWORK_DETACH		= 18,
	 ...
	 * @ref lte_lc_pdn_evt in the event.
	 */
	LTE_LC_EVT_PDN_APN_RATE_CONTROL_ON	= 19,
	 ...
	 * @ref lte_lc_pdn_evt in the event.
	 */
	LTE_LC_EVT_PDN_APN_RATE_CONTROL_OFF	= 20,
	 ...
	 * @ref lte_lc_pdn_evt in the event.
	 */
	LTE_LC_EVT_PDN_CTX_DESTROYED		= 21,
	 ...
	 */
	LTE_LC_EVT_CELLULAR_PROFILE_ACTIVE	= 23,
};
...
struct lte_lc_evt {
	...
	union {
		...
		/**
		 * Payload for PDN events: @ref LTE_LC_EVT_PDN_ACTIVATED,
		 * @ref LTE_LC_EVT_PDN_DEACTIVATED, @ref LTE_LC_EVT_PDN_IPV6_UP,
		 * @ref LTE_LC_EVT_PDN_IPV6_DOWN, @ref LTE_LC_EVT_PDN_NETWORK_DETACH,
		 * @ref LTE_LC_EVT_PDN_APN_RATE_CONTROL_ON, @ref LTE_LC_EVT_PDN_APN_RATE_CONTROL_OFF,
		 * @ref LTE_LC_EVT_PDN_CTX_DESTROYED, and @ref LTE_LC_EVT_PDN_ESM_ERROR.
		 */
		struct lte_lc_pdn_evt pdn;

		/**
		 * Payload for @ref LTE_LC_EVT_CELLULAR_PROFILE_ACTIVE event.
		 * This is the cellular profile ID of the active cellular profile.
		 */
		uint8_t cellular_profile_id;
	};
};
...
struct lte_lc_pdp_context_opts {
	/**
 ...
 */
int lte_lc_context_create(uint8_t *cid);
 ...
 * @return A negative errno otherwise.
 */
int lte_lc_context_configure(uint8_t cid, const char *apn, enum lte_lc_pdn_family family,
			     struct lte_lc_pdp_context_opts *opts);
 ...
 * @return A negative errno otherwise.
 */
int lte_lc_context_auth_set(uint8_t cid, enum lte_lc_pdn_auth method,
 ...
 * @return A negative errno otherwise.
 */
int lte_lc_context_destroy(uint8_t cid);
 ...
 * @return A negative errno otherwise.
 */
int lte_lc_context_activate(uint8_t cid, int *esm, enum lte_lc_pdn_family *family);
 ...
 * @return A negative errno otherwise.
 */
int lte_lc_context_deactivate(uint8_t cid);
 ...
 * @return A negative errno otherwise.
 */
int lte_lc_context_pdn_id_get(uint8_t cid);
 ...
 * @return A negative errno otherwise.
 */
int lte_lc_default_apn_get(char *buf, size_t len);
