...
enum lte_lc_modem_evt {
	/** Indicates that light search has been performed. This event gives the
	 *  application the chance to stop using more power on searching networks
	 *  from possibly weaker radio conditions.
	 *  Before sending this event, the modem searches the cells based on previous
	...
	/** Indicates that network search has been performed. The modem has found
	 *  a network that it can select according to 3GPP network selection rules
	...
	/** The modem has detected a reset loop. A reset oop indicates that
...
/** @brief Enable modem domain events. Modem domain events informs of
 *	   notable events within the modem. See @ref lte_lc_modem_evt for
