...
/**
 * @file
 * @defgroup rtfw App-domain real-time framework (RTFW)
 * @{
 * @brief API for the app-domain real-time framework.
 */
 ...
 *
 * The @c requested field is the newest published command. The @c attempted and
 * @c apply_result fields describe the command acknowledged by the fast path.
 * The @c applied field is the most recent command completed successfully. A
 * failed attempt is acknowledged and clears @c pending without changing
 * @c applied.
 */
struct rtfw_status {
 ...
 * -EALREADY.
 *
 * @param config Client configuration. It must not be NULL, and all fields must
 *               be initialized. @c command_handler, @c fastpath_handler, and
 *               @c pend_source_irq are required. @c event_handler and both
 *               user-data fields can be NULL. RTFW copies the configuration
 *               before returning.
 *
 * @retval 0 The framework was initialized.
 * @retval -EINVAL @p config or a required callback is NULL.
 ...
 * @retval -EACCES The framework is not initialized.
 * @retval -EINVAL @p command is NULL.
 * @retval -EMSGSIZE The command payload is too large.
