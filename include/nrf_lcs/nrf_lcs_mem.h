...
#ifndef NRF_LCS_MEM_H__
#define NRF_LCS_MEM_H__
 ...
 * The memory can be a true or emulated OTP - the memory must be writable
 * only once, naturally blocking backward LCS transitions.
 *
 * The API is designed in such a way, that the memory backend does not need to
 ...
 * The read API uses a pointer to an output variable to distinguish between a
...
/**
 * The magic value indicating a binary version of the LCS definitions.
 */
...
/**
 * @brief Enter the PSA RoT provisioning Life Cycle State.
 *
 * @param[in] value  The value to write to the memory corresponding to the PSA RoT provisioning
 *                   state.
 *
 * @return 0 if successful, negative error code otherwise.
 */
...
/**
 * @brief Enter the secured Life Cycle State.
 *
...
/**
 * @brief Enter the decommissioned Life Cycle State.
 *
