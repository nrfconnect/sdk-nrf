...
/**
 * @file
 * @brief The PSA-compatible Life Cycle State (LCS) API for Nordic devices.
 *
 ...
 * available.
 *
 * The LCS names and descriptions are compliant with the PSA specification,
 * yet are extended by expectations and steps performed by the firmware,
 * which are based on Nordic design decisions.
 *
...
/**
 * @brief nRF Life Cycle State subsystem
 * @defgroup nrf_lcs nRF Life Cycle State subsystem
 * @{
 */
...
enum nrf_lcs {
	 ...
	 * an authenticated erase all procedure (if this feature is supported).
	 */
	NRF_LCS_UNKNOWN = 0x60ebef49U,
	 ...
	 * verification, but should ensure the integrity of the next firmware in
	 * the chain.
	 ...
	 * procedure (if this feature is supported).
	 */
	NRF_LCS_ASSEMBLY_AND_TEST = 0x5d44c48dU,
	 ...
	 *
	 * On Nordic devices, the bootloader(s) are expected to perform a signature verification
	 * of the firmware, but do not perform firmware updates.
	 ...
	 * procedure (if this feature is supported).
	 */
	NRF_LCS_PSA_ROT_PROVISIONING = 0xafe19732U,
	 ...
	 */
	NRF_LCS_SECURED = 0x87bb60e7U,

	/**
	 * The device is in the PSA Platform Root of Trust debug state.
	 * This state is not yet fully defined, but it is included here for completeness.
	 */
	NRF_LCS_PSA_PROT_DEBUG = 0x98985939U,

	/**
	 * The device is in the PSA Non-Platform Root of Trust debug state.
	 * This state is not yet fully defined, but it is included here for completeness.
	 */
	NRF_LCS_PSA_NON_PROT_DEBUG = 0x7a7e3456U,

	/**
	 * The device is decommissioned, and it can be discarded.
	 * When entering this state, all secret data must be removed
	 ...
	 */
	NRF_LCS_DECOMMISSIONED = 0x24152b9eU,
