 ...
 *
 * @param attr Service attribute
 *
 * @return The service value from the parsed attribute or NULL when attribute UUID value is unexpected.
 */
struct bt_gatt_service_val * bt_gatt_dm_attr_service_val(const struct bt_gatt_attr * attr);
 ...
 *
 * @note Use it only on the attribute parsed in this module.
 *       To access characteristic attribute use @ref bt_gatt_dm_char_next function.
 *
 * @param attr Characteristic attribute
 *
 ...
 *
 * The number of attributes counting the service descriptor.
 ...
 *             Note: It can be the previous characteristic attribute or the last descriptor inside the previous
 *             attribute. Function would start searching for the next characteristic from that point.
 *
 * @return The pointer for an attribute that describes the characteristic or NULL if no more characteristic is present.
 */
 ...
 *
 * @return The pointer to the attribute or NULL if there is no attribute with such a pointer.
 */
const struct bt_gatt_attr * bt_gatt_dm_attr_by_handle(const struct bt_gatt_dm *dm, u16_t handle);
 ...
 *
 * @param dm Discovery Manager instance.
 * @param prev Previous attribute or NULL if we wish to get first attribute (service).
 *
 * @return Attribute next to the @c prev or the first attribute if NULL is given.
 */
const struct bt_gatt_attr * bt_gatt_dm_attr_next(const struct bt_gatt_dm *dm, const struct bt_gatt_attr *prev);
 ...
 *
 * @param dm Discovery Manager instance.
 * @param attr_chrc The characteristic attribute where to search
 * @param uuid The UUID of the searched descriptor.
 *
 * @return Pointer to the attribute or NULL if the attribute cannot be found.
 */
const struct bt_gatt_attr * bt_gatt_dm_desc_by_uuid(const struct bt_gatt_dm *dm,
 ...
 * it returns NULL also when returned attribute appears to be next characteristic.
 *
 * @param dm Discovery Manager instance.
 * @param prev Previous attribute.
 *        The characteristic if we wish to get first descriptor or previous descriptor.
 *
 * @return The pointer to the descriptor attribute or NULL if there is no more descriptors in the characteristic.
 */
const struct bt_gatt_attr * bt_gatt_dm_desc_next(const struct bt_gatt_dm * dm, const struct bt_gatt_attr * prev);
 ...
 * @param[in] context Context argument that would be passed to callback functions
 *
