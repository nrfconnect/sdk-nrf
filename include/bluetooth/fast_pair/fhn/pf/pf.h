 ...
 *
 *  The API is only available when the @kconfig{CONFIG_BT_FAST_PAIR_FHN_PF} Kconfig option
 *  is enabled.
 *
...
struct bt_fast_pair_fhn_pf_ranging_mgmt_cb {
	 ...
	 *  parameter indicates the number of elements in the @p config_payloads array. The
	 *  @p config_payloads array is only valid during the callback execution.
	 *
	 *  This callback is executed in the cooperative thread context. You can learn about the
	 *  exact thread context by analyzing the @kconfig{CONFIG_BT_RECV_CONTEXT} Kconfig option.
	 *  By default, this callback is executed in the Bluetooth-specific workqueue thread
	 ...
	 *
	 *  In the case of a communication channel termination due to the disconnected callback
	 *  from the Zephyr Bluetooth stack, this callback is executed in the cooperative thread
	 *  context as part of the delayable work item defined in the system workqueue. The
	 *  cooperative properties of this callback depend on the
	 *  @kconfig{CONFIG_SYSTEM_WORKQUEUE_PRIORITY} Kconfig option.
	 *
	 *  In the case of a communication channel termination due to the @ref bt_fast_pair_disable
	 *  API call, this callback is executed in the context of the @ref bt_fast_pair_disable API
	 *  call.
	 *
	 *  @param conn Connection object.
	 */
 ...
 *
 *  If the @kconfig{CONFIG_BT_FAST_PAIR_FHN_PF} Kconfig option is enabled, you must call this API
