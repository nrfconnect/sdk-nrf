...
#ifndef BT_FAST_PAIR_FHN_H_
#define BT_FAST_PAIR_FHN_H_
...
/**
 * @defgroup bt_fast_pair_fhn Fast Pair Find Hub Network API
 * @brief Fast Pair Find Hub Network (FHN) extension API
 *
 *  It is required to use the Fast Pair FHN API in the cooperative thread context.
...
enum bt_fast_pair_fhn_ring_src {
	/** Ringing source type originating from the Bluetooth Fast Pair service that
	 *  is defined in the FHN Accessory specification (Beacon Actions operations).
	 */
	 ...
	 *  Used only when the @kconfig{CONFIG_BT_FAST_PAIR_FHN_DULT} is enabled.
	 */
	BT_FAST_PAIR_FHN_RING_SRC_DULT_BT_GATT,

	/** Ringing source type originating from the Motion detector defined in the
	 *  DULT specification.
	 *  Used only when the @kconfig{CONFIG_BT_FAST_PAIR_FHN_DULT_MOTION_DETECTOR}
	 *  is enabled.
	 */
 ...
 *  response passed to the @ref bt_fast_pair_fhn_ring_state_update API.
 */
#define BT_FAST_PAIR_FHN_RING_COMP_BM_ALL (0xFF)

/** Ringing volume. */
enum bt_fast_pair_fhn_ring_volume {
	/** Default level of the ringing volume. */
	BT_FAST_PAIR_FHN_RING_VOLUME_DEFAULT = 0x00,

	/** Low level of the ringing volume. */
	BT_FAST_PAIR_FHN_RING_VOLUME_LOW = 0x01,

	/** Medium level of the ringing volume. */
	BT_FAST_PAIR_FHN_RING_VOLUME_MEDIUM = 0x02,

	/** High level of the ringing volume. */
	BT_FAST_PAIR_FHN_RING_VOLUME_HIGH = 0x03,
};

/** Number of milliseconds per decisecond. */
#define BT_FAST_PAIR_FHN_RING_MSEC_PER_DSEC (100U)

/** @brief Convert the ringing timeout value from deciseconds to milliseconds.
 *
 * Deciseconds are the native time unit used by the FHN extension.
 *
 * @param _timeout_ds The ringing timeout value in deciseconds.
 */
#define BT_FAST_PAIR_FHN_RING_TIMEOUT_DS_TO_MS(_timeout_ds) \
	((_timeout_ds) * BT_FAST_PAIR_FHN_RING_MSEC_PER_DSEC)
 ...
 */
#define BT_FAST_PAIR_FHN_RING_TIMEOUT_MS_TO_DS(_timeout_ms) \
	((_timeout_ms) / BT_FAST_PAIR_FHN_RING_MSEC_PER_DSEC)

/** Ringing request parameters. */
struct bt_fast_pair_fhn_ring_req_param {
	/** Bitmask with the active ringing components that is composed of
	 *  the @ref bt_fast_pair_fhn_ring_comp identifiers.
	 */
	...
	/** Ringing volume using @ref bt_fast_pair_fhn_ring_volume values. */
	enum bt_fast_pair_fhn_ring_volume volume;
};

/** Ringing callback structure. */
struct bt_fast_pair_fhn_ring_cb {
	/** @brief Request the user to start the ringing action.
	 *
	 *  This callback is called to start the ringing action. The FHN
	 ...
	 *  @ref bt_fast_pair_fhn_ring_state_update API and the
	 *  @ref BT_FAST_PAIR_FHN_RING_TRIGGER_UI_STOPPED trigger).
	 *
	 *  The input parameters determine how the ringing actions should
	 *  be executed. See @ref bt_fast_pair_fhn_ring_req_param for more
	 *  details.
	 *
	 *  If the action is accepted for at least one requested component, you
	 *  shall indicate it using the @ref bt_fast_pair_fhn_ring_state_update API
	 *  and set the @ref BT_FAST_PAIR_FHN_RING_TRIGGER_STARTED as a trigger
	 *  for the ringing state change. If all components are out of range, you
	 *  shall set the @ref BT_FAST_PAIR_FHN_RING_TRIGGER_FAILED as a trigger.
	 *
	 ...
	 *  @ref bt_fast_pair_fhn_ring_state_update API.
	 *
	 ...
	 */
	void (*start_request)(enum bt_fast_pair_fhn_ring_src src,
	 ...
	 *  FHN module requests this action when the ringing timeout expires.
	 *
	 *  If the action is accepted for at least one requested component, you
	 *  shall indicate it using the @ref bt_fast_pair_fhn_ring_state_update API
	 *  and set the @ref BT_FAST_PAIR_FHN_RING_TRIGGER_TIMEOUT_STOPPED as
	 *  a trigger for the ringing state change. If all components are out of
	 *  range, you shall set the @ref BT_FAST_PAIR_FHN_RING_TRIGGER_FAILED
	 *  as a trigger.
	 *
	 ...
	 *
	 *  This callback is called to stop the ongoing ringing action. The
	 *  FHN module requests this action in response to the command
	 *  from the connected peer.
	 *
	 *  If the action is accepted for at least one requested component, you
	 *  shall indicate it using the @ref bt_fast_pair_fhn_ring_state_update API
	 *  and set the @ref BT_FAST_PAIR_FHN_RING_TRIGGER_GATT_STOPPED as
	 *  a trigger for the ringing state change. If all components are out of
	 *  range, you shall set the @ref BT_FAST_PAIR_FHN_RING_TRIGGER_FAILED
	 *  as a trigger.
	 *
...
/** @brief Register the ringing callbacks in the FHN module.
 *
 *  This function registers the ringing callbacks. If you declare at least one
 *  ringing component using the @kconfig{CONFIG_BT_FAST_PAIR_FHN_RING_COMP} Kconfig
 ...
 *
 *  You can call this function only in the disabled state of the FHN module
...
enum bt_fast_pair_fhn_ring_trigger {
	/** Ringing action started. */
	BT_FAST_PAIR_FHN_RING_TRIGGER_STARTED = 0x00,

	/** Ringing action failed (all requested components are out of range). */
	BT_FAST_PAIR_FHN_RING_TRIGGER_FAILED = 0x01,

	/** Ringing action stopped due to the timeout. */
	BT_FAST_PAIR_FHN_RING_TRIGGER_TIMEOUT_STOPPED = 0x02,

	/** Ringing action stopped due to the UI action (e.g button press, touch sense). */
	BT_FAST_PAIR_FHN_RING_TRIGGER_UI_STOPPED = 0x03,

	/** Ringing action stopped due to the GATT request. */
	BT_FAST_PAIR_FHN_RING_TRIGGER_GATT_STOPPED = 0x04,
};

/** Ringing state parameters. */
struct bt_fast_pair_fhn_ring_state_param {
	/** Trigger for the new ringing state. */
	...
	/** Ringing timeout in deciseconds.
	 *  Relevant only for the @ref BT_FAST_PAIR_FHN_RING_TRIGGER_STARTED trigger
	 *  Set to zero to preserve the existing timeout.
	 */
	uint16_t timeout;
};

/** @brief Update the ringing state in the FHN module.
 *
 *  This function updates the ringing state in the FHN module and notifies
 ...
 *  @ref bt_fast_pair_fhn_ring_cb structure. It is also possible to use
 ...
 *  (see the @ref BT_FAST_PAIR_FHN_RING_TRIGGER_UI_STOPPED trigger). In
 ...
 *               See the @ref bt_fast_pair_fhn_ring_state_param for
 ...
 *
 *  Used only if the @kconfig{CONFIG_BT_FAST_PAIR_FHN_DULT_MOTION_DETECTOR} Kconfig option
 *  is enabled.
 */
struct bt_fast_pair_fhn_motion_detector_cb {
	 ...
	 *  The user should notify the FHN module if motion was detected
	 ...
	 *  detector is no longer used by the FHN module. It concludes
 ...
 *  only be used when the @kconfig{CONFIG_BT_FAST_PAIR_FHN_DULT_MOTION_DETECTOR}
 ...
 *  @return 0 if the operation was successful. Otherwise, a (negative) error code is returned.
 */
int bt_fast_pair_fhn_motion_detector_cb_register(
	const struct bt_fast_pair_fhn_motion_detector_cb *cb);

/** Unknown battery level. */
#define BT_FAST_PAIR_FHN_BATTERY_LEVEL_NONE (0xFF)
 ...
 *
 *  By default, the @ref BT_FAST_PAIR_FHN_BATTERY_LEVEL_NONE value is used,
 ...
 *
 *  However, if the @kconfig{CONFIG_BT_FAST_PAIR_FHN_BATTERY_DULT} Kconfig is enabled,
 ...
 *  you must not call this API with the @ref BT_FAST_PAIR_FHN_BATTERY_LEVEL_NONE
 ...
 *  FHN Accessory specification in the advertising payload is implementation-specific.
 *  The mapping configuration is controlled by the following Kconfig options:
 *  @kconfig{CONFIG_BT_FAST_PAIR_FHN_BATTERY_LEVEL_LOW_THR} and
 *  @kconfig{CONFIG_BT_FAST_PAIR_FHN_BATTERY_LEVEL_CRITICAL_THR}.
 *
 *  @param percentage_level Battery level as a percentage [0-100%] or
 *                          @ref BT_FAST_PAIR_FHN_BATTERY_LEVEL_NONE value if the
...
/** Helper to declare FHN advertising parameters inline.
 *
 * @param _int_min     Minimum advertising interval.
 * @param _int_max     Maximum advertising interval.
 */
#define BT_FAST_PAIR_FHN_ADV_PARAM_INIT(_int_min, _int_max) \
{                                                            \
	.interval_min = (_int_min),                          \
	.interval_max = (_int_max),                          \
}

/** Default value of FHN advertising parameters. */
#define BT_FAST_PAIR_FHN_ADV_PARAM_DEFAULT                   \
	((struct bt_fast_pair_fhn_adv_param[]) {              \
		BT_FAST_PAIR_FHN_ADV_PARAM_INIT(             \
			0x0C80, /* 2s min interval */        \
			0x0C80  /* 2s max interval */        \
	)})

/** FHN advertising parameters. */
struct bt_fast_pair_fhn_adv_param {
...
/** @brief Set the FHN advertising parameters.
 *
 ...
 *  value @ref BT_FAST_PAIR_FHN_ADV_PARAM_DEFAULT is used for advertising.
 *
 ...
 *  during an ongoing FHN advertising.
 *
 ...
 *  Pair advertising together with FHN advertising (see respective
 *  specifications for details). This function does not validate if
 *  these constraints are met.
 *
 *  @param adv_param Configuration parameters for FHN advertising.
 *                   See the @ref bt_fast_pair_fhn_adv_param for the
...
/** @brief Set the Bluetooth identity for the FHN extension.
 *
 *  This function sets the Bluetooth identity for the FHN extension.
 ...
 *
 *  @param id Bluetooth identity for the FHN extension.
 *
 *  @return 0 if the operation was successful. Otherwise, a (negative) error code is returned.
 */
...
struct bt_fast_pair_fhn_info_cb {
	 ...
	 *  authenticated locally using the protocol defined in the FHN
	 *  Accessory specification. It is triggered on a successful Read
	 *  Provisioning State operation on the Beacon Actions GATT
	 *  characteristic.
	 *
	 *  This callback can be used to facilitate the FHN firmware update
	 ...
	 *  retrieved from the FHN accessory, may be used by the authenticated
	 ...
	 *
	 *  This callback is called to indicate that the FHN accessory has been
	 ...
	 *  the initial state, use the @ref bt_fast_pair_fhn_is_provisioned API.
	 *
	 *  This callback is executed in the cooperative thread context. You
	 *  can learn about the exact thread context by analyzing the
	 *  @kconfig{CONFIG_BT_RECV_CONTEXT} configuration choice. By default, this
...
/** @brief Check the FHN provisioning state.
 *
 *  This function can be used to synchronously check the FHN provisioning state.
 *  To track the provisioning state asynchronously, use the
 *  @ref bt_fast_pair_fhn_info_cb.provisioning_state_changed callback.
 *
...
/** @brief Register the information callbacks in the FHN module.
 *
 *  This function registers the information callbacks. You can call this function only
 *  in the disabled state of the FHN module (see @ref bt_fast_pair_is_ready function).
...
/** Read modes. */
enum bt_fast_pair_fhn_read_mode {
...
struct bt_fast_pair_fhn_read_mode_cb {
	/** @brief Read mode exited.
	 *
	 *  This callback is called to indicate that the read mode has been exited.
	 *  Read mode can be entered by calling the @ref bt_fast_pair_fhn_read_mode_enter API.
	 *
...
/** @brief Register the read mode callbacks in the FHN module.
 *
 ...
 *  @return 0 if the operation was successful. Otherwise, a (negative) error code is returned.
 */
int bt_fast_pair_fhn_read_mode_cb_register(
