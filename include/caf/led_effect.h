...
#define LED_COLOR(_r, _g, _b)                                                                      \
	{                                                                                          \
		.c = { COLOR_BRIGHTNESS_TO_PCT(_r),                                                \
		       COLOR_BRIGHTNESS_TO_PCT(_g),                                                \
		       COLOR_BRIGHTNESS_TO_PCT(_b) }                                               \
	}

#define LED_COLOR_OFF					 LED_COLOR(0, 0, 0)
#define _LED_COLOR_BRIGHTNESS_SCALE(_brightness, _color) ((_color * _brightness) / 100)
#define LED_COLOR_WHITE(_brightness)			 LED_COLOR(_brightness, _brightness, _brightness)
#define LED_COLOR_RED(_brightness)			 LED_COLOR(_brightness, 0, 0)
#define LED_COLOR_GREEN(_brightness)			 LED_COLOR(0, _brightness, 0)
#define LED_COLOR_BLUE(_brightness)			 LED_COLOR(0, 0, _brightness)
#define LED_COLOR_YELLOW(_brightness)			 LED_COLOR(_brightness, _brightness, 0)
#define LED_COLOR_MAGENTA(_brightness)			 LED_COLOR(_brightness, 0, _brightness)
#define LED_COLOR_CYAN(_brightness)			 LED_COLOR(0, _brightness, _brightness)
#define LED_COLOR_ORANGE(_brightness)                                                              \
	LED_COLOR(_brightness, _LED_COLOR_BRIGHTNESS_SCALE(_brightness, 165), 0)
#define LED_COLOR_PINK(_brightness)                                                                \
	LED_COLOR(_brightness, _LED_COLOR_BRIGHTNESS_SCALE(_brightness, 192),                      \
		  _LED_COLOR_BRIGHTNESS_SCALE(_brightness, 203))
#define LED_COLOR_TURQUOISE(_brightness)                                                           \
	LED_COLOR(_LED_COLOR_BRIGHTNESS_SCALE(_brightness, 64),                                    \
		  _LED_COLOR_BRIGHTNESS_SCALE(_brightness, 224),                                   \
		  _LED_COLOR_BRIGHTNESS_SCALE(_brightness, 208))
#define LED_COLOR_INDIGO(_brightness)                                                              \
	LED_COLOR(_LED_COLOR_BRIGHTNESS_SCALE(_brightness, 75), 0,                                 \
		  _LED_COLOR_BRIGHTNESS_SCALE(_brightness, 130))
#define LED_COLOR_BROWN(_brightness)                                                               \
	LED_COLOR(_LED_COLOR_BRIGHTNESS_SCALE(_brightness, 165),                                   \
		  _LED_COLOR_BRIGHTNESS_SCALE(_brightness, 42),                                    \
		  _LED_COLOR_BRIGHTNESS_SCALE(_brightness, 42))

#if CONFIG_CAF_LED_EFFECTS_COMPATIBILITY_MODE
/** Create LED color initializer for LED turned off.
 */
#define LED_NOCOLOR() LED_COLOR_OFF
#endif
 ...
 */
typedef uint8_t (*led_effect_custom_easing_func_t)(uint8_t fraction);
 ...
 */
#define LED_EFFECT_EASING(_type, _parameters) {.type = _type, .param = _parameters}
...
#define LED_EFFECT_EASING_STEP(_step_fraction)                                                     \
	{                                                                                          \
		.type = LED_EFFECT_EASING_TYPE_STEP, .param =                                      \
		{.step = {                                                                         \
...
#define LED_EFFECT_EASING_SIN(_offset, _max_value)                                                 \
	{                                                                                          \
		.type = LED_EFFECT_EASING_TYPE_SIN, .param = {                                     \
			.sin = {.offset = _offset, .max_value = _max_value}                        \
		}                                                                                  \
	}
...
#define LED_EFFECT_EASING_CUSTOM(_func)                                                            \
	{                                                                                          \
		.type = LED_EFFECT_EASING_TYPE_CUSTOM, .param =                                    \
		{.custom = {                                                                       \
			 .func = _func,                                                            \
		 } }                                                                               \
	}
...
#define LED_EFFECT_STEP(_color, _duration, _easing)                                                \
	{.color = _color, .duration = _duration, .easing = _easing}
...
#define LED_EFFECT_STEP_EASING_LINEAR(_color, _duration)                                           \
	LED_EFFECT_STEP(                                                                           \
		LED_MACRO_ARG_PASS(_color), _duration,                                             \
...
/** @brief Single LED effect step with cosine easing function which starts at LED_EFFECT_OUTPUT_MAX,
 * falls to 0, then rises back to LED_EFFECT_OUTPUT_MAX.
 * @param _color Color of the LED at the end of the step.
 * @param _duration Duration of the step in milliseconds.
 */
...
#define LED_EFFECT(_repeat_count, _step_count, ...)                                                \
	{                                                                                          \
		.steps = ((const struct led_effect_step[]){__VA_ARGS__}),                          \
 ...
 *                    (in milliseconds).
 */
#define LED_EFFECT_LED_ON_EASE_OFF(_color, _on_time, _off_delay)                                   \
	LED_EFFECT(1, 2,                                                                           \
 ...
 *
 * @param _repeats  Number of times the effect should repeat. 0 is forever.
 * @param _count    Number of LED on/off cycles.
 ...
 * @param _color	Selected LED color.
 */
#define LED_EFFECT_LED_COUNTER2_DELAY(_repeats, _count, _delay, _on_period, _off_period, _color)   \
	LED_EFFECT(_repeats, 1 + (_count) * 2,                                                     \
		   LED_EFFECT_STEP_EASING_STEP_INSTANT(LED_COLOR_OFF, _delay),                     \
		   LISTIFY(_count, _LED_EFFECT_LED_COUNTER2_DELAY_STEP, (), _on_period,            \
			   _off_period, LED_MACRO_ARG_PASS(_color)))
...
#define LED_EFFECT_LED_COUNTER2(_count, _on_period, _off_period, _easing, _color)                  \
	LED_EFFECT(_count, 3, LED_EFFECT_STEP_INSTANT(LED_COLOR_OFF),                              \
 ...
 * and the selected color using sine easing function.
 *
 * @param _count    Number of times the effect should repeat. 0 is forever.
 * @param _on_period	Period of time for a cycle.
 * @param _off_period   Period of time for which LED is off.
...
#define LED_EFFECT_LED_BREATH_COUNTER2(_count, _on_period, _off_period, _color)                    \
	LED_EFFECT_LED_COUNTER2(                                                                   \
		_count, _on_period, _off_period,                                                   \
...
#define LED_EFFECT_LED_BREATH_COUNTER(_count, _period, _color)                                     \
	LED_EFFECT_LED_BREATH_COUNTER2(_count, _period, 0, LED_MACRO_ARG_PASS(_color))
...
#define LED_EFFECT_LED_BREATH(_period, _color)                                                     \
	LED_EFFECT_LED_BREATH_COUNTER(0, _period, LED_MACRO_ARG_PASS(_color))
 ...
 * @param _color	Selected LED color.
 */
#define LED_EFFECT_LED_BREATH_LINEAR(_period, _color)                                              \
	LED_EFFECT(0, 4, LED_EFFECT_STEP_EASING_LINEAR(LED_MACRO_ARG_PASS(_color), _period),       \
...
#define LED_EFFECT_FADE(_fade_time, _start_color, _end_color)                                      \
	LED_EFFECT(1, 2, LED_EFFECT_STEP_INSTANT(LED_MACRO_ARG_PASS(_start_color)),                \
		   LED_EFFECT_STEP_EASING_LINEAR(LED_MACRO_ARG_PASS(_end_color), _fade_time))
 ...
 * which in this case is LED color. Period is defined separately. To use custom
 * period, use LED_EFFECT_LED_COUNTER2_DELAY macro.
 *
 * @param _ticks	Number of ticks.
 * @param _color	Selected LED color.
 */
#define LED_EFFECT_LED_CLOCK(_ticks, _color)                                                       \
	LED_EFFECT_LED_COUNTER2_DELAY(0, _ticks, LED_CLOCK_SLEEP_PERIOD, LED_CLOCK_BLINK_PERIOD,   \
				      LED_CLOCK_BLINK_PERIOD, LED_MACRO_ARG_PASS(_color))

#if CONFIG_CAF_LEDS_ENABLE_DEPRECATED_MACROS

/** Create LED turned on for a brief period effect initializer.
 *
 * LED color remains constant for a defined time, then goes off.
 *
 * @deprecated This macro is renamed to LED_EFFECT_LED_ON_EASE_OFF.
 *
 * @param _color      Selected LED color.
 * @param _on_time    Time LED will remain on in milliseconds.
 * @param _off_delay  Time in which LED will gradually switch to off
 *                    (in milliseconds).
 */
#define LED_EFFECT_LED_ON_GO_OFF LED_EFFECT_LED_ON_EASE_OFF

/** @def _BREATH_SUBSTEPS
 *
 * @brief Substeps for color update for LED breathing effect.
 * @deprecated This macro is deprecated and is no longer used in this module.
 */
#define _BREATH_SUBSTEPS 15

#undef LED_EFFECT_LED_BREATH
/** Create LED breathing effect initializer.
 *
 * LED color is smoothly, gradually changed between the LED turned off
 * and the selected color.
 *
 * @deprecated This macro is renamed to LED_EFFECT_LED_BREATH_LINEAR.
 *
 * @param _period	Period of time for single substep.
 * @param _color	Selected LED color.
 */
#define LED_EFFECT_LED_BREATH LED_EFFECT_LED_BREATH_LINEAR

#endif /* CONFIG_CAF_LED_EFFECTS_COMPATIBILITY_MODE */
