...
#ifndef NRF_CLOUD_PGNSS_H_
#define NRF_CLOUD_PGNSS_H_

/** @file nrf_cloud_pgnss.h
 * @brief Module to provide nRF Cloud Predicted GNSS (PGNSS) support to nRF9160 SiP.
 */
...
/** @defgroup nrf_cloud_pgnss nRF Cloud PGNSS
 * @{
 */

#define NRF_CLOUD_PGNSS_NUM_SV				(32U)

/** @brief nrf_cloud_pgnss_system_time is a special version of
 ...
 * validity period.
 */
struct nrf_cloud_pgnss_system_time {
...
/** @brief PGNSS prediction Flash storage format. */
struct nrf_cloud_pgnss_prediction {
	/** Set to NRF_CLOUD_AGNSS_GPS_SYSTEM_CLOCK. */
	uint8_t time_type;
	/** Will be 1. */
	uint16_t time_count;
	/** Information about when this prediction is applicable. */
	struct nrf_cloud_pgnss_system_time time;
	...
	/** Usually will be NRF_CLOUD_PGNSS_NUM_SV. */
	uint16_t ephemeris_count;
	/** Array of satellite orbital equation coefficients. */
	struct nrf_cloud_agnss_ephemeris ephemerii[NRF_CLOUD_PGNSS_NUM_SV];
	/** Not from cloud; appended during storage to verify integrity on retrieval. */
	uint32_t sentinel;
} __packed;

/** Omit the prediction count from PGNSS request */
#define NRF_CLOUD_PGNSS_REQ_NO_COUNT	0
/** Omit the prediction validity period from PGNSS request */
#define NRF_CLOUD_PGNSS_REQ_NO_INTERVAL	0
/** Omit the GPS day from PGNSS request */
#define NRF_CLOUD_PGNSS_REQ_NO_GPS_DAY	0
/** Omit the GPS time of day from PGNSS request */
#define NRF_CLOUD_PGNSS_REQ_NO_GPS_TOD	(-1)

/** @brief PGNSS request type */
struct gps_pgnss_request {
...
/** @brief nRF Cloud Predicted GNSS (PGNSS) result; the location of
 *         the PGNSS data file which is to be downloaded and provided
 *         to nrf_cloud_pgnss_process().
 */
...
/** @brief Data required for nRF Cloud Predicted GNSS (PGNSS) request.
 *
 * Defined unconditionally so the CoAP codec test can exercise the encode path
 * via @ref nrf_cloud_coap_pgnss_url_get without depending on the full
 * CoAP transport machinery being enabled.
 */
struct nrf_cloud_coap_pgnss_request {
	/** Data to be included in the PGNSS request. To omit an item
	 * use the appropriate `NRF_CLOUD_PGNSS_REQ_NO_` define.
	 */
	const struct gps_pgnss_request *pgnss_req;
};

/** @brief PGNSS error code: current time unknown. */
#define ETIMEUNKNOWN	8000
/** @brief PGNSS error code: not found but loading in progress. */
 ...
 */
#define NRF_CLOUD_PGNSS_EMPTY_EPHEM_HEALTH (0xff)

/** @brief PGNSS event passed to the registered pgnss_event_handler. */
enum nrf_cloud_pgnss_event_type {
	/** PGNSS initialization beginning. */
	PGNSS_EVT_INIT,
	/** There are currently no PGNSS predictions available. */
	PGNSS_EVT_UNAVAILABLE,
	/** PGNSS predictions are being loaded from the cloud. */
	PGNSS_EVT_LOADING,
	/** A PGNSS prediction is available now for the current date and time.
	 *  The prediction pointer in the associated nrf_cloud_pgnss_event
	 *  structure points to the prediction.
	 */
	PGNSS_EVT_AVAILABLE,
	/** All PGNSS predictions are available. */
	PGNSS_EVT_READY,
	/** A PGNSS request has been created for missing predictions.
	 *  The event has payload in the form of @ref gps_pgnss_request.
	 *  The event is intended to be used when CONFIG_NRF_CLOUD_PGNSS_TRANSPORT_MQTT
	 *  is disabled to let the application decide when and how to use
	 *  the request information.
	 *  This event type is not received if  CONFIG_NRF_CLOUD_PGNSS_TRANSPORT_MQTT
	 *  is enabled.
	 */
	PGNSS_EVT_REQUEST,
};

struct nrf_cloud_pgnss_event {
	enum nrf_cloud_pgnss_event_type type;
	union {
		struct nrf_cloud_pgnss_prediction *prediction;
 ...
 */
typedef void (*pgnss_event_handler_t)(struct nrf_cloud_pgnss_event *event);

/** @brief Initialization parameters for the module. */
struct nrf_cloud_pgnss_init_param {
	/** Event handler that is registered with the module. */
	pgnss_event_handler_t event_handler;
 ...
 * the PGNSS subsystem is built with A-GNSS disabled, or when A-GNSS data is
...
/** @brief Requests specified PGNSS data from nRF Cloud via MQTT.
 * Not used for other transports.
 *
 * @param request Pointer to structure specifying what PGNSS data is desired.
 ...
 */
int nrf_cloud_pgnss_request(const struct gps_pgnss_request *request);

/** @brief Requests all available PGNSS data from nRF Cloud via MQTT.
 * Not used for other transports.
 *
 * @return 0 if successful, otherwise a (negative) error code.
 */
int nrf_cloud_pgnss_request_all(void);

/** @brief Use this function when you use a custom transport mechanism for downloading prediction
 * data. When your pgnss_event_handler_t receives a PGNSS_EVT_REQUEST event, send
 * the PGNSS request to the cloud over your transport, then call this function.
 ...
 * @retval -ENOMEM No free space in PGNSS flash storage area.
 * @retval -EBUSY PGNSS download already in progress.
 ...
 * The library calls this automatically for HTTP(S) transports.
 *
 * @param buf Pointer to PGNSS data received from transport.
 * @param len Size of PGNSS data chunk received from transport.
 * @retval 0 Buffer processed.
 * @retval -EINVAL Invalid parameter to function or invalid header in PGNSS data received.
 * @retval -ENODATA Unable to determine current date and time.
 * @retval -ENOMEM No free flash in PGNSS storage area.
 ...
 * a call to nrf_cloud_pgnss_process_update(), as well as successful completion. Further,
...
/** @brief If previous request for PGNSS data failed, re-enable future retries.
 * This should be called by the application after it attempts to
 * handle PGNSS_EVT_REQUEST, but is unable to complete it successfully. For
...
/** @brief Processes binary PGNSS data received from nRF Cloud.
 *
 * @param buf Pointer to data received from nRF Cloud.
 * @param buf_len Buffer size of data to be processed.
 *
 * @retval 0 PGNSS data successfully processed.
 * @retval -EFAULT An nRF Cloud PGNSS error code was processed.
 * @return A negative value indicates an error.
 */
int nrf_cloud_pgnss_process(const char *buf, size_t buf_len);

/** @brief Processes binary PGNSS data using URL received from nRF Cloud.
 *
...
/** @brief Injects binary PGNSS data to the modem. If request is NULL,
...
/** @brief Find out if PGNSS update is in progress
 *
 * @return True if request sent but loading not yet completed.
 */
bool nrf_cloud_pgnss_loading(void);

/** @brief Download more predictions if less than CONFIG_NRF_CLOUD_PGNSS_REPLACEMENT_THRESHOLD
...
/** @brief Initialize PGNSS subsystem. Validates what is stored, then
 ...
 *
 * @note It must return successfully before using PGNSS services.
 *
