 ...
 *			 than @ref CONFIG_NRF_CLOUD_REST_FRAGMENT_SIZE.
 *
 * @retval 0 If successful.
 *           If result is NULL and the A-GPS data is larger than
 *           @ref CONFIG_NRF_CLOUD_REST_FRAGMENT_SIZE, a positive value is
...
/**
 * @brief nRF Cloud Predicted GPS (P-GPS) data file request.
 *
 * @param[in,out] rest_ctx Context for communicating with nRF Cloud's REST API.
 * @param[in]     request Data to be provided in API call.
 * @param[in,out] result Optional; parsed results of API response.
 ...
 *         1 Device is already provisioned.
 *         Otherwise, a (negative) error code is returned.
 */
