...
enum nrf_provisioning_event {
	 ...
	 * Application should call lte_lc_offline() or similar to deactivate LTE
	 * connectivity. This event is generated when the library needs to write
	 * certificates or credentials to the modem, which requires the LTE connection
	 * to be offline.
	 */
	...
	/**
	 * A fatal error occurred during provisioning that cannot be recovered from.
 ...
 * - Registering LTE link controller event handlers
 * - Starting automatic provisioning if CONFIG_NRF_PROVISIONING_AUTO_START_ON_INIT is enabled
 *
 ...
 * @retval -ENOMEM if insufficient memory is available for initialization.
 * @retval -ENODEV if required hardware resources are not available.
 * @retval -ENETDOWN if network interface is not available.
 */
 ...
 * @retval -EFAULT if unable to convert interval to string format.
