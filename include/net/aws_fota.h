 ...
 *
 *  For causes that describe a failure of the update itself, the same cause is reported to
 *  AWS IoT Jobs in the ``statusDetails`` field of the job execution update that marks the
 *  job as ``FAILED``.
 *
 *  @ref AWS_FOTA_ERROR_CAUSE_JOB_UPDATE_REJECTED and
 *  @ref AWS_FOTA_ERROR_CAUSE_JOB_UPDATE_FAILED are the exceptions. They describe a failure
 *  of the job execution update itself, so for those causes no status details are accepted
 *  by, or published to, AWS IoT Jobs.
 */
